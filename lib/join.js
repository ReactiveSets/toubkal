  ...
  */
  ...
  Pipelet.build( 'half_join', Half_Join, {
    make_organizer_and_transform: function( position, on ) {
      ...
      var u, p = this,position, all = this.options.all;
      
      var transform = new Code( 'join..transform(), position: ' + p + ', all: ' + all )
        ._function( 'this.transform', u, [ 'values' ] )
          .var( 'filter = this.filter', 'merge = this.merge', 'out', 'i', 'l', 'v', 'locations' );
          ...
          .add( 'locations = filter.locate( values, { all: true } )' )
          
          // Merge joined values
          .add( 'out = [], i = -1; l = locations.length' )
          
          ._while( '++i < l' )
            ._var( 'l = locations[ i ]', 'v = l.o' + ( renames ? '.__v' : '' ), 'matches = l.all_matches' )
            
            ._if( 'matches' );
              .var( 'j = -1, ml = matches.length' )
              
              .unrolled_while(
                'out.push( merge( ' + ( p ? 'matches[ ++j ], v' : 'v, matches[ ++j ]' ) + ' ) )',
                u, u, { index: 'j', len: 'ml' }
              );
              
              if ( all ) {
                transform._else().add( 'out.push( merge( ' + ( p ? '{}, v' : 'v, {}' ) + ' ) )' )
              }
