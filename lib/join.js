...
( function( exports ) {
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Parameters:
       ...
       - merge: function( a_value, b_value ) where a_value is an Object from the 'a' set, and
           ...
           alter neither a_value nor b_value. Altering a_value or a_value may produce
  ...
  */
  function Half_Join( position, filter, on, merge, options ) {
    ...
    this._half_join_filter = filter.order( this.organizer, this.options );
  ...
  Pipelet.Build( 'half_join', Half_Join, {
    ...
    _make_organizer_and_transform: function( position, on ) {
      ...
      var transform = new Code( 'join.._transform(), position: ' + position + ', all: ' + all )
        ._function( 'this._transform', u, [ 'values' ] )
