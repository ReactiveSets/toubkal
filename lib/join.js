  ...
  Pipelet.build( 'half_join', Half_Join, {
      ...
      if ( renames.length ) {
        var code = new Code()
          ._function( 'this.prepare', null, [ 'values' ] )
            ...
            .unrolled_while( 'v = values[ ++i ]; out.push( { ' + attributes.join( ', ' ) + ' } );' )
            .add( 'return out' )
          .end( 'prepare' )
          .get()
        ;
    ...
    transform: function( values ) {
      ...
      if ( this,prepare ) {
        ...
        query = this.prepare( values );
        
        // ToDo: need to sort values and query together
      ...
      var locations = filter.locate( query, { all: true } );
      
      // Merge found values
      // ToDo: make code generator and inline merge function call when possible 
      for ( var i = -1, l = locations.length, all = this.options.all, out = [], p = this.position, merge = this.merge; ++i < l; ) {
        var l = locations[ i ], v = values[ i ], matches = l.all_matches;
