...
( function( exports ) {
  var fs = require( 'fs' )
    , XS = require( 'excess/lib/pipelet.js' ).XS
    , xs         = XS.xs
    , log        = XS.log
    , extend     = XS.extend
    //, Code       = XS.Code
    ...
    add: function( files ) {
      ...
      function add_file( file ) {
        ...
        fs.readFile( filename, 'utf8', function( error, data ) {
          ...
          de&&ug( 'watch(), read file "' + filename + '", length: ' + data.length );
          ...
          watcher.on( 'change', function( event ) {
            de&&ug( 'watch(), file "' + filename + '" changed, event: ' + event );
            ...
            // ToDo: To prevent reading more than once the same file, a strategy must be found to figure out which
            // one is the last.
            
            fs.readFile( filename, 'utf8', function( error, data ) {
              if ( error ) {
                de&&ug( 'watch(): no longer able to read fle "' + filename + '", error: ' + log.s( error ) );
                
                watcher.close();
                
                Set.prototype.remove.call( that, file );
