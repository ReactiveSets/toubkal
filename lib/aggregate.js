  ...
  Pipelet.subclass( Aggregate_Measures, {
    ...
    build_reduce_groups: function( measures, dimensions, done ) {
      var u, i, m, ids = [], id, first, inner
        ...
        , vars = [ 'keys = groups.keys', 'positions = {}', 'key', 'out = []', 'hash = {}' ]
        , init_measures = ''
      ;
  ...
  Set.build( 'aggregate', Aggregate, {
    ...
    */
    build_merge: function( measures, done ) {
      ...
      this.merge = function( aggregates, options ) {
        ...
        for ( var i = -1, l = keys1.length; ++i < l; ) {
          ...
          if ( g0 ) {
            ...
            if ( g1._count += g0._count ) all_zeros = false;
            
            for ( var j = -1; ++j < ml; ) {
              var m = measures[ j ].id;
              
              if ( g1[ m ] += g0[ m ] ) all_zeros = false;
            }
    ...
    */
    add: function( values, options ) {
      ...
      if ( this.aggregates ) return this.merge( a, options );
