/*  load_images.js
    
    ----
    
    Copyright (C) 2013, Connected Sets

    This program is free software: you can redistribute it and/or modify
    it under the terms of the GNU Affero General Public License as
    published by the Free Software Foundation, either version 3 of the
    License, or (at your option) any later version.

    This program is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU Affero General Public License for more details.

    You should have received a copy of the GNU Affero General Public License
    along with this program.  If not, see <http://www.gnu.org/licenses/>.
*/
"use strict";

( function( exports ) {
  var XS;
  
  if ( typeof require === 'function' ) {
    XS = require( './pipelet.js' ).XS;
    
    require( './code.js'     );
    require( './selector.js' );
  } else {
    XS = exports.XS;
  }
  
  var xs      = XS.xs
    , log     = XS.log
    , extend  = XS.extend
    , Code    = XS.Code
    , Pipelet = XS.Pipelet
    , Set     = XS.Set
  ;
  
  /* -------------------------------------------------------------------------------------------
     de&&ug()
  */
  var de = true;
  
  function ug( m ) {
    log( "xs load_images, " + m );
  } // ug()
  
  /* -------------------------------------------------------------------------------------------
     load_images( options )  ToDo: change name to throttled_image_load() or other more descriptive name
     
     load images in the DOM, throttled, one by one, or loading_max by loading_max at a time.
     
     Options:
       - node_id: DOM node id of the div where images are loaded, default is 'xs_load_images'
       - display: images style display, default is 'none'
       - loading_max: maximum number of images loading simultaneously, default is 1
     
     image object:
       id           : image ID
       uri          : image source
       title*       : image title
       description* : image description
       style*       : custom css classes
       
       ( * ) : optional attributes
  */
  function Load_Images( options ) {
    Set.call( this, options );
    
    this._loading_count = 0;
    this._loading_max = this.options.loading_max || 1;
    this._next_images_parameters = [];
    
    return this._set_node();
  } // Load_Images()
  
  var Set_proto  = Set.prototype
    , Set_add    = Set_proto.add
    , Set_remove = Set_proto.remove
  ;
  
  /* -------------------------------------------------------------------------------------------
     .load_images( options )
  */
  Set.build( 'load_images', Load_Images, {
    _set_node: function() {
      var options = this.options
        , node_id = options.node_id || 'xs_load_images'
      ;
      
      var $node = document.getElementById( node_id );
      
      if( ! $node ) {
        var body = document.body;
        
        // create images container if it does not exist
        $node = document.createElement( 'div' );
        
        $node.id = node_id;
        
        $node.style.display = options.display || 'none';
        
        // insert the container at the beginning of body 
        document.body.insertBefore( $node, body.childNodes[ 0 ] );
      }
      
      this._$node = $node;
      
      return this;
    }, // _set_node()
    
    _get_image_from_uri: function( uri ) {
      var image;
      
      for(image = this._$node.childNodes[ 0 ]
        ; image && image.src != uri
        ; image = image.nextSibling
      );
      
      return image;
    }, // _get_image_from_uri()
    
    _load_next_image: function() {
      var parameters;
      
      while ( this._loading_count < this._loading_max
        &&  ( parameters = this._next_images_parameters.shift() )
      ) {
        this._load_image.apply( this, parameters );
      }
      
      return this;
    }, // _load_next_image()
    
    _operation_add: function( operation, value ) {
      if ( this._loading_count >= this._loading_max ) {
        this._next_images_parameters.push( [ operation, value ] );
      } else {
        this._load_image( operation, value );
      }
      
      return this;
    }, // _operation_add()
    
    _load_image: function( operation, value ) {
      de&&ug( '_load_next_image(): ' + log.s( value ) );
      
      var that = this;
      
      that._loading_count += 1;
      
      if( value.uri ) {
        // Find out if image is already loaded or loading
        image = this._get_image_from_uri( value.uri );
        
        if ( image ) {
          // ToDo: add test for image already loaded
          var previous_onload = image.onload;
          
          image.onload = onload;
          
          that._load_next_image();
        } else {
          var $node = this._$node
            , image = document.createElement( 'img' )
          ;
          
          $node.appendChild( image );
          
          image.src = value.uri;
          
          image.onerror = onerror;
          image.onload  = onload;
        }
      } else {
        de&&ug( 'image uri is not defined, image id: ' + value.id );
        
        onerror()
      }
      
      return;
      
      function onerror() {
        that._loading_count -= 1;
        
        if ( image ) {
          de&&ug( 'image failed to load, image id: ' + value.id );
          
          $node.removeChild( image );
        }
        
        that._operation_no_value( operation );
        
        that._load_next_image();
      }
      
      function onload( e ) {
        that._loading_count -= 1;
        
        previous_onload && previous_onload( e );
        
        de&&ug( 'image: ' + log.s( value ) );
        
        image.setAttribute( 'loaded', 1 );
        
        that._operation_emit_add( operation, value );
        
        previous_onload || that._load_next_image();
      }
    }, // _load_image()
    
    _operation_remove: function( operation, value ) {
      var image = this._get_image_from_uri( value.uri );
      
      if ( image ) {
        var loaded = image.getAttribute( 'loaded' );
        
        de&&ug( '_operation_remove(), loaded_count: ' + loaded );
        
        if ( loaded ) {
          this._operation_emit_remove( operation, value );
        } else {
          // this image was loading, can therefore load one more image
          // will call this._load_next_image() after removing image div bellow
          this._operation_no_value( operation );
        }
        
        // ToDo: should we really remove the image from the DOM? This is just a cache.
        // Unless this is used by a derived class to actually manage another DOM element
        // There is at least one legitimate use: to abort loading an image which has not yet fully loaded
        this._$node.removeChild( image );
        
        loaded || this._load_next_image();
      } else {
        this._operation_no_value( operation );
      }
    }, // _operation_remove()
    
    _operation_emit_add: function( operation, values ) {
      de&&ug( '_operation_emit_add(): values: ' + log.s( values ) );
      
      return values
        ? Set_add.call( this, values instanceof Array ? values: [ values ], --operation.count ? operation.more : operation.done )
        : this._operation_no_value( operation )
      ;
    }, // _operation_emit_add()
    
    _operation_emit_remove: function( operation, values ) {
      de&&ug( '_operation_emit_remove(): values: ' + log.s( values ) );
      
      return values
        ? Set_remove.call( this, values instanceof Array ? values: [ values ], --operation.count ? operation.more : operation.done )
        : this._operation_no_value( operation )
      ;
    }, // _operation_emit_remove()
    
    _operation_no_value: function( operation ) {
      de&&ug( '_operation_no_value(): operation: ' + log.s( operation ) );
      
      var more = operation.more;
      
      --operation.count || more.more || Set_add.call( [], more );
      
      return this;
    }, // _operation_no_value()
    
    _operation_start: function( l, options ) {
      if ( l == 0 ) {
        if ( options && options.more ) return;
        
        Set_add.call( [], options );
        
        return;
      }
      
      var options_more;
      
      if ( l > 1 ) options = XS.no_more( options_more = XS.more( options ) );
      
      return { count: l, done: options, more: options_more };
    }, // _operation_start()
    
    add: function( values, options ) {
      var l = values.length
       , operation = this._operation_start( l, options );
      ;
      
      de&&ug( 'add(), count: ' + l + ', operation: ' + log.s( operation ) );
      
      for ( var i = -1; ++i < l; ) this._operation_add( operation, values[ i ] );
      
      return this;
    }, // add()
    
    remove: function( values, options ) {
      var l = values.length
       , operation = this._operation_start( l, options );
      ;
      
      de&&ug( 'remove(), count: ' + l + ', operation: ' + log.s( operation ) );
      
      for ( var i = -1; ++i < l; ) this._operation_remove( operation, values[ i ] );
      
      return this;
    } // remove()
  } ); // Load_Images instance methods
  
  /* -------------------------------------------------------------------------------------------
     module exports
  */
  eval( XS.export_code( 'XS', [ 'Load_Images' ] ) );
  
  de&&ug( "module loaded" );
} )( this ); // load_images.js
