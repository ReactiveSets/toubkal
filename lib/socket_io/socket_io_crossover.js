  ...
  */
  ...
  Pipelet.subclass( 'Socket_IO_Crossover', Socket_IO_Crossover, {
    ...
    _disconnect: function() {
      de&&ug( _get_name( this, '_disconnect' ) );
    ...
    _close: function() {
      ...
      de&&ug( _get_name( this, 'close' ) + 'terminating pending fetches, fetch ids:', fetch_ids );
      
      fetch_ids.forEach( function( id ) {
        // ToDo: emit error to global error dataflow
        receivers[ id ]( [ { flow: 'errors', cause: 'connection_closed' } ], true );
    ...
    _tx_connection: function( rs_connection ) {
      var that           = this
        , name           = de && _get_name( that, '_tx_connection' )
