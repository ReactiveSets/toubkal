  ...
  Pipelet.subclass( 'Socket_IO_Crossover', Socket_IO_Crossover, {
    ...
    connect: function() {
      ...
      if( that.disconnected ) {
        ...
        de&&ug( that._get_name( 'connect' ) + 'emit ' + l + ' queued operations' );
        
        while( v = q.shift() ) emit.apply( s, v );
    ...
    _send: function( method, data ) {
      var that  = this
        , queue = that._emit_queue
      ;
      
      if( queue.length || that.disconnected ) {
        de&&ug( that._get_name( '_queue' ) + method, data );
        
        queue.push( [ method, data ] );
      } else {
        de&&ug( that._get_name( '_send'  ) + method, data );
        
        // ToDo: implement retransmission mechanism if not acknowledged before diconnection
        that.socket.emit( method, data );
      }
    }, // _send()
