...
( 'socket_io_crossover', [ '../core/pipelet' ], function( rs ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      - ```"rs_connection"```: packet emitted on connection and reconnections:
  ...
  */
  function Socket_IO_Crossover( socket, options ) {
    ...
    that._output.source = {
      ...
      _reset_peer: function() {
        ...
        if( ql ) {
          de&&ug( get_name() + 'removing input query terms:', query );
    ...
    function init_rx( name ) {
      de&&ug( _get_name( that, 'init_rx' ) + '(re)set last_processed to zero' + name );
  ...
  Pipelet.subclass( 'Socket_IO_Crossover', Socket_IO_Crossover, {
    ...
    _connect: function( connection, wait ) {
      var that           = this
        , name           = de && _get_name( that, '_connect' )
        , last_tx        = connection.last_tx
        , last_processed = typeof last_tx == 'number' ? that._last_processed = last_tx : that._last_processed
      ;
      
      de&&ug( name
        + 'peer rs_connection:', connection
      ...
      connection.last_tx = that._last_tx = that._acknowledged;
      
      de&&ug( name + 'tx "rs_connection":', connection );
      
      that.socket.emit( 'rs_connection', connection );
    ...
    _close: function() {
      ...
      for( id in receivers )
        if( receiver = receivers[ id ] ) {
          de&&ug( _get_name( this, 'close' ) + 'delete receiver:', id );
