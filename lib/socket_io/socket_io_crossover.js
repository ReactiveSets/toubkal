...
( 'socket_io_crossover', [ '../core/pipelet' ], function( rs ) {
  ...
  */
  function Socket_IO_Crossover( socket, options ) {
    var that = this
      , input
      , update_upstream_query = Pipelet.Input.prototype.update_upstream_query
    ;
    ...
    this._output || ( this._output = new Socket_IO_Crossover.Output( this, options && options.name ) );
  ...
  Socket_IO_Crossover.Output = Pipelet.Output.subclass(
    ...
    update_upstream_query: function( changes, input ) {
      if ( input ) { // input is undefined if query is only resubmitted to peer
        changes = this._update_query( changes, input ); // ToDo: overload this.source instead, this breaks encapsulation
      }
      
      changes[ 0 ].length + changes[ 1 ].length
        && this.pipelet.send( 'rs_query', changes )
      ;
      
      return this;
    } // Socket_IO_Crossover.Output..update_upstream_query()
  } ); // Socket_IO_Crossover.Output
