...
( 'socket_io_crossover', [ '../core/pipelet' ], function( rs ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      - ```"rs_connection"```: packet emitted as a requestion on connection
  ...
  */
  ...
  function Socket_IO_Crossover( socket, options ) {
    ...
    that._output.source = {
      _get_name: function( method ) { return get_name( that, method ) },
      ...
      update_upstream_query: function( changes ) {
        ...
        // ToDo: DRY code with Input..update_upstream_query()
        input._update_query.call( this, changes, ___, future_query_s );
        
        // ToDo: DRY code with Plug..update_upstream_query(), implement re-oridering of query updates
        changes = input._update_query.call( this, changes );
        
        // Send optimized changes
        changes_count( changes ) && that._send( [ 'query', changes ] );
      ...
      _reset_peer: function() {
        ...
        if( ql ) {
          de&&ug( get_name( that, name ) + 'removing input query terms:', query );
        ...
        while( message = queue[ ++i ] ) {
          switch( message[ 0 ] ) {
            case 'query':
            case 'fetched':
              de&&ug( get_name( that, name ) + 'removing from queue obsolete message:', message );
              
              queue.splice( i--, 1 );
            break;
            
            case 'fetch':
              de&&ug( get_name( that, name ) + 'removing obsolete query changes from fetch:', message );
              
              message[ 3 ] = ___;
          }
        }
        
        // Resubmit output query
        q     = this.query;
        query = q && q.query
        ql    = query && query.length;
        
        if ( ql ) {
          de&&ug( get_name( that, name ) + 'resubmit query ' + ql + ' terms' );
      ...
      _fetch: function( receiver, query, query_changes ) {
        // Optimize query_changes, see Input.._fetch()
        // ToDo: DRY with Input.._fetch()
        var _query_changes;
        
        if ( query_changes ) {
          // Find-out query_changes differences on future query
          
          _query_changes = input._update_query.call( this, query_changes, ___, future_query_s );
        ...
        // ToDo: apply ongoing fetches' query changes on output disconnection, this should be done in Plug
        that._fetches[ ++fetch_id ] = {
          receiver: receiver,
          
          query_changes: _query_changes
        };
        
        that._send( [ 'fetch', fetch_id, query, _query_changes ] );
      } // Socket_IO_Crossover.Output.._fetch()
    }; // Socket_IO_Crossover.Output..source
    
    function init_rx( name ) {
      de&&ug( get_name( that, 'init_rx' ) + '(re)set last_processed to zero' + name );
      ...
      that.socket = socket
        ...
        .on( 'rs', function( peer_position, operation, _2, _3, _4, _5, _6, _7 ) {
          ...
          switch( operation ) {
            ...
            case 'fetched':
              var fetches = that._fetches
                , fetch_object = fetches[ _2 ]
              ;
              ...
              if( fetch_object ) {
                ...
                if( no_more ) {
                  var query_changes = fetch_object.query_changes;
                  
                  // ToDo: DRY with Input.._fetch(), enforce query updates order
                  query_changes
                    && input._update_query.call( output.source, query_changes )
                  ;
                  
                  delete fetches[ _2 ];
                }
                
                fetch_object.receiver( _4, no_more, _5, _6, _7 );
              } else {
