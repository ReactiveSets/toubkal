    ...
    var socket = io.connect( address, { transports: options.transports || [ 'xhr-polling', 'polling', 'websocket' ] } )
      ...
      .on( 'rs_connection', function( _connection ) {
        ...
        if ( previous_id ) {
          ...
          if( ! same_id ) {
            // Cleanup sending queue and resubmit queries
            that._resubmit_query(); // this will allow to receive operations from previous queries
            ...
            */
            add_state_change( that, true, same_id );
          }
        }
  ...
  Socket_IO_Crossover.Build( 'socket_io_server', Socket_IO_Server, function( Super ) {
    return {
      _connect: function( connection ) {
        ...
        add_state_change( this, true );
      }, // _connect()
      
      _disconnect: function( connection ) {
        Super._disconnect.call( this, connection );
        
        add_state_change( this, false );
      } // _disconnect()
    };
  } ); // socket_io_server()
