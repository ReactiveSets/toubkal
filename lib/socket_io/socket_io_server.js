...
function( io, rs, Socket_IO_Crossover, timestamp_string ) {
  ...
  */
  ...
  Socket_IO_Server.Output = Socket_IO_Crossover.Output.subclass(
    ...
    function( Super ) {
      var _update_upstream_query = Super.update_upstream_query;
      
      return {
        // ToDo: should this move this to Socket_IO_Crossover.Output to also optimize queries for socket_io_clients()?
        // ToDo: overlaod _update_query instead, calling Input.._update_query() then Super._update_query()
        // Optimizes query updates before sending over to server and allows to resubmit query
        update_upstream_query: function( changes, input ) {
          var that    = this
            , query   = that._query
            , removes = changes[ 0 ]
            , adds    = changes[ 1 ]
          ...
          _update_upstream_query.call( that, query.discard_operations(), input );
        ...
        resubmit_query: function() {
          ...
          return _update_upstream_query.call( that, [ [], that._query.query ] );
        } // resubmit_query()
      };
    } // methods()
