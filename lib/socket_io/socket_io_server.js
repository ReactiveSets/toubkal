...
function( io, rs, Socket_IO_Crossover, timestamp_string ) {
  ...
  /* --------------------------------------------------------------------------
      ...
      - resynchronize caches on hard reconnections where the server has lost
        ...
        which uses this pipelet and takes care of concurent caches
        resynchronization.
  ...
  */
  var state_change_id = 0
    ...
    , socket_io_synchronizing = 'socket_io_' + synchronizing
    , state_changes = rs
        .map(
          function( v ) {
  ...
  rs
    .Singleton( state_changes_name, function( source, options ) {
      return source
    ...
    /* ------------------------------------------------------------------------
        ...
        @short Emits start of synchronization events, collects synchronized
        events
        ...
        - Resynchronize a cache of ```"users"``` on hard reconnection with a new id:
          ```javascript
            rs
              ...
              // No-longer required on this branch
        ...
        to allow the concurent synchronization of multiple caches. Once
    ...
    */
    .Singleton( socket_io_synchronizing, function( source, options ) {
      return source
        // Only enit last value at the end of the transaction
        .optimize( { tag: synchronizing } )
        .last()
        .split_updates()
        .adds()
        ...
        .filter( [ { state: synchronizing } ], { name: options.name } )
        
        .alter( function( state ) { delete state.same_id }, { fork_tag: synchronizing } )
      ;
    } )
