...
function( io, rs, Socket_IO_Crossover, timestamp_string ) {
  ...
  */
  ...
  function Socket_IO_Server( options ) {
      ...
      .on( 'rs_connection', function( _connection ) {
        ...
        if ( previous_id ) {
          if ( previous_id == _id ) {
            ...
            that._output.resubmit_query(); // this will only get us new operations, not lost data since last session
            ...
            /*
                ...
                - fetch using current query, ignore duplicates (unique_set() would do this)
    ...
    that._output || ( that._output = new Socket_IO_Server.Output( that, options && options.name ) );
  ...
  Socket_IO_Server.Output = Socket_IO_Crossover.Output.subclass(
    ...
    function( p, name ) {
      ...
      this._query = new Query( [] ); // Optimized local query as Input.._update_query does
    },
    
    function( Super ) {
      return {
        // ToDo: should this move this to Socket_IO_Crossover.Output to also optimize queries for socket_io_clients()?
        // Optimizes query updates before sending over to server and allows to resubmit query
        _update_query: function( changes, input ) {
          var that    = this
            , query   = that._query
            , removes
            , adds
          ...
          // !!! Do not optimize query before submitting to query tree
          // because this will fail if optimizations happen on terms submitted by another input than the input that submitted the first term
          changes = Super._update_query.call( that, changes, input ); // updates output query tree
          
          // Optimize query shared from all inputs AFTER updating query tree
          removes = changes[ 0 ];
          adds    = changes[ 1 ];
          ...
          return query.discard_operations();
        }, // _update_query()
        
        resubmit_query: function() {
          // ToDo: we also need to fetch differences from previous session
          var that = this;
          
          de&&ug( that._get_name( 'resubmit_query' ), ', query terms:', query.query.length );
          
          return that.update_upstream_query( [ [], that._query.query ] ); // no input because this is a resubmission to server
        } // resubmit_query()
      };
    } // methods()
