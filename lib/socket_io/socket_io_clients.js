...
/* --------------------------------------------------------------------------
   ...
   Options:
     - remove_timeout : (Integer) in seconds, timeout to remove a connection after a
         disconnection, waiting for a reconnection. Default is 1 minute.
     ...
     - get_session (Function): to retrieve session information and passport
       user id.
*/
    ...
    function start_server( address ) {
      ...
      io.configure( function() {
        ...
        io.set( 'authorization', function ( handshake, callback ) {
          de&&ug( 'authorization:', handshake );
          
          var get_session = that._options.get_session;
          
          get_session && get_session( handshake, function( error, session ) {
            if ( session ) {
              //session.resetMaxAge();
              session.save( function( error ) {
                ...
                de&&ug( 'client, saved session' );
              } );
              
              var passport = session.passport
                , user_id  = passport && passport.user // ToDo: this is passport-specific
              ;
              
              handshake.user_id = user_id;
            }
            
            de&&ug( 'client, session:', { error: error, session: session, user_id: user_id } );
          } );
