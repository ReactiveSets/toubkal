...
/* ----------------------------------------------------------------------------
    ...
    - socket (@@class:Socket_IO_Crossover\): to communicate with the remote client:
    ...
    function start_server( address ) {
      ...
      function on_connection( socket ) {
        ...
        function on_disconnect() {
          if( connection ) {
            ...
            if( timeout ) {
              ...
              connection.timeout_id = setTimeout( remove_connection, timeout * 1000 );
          ...
          function remove_connection() {
            ...
            connection.timeout_id = 0;
            
            delete connections[ connection.id ];
            
            Set.prototype._remove.call( that, [ connection ], { no_fetch: true } );
        ...
        function on_rs_connection( _connection ) {
          ...
          if ( id ) {
            ...
            if ( connection = connections[ id ] ) {
              var timeout_id = connection.timeout_id;
              
              // We have a reference to this connection, allow reconnection
              de&&ug( get_name() + 'accept rs_connection as a reconnection:', _connection, timeout_id ? 'clearTimeout: ' + timeout_id : '' );
          ...
          if ( connection ) {
            ...
            Set.prototype._add.call( that, [ connection ] ); // ToDo: this may be too early before sending rs_connection and connecting socket
          }
        ...
        function update_connection( attributes ) {
          ...
          that._update( [ [ connection, updated ] ] );
