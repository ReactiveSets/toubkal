...
/* --------------------------------------------------------------------------
   get_session( session_options )
   
   Socket-io session middleware
   
   Parameters:
   - session_options (Object): options for express-session middleware
*/
...
function Socket_IO_Clients( options ) {
  ...
  if ( session_options ) {
    this._get_session = get_session( session_options );
  } else {
    this._get_session = null;
  }
    ...
    function start_server( address ) {
      ...
      io.configure( function() {
        ...
        io.set( 'authorization', function ( handshake, callback ) {
          ...
          that._get_session && that._get_session( handshake, function( error, session ) {
            if ( session ) {
              //session.resetMaxAge();
