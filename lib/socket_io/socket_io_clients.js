...
/* ----------------------------------------------------------------------------
    ...
    - **options** (Object): pipelet options:
      ...
      - Additional options for @@class:Socket_IO_Crossover\:
        - **max_concurrent_transactions** (Number): default is 2
        - **allowed_tags** (Array of Strings): default is []
    ...
    - **connected** (Boolean): current state of the low-level connection with
      peer, false means that sending of packets is currently suspended until
      client reconnects. After the reconnection timeout, if the client has
      not recconected the connection will be removed.
      ...
      function on_connection( socket ) {
        ...
        function on_disconnect() {
          if( connection ) {
            ...
            connection.socket._disconnect(); // force emited operations to be queued until reconnection
            ...
            if( timeout ) {
              ...
              update_connection( { connected: false } );
            } else {
              remove_connection();
            }
          } else {
        ...
        function on_rs_connection( _connection ) {
          ...
          if ( connection ) {
            ...
            crossover = connection.socket;
            
            // disconnect and terminate previous socket
            crossover.socket.disconnect();
            
            // replace previous socket with new socket to allow to emit and receive operations
            crossover._init_socket( socket );
            
            update_connection( { connected: true } );
          } else {
            id = _connection.id = uuid.v1();
            
            crossover = new Socket_IO_Crossover( socket, extend( {}, options, { name: id } ) ).set_namespace( that );
            ...
            connections[ id ]
              ...
              = {
                  ...
                  count         : 1,
                  timeout_object: null,
                  // ToDo: API change: rename socket into crossover
                  socket        : crossover
                }
            ;
          ...
          // Acknowledge connection request, providing (possibly new) connection id to peer
          crossover._tx_connection( _connection );
          
          // Then connect and possibly force (re)transmit queued packets
          crossover._connect();
...
/* --------------------------------------------------------------------------
   module exports
*/
RS.Socket_IO_Clients = Socket_IO_Clients;
