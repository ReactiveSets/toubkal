  ...
  */
  function Trace_Domain( realm, emit, query ) {
    ...
    function query_remove( query ) {
      ...
      // Assume that the remove term was previously added by a corresponding query_add() on this trace domain
      ...
      query
        ...
        .forEach( function( term ) {
          ...
          if ( level == _trace_level ) {
            // ToDo: Set the next hihgest trace level
          }
          
          var name = term._name || Trace_Domain.any_other_instance;
          
          if ( level == _instance_levels[ name ] ) {
            // ToDo: find the next highest level for this instance
            // If none are available remove this name from _instance_levels[]
          }
          
          var method = term._method || Trace_Domain.any_other_method;
          
          if ( level == _methods_instance_levels[ method ][ name ] ) {
            // ToDo:  find next highest level for this method at this instance
            // If no more is available for this method remove it from _methods_instance_levels[ method ][]
            
            if ( _methods_instance_levels[ method ].length == 0 ) {
