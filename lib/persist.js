...
function File( source, name, options ){
  ...
  l8.nextTick( function(){ that.load(); } );
...
// I don't want to use l8.flow because I want to handle the errors myself,
// go-lang style. ToDo: add a l8.go!
var nodish = function( err ){
  err && de&&bug( "error detected in nodish callback", err );
  // ToDo: try "return arguments", make it work in l8 if needed.
  ...
  l8.task( function(){
    ...
    // If no error, process each line in file
      ...
      l8.step(function(){
        ...
        fs.read(
          ...
          this.proceed( nodish ) // cb, moves to next step
        );
      ...
      // Process each line from buffered read, unless read error
        ...
        if( !that.buffer ){
          ...
          fs.close( that.fd );
          that.fd = null;
          that.notify( [ {action:"add", objects: that.dataset.get() } ] );
          that.state = "running";
        }
      });
    ...
    // Reopen file, in append mode
      ...
      fs.open(
        that.filename,
        "a",
        this.proceed( function( e, fd ){ return [ e, fd ]; } )
      );
    ...
    // If ok, add some versionning info into the file
      ...
      var info = {
        ...
        id: that.next_id
        // ToDo: add/update/remove counters
      };
      that.next_id++;
      that.put( info );
...
smoke2.add( [ { article: 5, text: "hello" }, { article: 6, text: "world"} ] );

l8.countdown( 10 );

de&&bug( "file module loaded" );
