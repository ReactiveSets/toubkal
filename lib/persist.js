/*  persist.js
      ...
      .file( [filename] ) - restore from file and log changes
      .file( "in"  )      - read actions from stdin
      .file( "out" )      - write actions to stdout
...
function File( source, name, options ){
  ...
  this.fd            = name === "in"  ? 0 : name === "out" ? 1 : 0;
  this.state         = name === "out" ? "running" : "loading";
  this.error         = null;
  this.queue         = l8.queue();
  this.buffer        = "";
  this.version       = 0;
...
// private
  ...
  l8.task(function(){
    ...
    l8.step(function(){
      ...
      if( that.name === "in"       )return [ 0, 0 ]; // No err, stdin fd
      if( that.name === "out"      )return [ 0, 1 ]; // No err, stout fd
    ...
    // If no error, process each line in file, when appropriate
    }).repeat(function(){
      
      if( that.name !== "in" ){
        if( that.state !== "loading" ) this.break;
        if( that.name === "out"      ) this.break;
      ...
      // Process each line from buffered read, unless read error
        ...
        while( line = that.read_line() ){
          ...
          // For stdin, emit without delay, no async write to file is involved
          if( that.fd === 0 ){
            if( op.name !== "version" ){
              if( !op.id ){
                ...
                if( op.id > that.next_id ){
                  that.next_id = op.id + 1;
                }
              }
        ...
        if( !that.buffer ){
          // Don't close stdin, probably already done by the os anyways, empty
          if( that.fd === 0 ) return;
    ...
    // Reopen file, in append mode (or append/truncate), unless stdin/stdout
    }).step(function(){
      if( that.name === "in" || that.name === "out" ) return [ 0, that.fd ];
    ...
    // If ok, remember fd and maybe add some versionning info into the file
    }).step(function( e, fd ){
      if ( e ){
        that.state = "error";
        that.error = e;
      ...
      // Move to next step if stdin/stdout
      if( that.fd <= 1 ) return [0,1]; // No err, nothing written
    ...
    // Including so far queued operations, first.
      ...
      // Add monotone increasing id and timestamp and put into file
        ...
        op.id = that.next_id;
        that.next_id++;
        //if( that.fd < 1 ) return [0,0]; // stdin, no error, nothing written
        // Writing to stdin will be refused by the OS
        that.put( op );
        // ToDo: op = that.queue.tryGet() to handle batch of operations faster.
        // I could then .put() an array and issue a single write() to the file
    ...
    // to wait until the previous write succceeds before I can issue a new one,
...
// Export in & out
fluid.in  = fluid.file( "in"  );
fluid.out = fluid.file( "out" );
File.register( "stdin",  fluid.in  );
File.register( "stdout", fluid.out );

// In debug mode, trace all inputs/outputs on stdin/stdout
de && fluid.in.trace();
de && fluid.out.trace();

// ToDo: .to(), to be able to write this:
//   xs.in.to( xs.out )
// and with .pipe() alias: in.pipe( out )
// CoffeeScript: in.pipe out -- that's even cooler
