  ...
  Set.build( 'order', Order, {
    ...
    locate: function( objects, options ) {
      ...
      function locate( a, organizer, locations, begin, end, exact, all ) {
          ...
          for ( var i = begin - 1; ++i < end; ) {
            ...
            if ( ( location.order = organizer( a[ guess ], o ) ) === 0 ) {
              ...
              if ( exact ) {
                ...
                var all_matches = location.all_matches = [ a[ guess ] ];
              }
              ...
              while ( true ) {
                ...
                if ( all ) all_matches.push( a[ insert ] );
              }
              // insert location either at (insert_before) or after last matching value
              
              location.insert = insert;
              
              if ( all || ( exact && location.found === u ) ) {
                // All matches requested or still no key match and exact location requested
                ...
                while( true ) {
                  ...
                  if ( all ) {
                    all_matches.push( a[ guess ] );
                  } else if ( key === make_key( a[ guess ] ) ) {
        ...
        de&&ug( "Order.locate(), name: " + _options.name + ", locations: " + log.s( locations, replacer ) );
