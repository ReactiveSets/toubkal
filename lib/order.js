     ...
     Paraneters:
       - organizer: can be:
         ...
         - A JavaScript Object, such as a Pipelet, providing the functions fetch_all() and
           on() with the same behavior as that of a Pipelet, to allow the dynamic change
  ...
  */
  ...
  Set.build( 'order', Order, {
    set_organizer: function( organizer ) {
      ...
      if ( typeof organizer.on == 'function' ) {
        organizer.on( 'complete', function() {
    ...
    remove: function( objects, options ) {
      ...
      // de&&ug( "remove(), values: " + log.s( objects ) );
      ...
      // ordered in the same order as objects, removal could fail
      locations.sort( function( a, b ) { return a.found > b.found ? 1 : a.found < b.found ? -1 : 0 } );
      objects = []; // will be rebuilt from locations
      ...
      for ( var i = locations.length; i; ) {
        ...
        if ( f !== u ) {
          ...
          de&&ug( 'remove(), value not found: ' + log.s( objects[ i ] ) );
