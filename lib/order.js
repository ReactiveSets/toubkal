  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Paraneters:
       - organizer: can be:
         ...
         - A JavaScript Object, such as a Pipelet, providing the functions fetch_all() and
           on_change() with the same behavior as that of a Pipelet, to allow the dynamic change
  ...
  */
  ...
  Set.build( 'order', Order, {
    set_organizer: function( organizer ) {
      ...
      if ( typeof organizer.on_change == 'function' ) {
        organizer.on_change( function( operation, values, options ) {
          if ( true && options && options.more ) { // This code is not used, can be removed by minifier
            de&&ug( 'Order.update_organizer(), waiting for more updates' );
            
            return;
          }
          // There are no more updates to come: update organizer
          de&&ug( 'Order.update_organizer(), change: ' + operation + ' re-generate code' );
          ...
          ._function( 'sorter', null, [ 'a', 'b' ] )
            ...
            for ( var i = -1; ++i < l; ) {
              var d = organizer[ i ], inferior, superior
                , no_undefined = options.no_undefined || d.no_undefined
                , no_null      = options.no_null      || d.no_null
                , id = d.id
              ;
    ...
    locate: function( objects, options ) {
      var u, start = 0, stop = this.a.length, n = objects.length, step, guess = 0
        , locations = [], previous
        , that = this
        , _options = this.options, insert_before = _options.insert_before
        , exact, all
      ;
      ...
      function locate( a, sorter, locations, begin, end, exact, all ) {
        ...
        de&&ug( "Order.locate(), name: " + _options.name + ", locations: " + locations.length );
    ...
    add: function( objects, options ) {
      ...
      if ( ! sorter ) return this.wait_for_sorter( function() { that.add( objects, options ) } );
      
      objects = objects.slice( 0 ); // shallow copy before sort
      ...
      if ( a.length === 0 ) {
        this.a = objects.slice( 0 ); // shallow copy to prevent the emission of objects that could be further modified by splice, especially if there is a delay pipelet after order()
      } else {
    ...
    remove: function( objects, options ) {
      ...
      if ( ! sorter ) return this.wait_for_sorter( function() { that.remove( objects ) } );
      ...
      var locations = this.locate( objects, { exact: true } )
        , _locations = options.locations = []
        , a = this.a, u
      ;
    ...
    update: function( updates, options ) {
      ...
      if ( ! sorter ) return this.wait_for_sorter( function() { that.update( updates ) } );
  ...
  */
  ...
  Set.build( 'ordered', Ordered, {
    ...
    add: function( added, options ) {
      ...
      return Pipelet.prototype.add.call( this, added, options );
    }, // add()
    
    remove: function( removed, options ) {
      this.remove_from( removed, options );
      
      return Pipelet.prototype.remove.call( this, removed, options );
    }, // remove()
    
    update: function( updates, options ) {
      this.update_from_to( updates, options );
      
      return Pipelet.prototype.update.call( this, updates, options );
    } // update()
  } );
