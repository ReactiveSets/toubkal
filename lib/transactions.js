...
( function( exports ) {
  ...
  */
    ...
    */
    _emit_event: function( event_name, a ) {
      ...
      if ( de ) {
        ...
        name = 'Event_Emitter ' + name + '_emit_event( ' + event_name + ' )';
      }
      
      if ( ( listeners = events[ event_name ] ) && ( l = listeners.length ) ) {
        de&&log( name, { listeners_count: l } );
        
        try {
          for( var i = -1, listener; listener = listeners[ ++i ]; ) {
            if ( listener.once ) {
              de&&log( name, { message: 'removing "once" event listener', position: i } );
  ...
  /* -------------------------------------------------------------------------------------------
     RS.uuid_v4()
  */
  var uuid_v4 = RS.uuid_v4 = uuid.v4;
    ...
    */
    get_transaction: function( count, options, output, fork ) {
      ...
      function record_tid( tid ) {
        de&&ug( 'Transactions()', { message: 'new transaction', id: tid } );
      ...
      function on_transaction_ended() {
        de&&ug( 'Transactions()', { message: 'transaction ended, delete transaction', id: tid } );
  ...
  */
  ...
  Event_Emitter.subclass( Transaction, {
    ...
    add_operations: function( count ) {
      ...
      de&&ug( this.debug_name + 'add_operations()', { adding_operations: count, new_total: this.count } );
    ...
    get_emit_options: function() {
      ...
      function trace( message ) {
        ug( that.debug_name + 'get_emit_options()', message );
      }
    ...
    */
      ...
      if ( count || source_more ) {
        de&&ug( this.debug_name + 'end()', { message: 'not ending because'
          + ( count                ? ' count (' + count + ') > 0'         : '' )
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     are defined in the RS.Options object that provides methods which first parameter is an
     optional options Object.
  */
  
  /* -------------------------------------------------------------------------------------------
     RS.Options.forward( [ options ] )
     RS.options_forward( [ options ] ) deprecated form
  ...
  */
  ...
  RS.options_forward = options_forward;
