...
!function( exports ) {
  ...
  if ( typeof require === 'function' ) {
    module.exports = Lazy_Logger;
  } else {
    exports.Lazy_Logger = Lazy_Logger;
  }
  
  // de&&ug( messages ), set de to true for debugging Lazy_Logger itself
  ...
  if ( typeof console != "object" || typeof console.log != "function" ) {
    ...
    Lazy_Logger.to_console = function( emitted_log ) {
      de&&ug( 'to_console()', emitted_log );
      
      var diff = Lazy_Logger.time_difference_string( emitted_log._timestamp );
      
      diff && console.log( diff );
      
      console.log.apply( console, Lazy_Logger.to_console_parameters( emitted_log ) );
    };
  } // Lazy_Logger.to_console()
  
  // ToDo: document and test Lazy_Logger.time_difference_string()
  Lazy_Logger.time_difference_string = ( function() {
    var _start = new Date();
    
    return function( date ) {
      var miliseconds = date - _start;
  ...
  } )(); // Lazy_Logger.time_difference_string()
  
  // Test and document Lazy_Logger.date_to_timestamp_string()
  Lazy_Logger.date_to_timestamp_string = function( date ) {
    ...
    return year + '/' + month + '/' + day + ' '
      ...
      + '.' + ms
    ;
  }; // Lazy_Logger.date_to_timestamp_string()
  
  Lazy_Logger.to_console_parameters = function( t ) {
    var parameters = [
        Lazy_Logger.date_to_timestamp_string( t._timestamp )
      + ' #' + t._level
      + ' ' + t._realm
      + '[ ' + t._name
      + ' ].' + t._method + '()'
    ...
    return parameters;
  }; // Lazy_Logger.to_console_parameters()
}( this ); // lazy_logger.js
