...
( function( exports ) {
  ...
  */
  function Table_Columns( columns, table, options ) {
    ...
    return this.add_source( this.columns = columns );
  } // Table_Columns()
  
  Ordered.subclass( Table_Columns, {
    add: function( added, options ) {
      de&&ug( "Table_Columns..add(), added: " + log.s( added ) );
      
      if( ! added.length ) return this;

      this.lines = this.table.fetch_all();
    ...
    remove: function( removed, options ) {
      de&&ug( "Table_Columns..remove(), removed: " + log.s( removed ) );
    ...
    update: function( updates, options ) {
      de&&ug( "Table_Columns..update(), updated: " + log.s( updates ) );
  ...
  */
  ...
  Ordered.build( 'table', Table, {
    ...
    add: function( added, options ) {
      de&&ug( "Table..add(), added: " + log.s( added ) );
      
      if( ! added.length ) return this;
      
      this.cols = this.columns.fetch_all();
    ...
    remove: function( removed, options ) {
      de&&ug( "Table..remove(), removed: " + log.s( removed ) );
    ...
    update: function( updates, options ) {
      de&&ug( "Table..update(), updates: " + log.s( updates ) );
