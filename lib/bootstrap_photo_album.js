...
( function( exports ) {
  ...
  */
  XS.Compose( 'bootstrap_photo_album', function( source, photos_matrix_node, carousel_node, options ) {
    ...
    // photo album graph
    var carousel_events = xs.pass_through().trace( '' ); // tmp pipelet for graph loop
    ...
    return source
      ...
      .load_images()
      .bootstrap_carousel( carousel_node, { input_events: thumbnails_events } )
  ...
  */
  function Bootstrap_Photos_Matrix( photos_matrix_node, options ) {
    ...
    if( options.input_events ) {
      options.input_events.on_change( function( caller, values, options ) {
      }, this );
    }
...
use case :
  ...
  xs.socket_io_server()
    .bootstrap_photo_album( "album_1_matrix", "album_1_carousel", { album: 'album_1' } )
  ;
