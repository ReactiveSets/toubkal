...
( 'group_by', [ './stateful' ], function( rs ) {
  ...
  /* --------------------------------------------------------------------------
      ...
      @parameters
      - **f** (Function( Object ) -> Object): returns a value uniquely
        identifying output value groups. It defines the equivalent of
        dimensions in @@pipelet:aggregate().
      
      - **options** (Object): optional pipelet options
        - **key** (Array of Strings): output @@key attributes, default is
          ```Object.keys( f( {} ) )```. It should usually match the the keys
          of objects returned by ```f()```, or a subset of these keys.
        ...
        - **initial_groups** (Array of groups): Optional groups initialized
          ...
          defined with the same attributes returned by ```f()```.
          These groups are never deleted when they become empty after a remove
          operation.
  ...
  */
  function Group( f, options ) {
    ...
    options.key = options.key || Object.keys( f( {} ) );
  ...
  Set.Build( 'group', Group, {
    _emit_on_complete: function( options ) {
      ...
      de&&ug( that._get_name( '_emit_on_complete' ), 'added:', added.length, 'removed:', removed.length, 'updated', updated.length );
