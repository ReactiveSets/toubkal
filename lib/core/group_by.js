...
( 'group_by', [ './stateful' ], function( rs ) {
  ...
  /* --------------------------------------------------------------------------
      ...
      - **options** (Object): optional pipelet options
        ...
        - **sticky_groups** (Array of groups): Optional groups initialized
          as empty groups, i.e. with their content attribute set as an empty
          Array - e.g. ```{ id: 1, content: [] }```. Each sticky group should
          be defined with the same attributes returned by ```by()```.
          Sticky groups are never deleted when they become empty after a
          remove operation.
      ...
      ```javascript
        rs.set( [
          ...
          .group( by_country, { sticky_groups: [ { country: 'Germany' }, { country: 'England' } ] } )
        ;
  ...
  */
  function Group( by, options ) {
    ...
    that._sticky_groups = sticky_groups;
    ...
    that.a =
      // ToDo: remove deprecated option initial_groups, by December 31st 2018 at the latest.
      ( options.sticky_groups || options.initial_groups || [] )
      
      .map( function( sticky_group ) {
        var k = JSON.stringify( sticky_group );
        
        sticky_groups[ k ] = true;
        
        groups[ k ]
          = sticky_group
          = extend_2( {}, sticky_group )
        ;
        
        sticky_group[ content ] = [];
  ...
  Set.Build( 'group', Group, {
    _emit_on_complete: function( options ) {
      // de&&ug( get_name( this, '_emit_on_complete' ), { options: options, has_more: has_more( options ) } );
    ...
    _remove: function( values, options ) {
      var that    = this
        ...
        , content = that._content
        , groups  = that._groups
        , sticky_groups  = that._sticky_groups
        , removed_groups = that._removed_groups
        , updated_groups = that._updated_groups
      ;
