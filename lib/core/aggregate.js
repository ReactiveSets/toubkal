...
( 'aggregate', [ './stateful' ], function( rs ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      - **options** (Object): optional @@pipelet:set() options. Do not set
        ...
        Additional options:
        - **sticky_groups** (Object []): Optional groups that will be
          emitted and initialized with zero count and default measures.
          
          These groups are never removed even when their counts go back
          to zero.
          
          Each sticky group must be defined with the same attributes
          as defined in dimensions.
          
          When sticky groups are defined, dimensions should never change
          because sticky groups cannot change.
          
          If there are no dimensions, i.e. when aggregating all source
          values into a single group, *sticky_groups* should be defined
          as an Array of one empty Object: ```[ {} ]```.
          
          See also *sticky_groups* option of pipelet group().
      ...
      - Count the number of cities per state as well as total
        ...
        we also add a sticky group for the state of Danemark:
        ...
        cities
          .aggregate( population, by_state, { sticky_groups: [ { state: "Danemark" } ] } )
        ;
        ```
      ...
      providing a sticky group even if there are no cities in that set:
      
        ```javascript
        var population = [ { id: "population" } ];
        
        cities
          .aggregate( population, [], { sticky_groups: [ {} ] } )
        ;
        ```
  ...
  */
  function Aggregate( measures, dimensions, options ) {
    var that          = this
      , sticky_groups = options.sticky_groups
      , name
      , _dimensions
      , _measures
    ;
      ...
      this._merge = function( aggregates, options, operation ) {
        ...
        for ( i = -1; k1 = keys1[ ++i ]; ) {
          ...
          if ( g0 ) {
            ...
            if ( g1._count || ! all_default || sticky && sticky[ k1 ] ) {
