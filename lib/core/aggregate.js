...
( 'aggregate', [ './stateful' ], function( rs ) {
  ...
  p._add = p._remove = p._update = function( _, options ) {
    ...
    that._output.fetch_all( function( dimensions ) {
      that.build_group( dimensions, function( dimensions ) {
        that.aggregate.dimensions_changed( dimensions, options );
      } );
    } );
  ...
  */
  function Aggregate( measures, dimensions, options ) {
    ...
    // ToDo: Aggregate_Dimensions should become Input
  ...
  Set.Build( 'aggregate', Aggregate, {
    ...
    dimensions_changed: function( dimensions, options ) {
      var that     = this
    ...
    }, // Aggregate..dimensions_changed()
    
    _measures_changed: function( measures, options ) {
      this.build_merge( measures );
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        Aggregate..build_merge( measures )
        
        Builds merge() method new measures
    */
    build_merge: function( measures ) {
    ...
    */
    _add: function( values, options ) {
      ...
      if ( that._aggregates ) return that.merge( a, options, 'add' );
    ...
    */
    _remove: function( values, options ) {
      this.merge( this._aggregate( values ), Options.forward( options ), 'remove' )
    }, // Aggregate.._remove()
