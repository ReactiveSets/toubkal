  ...
  */
  ...
  /* --------------------------------------------------------------------------
    ...
    - **hashes** (Object): groups by key, ```{ key: group }``` will be added
      to it.
    - **positions** (Object): positions by ```key``` of groups in
      ```groups```, as well as positions by ```key``` of group's
      ```key``` in ```keys```, ```{ key: added_position }``` will
      be added to it.
    - **key** (String): group's key, indexing hashes and positions.
    - **group** (Object): to push to ```groups``` at new position in
      ```positions``` indexed by ```key``` and to add to ```hashes```
      indexed by ```key```
  */
  function add_group( groups, keys, hashes, positions, key, group ) {
