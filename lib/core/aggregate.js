...
( 'aggregate', [ './pipelet' ], function( rs ) {
  ...
  /* -------------------------------------------------------------------------------------------
     de&&ug()
  */
  var de = false, ug = RS.log.bind( null, 'aggregate' );
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      @description:
        ...
        Groups source values by dimensions and compute aggregate measures for each group.
        
        This provides an equivalent to SQL GROUP BY clause.
        
        Example: Count the number of cities per state as well as total population by state:
        
        ```SQL
        SELECT state, count(*), sum( population ) FROM cities GROUP BY state
        ```
        
        Which can be acomplished with aggregate():
        
        ```JavaScript
        source.aggregate( [ { id: "population" } ], [ { id: 'state' } ] )
        ```
      ...
      @parameters:
        - measures (Pipelet or Array): each measure is defined by attributes:
          ...
          - type (String): tells how this measures aggregates values. Default
            is "sum", possible values are:
            - "sum": Sum all values
        ...
        - aggregate (Pipelet or Array): each dimension is defined by
          attributes:
          - id (String): source attribute name
        
        - options (Object): optional @set() attributes
  */
  function Aggregate( measures, dimensions, options ) {
    this._output || ( this._output = new Aggregate.Output( this, options && options.name ) );
  ...
  Set.Build( 'aggregate', Aggregate, {
    dimensions_changed: function( dimensions ) {
      ...
      if ( measures ) {
        measures._output.fetch_all( function( measures ) {
          measures.build_reduce_groups( measures, dimensions, rebuild_aggregates.bind( that ) );
        } );
      }
    }, // dimensions_changed()
