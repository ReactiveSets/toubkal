  ...
  Loggable.subclass( 'Aggregate_Measures', Aggregate_Measures, {
    ...
    build_reduce_groups: function( measures, dimensions ) {
      var u, i, m, ids = [], id
        ...
        , vars = [ 'keys = groups.keys', 'key', 'out = []', 'hash = {}', 'v', 'u' ]
        , init_measures = ''
        , first = '', inner, last
        , l = measures.length
      ;
      ...
      var code = new Code()
        ._function( 'this.aggregate._reduce_groups', null, [ 'groups' ] )
          ...
          dl ? code._for( 'var j = -1', 'key = keys[ ++j ]' ) : code.add( 'key = keys[ 0 ]', 1 );
          
            code._var( 'g = groups[ key ]', 'i = -1', 'l = g.length' );
  ...
  */
  ...
  Set.Build( 'aggregate', Aggregate, {
    ...
    _dimensions_changed: function( dimensions, options ) {
      ...
      // If this is the first time, during initialization, measures is not defined yet
      if ( measures ) { // ToDo: do not build reduce groups until measures have been fetched once
