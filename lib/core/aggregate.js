  ...
  Set.Build( 'aggregate', Aggregate, {
    ...
    _dimensions_changed: function( dimensions, options ) { // safe from tested safe callers
      var that            = this
        ...
        , key             = that._set_key( dimensions.length && dimensions )
      ;
      ...
      if ( measures_values ) {
        ...
        that._input.fetch_all( function( values ) {
          ...
          options = extend( {}, options, { new_key: key } );
          
          // ToDo: emit new key with the next non-empty emission of groups, or implement set_key operation as defined in stateful.js
