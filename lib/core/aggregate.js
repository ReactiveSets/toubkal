  ...
  Set.Build( 'aggregate', Aggregate, {
    ...
    _dimensions_changed: function( dimensions, options ) { // safe from tested safe callers
      var that            = this
        ...
        , key             = that._key = dimensions.length ? dimensions : [ 'id' ]
      ;
      ...
      if ( measures_values ) {
        ...
        that._input.fetch_all( function( values ) {
          ...
          if ( removes ) {
            t = that._transaction( 2, options, function( _t ) { t = _t } );
            
            next_options();
            
            that.__emit_remove( removes, options );
          ...
          options = extend( {}, options, { new_key: key } ); // ToDo: implement and add test for new_key propagation downstream
          
          that.__emit_add( aggregates.groups, options );
