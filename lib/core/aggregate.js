...
( 'aggregate', [ './stateful' ], function( rs ) {
  ...
  */
  ...
  Loggable.subclass( 'Aggregate_Measures', Aggregate_Measures, {
    // ToDo: mitigate remaining injection attacks
    build_reduce_groups: function( measures, safe_dimensions ) {
      ...
      if ( ml ) {
        ...
        for ( i = -1; m = measures[ ++i ]; ) {
          safe_ids.push( safe_string( id = m.id ) );
          ...
          */
          ...
          switch( m.type ) {
            ...
            default:
              // ToDo: emit error
              log( name + 'unsupported measure type:', m.type );
          ...
          */
          ...
          m.no_null || ( safe_v = '( u == ( ' + safe_v + ' ) ? ' + m[ default_s ] + ' : v )' );
          
          switch( m.type ) {
            case 'sum':
              first += measure_var + ' += ' + safe_v;
          ...
          ._while( 'key = keys[ ++j ]' )
            ...
            .line( dl ? 'out.push( hash[ key ] = {' : 'out = {' );
              ...
              if ( dl )
                ...
                code.line( safe_indent + 'id: 1,' );
              
              // Add aggregated measures
              if ( ml )
                for ( i = -1; ( safe_id = safe_ids[ ++i ] ) !== u; )
                  code.line( safe_indent + safe_id + ': ' + measures_vars[ i ] + ',' ); // ToDo: safe this
