...
( 'aggregate', [ './stateful' ], function( rs ) {
  ...
  */
  ...
  Loggable.subclass( 'Aggregate_Dimensions', Aggregate_Dimensions, {
    ...
    fetch_safe: function( receiver ) {
      this.input.fetch_all( function( dimensions ) {
        receiver( dimensions.map( function( d ) { return safe_string( d.id ) } ) ) // safe, tested
      } );
    }, // Aggregate_Dimensions..fetch_safe()
    
    build_group: function( safe_dimensions ) {
      if ( safe_dimensions.length ) {
        var safe_key_code = '"#" + v[ ' + safe_dimensions.join( ' ] + "#" + v[ ' ) + ' ]'; // safe, from tested variables
  ...
  p._add = p._remove = p._update = function( _, options ) {
    if ( ! Options.has_more( options ) ) {
      ...
      that.fetch_safe( function( safe_dimensions ) { // safe from tested safe method
  ...
  Loggable.subclass( 'Aggregate_Measures', Aggregate_Measures, {
    build_reduce_groups: function( measures, safe_dimensions ) {
      ...
      if ( ml ) {
        ...
        for ( i = -1; measure = measures[ ++i ]; ) {
          safe_ids.push( safe_string( measure_id = measure.id ) ); // safe, tested
            ...
            .line( dl ? 'out.push( hash[ key ] = {' : 'out = {' ); // safe, constant strings
              ...
              if ( dl )
                for ( i = -1; safe_dimension = safe_dimensions[ ++i ]; )
                  safe_code.line( safe_indent
                    ...
                    + '[ ' + safe_dimension + ' ]'
                    + ','
                  ); // safe, from tested variables and constant strings
              ...
              if ( ml )
                for ( i = -1; ( safe_id = safe_ids[ ++i ] ) !== ___; )
                  safe_code.line( safe_indent + safe_id + ': ' + safe_measures[ i ] + ',' ); // safe, from tested variables
    ...
    _update_measures: function( measures, options ) {
      if ( ! Options.has_more( options ) ) {
        ...
        that.aggregate._dimensions.fetch_safe( function( safe_dimensions ) { // safe from tested safe method
  ...
  */
  ...
  Set.Build( 'aggregate', Aggregate, {
    ...
    _dimensions_changed: function( safe_dimensions, options ) { // safe from tested safe callers
      var that            = this
      ...
      if ( measures_values ) {
        ...
        that._input._fetch( function( values ) {
          ...
          that._key = safe_dimensions; // ToDo: we need dimensions here, not safe strings
          
          // ToDo: emit key change between remove and add, should be emitted and used by downstream set
