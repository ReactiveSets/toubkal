  ...
  Loggable.subclass( 'Aggregate_Dimensions', Aggregate_Dimensions, {
    ...
    build_group: function( dimensions ) {
      if ( dimensions.length ) {
        ...
        this.aggregate._group = group_all
      ;
    } // Aggregate_Dimensions..build_group()
  } ); // Aggregate_Dimensions instance methods  
  
  function group_all( values ) {
    return { groups: { '#': values }, keys: [ '#' ] }
  } // group_all()
  ...
  */
  ...
  Loggable.subclass( 'Aggregate_Measures', Aggregate_Measures, {
    ...
    build_reduce_groups: function( measures, dimensions ) {
      ...
      if ( ml ) {
        ...
        // ToDo: merge 2 for loops on measures
        for ( i = -1; m = measures[ ++i ]; ) {
          ids.push( id = m.id );
          
          operations[ m.type || ( m.type = 'sum' ) ] = true;
          
          // ToDo: allow source to specify other defaults, checked against possible injection attacks
          switch( m.type ) {
            case 'max':
              m[ default_s ] = -Infinity;
              m.init = [];
            break;
            ...
            case 'sum':
              m.init = m[ default_s ] = 0;
            break;
          }
          
          id = '_' + id;
          
          dl && measures_ids.push( id );
          
          init_measures.push( id + ' = ' + m.init );
        }
        ...
        for ( i = -1; m = measures[ ++i ]; ) {
          ...
          */
          ...
          switch( m.type ) {
            ...
            default:
              // ToDo: do not throw, emit error, ignore measure
      ...
      // Generate optional variables
      dl > 1 && vars.push( 'o' );
      
      dl
        ? vars.push( 'j = -1', 'key' )
        : vars.push( 'key = keys[ 0 ]' )
      ;
      
      if ( dl || ml ) {
        var g = 'g = groups' + ( !dl && ml ? '.groups': '' ) + '[ key ]';
        
        vars.push( dl ? 'g' : g );
      }
      
      if ( ml ) {
        push.apply( vars, dl ? measures_ids : init_measures );
      ...
      var code = new Code()
        ._function( 'this.aggregate._reduce_groups', null, [ 'groups' ] )
          ...
          ._while( 'key = keys[ ++j ]' )
            ...
            code
            .line( indent + '_count: ' + ( ml ? 'l' : dl ? 'g.length' : 'groups.groups[ key ].length' ) )
            .add( ( dl ? '} )' : '} ]' ), 1 );
          
          dl && code.end(); // end while dimensions
          
          code.add( 'return { groups: out, keys: keys, hash: hash }' )
        .end( '_reduce_groups()' )
      ;
  ...
  /* --------------------------------------------------------------------------
      ...
      - **measures** (Pipelet or Array of Objects): optional.
        ...
        - **init**: initialization value for each group before computation
        of source values. The value depends on type:
          - ```"sum"```: 0
          - ```"min"```: [], the array of all source values
          - ```"max"```: [], the array of all source values
