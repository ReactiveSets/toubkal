    ...
    */
    update_upstream_query: function( changes, destination, transformed ) {
      ...
      if ( transformed
        ...
        // ToDo: in synchronized mode, delay update if there are fetches in progress
    ...
    _update_subscription: function( changes, options, source, then ) {
      ...
      if ( source || ( source = input.source ) ) {
        ...
        var removes = changes[ 0 ]
          ...
          , transaction = {}
          , sync    = fetch //&& input.options.synchronous // ToDo: remove sync variable once fully validated with applications
          , _t
        ;
        ...
        if( ! count && options && ( _t = options._t ) && _t.forks && ! _t.more ) {
        ...
        // ToDo: remove update_upstream_query() once sync is always true:
