  ...
  Loggable.subclass( 'Pipelet', Pipelet, {
    ...
    */
    ...
    pop_scope: function() {
      ...
      that._scope = that._scope.parent;
    ...
    */
    set_output: function set_output( output_name, scope ) {
      scope = scope || this._scope || outputs;
    ...
    output: function output( output_name, scope ) {
      return ( scope || this._scope || outputs )[ output_name ]
        || this._error( 'output', 'not set, name: ' + output_name )
      ;
    } // output()
  } ); // Pipelet instance methods
  
  // global scope for Pipelet..set_output() and Pipelet..output()
  var outputs = {};
  ...
  Pipelet.Add = function( name, pipelet_factory ) {
    ...
    function singleton() {
      ...
      function singleton_factory( source, parameters ) {
        var singletons = get_singletons( source )
    ...
    function multiton( get_name ) {
      ...
      function multiton_factory( source, parameters ) {
        var _name      = name + '#' + get_name.apply( source, parameters )
          , singletons = get_singletons( source )
    ...
    function get_singletons( source ) {
      var scope = source._scope || Pipelet.scope;
      
      return scope.singletons = scope.singletons || {};
    } // get_singletons()
