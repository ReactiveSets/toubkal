  ...
  Event_Emitter.subclass( 'Plug', Plug, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        The call @@method:Plug.._upstream_query_changes() if defined that
        may make any transformation for changes to forward upstream.
    ...
    update_upstream_query: function( changes, destination ) {
      ...
      if ( ! query_transform
        ...
        if ( _upstream_query_changes = that._upstream_query_changes ) {
          // ToDo: document Plug.._upstream_query_changes()
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        @short Virtual method, updates local @@class:Output query tree
      ...
      if ( query_transform ) {
        ...
        if ( query ) {
          ...
          if ( ! ( _query.length || _query_changes && changes_count( _query_changes ) ) ) {
            // Nul query with subscription changes
            
            // ToDo: isn't this case covered by the nul query case bellow?
            
            query_updates && push__query_changes(); // ToDo: do we need this if there are no query changes?
            
            // ToDo: forward upstream subscription changes?
