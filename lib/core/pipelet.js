...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  Plug.subclass( 'Input', Input, {
    ...
    */
    add_source: function( source, options ) {
      // ToDo: modifying parent pipelet scope should be controlled by parent
      if ( source.pipelet && source.pipelet._scope ) this.pipelet._scope = source.pipelet._scope;
  ...
  */
  ...
  Loggable.subclass( 'Pipelet', Pipelet, {
    ...
    */
    ...
    /* ---------------------------------------------------------------------------------------------
        @method Pipelet..set_scope( pipelet_or_scope )
        
        @section reference
        
        @short Sets the scope of current pipelet instance
        
        @description:
        If pipelet_or_scope is defined, it is used to replace the scope of the current pipelet,
        otherwise a new scope is created with the current scope as prototype, allowing to continue
        to provide access to previous scope attributes.
        
        @parameters:
        - pipelet_or_scope:
          - (Pipelet): use this pipelet scope to set current scope
          - (Object): use this object to set current scope
        
        @returns (Pipelet): current pipelet instance
    */
    set_scope: function( pipelet_or_scope ) {
      var that   = this
        , parent = that._scope
      ;
      
      pipelet_or_scope
        ? that._scope = pipelet_or_scope._scope || pipelet_or_scope
        : ( that._scope = Object.create( parent ) )._parent = parent
      ;
  ...
  */
  var rs = new Pipelet( { name: 'rs' } );
  
  // give rs an empty scope
  rs.set_scope();
    ...
    */
    _update: function( updates, options ) {
      ...
      return this._transaction( l * 2, options, function( t ) {
        ...
        if ( moves ) {
          while ( l-- )
            this._update_value( t, updates[ l ][ 0 ], updates[ l ][ 1 ], moves[ l ] )
          ;
        } else {
          var i = -1;
