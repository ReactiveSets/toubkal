  ...
  Event_Emitter.subclass( 'Plug', Plug, {
    ...
    */
    _fetch: function( receiver, query, query_changes, destination, __query_changes ) {
      ...
      // ToDo: return abort_this_fetch
      // ToDo: implement abort for this fetch, should cancel query changes if any and abort upstream
      ...
      if ( ! ( source.is_namespace == true ) ) { // == true fails with function Pipelet..is_namespace(), as intended
        ...
        if ( source._fetch )
          ...
          /*
            ...
            _update_subscription() (see filter) has fully completed.
            
            This requires to cancel ongoing subscription update by:
            - cancelling fetches as already done bellow
            - preventing query updates application so that
              _remove_source_subscription() does not attempt to remove
              cancelled source subscription update
          */
