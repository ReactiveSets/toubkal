  ...
  */
  ...
  function Plug( pipelet, options ) {
    ...
    if ( synchronous ) {
      ...
      /*
        query_updates (Array): list of query updates (Object):
          - done:
  ...
  Event_Emitter.subclass( 'Plug', Plug, {
    ...
    update_upstream_query: function( changes, destination, transformed ) {
      ...
      if ( transformed
        ...
        if ( synchronous && ( query_updates = that.query_updates ).length ) {
          ...
          query_updates.push( { done: apply_upstream, changes: changes } );
        } else {
      ...
      function apply_upstream( changes ) {
        var update_upstream_query = source && source.update_upstream_query;
    ...
    */
    _fetch: function( receiver, query, query_changes, destination, __query_changes ) {
      ...
      query_updates
        && query_updates.push( __query_changes = { done: 0, changes: __query_changes } )
      ;
        ...
        if ( no_more && _update_query ) {
          if ( query_updates ) {
            ...
            while ( ( query_update = query_updates[ 0 ] ) && ( done = query_update.done ) ) {
              var changes = _update_query.call( that, query_updates.shift().changes, destination )
