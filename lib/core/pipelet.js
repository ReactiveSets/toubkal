...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  var de = true, ug = log.bind( null, 'pipelet' );
  ...
  */
  function Input( p, name ) {
    ...
    // incoming subscriber index in current operation
    ...
    */
    add_source: function( source, options ) {
      ...
      if ( source instanceof Output ) {
        ...
        throw new Error( this._get_name( 'add_source' ) + ', expected instance of Output or Array' );
      }
    ...
    */
    remove_source: function( source, options ) {
      ...
      if ( source instanceof Output ) {
        ...
        throw new Error( this._get_name( 'remove_source' ) + ', expected instance of Output or Array' );
      }
    ...
    _transactions_remove_source: function( output ) {
      ...
      that.transactions
        ...
        .forEach( function( tid ) {
          ...
          de&&ug( that._get_name( '_transactions_remove_source' ) + ', removing pipelet connection in the middle of transaction, tid:', tid );
    ...
    */
    fetch_source: function( receiver, _query ) {
      ...
      function rx_filtered( values, no_more ) {
        // de&&ug( 'fetch_source(), before filtering, values: ' + log.s( values ) + ', no_more: ' + no_more );
        
        if ( values && values.length ) values = filter( values );
        
        // de&&ug( 'fetch_source(), after filtering values with query: values: ' + log.s( values ) );
        
        if ( no_more || ( values && values.length ) ) receiver.call( p, values, no_more );
      } // rx_filtered()
      
      function get_name( that ) {
        return name || ( name = that._get_name( "fetch_source" ) );
      } // get_name()
    }, // fetch_source()
    ...
    fetch_source_all: function( receiver, query ) {
      ...
      this.fetch_source( rx_concat, query );
  ...
  Plug.subclass( 'Output', Output, {
    ...
    */
    fetch_all: function( receiver, query ) {
      var that = this, p = this.pipelet, out, name;
      ...
      out || receiver || this.error( 'fetch_all', 'is asynchronous and no receiver function was provided' );
      ...
      function rx_concat( values, no_more ) {
        if ( out ) that.error( 'fetch_all', 'received extra chunck after no_more' );
    ...
    */
    emit: function( event_name, values, options ) {
      ...
      // ToDo: move this code to _route() and merge emit() with _route()
      // ToDo: be lazy, do not invoke _emit_event() unless there are listeners
      /* this[ event_name + '_listeners' ] && */ this._emit_event( event_name, [ values, options ] );
      
      more || /* this.complete_listeners && */this._emit_event( 'complete', [ options ] );
    ...
    _route: function( operation, values, options ) {
      // ToDo: bypass query tree router if all queries are greedy. if ( this._all_greedy ) return this.destinations.forEach( function( input ) { operation == 'clear' ? input.clear( options ) : input[ operation ]( values, options ) } )
      ...
      var subscribers_operations = this.tree.route( operation, values )
        ...
        , name
      ;
      // ToDo: consider moving all transaction code to transactions.js through a single method call e.g. this.transactions.set_options( o, this.destinations, subscribers_operations )
      // ToDo: this.transactions.set_options() would return an updated list of subscribers to notify with proper options
      ...
      */
      ...
      if ( subscribers_operations.length ) {
        de&&ug( ( name = this._get_name( '_route' ) ) + ', subscribers_operations:', subscribers_operations.length );
        
        // Emit accumulated values to subscribers
        while ( subscribers_operation = subscribers_operations[ ++i ] ) {
          // ToDo: query tree.route(): stop emitting empty operations
          ...
          while ( r = subscribers[ ++j ] ) {
            if ( t = r.t ) {
              ...
              if ( --t.count == 0 ) t.t.end(); // done with this transaction
            } else {
              o = options;
            }
      ...
      if ( options && ( _t = options._t ) && ! _t.more ) {
        ...
        if ( forks && this.destinations.length ) {
          // If there are fork tags, we need to make sure than all downstream pipelets are notified of the end of the transaction
          // Each destination input in this.destinations must be notified once and only once of the transaction termination
          // This is accomplished by creating the Output_Transaction object for all destination inputs for which the transaction
          // was not yet started
          var output_transaction = transactions.get_transaction( tid ); // may create a transaction if it does not exist yet
          
          this.destinations.forEach( function( input ) {
            // Add this input transactions if not already added to this output transaction
            // ToDo: optimize to not add the remove destination inputs if input.transactions is not yet in the transaction
            // ToDo: input.transactions.get_from_tid( tid ) || input.transactions.end_from_tid( tid );
            
            input.transactions.get_from_tid( tid ) ||
            
            output_transaction
              ._add_destination( input.transactions, forks );
            ;
          } );
        }
        
        if ( transactions.count ) {
          de&&ug( name + ', terminating transaction, inputs:', transactions.count );
          
          // This transaction is terminated here
          // Notify all inputs associated with an Output_Transaction for this transaction
          transactions
            .get_terminated_input_transactions( options )
            
            .forEach( function( input_transactions ) {
              de&&ug( name + ', terminated input:', input_transactions.container._get_name() );
              
              input_transactions.container.add( [], options )
            } )
          ;
        }
      }
  ...
  */
  function Pipelet( options ) {
    ...
    this._input  || ( this._input  = new Pipelet.Input ( this, options.name ) );
    this._output || ( this._output = new Pipelet.Output( this, options.name ) );
  ...
  Root.subclass( Pipelet, {
    /* ------------------------------------------------------------------------
       ...
       traces. Optionally, if a name is provided assume this is a method name.
  ...
  */
  Pipelet.set_default_options = function set_default_options( f, source, parameters, defaults ) {
    parameters = slice.call( parameters, 0 );
    
    var default_options = f.default_options
    ...
    switch( typeof default_options ) {
      ...
      default:
      throw new Error( name() + 'expected function or object for default_options' );
    }
    ...
    if ( l === fl ) {
      ...
      if ( typeof last !== 'object' ) {
        fatal( 'expected last parameter to be an options object but found ' + typeof last );
      }
      
      parameters[ l ] = extend_2( options, last );
    } else if ( l < fl ) {
      // Set options as last expected parameter of constructor or composition
      parameters[ fl - 1 ] = options;
    } else {
      fatal( 'too many parameters' );
    }
    
    // de&&ug( name() + 'options:', parameters[ f.length - 1 ] );
    ...
    function fatal( message ) {
      throw new Error( name()
        + message
        + ', expecting ' + fl + ' parameters max'
      );
    } // fatal()
    
    function name() { return 'set_default_options(), ' }
  ...
  */
  Pipelet.subclass = function( derived, methods ) {
    RS.subclass( this, derived, methods );
  ...
  */
  ...
  /* --------------------------------------------------------------------------
     ...
     Architect Usage, displays sales from USA in a table:
       rs.file( 'sales' ).from_USA().table( '#sales_from_usa' );
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     rs.encapsulate() to prevent possible surprises (see Warning bellow).
     ...
     Parameters:
       ...
       - composition : (function) this is the constructor for the composed pipelet that should
           ...
           - parameters: optional parameters to the pipelet coming from the invocation of
             the pipelet
           - options   : coming from the pipelet options augmented with default options, see
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Parameters:
       ...
       - output: the output pipelet of the graph
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     A controllet is a pipelet such union(), filter(), pass_through(), that does not modify data
     and therefore does not need to process data operation events such as add, remove, update,
     clear.
  ...
  */
  function Controllet( options ) {
    ...
    this._input  || ( this._input  = new Controllet.Input ( this, name ) );
  ...
  */
  ...
  Union.Input = Controllet.Input.subclass(
    ...
    _add_source: function( source ) {
      if ( source ) {
        ...
        sources.indexOf( source ) != -1 && this.error( 'invalid source: already there' );
        
        sources.push( source );
        
        this.transactions.set_branches( sources.length );
      }
    ...
    _remove_source: function( source ) {
      ...
      position === -1 && this.error( 'source not found in this' );
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Dispatch is a basic building block for managing parallelism with Toubkal.
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Alters a set by altering all added and removed values by transform().
     
     Note: alter() is greedy unless option query_transform is a Function. When greedy, alter()
     will:
       - fetch all values from upstream regardless of downstream query
     ...
     Parameters:
       - transform:
           (Function): a function to transform values which signature is:
             ...
             !! The behavior of transform is different based on the value of alter()
             option 'no_clone':
               - if options no_clone is falsy, transform() MUST modify value which will have
               been cloned prior to calling transform(). Any returned value by transform()
               is ignored.
       ...
       - options: (Object)
         ...
         - query_transform (Function): A transform to alter query updates. If provided, alter
           becomes lazy.
     ...
     ToDo: alter(), provide option, or another pipelet, for assynchronous transform
  ...
  */
  function Alter( transform, options ) {
    ...
    if ( typeof transform == 'object' ) {
      ...
      if ( typeof transform != 'function' ) throw new Error( 'Alter(): transform must be an Object or a function' );
        ...
        update_upstream_query: function( removes, adds ) {
          ...
          if ( this.query_transform ) {
            Super.update_upstream_query.call( this
              , removes.map( query_transform )
              , adds   .map( query_transform )
            );
          }
    ...
    function ( Super ) {
      return {
        _fetch: function( receiver, query ) {
          ...
          if ( query ) {
            if ( query_transform ) {
              query = query.map( query_transform );
            } else {
  ...
  */
  function Set( a, options ) {
    ...
    if ( a && ! ( a instanceof Array ) ) throw new Error( 'Set(), first parameter must be an Array' );
    ...
    */
    _fetch: function( receiver, query ) {
      if ( query ) {
        ...
        de&&ug( ( name = 'Set ' + this._get_name( '_fetch' ) ) + 'query: ' + log.s( query ) );
      ...
      function send( values, no_more ) {
        ...
        if ( l || no_more ) { // filter out empty chunks
          // ToDo: split large result sets in chunks to allow incremental processing
          receiver( values, no_more );
        }
      }
    } // _fetch()
  ...
  Set.Build( 'unique_set', Unique_Set, {
    _add: function( values, options ) {
      ...
      while ( ++i < l ) {
        ...
        ( v.flow === 'error' || this.index_of( v ) === -1 ? added : duplicates )
          .push( v )
        ;
      }
  ...
  */
  ...
  Pipelet.Build( 'trace', Trace, {
    log: function( method, object ) {
      var u, s = 'rs "' + this._get_name() + '".' + method + '()';
    ...
    _input_add_source: function( source ) {
      this.pipelet.log( 'add_source', { source: source._get_name() } );
      
      return Pipelet.Input.prototype.add_source.call( this, source );
    }, // _input_add_source()
    
    _input_fetch_source: function( receiver, query ) {
      var u, s = this.source, that = this, name = "fetch_source", p = this.pipelet;
