  ...
  Loggable.subclass( 'Pipelet', Pipelet, {
    ...
    */
    _update: function( updates, options ) {
      ...
      if ( moves ) delete options.moves; // ToDo: make shallow copy of options first
      
      de&&ug( get_name( that, '_update' ) + 'updates:', l );
      
      t = that._transaction( l ? 2: 0, options );
      
      add_options = remove_options = options = t.next().get_emit_options(); // ToDo: implement Transaction..next_options()
      
      if ( --l ) {
        // there are more than one update ToDo: add test for more than one update
        // process all of them but last with first options
        for ( l -= 1; i < l; ) {
          update = updates[ ++i ]; // ToDo: add this into move_options() renamed next_update()
