  ...
  Plug.subclass( 'Input', Input, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       ...
       Parameters:
         - source (Output or Array of Obects): the source output to add or
    ...
    */
    _remove_source: function( p ) {
      this.source !== p && this.error( '_remove_source', 'this.source ( ' + this.source._get_name() + ' ) is not ' + p._get_name() );
  ...
  Plug.subclass( 'Output', Output, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       ...
       Removes the connection between this output and one of its destinations:
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       ...
       A "complete" event is also emited when emitted objects delimit the end
    ...
    */
    emit: function( event_name, values, options ) {
      ...
      // ToDo: be lazy, do not invoque _emit_event() unless there are listeners
  ...
  */
  function Dispatch( source, branches, branch_dataflow, options ) {
    ...
    var that = this
      ...
      , dispatcher = this.dispatcher = no_encapsulate ? source : rs.pass_through( options )
      , gatherer   = this.gatherer   = rs.union( [], { name: 'gatherer (union)' } )
    ;
    
    // ToDo: use greedy input port instead of adding set() in branches dataflow
    if ( ! branches.make_key ) branches = branches.set( [] );
    ...
    remove_branches: function( removed_branches, options ) {
      ...
      for( var i = -1; ++i < l; ) {
        ...
        if ( branch ) {
          ...
          branch.input && this.dispatcher._output.remove_destination( branch.input._input, { no_fetch: true } );
