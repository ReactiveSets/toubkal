...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     ToDo: consider a map() pipelet which is alter() with option no_clone, then remove option no_clone from alter()
  */
  ...
  */
  ...
  Pipelet.Build( 'trace', Trace, {
    ...
    _input_fetch_source: function( receiver, query ) {
      ...
      p._log( name, { source: s && s._get_name(), query: _query && _query.query } );
