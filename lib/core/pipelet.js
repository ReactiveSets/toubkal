...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      - **transactions_options** (Object): options for class Input_Transactions():
        ...
        - **tag** (String): **untag** alias, deprecated, throws an Error()
  ...
  */
  function Input( p, name, options, input_transactions ) {
    ...
    if ( options.tag )
      throw new Error( 'deprecated option tag is no-longer allowed' )
    ;
    
    that.untag = options.untag || null;
  ...
  Plug.subclass( 'Input', Input, function( Super ) { return {
    /* ------------------------------------------------------------------------
        @method Input..set_untag( tag )
        
        @short Sets tag to remove from input transactions
        
        @parameters
        - **tag** (String): optional transaction tag from an upstream
          fork tag dispatching operations over a number of concurrent
          bramches.
        ...
        If tag is specified this method sets ```this.untag``` then calls
        method Input_Transactions..set_untag() for ```this.transactions```.
        
        This may be called by an upstream class Controllet() instance
        to apply controllet's input tag to all its destinations, the
        controllet, appearing as the synchronization point from a
        programmer's perspective.
        
        ### See Also
        - Method Input..add_branches()
        - Method Controllet.Input..set_untag()
        
        ### ToDo
        ToDo: set_untag() allow to set and unset multiple tags
    */
    set_untag: function( tag ) {
      tag && this.transactions.set_untag( this.untag = tag );
    }, // Input..set_untag()
  ...
  /* --------------------------------------------------------------------------
      ...
      - **options** (Object): optional pipelet parameters:
        ...
        - **fork_tag** (String): @@output @@transaction @@tag for
          ...
          the recombining point where the option untag (bellow) is set to the
        ...
        - **tag** (String): was a deprecated alias of **untag**, now throws
          an Error() if set.
      ...
      - To retrieve and subscribe to output data change events:
        ...
        for details.
  */
  // ToDo: rename fork_tag option into tag
  ...
  function Pipelet( options ) {
    ...
    if ( options.tag )
      throw new Error( 'option tag was deprecated, you must now use option "untag" instead' )
    ;
    
    // Set tag to remove from input.transactions if any.
    // !! This must be done after that._output is set for Controllets which route tags to their destinations
    input.set_untag( options.untag );
    
    // Output transactions fork tag to add to all emitted operations' transactions if defined
    // ToDo: rename fork_tag into tag, deprecate fork_tag
  ...
  */
  ...
  Controllet.Input = Input.subclass(
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        @method Controllet.Input..set_untag( tag )
        
        @short Route to all destinations method Input..set_untag()
        
        @description
        Also sets ```this.untag``` to ```tag``` parameter if defined so that
    ...
    */
    set_untag: function( tag ) {
      tag && this
        [ route_to_destinations_s ]( 'set_untag', [ this.untag = tag ] )
    }, // Controllet.Input..set_untag()
  ...
  Controllet.Output = Output.subclass(
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        @description
        Sets @@tag of paramenter ```input``` to this controllet input untag
        attribute as set by method Controllet.Input..set_untag().
        
        Then calls method Output.._add_destination().
    */
    _add_destination: function( input ) {
      input.set_untag( this.source.untag );
