    ...
    */
    _fetch: function( receiver, query, query_changes, destination ) {
      ...
      if ( _update_query ) {
        ...
        // ToDo: optimize this, should only be done on Inputs eventually because Output Query_Tree does not optimize queries:
        _query_changes = _update_query.call( that, query_changes, destination );
        
        // undo query changes for now, will be applied later by rx() if any
        ...
        if ( _query_changes[ 0 ].length + _query_changes[ 1 ].length == 0 ) {
          ...
          _query_changes = _update_query = null;
        }
