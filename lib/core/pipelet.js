    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        - Removing source from destination then adding back a new source
          ...
          ```javascript
          var RS      = rs.RS
            , tid     = RS.uuid_v4() // a unique transaction identifier
  ...
  Plug.subclass( 'Output', Output, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        @parameters
        - **input** (@@class:Input\): the destination input to add
    ...
    _inject_branches: function( input, tid, branches, injected_branches ) {
      ...
      while( branch = branches[ ++i ] )
        if ( tid == branch.tid && input === branch.destination ) {
          if ( branch.output_transactions.move_to( to, input, tid ) ) {
            // terminate the transaction here, ToDo: add test that terminates a transaction here
            options = to.get_options( input, { _t: { id: tid } } );
            
            options._t.more || input.listen( 0, [], options );
          }
  ...
  */
  ...
  Union.Output = Controllet_Output.subclass(
    ...
    */
    ...
    _inject_branches: function( input, tid, branches, injected_branches ) {
      this.source.sources.forEach( function( source ) {
        var _inject_branches = source._inject_branches;
        
        _inject_branches
          && _inject_branches.call( source, input, tid, branches, injected_branches )
      } )
    }, // Union.Output.._inject_branches()
    
    _extract_branches: function( input, tid, branches ) {
      this.source.sources.forEach( function( source ) {
        var _extract_branches = source._extract_branches;
        
        _extract_branches
          && _extract_branches.call( source, input, tid, branches )
      } )
    } // Union.Output.._extract_branches()
