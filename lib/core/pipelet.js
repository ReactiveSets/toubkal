...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  Plug.subclass( 'Output', Output, {
    ...
    */
    _transactional_fetch: function( destination, query, options, next ) {
      ...
      /*
        ToDo: update upstream query with fetch query, but not the local query to allow synchronization
        of fetch and query update.
        
        This works if the upstream pipelet responding to fetch issues fetch responses consistent with
        emitted operations and that no pipelet in the pipeline between this pipelet and responding
        pipelet reorganzes the order of operations versus fetches.
        
        If this is the case, when fetch response and operations arrive at this pipelet they should still
        be consistent, allowing to update the local query synchronously to maintain consistency.
      */
      ...
      // ToDo: next() is deprecated, transaction is returned instead
  ...
  */
  ...
  Loggable.subclass( 'Pipelet', Pipelet, {
    /* ------------------------------------------------------------------------
        @method Pipelet.._add_input( output, Input, name, tag, methods )
