...
/* -------------------------------------------------------------------------------------------
   @manual Toubkal Pipelet Programer's Guide
...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
      @pipelet pipelet( options )
      
      @short Base pipelet for all pipelets
      ...
      @parameters:
        - options: (Object) optional parameters:
          ...
          _ _t_postfix (String): postifx to add to transaction objects 'id'
            attribute.
  ...
  */
  Pipelet.set_default_options = function set_default_options( constructor, source, parameters, defaults ) {
    ...
    if ( parameters_length == constructor_length && last_parameter ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      @short Get query updates from pipelet "from"
      
      @parameters:
        - from (Pipelet): the pipelet which inputs emit query updates that
          trigger "pull" events.
      ...
      @description:
        ...
        Emits adds and removes corresponding to "from" query updates.
        
        It allows to handle downstream query updates to create complex
        lazy pipelines, i.e. adding and removing pipelets from query
        updates.
        
      @examples:
        - Implement a multi-flow reactive cache:
        
          ```javascript
          rs.Compose( 'cache', function( source, options ) {
            var rs     = source.namespace()
              , output = rs.pass_through()
            ;
            
            var query_adds = rs
              .query_updates( output )
              ...
              .map( function( term ) {
                ...
                if ( v = term.flow ) out.flow = v;
                if ( v = term.id   ) out.id   = v;
