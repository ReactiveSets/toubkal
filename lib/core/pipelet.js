...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  function Alter( transform, options ) {
    // Use a greedy input, although we do not want the initial fetch
    this._input = this._input || new Greedy.Input( this, options && options.name );
    
    // Prevent initial fetch on connection, will only fetch after being fetched once from downstream
    this._input.no_fetch = true;
  ...
  Pipelet.Build( 'alter', Alter, {
    ...
    //
    // e.g.: if the alter function is set_flow() it could remove from incoming queries the flow key
