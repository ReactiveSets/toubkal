...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        - **receiver** (Function): signature:
          ```receiver( values, no_more, operation, updates, options, terminated_source )```
  ...
  */
  ...
  Loggable.subclass( 'Pipelet', Pipelet, {
    ...
    */
    // __transform: function( values ) { return values }, // Pipelet..__transform()
    ...
    */
    _update: function( updates, options ) {
      ...
      if ( transform && transform.length > 1 ) // transform accepts at least values and operation, so it can deal with updates
        return operation_transform( that, 'update', updates, options );
