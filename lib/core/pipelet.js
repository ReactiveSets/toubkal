...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  ...
  Event_Emitter.subclass( 'Plug', Plug, {
    ...
    */
    ...
    _transform_query_changes: function( query_changes ) {
      var that            = this
    ...
    /* ----------------------------------------------------------------------------------
        ...
        @description:
          ...
          queries calling _update_query() when fetched values are received.
          
          Assuming that fetch and other operations travel without changes in order, this
          will allow synchronization, maintaining dataflow consistency over asynchronous
          networks.
          
          This may be challenged by delayed transactions that only emit when complete.
          If a fetch happens in the middle of such a transaction, synchronization would
          no-longer be effective.
          
          So when fetched values arrive at a pipelet the solution is to also delay
          sending fetch response until all transactions are complete.
          
          Likewise as fetch results travel downstream towards its destination, it may be
          further delayed until other transactions complete at each intermediate step.
          
          This could be done naively by delaying all fetches until all transactions at a
          pipelet are complete.
          
          But this could lead to starvation if many transactions happen at that pipelet
          and are interlaced, there could always exist one or more ongoing transactions
          preventing fetches from delivering data.
          
          To prevent such stravation, a fetch might be delayed conditionally depending on
          the content of delayed transactions in progress matching fetch query.
    ...
    */
    _fetch: function( receiver, query, query_changes, destination ) {
      ...
      if ( query_transform ) {
        _query_changes = query_changes && that._transform_query_changes( query_changes );
        ...
        if ( query ) {
          ...
          if ( _query.length == 0
            && ! ( _query_changes && _query_changes[ 0 ].length + _query_changes[ 1 ].length )
          )
            return rx( [], true )
          ;
        }
      ...
      function rx( adds, no_more, removes, updates, options ) {
        ...
        no_more && _update_query && _update_query.call( that, query_changes, destination );
  ...
  Plug.subclass( 'Input', Input, function( Super ) { return {
    ...
    */
    add_source: function( source, options, then ) {
      ...
      if ( ! source.is_void ) { // ToDo: rename is_void by is_namespace or test if not a namespace
        ...
        if ( source._fetch ) {
          ...
          return that
            ._add_source             ( source ) // may throw if source already added, ToDo: use then to capture errors, abort adding, not throwing error
    ...
    */
    _source_error: function( function_name, message, source, then ) {
      ...
      this.error( function_name,
        ( message || 'expected instance of Output or Array, got a ' ) +
    ...
    */
    _add_source: function( source ) {
      ...
      s && this._source_error( '_add_source', 'already has a source: ', s );
    ...
    */
    _remove_source: function( source ) {
      ...
      s !== source
        && that._source_error( '_remove_source', 'expected ' + ( s._get_name ? s._get_name() : log.s( s ) ) + ' instead of ', source )
      ;
    ...
    */
    _fetch: function( receiver, query, query_changes, destination ) {
      ...
      if ( _update_query ) {
        ...
        if ( _query_changes[ 0 ].length + _query_changes[ 1 ].length == 0 ) {
          // There are no changes to forward upstream
          
          _query_changes = null;
        }
        
        query_changes = _query_changes;
      }
      
      Super._fetch.call( that, receiver, query, query_changes, destination );
    }, // Input.._fetch()
    ...
    */
    _update_query: function( changes ) {
      ...
      if ( rl + al ) {
        ...
        // log( this._get_name( '_update_query' ) + 'changes:', changes, ', query:', q.query, ', optimized:', q.optimized );
        ...
        // log( '_update_query:', changes, q.query, q.optimized );
      }
  ...
  Plug.subclass( 'Output', Output, {
    ...
    */
    _update_subscription: function( input, changes, options, then ) {
      ...
      // log( source._get_name( '_update_subscription' ) + 'changes:', changes, ', sync:', sync, 'count:', count );
      
      if ( rl * fetch ) {
        // log( '_update_subscription remove:', removes );
    ...
    /* ------------------------------------------------------------------------
        @method _transactional_fetch( destination, transaction, count, query, options, revert, then )
    ...
    */
    _transactional_fetch: function( destination, transaction, count, query, query_changes, options, revert, next ) {
      ...
      function receiver( adds, no_more, removes, updates, _options ) { // ToDo: use _options
        ...
        de&&ug( that._get_name( name ), { no_more: no_more, adds: al, removes: rl, updates: ul, options: _options } );
  ...
  */
  var authorized_name = [
    'adds', 'map', 'set', 'flow', 'join', 'last', 'next', 'form', 'once', 'beat', 'pick'
  ];
    ...
    */
    _fetch: function( receiver, query, query_changes, destination ) {
      var that    = this
        ...
        , count   = sources.length || ( sources = [[]], 1 ) // ToDo: consider always having a dummy source in Union
        , filter
        , paused  = []
      ;
      ...
      function fetch_source( source ) {
        ...
        if ( source._fetch ) {
          ...
          if ( query && source.length ) {
            filter = filter || new Query( query ).generate().filter;
    ...
    update_upstream_query: function( changes, destination ) {
      var sources = this.source.sources;
  ...
  */
  function Set( values, options ) {
    ...
    de&&ug( that._get_name( 'Set' ) + 'name:', options.name, 'length:', that.a.length );
  } // Set()
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      @description:
        When duplicates are added, they are emitted in bulk in the 'error' flow unless option
      ...
      @parameters:
        ...
        - options (Optional Object):
          - silent (Boolean): discard duplicates silently, do not emit errors, default is false.
  ...
  */
  ...
  Set.Build( 'unique', Unique_Set, function( Super ) { return {
    _add: function( values, options ) {
      ...
      if ( duplicates.length && ! that._options.silent ) {
        de&&ug( that._get_name( '_add' ) + 'discard duplicates, identities:', duplicates.map( that.make_key.bind( that ) ), ', key:', that._key );
    ...
    _remove: function( values, options ) {
      ...
      Super._remove.call( that, removed, options );
    }, // _remove()
    ...
    _update: function( updates, options ) {
      ...
      updates
        .forEach( function( update ) {
          var id = that.make_key( update[ 0 ] );
          
          // Does not verify identity of added value, see documented warning
          
          identities[ id ] && updated.push( update );
        } )
      ;
      
      Super._update.call( that, updated, options );
    },
  ...
  function Trace( name, options ) {
    ...
    function fetch( receiver, query, query_changes, destination ) {
      var name   = 'fetch'
        ...
        , output = source && source._get_name()
      ;
