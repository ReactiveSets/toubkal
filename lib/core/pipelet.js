  ...
  */
  function Pipelet( options ) {
    ...
    if( options.transactional )
      output.fetch_unfiltered = function( receiver ) { receiver( [], true ) }
    ;
  } // Pipelet()
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      - from (@@class:Pipelet\): which @@class:Output receives query updates
        with @@method:Plug..update_upstream_query().
      ...
      hijacking its @@method:Plug..update_upstream_query().
      
      Restores the original @@method:Plug..update_upstream_query()
      when its output is disconnected from all destinations, i.e.
      when its output method Output.._remove_destination() emits
      event ```"disconnected"```.
  ...
  */
  function Query_Updates( from, options ) {
    ...
    function update_upstream_query( changes, destination ) {
      ...
      // log( get_name( that, 'update_upstream_query' ) + 'to:', get_name( destination ), removes, adds );
