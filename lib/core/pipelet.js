  ...
  Plug.subclass( 'Input', Input, function( Super ) { return {
    ...
    */
    add_source: function( source, options, then ) {
      ...
      if ( ! source.is_void ) {
        that._source_position( source ) != -1
          && this.error( 'add_source', 'already has added source: ' + ( source._get_name ? source._get_name() : log.s( source ) ) );
        ;
    ...
    */
    remove_source: function( source, options, then ) {
      ...
      if ( ! source.is_void ) {
        ...
        /*
        that._source_position( source ) == -1
          && this.error( 'remove_source', 'does not have removed source: ' + ( source._get_name ? source._get_name() : log.s( source ) ) );
        ;
        */
