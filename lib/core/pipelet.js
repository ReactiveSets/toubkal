...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  ...
  function Alter_Input( p, name, query_transform, tag, input_transactions ) {
    ...
    if ( query_transform ) {
      // Input will be lazy and transform queries
      
      that._query_transform = query_transform;
    } else {
      // Query everything from upstream (be greedy)
      ...
      return {
        ...
        */
        update_upstream_query: function( changes ) {
          ...
          this._query_transform
            && Super.update_upstream_query.call( this, this._transform_query_changes( changes ) )
          ;
  ...
  function Alter_Output( p, name, query_transform ) {
    ...
    if ( ! query_transform ) {
      // alter() is greedy, need to filter fetched results
