...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  ...
  function Alter_Output( p, name, query_transform ) {
    ...
    if ( ! query_transform ) {
      // alter() is greedy, need to filter fetched results, tranformed by Alter.Input
      
      that.fetch_unfiltered = function( receiver, query_changes ) {
        that.source._fetch( receiver, null, query_changes, that );
      };
    }
  } // Alter_Output()
