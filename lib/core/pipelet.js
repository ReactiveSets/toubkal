  ...
  /* -------------------------------------------------------------------------------------------
     ...
     - transform (Function or Object): behavior varies with type:
       - (Function): a function to transform values which signature is:
         ...
         !! The behavior of transform depends on "no_clone" options:
         - if falsy, transform() MUST modify value which will have been cloned prior to
           calling transform(). Any returned value by transform() is ignored.
           
         - if truly, transform() MUST NOT modify value and MUST return either:
     ...
     - options (Object):
       - no_clone (Boolean): if true, values will not be cloned prior to calling
         transform() which must not make any side effects on value and must return
         zero or one Object value. Defaults to false.
     ...
     Example: Alter a source dataflow of stocks to produce a dataflow of P/E ratios
     from price and earnings attributes. Optionally provide query_transform for lazy
     behavior:
     
     - Without option "no_clone":
       stocks
     ...
     ToDo: alter(), allow to return any number of values with option no_clone.
     ...
     ToDo: alter(), implement map() deriving from alter() with no_clone behavior, replace option no_clone from alter() with _no_clone attribute
