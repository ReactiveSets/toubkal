...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  ...
  Loggable.subclass( 'Pipelet', Pipelet, {
    ...
    */
    ...
    /* ---------------------------------------------------------------------------------------------
        @method Pipelet..create_namespace( name )
        
        @short Create a child @namespace from this pipelet's namespace
        
        @parameter name (String): created namespace @loggable name
    ...
    */
    create_namespace: function( name ) {
      return create_namespace( assert_scope( this ), name );
    }, // create_namespace()
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      @description:
        ...
        from each other in scope-tree fashion. To create a child use the @@create_namespace()
        method.
        
        All @@pipelet instances are associated with a namespace. To retrieve this namespace
        use the @@namespace() method.
        
        To set the modify the namespace of a pipelet, use the @@set_namespace() method.
  */
  
  /* -------------------------------------------------------------------------------------------
      @function create_namespace( pipelet, name ) -> Namespace
      
      @short Create a @@namespace
      
      @description:
        Create a Namespace class, deriving from Pipelet and returns a singleton namespace
        instance.
        
      @parameters:
      - pipelet:
        - null or undefined: create root namespace @rs
        - (Pipelet): use pipelet's scope
        
      - name (String): used to compose a @@Loggable class name as 'Namespace.' + name
      
      @manual internal
  */
  function create_namespace( pipelet, name ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
      @class_method Pipelet.Add( name, pipelet_factory )
  ...
  */
  ...
  Greedy.Build( 'set', Set, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        - options (Object): optional operation metadata:
          ...
          - moves (Array): from upstream order()
        
        This implementation creates a transaction with twice as many operations
        as there are updates, then invokes _update_value() for each update.
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        @method _update_value( t, remove, add, move )
