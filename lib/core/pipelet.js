  ...
  Plug.subclass( 'Input', Input, {
    ...
    */
    _update_subscription: function( changes, options ) {
      ...
      de&&ug( input._get_name( '_update_subscription' ), 'adds:', adds, '- removes:', removes );
      
      if ( source && removes.length ) {
        o.operation = 'remove';
        o.count = adds.length ? 2 : 1
        
        transaction = source._transactional_fetch( input, removes, o );
      }
      ...
      input.update_upstream_query( removes, adds );
      
      if ( source && adds.length ) {
        o.transaction = transaction; // ToDo: add test to verify that second fetch uses transaction
