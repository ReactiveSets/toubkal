  ...
  Event_Emitter.subclass( 'Plug', Plug, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        - ToDo: update_upstream_query(): process transaction objects:
          ...
          downstream dataflow.
    */
    update_upstream_query: function( changes, destination, transformed ) {
      var that            = this
      ...
      if ( transformed
        ...
        if ( synchronous && ( query_updates = that.query_updates ).length ) {
          ...
          // ToDo: forward changes upstream immediately, calculating future changes similiraly to Input.
          // This will reduce latency added when pending fetches complete upstream
          query_updates.push( { done: apply_upstream, changes: changes, destination: destination } );
        } else {
          apply_upstream( that._update_query( changes, destination ) );
        }
      }
    ...
    */
    _fetch: function( receiver, query, query_changes, destination, __query_changes ) {
      ...
      function cancel() {
        // ToDo: implement fetch cancellation
      }
    ...
    // ToDo: use _cancel_fetches() from Input..remove_source()
