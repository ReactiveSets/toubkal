...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  ...
  Loggable.subclass( 'Pipelet', Pipelet, {
    /* ------------------------------------------------------------------------
        _add_input( output, Input, name, methods )
        
        Instanciate Input and connect it to output.
        ...
        - methods (Object): implementing _add(), _remove, _update(), _clear().
        
        Returns:
        - Input instance
    */
    _add_input: function( output, Input, name, methods ) {
      var input = new Input( methods, name )
    ...
    */
    ...
    /* ---------------------------------------------------------------------------------------------
        ...
        @short A javascript expression composed of @@pipelets.
        
        @description:
          A pipeline is a javascript expression starting with a Toubkal @@namespace, typically "rs",
    ...
    */
    ...
    /* ---------------------------------------------------------------------------------------------
        ...
        @short A reference to a set of @@singletons, @@multitons, and @@outputs names.
        ...
        @description:
          ...
          To set the namespace of a pipelet, use the @@Pipelet..set_namespace() method.
          
          A namespace can also be created as a repository for pipelets and @@compositions. Check
          @@Pipelet..create_namespace() for details.
    */
    ...
    */
    set_namespace: function( pipelet ) {
      //if ( this._scope == pipelet._scope ) return this;
      
      /*
      if ( this.__proto__ && pipelet._scope._namespace != rs ) {
        var proto = this.__proto__ = Object.create( this.__proto__ )
          , namespace = pipelet._scope._namespace.__proto__
        ;
        
        for ( var p in namespace ) {
          if ( p != 'constructor' && namespace.hasOwnProperty( p ) ) {
            log( 'set_namespace, property:', p );
            proto[ p ] = namespace[ p ];
          }
        }
      }
      */
      ...
      this._scope = assert_scope( pipelet )._scope;
    ...
    */
    ...
    /* ---------------------------------------------------------------------------------------------
        ...
        @short Create a child @namespace from this pipelet's namespace
        
        @description:
        If the created namespace is a repository, Compositions and Pipelets registered in this
        namespace are only accessible from other pipelets or compositions registered in this
        namespace. In order to connect these pipelets to upstream pipelets registered in
        another namespace one may use through(), e.g.:
        
        ```javascript
          
          rs = rs.create_namespace( 'example', true ); // this is a repository
          
          rs
            // Compositions read()  and draw() are defined in child rs repository
            .Compose( 'read', function( source, options ) {} )
            .Compose( 'draw', function( source, options ) {} )
            
            // set() defined in root namespace, is accessible from all namespaces
            .set()
            
            // draw() is not accessible after calling set() defined in root namespace
            
            .through(
              rs.read()
            )
            
            // Here is still the child namespace after going through read()
            .draw()
          ;
        ```
        
        Another option is to define a pipelet in the parent namespace that provides access
        ...
        ```javascript
          ...
          rs
            ...
            .example() // makes read()  and draw() accessible
    ...
    */
    Compose: function _Compose( name, options, composition ) {
      ...
      var constructor_apply = make_constructor_apply( composition )
        , union             = options.union
        , factory           = Pipelet.Add( name, instanciate, this.namespace() )
      ;
  ...
  */
  function create_namespace( pipelet, name, is_repository ) {
    Loggable.subclass.call( Pipelet, 'Namespace.' + name, Namespace );
    
    var namespace = new Namespace();
    
    log( 'create_namespace: ', Object.keys( namespace.constructor.class_names ) );
    
    return namespace;
    
    function Namespace() {
      var that   = this
        , parent = pipelet && pipelet._scope
        , scope  = that._scope = Object.create( parent )
      ;
      
      scope._namespace = that;
      
      // scope._scope_parent = parent;
      
      // This is not a real pipelet, we're only interested the repository of all defined pipelets
      // in Pipelet.prototype
      ...
      // provide a reference to RS, the dictionary of all Toubkal Pipelet Programmer classes
      that.RS = RS;
      
      that._repository = is_repository
        ? Namespace
        : parent ? parent._namespace._repository : Pipelet
      ;
    } // Namespace()
  } // create_namespace()
  ...
  */
  Pipelet.Add = function( name, pipelet_factory, namespace ) {
    ...
    log( 'Pipelet.Add(): name: ', name, ', namespace:', namespace ? Object.keys( namespace.constructor.class_names ) : null  );
    ...
    return ( namespace ? namespace._repository : Pipelet ).prototype[ name ] = factory;
    ...
    function singleton() {
      ...
      function singleton_factory( source, parameters ) {
        ...
        return assert_scope( pipelet
          ...
          : singletons[ name ] = _pipelet_factory( source, parameters )
        );
      } // singleton_factory()
    } // singleton()
    ...
    function multiton( get_name ) {
      ...
      function multiton_factory( source, parameters ) {
        ...
        return assert_scope( pipelet
          ? source.through( pipelet )
          : singletons[ _name ] = _pipelet_factory( source, parameters )
        );
      } // multiton_factory()
    } // multiton()
  ...
  */
  ...
  Controllet.Build( 'union', Union, {
    __emit: function( event_name, values, options ) {
      // ToDo: use _add_input() instead of new Pipelet()
      this._pipelet_input
        || ( this._pipelet_input = new Pipelet( { name: this._get_name() + ' input' } ).set_namespace( this ) ).through( this )
      ;
