        ...
        - options: (Object) For maximum performances, e.g. for very large dispachers with
          ...
          - loop: (Boolean) true if the dispatcher is has the gatherer union for input, so
              ...
              Beware that when looping:
                a) the output of the dispatcher includes dispatch source events because it
                  is also the gatherer.
                
                b) all branches outputs are connected to themselves which may yield infinite
                  recursions unless proper filter() non branch inputs and set_flow() on branch
  ...
  */
  ...
  function Dispatch( source, branches, branch, options ) {
    var that            = this
      , rs              = source.namespace()
      , name            = options.name + '-'
      , dispatcher_name = name + 'dispatcher'
      , loop            = options.loop
      , no_encapsulate  = loop || options.no_encapsulate
    ;
    ...
    that._branch   = branch;
    that._branches = {};
    that._gatherer = rs.union( [], { name: ( loop ? dispatcher_name + '-' : name ) +  'gatherer' } );
    that._dispatcher
      = no_encapsulate
      ? ( loop
      ? that._gatherer
      : source )
      : rs
        .pass_through( { name: dispatcher_name } )
        //.trace( dispatcher_name, { update_upstream_query: true } )
    ;
    
    // This pipelet is instancated as a composition, we need to add branches as a source explicity:
    that._add_source( branches );
    
    return loop || no_encapsulate
      ? that._gatherer
      : source.encapsulate( that._dispatcher, that._gatherer, options )
    ;
  } // Dispatch()
