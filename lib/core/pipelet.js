...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        - **options** (Object): optional, define optional attributes:
          ...
          - **singleton**: to make this @@composition a @@singleton or a @@multiton,
          i.e. having less instances than invocations:
            - falsy: this composition is neither a singleton not a multiton
            - ```true```: this composition is a singleton, it will have at most
            one instance per instance_scope (see instance_scope option bellow).
            - (Function): this composition is a multiton, for each invocation
            singleton() is called with the same parameters as composition() without
            the source pipelet as first parameter; singleton() should return a unique
            string for each singleton of the multiton. The first time a string
            is returned, composition() is called to instantiate the pipelet.
            Subsequent invocations of singleton() returning the same string
            reuse the instance returned by the first call of composition().
          
          - **instances_scope** (@@class:Pipelet or @@method:Pipelet..namespace\):
          optional global scope for singletons and multitons. The default
          scope is that of the source pipelet instantiating the singleton
          or multiton.
  ...
  */
  Pipelet.Add = function( name, pipelet_factory, namespace ) {
    ...
    function singleton( get_name, instances_scope ) {
      var multiton                = typeof get_name == function_s
      ...
      function singleton_factory( source, parameters ) {
        var _name   = multiton ? name + '#' + get_name.apply( source, parameters ) : name
          , scope   = instances_scope || source[ _scope_s ]
          , pipelet = scope[ _name ]
        ;
        
        de&&ug( singleton_or_multiton_s
          , name + '(),'
          + ( multiton ? ' instance name: ' + _name : '' )
          + ( !pipelet ? ' not' : '' )
        ...
        return assert_scope( pipelet
          ...
          : scope[ _name ] = pipelet_factory( source, parameters )
        );
      } // singleton_factory()
    } // singleton()
    ...
    */
    _fetch: function( receiver, query, query_changes, destination ) {
      ...
      function source_add( source ) {
        ...
        log( get_name( that, 'fetch#add_source' ) + 'source:', get_name( source ), ', count:', count, ', stack:', RS.stack().toString() );
