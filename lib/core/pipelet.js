...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  function Input( p, name, tag ) {
    ...
    tag && this.transactions.set_tag( tag );
  } // Input()
  ...
  Plug.subclass( 'Output', Output, {
    ...
    */
    emit: function( event_name, values, options ) {
      ...
      this.destinations.length && this._route( event_name, values, options );
    ...
    _route: function( operation, values, options ) {
      ...
      */
      if ( subscribers_operations.length ) {
        de&&ug( name + ', subscribers_operations:', subscribers_operations.length );
      ...
      if ( end_of_transaction ) {
        /*
          ...
          Otherwise (if no fork tag is in options), only notify downstream pipelets which have
          already received something from this transaction and are therefore waiting for the
          end of the transaction.
        */
        
        if ( terminated_inputs ) {
          // There are fork tags, we need to make sure that all downstream pipelets are notified
          // of the end of this transaction from this branch.
          
          // ToDo: find all downstream destination, as possibly forwarded by downstream controllet
          
          false && this.destinations.forEach( function( input ) {
            // Each destination input in this.destinations must be notified ONCE AND ONLY ONCE of
            // the transaction termination.
            if ( terminated_inputs.indexOf( input ) !== -1 ) return; // already notified above
            
            // terminate this transaction for this input
            var o = transactions.get_options( input.transactions, options );
            
            de&&ug( name + ', terminated branch, input:', input._get_name(), ', options:', o );
            
            o._t.more || input.add( [], o ); // only emit if there is no more expected from other branches
          } );
        }
        
        // ToDo: once the above ToDo is done, this should become and else-if
        
        if ( transactions.count ) { // there are ongoing output transactions
          ...
          transactions
            ...
            .forEach( function( destination ) {
              ...
              de&&ug( name + ', terminated transaction, input:', input_transactions.container._get_name(), ', options:', options );
  ...
  */
  function Pipelet( options ) {
    ...
    this._input  || ( this._input  = new Pipelet.Input ( this, options.name, options.tag ) );
