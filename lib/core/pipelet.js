...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  var de = false, ug = log.bind( null, 'pipelet' );
  ...
  */
  var authorized_name = [
    'map', 'set', 'flow', 'join', 'last', 'next', 'form', 'once', 'beat'
  ];
  ...
  */
  function Alter( transform, options ) {
    ...
    if ( options.no_clone ) {
      ...
      throw new Error( 'alter(): option no_clone is no-longer supported, use map() instead' );
    }
