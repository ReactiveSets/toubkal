...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  ...
  Plug.subclass( 'Input', Input, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       Input..add_source( source [, options ] )
    ...
    */
    add_source: function( source, options ) {
      ...
      // ToDo: use source._is_a( 'Namespace' )
      source.pipelet
        && this.pipelet.set_namespace
        && this.pipelet.set_namespace( source.pipelet )
      ;
      ...
      if ( source._is_a && source._is_a( 'Output' ) ) {
        ...
        this
          ...
          ._add_source_subscription( source, options )
        ;
    ...
    /* ------------------------------------------------------------------------
       Input..remove_source( source, options )
    ...
    */
    remove_source: function( source, options ) {
      ...
      if ( source._is_a && source._is_a( 'Output' ) ) {
        ...
        this
          ._remove_source             ( source ) // may throw if source not this source
          ._remove_source_subscription( source, options )
    ...
    /* ------------------------------------------------------------------------
       Input.._source_error( function_name, source )
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       Input.._update_source_array( source, options )
    ...
    */
    _update_source_array: function( source, options ) {
      if ( this._do_not_fetch( options ) ) return this; // Don't add or remove anything
      
      de&&ug( this._get_name( '_update_source_array' ) + 'source:', typeof source );
    ...
    /* ------------------------------------------------------------------------
        ...
        @short Update source query and fetch
        
        @parameters:
        - source (Output): upstream output subscribed-to
        
        - options (Object):
          - _t (Object): transaction object
          
          - no_fetch (Boolean): do not fetch if true, set to false by
            @@Input..fetch_source() on first fetch, means something is
            listening downstream.
    */
    _add_source_subscription: function( source, options ) {
      var that  = this
        , q     = that.query
        , query = q && q.query
      ;
      
      // ToDo: synchronize query_update() and _transactional_fetch()
      query && source.query_update( [], query, that );
      
      this._do_not_fetch( options )
        || source._transactional_fetch( that, query, options )
      ;
    ...
    /* ------------------------------------------------------------------------
        ...
        - options (Object):
          ...
          - operation (String): "remove", passed to @@Output.._transactional_fetch()
          
          - no_fetch (Boolean): do not fetch if true, set to false by
            @@Input..fetch_source() on first fetch, means something is
            listening downstream.
    */
    _remove_source_subscription: function( source, options ) {
      var that  = this
        , q     = that.query
        , query = q && q.query
      ;
    ...
    /* ------------------------------------------------------------------------
        @method Input.._do_not_fetch( options )
        
        @short True if pipelet should not fetch on connection events
        ...
        - no_fetch (Boolean): do not fetch if true, set to false by
          ...
          listening downstream.
    */
    _do_not_fetch: function( options ) {
      return this.is_lazy()
    ...
    /* ------------------------------------------------------------------------
        @method Input.._update_subscription( changes, options )
    ...
    */
    _update_subscription: function( changes, options ) {
      var input   = this
        , source  = input.source
        , removes, rl
        , adds   , al
      ...
      de&&ug( input._get_name( '_update_subscription' ), 'changes:', changes );
      ...
      if ( source ) {
        ...
        if ( rl ) {
          ...
          options.count = al ? 2 : 1
          
          transaction = source._transactional_fetch( input, removes, options );
        }
        ...
        rl + al && source.query_update( removes, adds, input );
        
        if ( al ) {
          options.transaction = transaction; // ToDo: add test to verify that second fetch uses transaction
          
          source._transactional_fetch( input, adds, options );
        }
      }
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        @method Input..update_upstream_query( changes )
        
        @short Update upstream source query with minimum changes.
        
        @description:
          Apply removes and adds to input query, find minimum set of changes
          calling @@Input.._update_query(), if there are any changes apply
          them upstream calling source @@Output..query_update()
        
        @parameter changes (Array): changed query terms:
    ...
    */
    update_upstream_query: function( changes ) {
      ...
      removes.length + adds.length
        && source
        && source.query_update( removes, adds, input ) // ToDo: query_update(): use changes instead of removes and adds
      ;
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        Input..fetch_operations( receiver [, query] )
        
        ToDo: implement fetch_operations().
        
        API in progress: this method is meant to eventually replace
        fetch_source(), providing additional services that could not be
        implemented with fetch_source() API. The new API should allow:
        
        - to emit all types of operations, in addition to add, allow
          removes, update, and clear operations;
        
        - operations options, e.g. with transactions;
        
        - synchronization between fetch() and incoming operations to prevent
          dataloss and duplicates;
        
        - fetched operations to participate in transactions just as if
          these had been emitted by the pipelet, handling fork tags. If a
          pipelet has a fork tag set, all fetched values happening concurently
          from several branches should be part of the same transaction;
        
        - query to specify a starting point for desired operations, allowing
          reconnections and optimized replication;
        
        - to be backward-compatible with source outputs that do not implement
          fetch_operations().
        
        ---
        
        Fetches operations from source, possibly in chunks.
        
        If source output implements fetch_operations() use it, otherwise use
        _fetch() to emit a single add operation.
        
        This method should not be called directly but may be overloaded.
        
        Parameters:
        - receiver (Function): signature receiver( operations, no_more ):
          - operations (Array): for each operation provide a first String
            value for the operation name, e.g. 'add', 'remove','update',
            followed by an Array of values, followed by an options Object.
            
            If the operation is 'update', values come in couples where the
            first is the removed value, and the second is the added value.
            
            Example: operations containing an add with 3 values and options, a
            remove with 1 value and options, an update with 2 updates and
            options, and a final remove with 2 values and options.
            ...
            options may include a Lamport clock value
            
          - no_more: if falsy, more operations will come later, if tryly
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       Input..fetch_source( receiver [, query] )
       ...
       - query (optional Array of Objects): see _output._fetch() for
         definition
    */
    fetch_source: function( receiver, query ) {
      var p = this.pipelet
        , s = this.source
        , name
      ;
      ...
      de&&ug( name = this._get_name( "fetch_source" ), 'query:', query );
  ...
  */
  ...
  Plug.subclass( 'Output', Output, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       Output..add_destination( destination [, options ] )
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       Output..remove_destination( destination [, options ] )
    ...
    */
    _remove_destination: function( input ) {
      ...
      position == -1 && this._error( '_remove_destination', 'not found, destination: ' + input._get_name() );
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        @method Output.._transactional_fetch( destination, query, options )
        ...
        - options (Optional Object): options for the transactional fetch
          ...
          - count (Integer): the number of fetch operations, including this
            ...
            count is greater than 1, a transaction will be returned which
            must then be provided as option "transaction" of the next call
            to _transactional_fetch().
            
            This option is deleted after use.
          
          - transaction (Transaction): transaction returned previous
        ...
        @returns transaction (Transaction): allow next _transactional_fetch()
          in the same transaction to proceed by passing this transaction object as
          option "transaction".
    */
    _transactional_fetch: function( destination, query, options ) {
      var that = this
      ...
      */
      ...
      function receiver( values, no_more ) {
        ...
        /*
           ToDo: update local query, upstream query should already have been updated
        */
      } // receiver()
    ...
    /* ------------------------------------------------------------------------
       Output..fetch_all( [ receiver [, query ] ] )
    ...
    */
    ...
    // ToDo: document Output..query_update()
    // ToDo: query_update(): use changes parameter instead of removes and adds
    ...
    // ToDo: document Output..update_upstream_query()
    ...
    update_upstream_query: function( changes ) {
      ...
      this.pipelet._input.update_upstream_query( changes );
      
      return this;
    }, // Output..update_upstream_query()
  ...
  */
  function Pipelet( options ) {
    ...
    // inputs in addition to this._input
  ...
  Loggable.subclass( 'Pipelet', Pipelet, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        @method Pipelet..through( pipelet )
        
        @short Getting dataflow through another @@pipelet.
    ...
    */
    log_namespace: function( name ) {
      ...
      RS.log( namespace._get_name() + ', ' + name
  ...
  Controllet.Input = Pipelet.Input.subclass(
    ...
    */
    ...
    _add_source_subscription: function( source, options ) {
      ...
      return this.route_to_destinations( '_add_source_subscription', [ source, options ] );
    }, // Controllet.Input.._add_source_subscription()
    
    _remove_source_subscription: function( source, options ) {
    
      return this.route_to_destinations( '_remove_source_subscription', [ source, options ] );
  ...
  */
  function Union( sources, options ) {
    ...
    sources && sources.forEach( that._add_source.bind( that ) );
  } // Union()
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     ToDo: alter(), no longer provide position and values to transform(), modify form_process() accordingly
        ...
        */
        update_upstream_query: function( changes ) {
          ...
          function terms_transform( terms ) {
            ...
            while ( ++i < l ) {
              term = query_transform( terms[ i ] );
              
              term && _terms.push( term );
            }
  ...
  */
  ...
  Greedy.subclass( 'Dispatch', Dispatch, {
    _add: function( new_branches, options ) {
      ...
      while ( branch = new_branches[ ++i ] ) {
        id = that.make_key( branch );
        
        typeof id === "undefined" && fatal( 'branches must define a unique id' );
        
        branches[ id ] && fatal( 'branch id: ' + id + ', already exists' );
        
        de&&ug( that._get_name( name ) + 'id:', id, ', options:', options );
        
        if ( requires_pass_through )
          input = that.dispatcher.pass_through( { name: options.name + '-input' } )
        ;
        
        output = that.branch_dataflow.call( branch, input, options );
        
        output = ( branches[ id ] = input_output
          ? output
          : { input: requires_pass_through ? input : output, output: output }
        ).output;
    ...
    _remove: function( removed_branches, options ) {
      ...
      while ( branch = removed_branches[ ++i ] ) {
        branch = branches[ id = that.make_key( branch ) ]
        ...
        // Unplug using no_fetch ToDo: consider a dispatch() option to fetch source during disconnection
        ( output = branch.output )
          && output._output.remove_destination( that.gatherer._input, no_fetch )
        ;
        
        // Unplug using no_fetch because there is nowhere to send removes to
        ( input = branch.input )
          && branch.input._input.remove_source( that.dispatcher._output, no_fetch )
        ;
        
        // ToDo: move this before disconnecting pipelets in case disconnection has a side effect that restarts the same branch which would fail
        delete branches[ id ];
      }
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      - options (Object): optional attributes:
        ...
        - All other options are booleans controlling which pipelet methods are traced:
          ...
          - fetch               (Boolean, default false): trace _input.fetch_source()
          - fetched             (Boolean, default true ): trace _input.fetch_source#receiver()
          ...
          - query_update        (Boolean, default false): trace _output.query_update()
  ...
  */
  function Trace( name, options ) {
    ...
    if ( options.fetch || options.fetched ) that._input.fetch_source  = fetch_source;
    if ( options.query_update             ) that._output.query_update = output_query_update;
    ...
    function fetch_source( receiver, query ) {
      var name   = 'fetch'
        , that   = this
        , source = that.source
      ;
      ...
      source
        ? Super_Input.fetch_source.call( that, rx, query )
    ...
    function output_query_update( removes, adds, input ) {
      log( 'query_update', { removes : removes, adds: adds, input: input._get_name() } );
      
      return Super_Output.query_update.call( this, removes, adds, input );
    } // output_query_update()
  } // Trace()
  ...
  */
  function Delay( delay, options ) {
    ...
    that._input.fetch_source  = input_fetch_source;
    that._output.query_update = output_query_update;
    
    that.delay = delay;
    
    de&&ug( 'new Delay(): delay: ' + delay + ' ms' )
    
    function input_fetch_source( receiver, query ) {
      var that = this;
