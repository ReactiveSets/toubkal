...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  ...
  function Plug( pipelet, options ) {
    ...
    if ( synchronous ) {
      ...
      /*
        query_updates (Array): list of ongoing query updates (Object):
          - done:
            - 0: waiting for fetch to complete
            - 1: complete
            - (Function): complete, function to call after applying changes
            - 2: applied
  ...
  Event_Emitter.subclass( 'Plug', Plug, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        @method Plug..update_upstream_query( changes, destination, transformed )
        
        @short Update upstream source query with minimum changes.
        
        @parameters
        - **changes** (Array): changed query terms to apply to
        ...
        - transformed (Boolean): provided by
          @@method:Input..update_upstream_query() to ```1``` where
          changes are already transformed with at least one resulting
          change. Can also be set if there is no query transform
        
        - **upstream_changes** (Array): changed query terms to apply upstream
          immediately regardless of the result of ```_update_query( changes )```
        ...
        @@method:Input.._update_query() or @@method:Output.._update_query().
        
        Changes applied upsrteam are ```upstream_changes``` if defined or the
        result of call to @@method:Plug.._update_query(), which may be delayed
        when fetches are in progress.
        ...
        - ToDo: update_upstream_query(): process transaction objects:
          ...
          downstream dataflow.
    */
    update_upstream_query: function( changes, destination, transformed, upstream_changes ) {
      var that            = this
      ...
      if ( transformed
        ...
        // if upstream_changes are availlable, forward changes upstream immediately
        upstream_changes
          ? apply_upstream( upstream_changes )
          : _apply_upstream = apply_upstream
        ;
        
        if ( synchronous && ( query_updates = that.query_updates ).length ) {
          // There are fetches in progress, delay query update
          
          // This will reduce latency added when pending fetches complete upstream
          query_updates.push( { done: _apply_upstream, changes: changes, destination: destination } );
        } else {
          changes = that._update_query( changes, destination );
          
          _apply_upstream && _apply_upstream( changes );
        }
      }
    ...
    /* ------------------------------------------------------------------------
        @method Plug.._transform( value )
        
        @short Optional virtual method to transform emitted values
        
        @parameters
        - **value** (Object): to transform
        
        @returns
        - (Object): optional transformed value. If falsy, no value is emmited.
        
        @description
        When this method is defined, it is called by @@method:Plug.._fetch().
        
        This method should not modify its ```value``` paraneter,
        a new object should be returned instead.
    */
        ...
        ```javascript
          function query_transform( term ) {
            if( term.flow == 'issues' ) {
              ...
              delete term.flow; // term is now greedier
              
              return term;
            }
            
            // Returns undefined for all other values of flow, making query greedier
          }
        ```
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        Operation:
          ...
          - Call ```receiver()``` immediately with an empty set of values if
            ...
            otherwise call ```receiver()``` immediatly with an empty set of
            values;
  ...
  /* --------------------------------------------------------------------------
      ...
      - **input_transactions** (Input_Transactions): optional shared with other
        ...
        transactions.
  */
  var future_query_s = 'future_query';
    ...
    */
    update_upstream_query: function( changes, destination ) {
      ...
      if( ! query_transform || changes_count( changes = map_query_changes( query_transform, changes ) ) ) {
        ...
        that._update_query( changes, destination, future_query_s );
        
        Super.update_upstream_query.call( that, changes, destination, 1 );
      }
    ...
    /* ------------------------------------------------------------------------
        ...
        This method is currently commented-out to prevent a bug where two
        simultaneous fetches happen while the first one did not have the
        time to complete, and therefore did not yet update the local query.
        The optimization that this pipelet is supposed to do fails trying
        to remove a query term not yet added.
        ToDo: remove corresponding code in Plug.._fetch() for __query_changes parameter
        
        This method, when not commented-out does the following:
        In addition to fetching upstream source, transmits upstream optimized
        query changes, if any.
        
        Calls method Plug.._fetch() to fetch source,
        
        Calls method Input.._update_query() if defined and ```query_changes```
        is defined.
    */
    ...
    */
    _update_query: function( changes, destination, query_name ) {
      ...
      if( rl + al ) {
        ...
        q = that[ query_name ] = that[ query_name ] || new Query( [], get_name( that ) + '-input' );
  ...
  */
  function Pipelet( options ) {
    ...
    input  = that._input  || ( that._input  = new Pipelet.Input ( that, name, options ) );
    output = that._output || ( that._output = new Pipelet.Output( that, options ) );
  ...
  */
  function Controllet( options ) {
    ...
    that._input  || ( that._input  = new Controllet.Input ( that, name, options ) );
    that._output || ( that._output = new Controllet.Output( that, options ) );
  ...
  */
  Controllet.Input = Pipelet.Input.subclass(
    ...
    function( p, name, options, input_transactions ) {
      Pipelet.Input.call( this, p, name, options, input_transactions );
    }, {
  ...
  */
  Controllet.Output = Pipelet.Output.subclass(
    ...
    function( p, options ) { Pipelet.Output.call( this, p, options ) }, function( Super ) { return {
  ...
  */
  function Union( sources, options ) {
    ...
    that._input  || ( that._input  = new Union.Input ( that, name, options ) );
    that._output || ( that._output = new Union.Output( that, options ) );
  ...
  Union.Input = Controllet.Input.subclass(
    ...
    function( p, name, options, input_transactions ) {
      Controllet.Input.call( this, p, name, options, input_transactions );
  ...
  Union.Output = Controllet.Output.subclass(
    ...
    function( p, options ) { Controllet.Output.call( this, p, options ) }, {
