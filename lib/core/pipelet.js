...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  Loggable.subclass( 'Pipelet', Pipelet, {
    ...
    */
    through: function( pipelet ) {
      this._output.add_destination( pipelet._input );
      
      return pipelet;
    }, // through()
    
    /* ---------------------------------------------------------------------------------------------
        Compose( name, composition, options )
        ...
        -------
          ...
          3/ Use composed aggregate_from() to aggregates sales yearly from USA:
            ...
            ;
    */
    Compose: function( name, composition, options ) {
      ...
      function add_compose() {
        ...
        pipelet = new constructor_apply( a );
        
        return single
          ? pipelet
          : add_union( pipelet, a[ a.length - 1 ] )
        ;
      } // add_compose()
      
      function add_union( pipelet, options ) {
        var input = rs
  ...
  */
  var Compose = Pipelet.Compose = function( name, composition ) {
    ...
    return rs.Compose( name, composition, { single: true } );
  } // Compose()
  ...
  */
  ...
  rs.Compose( 'dispatch', Dispatch, { single: true } );
