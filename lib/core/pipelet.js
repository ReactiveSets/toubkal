...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  var Code                   = RS.Code
    ...
    , operation_values       = RS.operation_values = { add: 0, remove: 1, update: 2 /* clear is not defined here, it is deprecated */ }
    ...
    */
    _fetch: function( receiver, query, query_changes, destination, __query_changes ) {
      ...
      var that             = this
        ...
        , _update_query    = that._update_query
        , _rx              = receiver
        , source           = that.source
      ...
      if ( _update_query = _update_query && __query_changes ) {
      ...
      function rx( values, no_more, operation, options, terminated_source ) {
        // ToDo: hamdle terminated_source to get possible optional data from this source as input events
          ...
          if ( query_updates ) {
            ...
            if ( __query_changes.done < 2 ) {
              ...
              __query_changes.done = function() {
                ...
                rx( values, no_more, operation, options );
              };
              
              // delay receiver until query is updated, re-ordering receiver's calls in fetch order
              
              // ToDo: should we instead terminate immediately fetch or risk loosing data here?
              // ToDo: this might require to listen data events from Output..emit() until query tree is updated
              return;
            }
          } else
      ...
      function emit_subscribers_operation( subscribers_operation ) {
        ...
        while ( r = subscribers[ ++i ] ) {
          ...
          if ( operation == 'clear' )
            ...
            input.listen( operation_values[ operation ], r.v, o, that );
          ;
