...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
      ...
      function rx( values, no_more, operation, options, terminated_source ) {
        ...
        function manage_output_listeners() {
          ...
          if ( no_more )
            ...
            listener.listeners
              .push( listener = function( operation, values ) {
  ...
  */
  ...
  Loggable.subclass( 'Pipelet', Pipelet, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        @description
        @@method:Pipelet.._make_key() is reset to its original so that the
    ...
    */
    _set_key: function( key ) {
      ...
      that._make_key = pipelet_make_key;
    ...
    /* ------------------------------------------------------------------------
        ...
        ```javascript
          this._make_key( {} ); // force generation of this._make_key
          
          var make_key   = this._make_key
            , identities = value.map( make_key )
          ;
        ```
    */
    // ToDo: rename into _get_identity()
    // ToDo: implement _get_identities(), _make_identity_methods(), _find_identity()
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        - **f** (Function): optional, deprecated, transaction processor,
          signature:
            f( transaction, count ):
            - **this** (Pipelet): context for this pipelet
            - **transaction** (@@class:Transaction\): to @@emit operations to
            - **count** (Integer): number of operations, the same as count
              parameter above.
        
        @returns
        (@@class:Transaction\): closing transaction to emit count operations.
        ...
        ```javascript
          ...
          this.__emit_remove( removes, t.next().get_emit_options() );
          this.__emit_add   ( adds   , t.next().get_emit_options() );
        ```
  ...
  var pipelet_make_key = Pipelet[ prototype_s ]._make_key;
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      @description
      Used by @@method:Pipelet.._make_key() to generate safe code
  */
