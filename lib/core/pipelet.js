  ...
  */
  ...
  Plug.subclass( 'Input', Input, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        @method Input..update_upstream_query( removes, adds )
        ...
        @parameters:
          - removes (Array): query terms to remove
          - adds    (Array): query terms to add
    */
    update_upstream_query: function( removes, adds ) {
      var input   = this
        , changes = input._update_query( [ removes, adds ] )
        , removes = changes[ 0 ]
        , adds    = changes[ 1 ]
        , source  = input.source
      ;
      ...
      removes.length + adds.length
        && source
        && source.query_update( removes, adds, input )
      ;
