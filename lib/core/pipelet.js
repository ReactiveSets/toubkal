...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
    ...
    /* ---------------------------------------------------------------------------------------------
        ...
        ----------
          ...
          - options (optional Object): define optional attributes:
            - single (Boolean): if false or undefined, the composition is encapsulated with a
              union input so that the composition is a single pipelet. See multi-pipelet
              compositions bellow for details.
              
            - singleton (Boolean): if true the composition will be instanciated once, at the
              first invocation. Subsequent invocations will reuse the singleton instance
              and parameters will be ignored.
        ...
        -------
          ...
          3/ Use composed aggregate_from() to aggregates sales yearly from USA:
            ...
            ;
    */
    Compose: function( name, options, composition ) {
      ...
      options.singleton && factory.singleton();
      ...
      function instanciate( source, a ) {
        ...
        function compose( input ) {
          ...
          a = Pipelet.set_default_options( composition, source, a, { name: name } );
    ...
    /* ---------------------------------------------------------------------------------------------
        ...
        See Compose() for behavior with option single set to false and singleton set to true.
    */
    Singleton: function( name, composition ) {
      return this.Compose( name, { singleton: true }, composition );
    } // Singleton()
  ...
  */
  Pipelet.Build = function( name, constructor, methods ) {
    ...
    function pipelet_factory( source, a ) {
      a = Pipelet.set_default_options( constructor, source, a, { name: name } );
  ...
  */
  Pipelet.Add = function( name, pipelet_factory ) {
    ...
    function singleton() {
      ...
      de&&ug( 'pipelet ' + name + '() is now a singleton' );
  ...
  Pipelet.Check_Name = function check_name( name ) {
    ...
    if ( authorized_name.indexOf( name ) == -1 ) { // not a core-authorized name
      ...
      if ( ! /^[a-z]{2}[0-9a-z_]*$/.test( name ) )
        return 'should start with two lower-case letters followed by lower-case letters or digits or "_"'
      ;
    }
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     See Pipelet.Compose() for definition, always using option single set to true even if there
     are more than one pipelet in the composition.
  */
  var Compose = Pipelet.Compose = function( name, composition ) {
    log( 'Pipelet.Compose() is deprecated, used rs.Compose() instead for ' + name );
    
    return rs.Compose( name, { single: true }, composition );
  } // Compose()
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     The key of encapsulate() is forced to that of output.
  ...
  */
  ...
  rs.Compose( 'dispatch', { single: true }, Dispatch );
  ...
  */
  rs.Compose( 'set_flow', { single: true }, function( source, flow, options ) {
   ...
   */
  rs.Compose( 'auto_increment', { single: true }, function( source, options ) {
