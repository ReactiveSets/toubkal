...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  ...
  function map_query_changes( query_transform, query_changes ) {
    if ( query_transform && query_changes ) {
      query_changes = [ transform_query( query_transform, query_changes[ 0 ] ), transform_query( query_transform, query_changes[ 1 ] ) ];
      
      if ( ! changes_count( query_changes ) ) return;
    }
  ...
  Event_Emitter.subclass( 'Plug', Plug, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        @description
        Transform changes calling @@method:Plug.._query_transform()
        if defined.
        
        Then call @@method:Plug.._update_query() possibly delayed in
        synchronized order with @@method:Plug..fetch() when its receiver
        emits final chunks.
        
        Then call @@method:Plug.._upstream_query_changes(), if defined, that
        may transform changes to forward upstream.
        
        Finally forward upstream resulting changes calling
        @@method:Plug..update_upstream_query() on ```this.source``` if
        defined.
        
        Because @@method:Plug.._query_transform() is called first, it has
        a different semantic when applied to an @@Class:Input or
        @@Class:Output\.
        
        When applied on an Output, @@method:Plug.._query_transform()
        should only be used as a filter of query terms and should not
        change these terms otherwise the downstream pipelet may not
        receive expected values due to filtering by output query tree.
        ...
        can both filter and alter terms but it is not currenly implemented
        in _fetch(). // ToDo: remove this when implemented in _fetch()
    */
    update_upstream_query: function( changes, destination ) {
      ...
      if ( ! query_transform
        || ( changes = map_query_changes( query_transform, changes ) )
      ) {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        - **receiver** (Function): see @@method:Plug.._fetch() receiver
          parameter, may be null if only query_changes need to be updated
          and ```fetch_unfiltered.length > 1```
        ...
        being called with a nul ```receiver```.
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        - **receiver** (Function): signature:
          ```receiver( values, no_more, operation, updates, options )```
          ...
          - **terminated_source** (Output): optional, indicates termination
            of fetch fron that source when upstream pipelet is a
            @@pipelet:union().
    ...
    */
    _fetch: function( receiver, query, query_changes, destination, __query_changes ) {
      ...
      var that             = this
        ...
        , _update_query    = that._update_query // Output only
        , _rx              = receiver
        , source           = that.source
      ...
      if ( query_transform ) {
        ...
        if ( _update_query ) { // this plug applies query updates locally, i.e. this is an output
          // transform local query changes
          __query_changes = __query_changes === query_changes
            ? _query_changes
        ...
        if ( query )
          ...
          _query = _query_changes
            && _query_changes[ query_changes.indexOf( query ) ]
            || transform_query( query_transform, query )
        ;
      }
      
      // Handle output query tree changes
      if ( _update_query = _update_query && __query_changes ) { // ToDo: shouldn't this be __query_changes && __query_changes?
        query_updates = that.query_updates;
        
        query_updates
          .push( __query_changes = { done: 0, changes: __query_changes, destination: destination, cancel: cancel } )
        ;
        
        // this requires rx to apply __query_changes using _update_query()
        _rx = rx;
      }
      ...
      if ( fetch_unfiltered || is_array( source ) ) {
        if ( _query ) {
          if ( _query.length ) {
            transform = get_filter;
            
            _rx = rx;
          } else {
            // nul query
            
            // call _rx() immediately with no data, fetch_unfiltered() will not call _rx()
            _rx( [], true );
            
            if ( ! _query_changes || fetch_unfiltered.length < 2 )
              // no query changes of fetch_unfiltered ignores query changes
              return
            ;
            
            _rx = 0; // only forward upstream changes if any and if requested by fetch_unfiltered()
          }
        }
      ...
      } else if ( _query && ! _query.length ) {
        de&&ug( name, 'nul query' );
        
        // If any, forward transformed subscription changes upstream
        _query_changes
          && source
          && ( update_upstream_query = source.update_upstream_query )
        ...
        // No source, emit the empty set
        _rx( [], true );
      }
      
      function get_filter( /* ignore options */ ) {
        return filter = filter || new Query( _query, get_name( that ) + '-_fetch' ).generate().filter
      } // get_filter()
      
      function cancel() {
        var de = 1;
        
        log( name, 'cancelled, query changes:', __query_changes.changes, '- destination:', get_name( destination ) );
        
        __query_changes.done = 1; // allow to apply query update by _apply_done_query_updates()
        
        cancelled = 1;
        
        // ToDo: instead of emitting an end of values event with a canceled cause, revert all previously emitted operations
        receiver( [], true );
        
        receiver = 0; // de-reference downstream closure as this closure is still referenced upstream
        
        // ToDo: abort upstream fetch
      } // cancel()
      ...
      function rx( values, no_more, operation, options, terminated_source ) {
        ...
        if ( no_more && _update_query ) // Output only
          if ( query_updates ) {
            ...
            that._apply_done_query_updates( __query_changes );
            
            if ( __query_changes.done < 2 ) {
              // de&&ug( name, 'not applied:', __query_changes.done );
        ...
        output_listeners && ( no_more || terminated_source ) && manage_output_listeners(); // Input only
        
        // transform results if any
        if ( transform && l ) {
          _transform = transform( options );
          
          if ( operation > 1 ) return update( values );
        ...
        function update( updates ) {
          ...
          if ( rl )
            receiver( removes, no_more && ! al && ! ul, 1, options )
          ;
          
          if ( ul )
            receiver( updates, no_more && ! al, 2, options )
          ;
          
          if ( al || no_more && ! ul && ! rl )
            receiver( adds, no_more, 0, options )
          ;
        } // update()
        
        function manage_output_listeners() {
          var i              = -1
            , j
            , listener
            , listeners
          ...
          if ( no_more ) {
            ...
            // log( get_name( that, 'manage_output_listeners' ) + 'output_listener.listeners:', listener.listeners.length );
          }
          
          function added_receiver( operation, values ) {
            log( get_name( that, 'added_listener' ) + 'operation:', operation, values.length );
            
            // ToDo: filter values as with unfiltered fetch
            // ToDo: apply transforms
            receiver( values, 0, operation );
          }
        } // manage_output_listeners()
      } // rx()
    }, // Plug.._fetch()
    ...
    /* ------------------------------------------------------------------------
        ...
        source always emits adds on fetch.
  ...
  Plug.subclass( 'Input', Input, function( Super ) { return {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        a fetch invilving multiple outputs from a @@pipelet:union().
    ...
    */
    listen: function( operation, values, options, source, only_output_listeners ) {
      var listeners = this[ _output_listeners_s ]
        , listener
        , i
      ;
      
      if ( source && listeners.length )
    ...
    */
        ...
        if ( source._fetch ) {
          ...
          that
            ...
            ._remove_source_subscription( source, options, function() {
              ...
              // ToDo: should _cancel_fetches() be called with _remove_source()?
              source._cancel_fetches( that ); // ToDo: revert data events emitted by ongoing fetches
              
              // ToDo: handle the case where remove_source() is called before add_source() fully completed.
              
              that
                ._remove_source_branches    ( source ) // will close ongoing transactions, ToDo: fix competition with _transactions_remove_source() bellow
                ._remove_source_destination ( source )
                ._transactions_remove_source( source ) // ToDo: is this still necessary if _remove_source_branches() removes transactions?
                 // ToDo: _transactions_remove_source() should not emit when the transaction is the current transaction removing this source
              ;
    ...
    /* ------------------------------------------------------------------------
        ...
        ToDo: this method is not called so far, may have to be removed unless used, see _transactions_remove_source()
    */
    _terminate_source_transaction: function( source, options ) {
      ...
      this.listen( 0, [], source._concurrent_options( this, options ) );
  ...
  */
  ...
  Plug.subclass( 'Output', Output, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        @description
        This method is called by @@method:Input..remove_source().
    ...
    */
    _apply_done_query_updates: function( query_update ) {
      ...
      if ( query_updates[ i ] == query_update ) { // it is first
        top: for ( ;; ) {
          // de&&ug( name, 'apply:', query_update );
          
          query_updates.splice( i--, 1 );
          
          that._update_query( query_update.changes, destination )
      ...
      if ( operation == 'clear' || values.length ) { // tree.route() returns no subscribers_operations when no values
        ...
        /* subscribers_operations: (Array of Objects), operations:
           ...
           - 1: (Array of Objects), for each subscriber input:
             ...
             - v: (optional Array of Objects) values to emit for 'add', 'remove', or 'update'
        ...
        if ( terminated_inputs ) {
          ...
          that.tree.subscribers
            ...
            .forEach( function( subscriber ) {
              ...
              // ToDo: make this a loop to reduce synchronous stack depth
  ...
  /* --------------------------------------------------------------------------
      ...
      - **options** (Object): optional pipelet parameters:
        ...
        - **source_key** (Array of Strings): used if key option is not
  ...
  */
  // ToDo: rename tag option untag, rename fork_tag tag
  ...
  Loggable.subclass( 'Pipelet', Pipelet, {
    ...
    */
    _add_input: function( output, Input, name, methods ) {
      var that      = this
        ...
        , input     =
            new Input( methods, name, that._options.tag, that._input.transactions )
      ;
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        Derived classes should reset any other methods that rely on ```key```.
    */
    ...
    */
    _identity: function( value ) {
      return ( this._identity =
        new Function( 'o', 'return ' + Pipelet.safe_identity_code( this._key ) )
      )( value );
    }, // Pipelet.._identity()
    ...
    */
    _notify: function( transaction, options ) {
      ...
      transaction.forEach( function( a ) {
        that[ '_' + a.action ]( a.objects, t.next_options() );
      } );
    }, // Pipelet.._notify()
    ...
    */
    __emit_operations: function( added, removed, updates, options ) {
      ...
      if( l ) {
        if ( l > 1 ) t = that._transaction( l, options );
  ...
  */
  Pipelet.safe_identity_code = function( key ) {
    var plus   = ' + '
  ...
  */
  ...
  Controllet.Input = Input.subclass(
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        @method Controllet.Input.._terminate_source_transaction( source, options )
        
        @short Route to all destinations method Input.._terminate_source_transaction()
    ...
    */
    ...
    }, // Controllet.Input.._terminate_source_transaction()
    ...
    */
    _transactions_remove_source: function( source ) {
      ...
      return this.route_to_destinations( '_transactions_remove_source', [ source ] );
  ...
  Controllet.Output = Output.subclass(
    ...
    */
    ...
    _cancel_fetches: function( destination ) {
      ...
      source && source._cancel_fetches( destination )
    } // Controllet.Output.._cancel_fetches()
  ...
  */
  ...
  function Union_Output( p, options ) {
    ...
    /*
      Ongoing fetches source updates (Array of Functions):
        ( source ) ->
        - source (Output): added to this union input
    */
    this._source_adds = [];
  } // Union_Output()
  ...
  Union.Output = Controllet_Output.subclass(
    ...
    */
    _fetch: function( receiver, query, query_changes, destination ) {
      ...
      function fetch_source( source ) {
        ...
        function rx( values, no_more, operation, options, terminated_source ) {
          ...
          ( complete = no_more )
            && ! --count
            && source_adds.splice( source_adds.indexOf( source_add ), 1 )
          ;
    ...
    source_add: function( source ) {
      // on remove source, Plug.._cancel_fetches() terminates fetches after reverting already emitted data
    ...
    _cancel_fetches: function( destination ) {
      this.source.sources.forEach( function( source ) {
        var cancel_fetches = source._cancel_fetches;
        
        cancel_fetches && cancel_fetches.call( source, destination );
      } );
    }, // Union.Output.._cancel_fetches()
