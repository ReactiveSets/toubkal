  ...
  */
  ...
  Greedy.subclass( 'Dispatch', Dispatch, {
    _add: function( new_branches, options ) {
      ...
      if ( t && t.id == that.remove_branches_tid ) {
        // ToDo: Dispatch.._add() find cleaner option to deal with updates as remove + add
        de&&ug( get_name() + 'ignore update from previous remove, tid:', t.id );
        
        return
      }
      
      var branches              = that.branches
      ...
      while ( branch = new_branches[ ++i ] ) {
        ...
        if ( requires_pass_through ) input = that.dispatcher.pass_through( { name: options.name + '-input' } );
        
        var output = that.branch_dataflow.call( branch, input, options );
    ...
    _remove: function( removed_branches, options ) {
      ...
      if ( t && t.more ) {
        de&&ug( get_name() + 'more option set, this could be an update, tid:', t.id );
        
        that.remove_branches_tid = t.id;
        
        return
      }
      
      while ( ++i < l ) {
        id = that.make_key( removed_branches[ i ] );
        ...
        if ( branch ) {
          ...
          branch.output && branch.output._output.remove_destination( that.gatherer._input, { no_fetch: true } );
          
          // Unplug using no_fetch because there is nowhere to send removes to
          branch.input && branch.input._input.remove_source( that.dispatcher._output, { no_fetch: true } );
