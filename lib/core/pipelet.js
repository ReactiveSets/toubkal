    ...
    */
    _fetch: function( receiver, query, query_changes, destination, __query_changes ) {
      ...
      function cancel() {
        ...
        // ToDo: instead of emitting an end of values event with a canceled cause, revert all previously emitted values
      ...
      // ToDo: replace removes and updates parameter by a String operation parameter which defaults to 'add' and can also be 'remove' or 'update'
      function rx( adds, no_more, removes, updates, options, terminated_source ) {
    ...
    */
    _transactional_fetch: function( destination, transaction, count, query, query_changes, options, revert, next ) {
      ...
      function revert_receiver( adds, no_more, removes, updates, _options ) {
        receiver( removes || [], no_more, adds, updates && updates.map( swap_update ), _options );
      } // revert_receiver()
    ...
    _fetch: function( receiver, query, query_changes, destination ) {
      ...
      function fetch_source( source ) {
        ...
        function rx( adds, no_more, removes, updates, options, terminated_source ) {
          ...
          if ( no_more || adds.length + ( removes ? removes.length : 0 ) + ( updates ? updates.length : 0 ) )
            receiver( adds, ! count, removes, updates, options, terminated_source || no_more && source )
          ;
        } // rx()
      } // fetch_source()
    }, // Union.Output.._fetch()
  ...
  */
  function Delay( delay, options ) {
    ...
    function input_fetch( receiver, query ) {
      ...
      // A delayed receiver
      // ToDo. forward other receiver parameters
