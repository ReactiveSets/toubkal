    ...
    */
    _fetch: function( receiver, query, query_changes, destination, __query_changes ) {
      ...
      var that             = this
        ...
        , query_updates    = _update_query && that.query_updates
        , _rx
        , ___
        , _upstream_query_changes
      ...
      if ( query_transform ) {
        ...
        ;
      } else if ( _query_changes && ! changes_count( _query_changes ) )
        _query_changes = ___
      ;
      ...
      if ( fetch_unfiltered ) {
        if ( _query ) {
          if ( _query.length ) {
            ...
            _rx = ___; // only forward upstream changes if any and if requested by fetch_unfiltered()
          }
        }
