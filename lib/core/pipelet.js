  ...
  */
  ...
  function map_query_changes( query_transform, query_changes, filtered ) {
    if ( query_transform && query_changes ) {
      ...
      if ( removes.length ) {
        removes = transform_query( query_transform, removes, filtered && filtered[ 0 ] )
      }
      
      if ( adds.length ) {
        adds = transform_query( query_transform, adds, filtered && filtered[ 1 ] )
      }
      
      query_changes = [ removes, adds ];
