  ...
  Event_Emitter.subclass( 'Plug', Plug, {
    ...
    */
    _fetch: function( receiver, query, query_changes, destination, __query_changes ) {
      __query_changes = __query_changes || query_changes; // changes to possibly apply locally
      ...
      var that             = this
        ...
        , query_transform  = that._query_transform
        , _query           = query
        , _query_changes   = query_changes // changes to apply upstream
        , _update_query    = that._update_query // to apply __query_changes locally, Output only
        , _rx              = receiver
        , source           = that.source
      ...
      if ( query_transform ) {
        ...
        if ( _update_query && __query_changes )
          // This plug applies query updates locally, i.e. this is an output
          
          // transform local query changes
          __query_changes = __query_changes == query_changes
            ? _query_changes
            : map_query_changes( query_transform, __query_changes )
          ;
      ...
      if ( query_updates = that.query_updates ) { // Output only
        // __query_changes can be undefined, to allow cancellation even when there are no query changes
        // ToDo: [maintainability] stop reusing __query_changes var bellow for another purpose
        query_updates
          .push( __query_changes = { done: 0, changes: __query_changes, destination: destination, cancel: cancel } )
        ;
        
        // this requires rx to apply __query_changes.changes using _update_query() and/or remove __query_changes from query_updates
        _rx = rx
      }
      ...
      function cancel() {
        ...
        //de&&
        ug( name + 'cancelled, query changes:', __query_changes.changes, '- destination:', get_name( destination ) );
