...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  Event_Emitter.subclass( 'Plug', Plug, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        ToDo: rename Plug..error into() Plug.._error()
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        - **receiver** (Function): signature:
          ...
          - **terminated_source** (@@class:Output\): optional, indicates
            termination of fetch fron that source when upstream pipelet is
            a @@pipelet:union().
    ...
    */
    _fetch: function( receiver, query, query_changes, destination, __query_changes ) {
      ...
      if ( _query && ! _query.length ) {
        ...
        if ( fetch_unfiltered ) {
          if ( _query_changes && fetch_unfiltered.length > 1 )
        ...
        if ( _query && _query.length || that_transform ) {
          _get_transform = get_filter_transform;
      ...
      // return // Only closure functions bellow
      
      // closure references: _query, that_transform, filter, get_transform
      function get_filter_transform( options ) {
        var transform    = _query && _query.length ? get_filter() : null
          , _transform
        ;
        
        if ( that_transform ) {
          _transform = get_transform( options );
          
          transform = filter ? filter_tranform : _transform
        }
        
        return transform // null means: do not attempt to call later, would also return null with same options
        
        function get_filter() {
          return filter = filter || new Query( _query, get_name( that ) + '-_fetch' ).generate().filter
        } // get_filter()
        
        function filter_tranform( values ) {
          // filter upstream values then transform them
          return _transform( filter( values ) );
        }
      } // get_filter_transform()
      
      // closure references: that_transform
      function get_transform( options ) {
        return transform;
        
        function transform( values ) {
          return that_transform( values, options, 'fetch' );
        }
      } // get_transform()
      
      function cancel() {
        // ToDo: add tests for fetch cancellation
        var de = 1;
        ...
        // Canceller should then not attempt to revert the effect of this fetch by fetching again,
        // e.g. in Input..remove_source() cancelling a previous Input..add_source() in progress
        // Also an option to opt-in or opt-out of fetch cancellation might become necessary
        receiver( [], true );
        
        // ToDo: remove all listeners added by this fetch to output_listeners, see rx() bellow
        
        // de-reference downstream closure as this closure is still referenced upstream
        receiver = 0;
        
        // attempt to de-reference _rx
        abort && abort()
      } // cancel()
      ...
      function rx( values, no_more, operation, options, terminated_source ) {
        ...
        if ( _get_transform && l ) {
          _transform = _get_transform( options );
        ...
        function manage_output_listeners() {
          ...
          if ( no_more ) {
            ...
            _transform = ___; // will be set at the first received value, if any
          ...
          function added_receiver( operation, values ) {
            log( get_name( that, 'added_listener' ) + 'emit operation:', operation + ', values:', values.length );
            
            // optionally filter and transform values
            if ( _transform === ___ )
              // _transform is undefined but not null (e.g. never attempted to build it)
              _transform = get_filter_transform()
            ;
    ...
    /* ------------------------------------------------------------------------
        ...
        @parameters
        - **receiver** (Function): optional, see _fetch() for definition.
  ...
  Plug.subclass( 'Input', Input, function( Super ) { return {
    ...
    */
    listen: function( operation, values, options, source, only_output_listeners ) {
      ...
      if ( source && values.length && listeners.length )
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        - ToDo: cancel ongoing add_source()
        ...
        if ( source._fetch )
          ...
          return that
            ...
            ._remove_source_subscription( source, options, function() {
              ...
              /*
                ToDo: fully handle the case where remove_source() is called
                before add_source()#_add_source_subscription() or ongoing
                _update_subscription() has fully completed.
              */
    ...
    */
    _fetch: function( receiver, query, query_changes, destination ) {
      ...
      return Super._fetch.call( this, receiver, query, _query_changes, destination, query_changes );
    }, // Input.._fetch()
  ...
  Plug.subclass( 'Output', Output, {
    ...
    */
    _transactional_fetch: function( destination, transaction, count, query, query_changes, options, revert, next ) {
      ...
      return that._fetch( rx, query, query_changes, destination );
  ...
  Loggable.subclass( 'Pipelet', Pipelet, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        @short Returns true is pipelet is a @@namespace
  ...
  */
  ...
  /* ------------------------------------------------------------------------
      ...
      - **options** (Object): @@class:Controllet options.
  ...
  */
  ...
  function Union_Output( p, options ) {
    ...
    /*
      Ongoing fetches source updates (Array of Functions):
        ( source ) ->
          (@@class:Output\): source added to this union input
    */
    this._source_adds = [];
  } // Union_Output()
  ...
  Union.Output = Controllet_Output.subclass(
    ...
    /* ------------------------------------------------------------------------
        ...
        @paraneters
        See method Plug.._fetch().
        
        @returns
        - (Function): abort fetching on all ongoing source fetches that can be aborted.
    ...
    */
      ...
      function fetch_source( source ) {
        ...
        function terminate() {
          ...
          if ( abort )
            ...
            aborts[ aborts.indexOf( abort ) ] = ___
          ;
    ...
    // ToDo: document Union.Output..source_add()
