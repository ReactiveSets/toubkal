...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  Plug.subclass( 'Output', Output, {
    ...
    */
    fetch_all: function( receiver, query ) {
      ...
      if ( that._fetch === Output.prototype._fetch ) { // ToDo: remove this optimization, remove fetch_source_all() if no-longer used
      ...
      function rx_all( values ) {
        out = p.__transform( values, {}, 'fetch' );
  ...
  */
  function Alter( transform, options ) {
    var no_clone        = this._no_clone = this._no_clone || false // set to true by Map()
      , name            = options.name
    ...
    var code = new Code()
      ._function( 'this.__transform', void 0, [ 'X', 'o', 'operation' ] )
        ._var( vars )
    ...
    _remove: function( removed_branches, options ) {
      ...
      while ( branch = removed_branches[ ++i ] ) {
        ...
        ( input = branch.input )
          ...
          : unplug_output()
        ;
      } // end while there are branches to remove
