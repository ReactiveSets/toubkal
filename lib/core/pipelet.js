  ...
  */
  ...
  Plug.subclass( 'Input', Input, function( Super ) { return {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        @method Input..listen( operation, values, options, source )
        
        @short listens to output data events for pipelet and ongoing fetches
        ...
        Then emits operation to pipelet.
    */
      ...
      if ( source && listeners.length )
        for ( i = -1; listener = listeners[ ++i ]; )
          if ( listener.output == source ) {
            ...
            for ( i = -1; listener = listeners[ ++i ]; )
              listener( operation, values, options )
            ;
    ...
    // add(), remove(), and update() are deprecated, use listen() instead
    // ToDo: remove deprecated add(), remove(), and update()
    add: function( added, options, source ) {
      this.listen( 0, added, options, source );
    }, // Input..add()
    
    remove: function( removed, options, source ) {
      this.listen( 1, removed, options, source );
    }, // Input..remove()
    
    update: function( updated, options, source ) {
      this.listen( 2, updated, options, source );
    }, // Input..update()
    ...
    */
    _transactional_fetch: function( destination, transaction, count, query, query_changes, options, revert, next ) {
      ...
      function receiver( values, no_more, operation, _options ) { // ToDo: use _options
        ...
        if ( l ) {
          destination[ operations[ operation || 0 ] ]( values, get_options( more ) );
