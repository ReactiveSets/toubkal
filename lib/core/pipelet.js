    ...
    */
    fetch_source: function( receiver, _query ) {
      ...
      if ( s && q ) {
        ...
        de&&ug( name + 'no input query, requested query: ' + log.s( _query ) );
  ...
  */
  Pipelet.Add = function( name, pipelet ) {
    if ( Pipelet.prototype[ name ] )
      fatal( 'is already used' );
    
    if ( [ 'set', 'flow', 'join', 'last', 'next', 'form' ].indexOf( name ) == -1 ) { // not a core-authorized name
      if ( name.length < 5 )
        fatal( 'must be at least 5 characters long' )
      ;
      
      if ( ! /^[a-z]{2}[0-9a-z_]*$/.test( name ) )
        fatal( 'should start with two lower-case letters followed by lower-case letters or digits or "_"' )
      ;
    }
     ...
     Parameters:
       ...
       - options: (Object)
         ...
         - query_transform (Function): A transform to alter queries for both fetch and
           query updates. If provided, alter becomes lazy and only fetches the minimum
           set of values from upstream. query_transform( term ) is called for each added
           and removed terms on query updates or each term of a fetch query. It must not
           mutate its term parameter and must return a new term or null or undefined to
           tell that this term can never match any output. After query_transform is
    ...
    */
    ...
    _update: function( updates, options ) {
      ...
      for ( var i = -1, l = updates.length, updated = [], added = []; ++i < l; ) {
        var o = updates[ i ]
          , r = o[ 0 ], v = o[ 1 ]
          , p = this._a_index_of( r )
        ;
        
        if ( p == -1 ) {
          // This update may come before an add
          this.b.push( r ); // Must be before _b_index_of( v )
          
          p = this._b_index_of( v );
