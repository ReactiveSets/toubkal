    ...
    _add: function( new_branches, options ) {
      ...
      while ( branch = new_branches[ ++i ] ) {
        ...
        if ( requires_pass_through )
          input = that.dispatcher.pass_through( { name: _options.name + '-input' } )
        ;
    ...
    _remove: function( removed_branches, options ) {
      ...
      while ( branch = removed_branches[ ++i ] ) {
        ...
        ( output = branch.output )
          && output._output.remove_destination( that.gatherer._input, options )
        ;
        
        // Unplug input from dispatcher
        ( input = branch.input )
          && branch.input._input.remove_source( that.dispatcher._output, options )
        ;
