    ...
    _fetch: function( receiver, query, query_changes, destination, __query_changes ) {
      ...
      if ( query_updates = that.query_updates ) { // Output only
        query_updates.push( query_update = {
          ...
          changes    : __query_changes, // may be undefined
          destination: destination,
          cancel     : cancel
        } );
      ...
      function emit_waiter() {
        ...
        abort = that._fetch( receiver, query, query_changes, destination )
      } // emit_waiter()
      ...
      function rx( values, no_more, operation, options, terminated_source ) {
        ...
        if ( no_more && _update_query ) // Output only
          if ( query_updates ) {
            ...
            that._apply_done_query_updates( query_update );
  ...
  Plug.subclass( 'Input', Input, function( Super ) { return {
    ...
    */
    _fetch: function( receiver, query, query_changes, destination ) {
      ...
      // ToDo: do we still need the extra query_changes parrameter?
    ...
    function name_after_caller_location() {
      ...
      if ( stack.length ) {
        while(
          ...
          &&
            (
                frame.indexOf( 'pipelet.js' ) > 0 // ToDo: do this only on Node
              ||
                frame.indexOf( 'lib/toubkal-min.js' ) > 0 // ToDo: do this only in the Browser
            )
        );
        
        if ( RS.win32 ) frame = frame.replace( /\\/g, '/' );
        
        frame = frame.split( '/' );
        
        frame = frame
          .slice( Math.max( frame.length - 3, 0 ) )
          .join( '/' )
          .replace( /\)$/, '' )
        ;
