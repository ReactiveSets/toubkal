...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  ...
  function create_namespace( pipelet, name, is_repository ) {
    ...
    function Namespace() {
      ...
      // ToDo: provide and use remove() method to prevent memory leaks on prototypes copies when pipelet is removed
      
      function add_factory( name, factory ) {
        return is_repository
          ? ( de&&ug( that._get_name( _add_factory ), name ), Namespace_prototype[ name ] = factory ) : parent_namespace
  ...
  */
  Pipelet.Add = function( name, pipelet_factory, namespace ) {
    ...
    return ( namespace || rs )[ _add_factory ]( name, factory );
