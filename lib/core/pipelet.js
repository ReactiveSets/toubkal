/*  pipelet.js
    ...
    Main entities provided in this source:
      - Plug / Input / Output
      - Pipelet: the current base of all pipelet classes having one input and one
        output plug, provding Compose(), Multiton(), through(), namespaces, ...
      - Build: to derive pipelets
      - Add: to add pipelets to a namespace
      - Encapsulate: to encapsulate a pipeline into a pipelet
      - Controllet: base class for controllets
      - union(): a controlet with n sources and one destination
      - greedy(): base pipelet for greedy pipelets
      - alter(): base pipelet for stateless transforms
      - set(): base stateful pipelet
      - unique(): set rejecting duplicate values
      - dispatch()
      - trace()
      - delay()
...
/* ------------------------------------------------------------------------------------------------
   ...
   @manual_introduction introduction Toubkal Introduction
...
/* -------------------------------------------------------------------------------------------
   @manual_introduction Toubkal Pipelet Programer's Guide
...
( 'pipelet', [ './RS', './query', './transactions' ], function( RS, Query, Transactions ) {
  ...
  */
  ...
  Plug.subclass( 'Output', Output, {
    ...
    */
    emit: function( event_name, values, options ) {
      ...
      that._emit_event( event_name, [ values, options ] );
      
      more || that._emit_event( 'complete', [ options ] );
    }, // Output..emit()
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      @manual reference, programmer
      
      @short Base class for all pipelets
      
      @description:
        All pipelines are made of connected instances of this class.
        
        Provides service for a @@synchronous, @@stateless, @@lazy pipelet.
      
      @parameters:
      - options: (Object) optional parameters:
        - name (String): debugging name for this pipelet, default is the name of
          the pipelet class.
        
        - key (Array of Strings): dataflow values attribute names carrying
          objects' identities - i.e. for which there is one and only one value in
          the set.
          
          Default is upstream pipelet key or [ 'id' ] if there are no upstream
          pipelets.
        
        - fork_tag (String): output transactions tag for diamond graph entrance
          - i.e. fork at this pipelet to multiple downstream pipelets that
          recombine downstream.
          
          This allows proper synchronization of transactions at the recombining
          point where the option tag (bellow) is set to this fork tag.
        
        - tag (String): input transactions tag for diamond graph exit - i.e. this
          pipelet recombines dataflows from an upstream fork tagged with this tag.
        
        - _t_postfix (String): postifx to add to transaction objects 'id'
          attribute.
          
          This allows to differentiate transactions internal to a graph that is
          folded into a loop.
          
          This should not be used to circonvent untagged diamond graphs. Doing so
          will prevent proper synchronization of graphs, most likely leading to
          pipelets waiting for ever end of transactions, memory leaks, or worse.
  */
  ...
  Loggable.subclass( 'Pipelet', Pipelet, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        @method Pipelet.._error( method, message )
        
        @short Reports errors to global error dataflow then throws.
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        ```javascript
          ...
          this.make_key( { a: 1, b: 2, c: "test" } )
          
          // -> "1#test"
        ```
        
        @description:
        Use ```this._key``` to generate code JIT to return a unique a string
        for a value based on the key coordinates concatenation separated with
        "#".
        
        The name of this method will eventually be renamed as _make_key().
      ...
      function instanciate( source, a ) {
        if ( options.union ) {
          ...
          var input = source.namespace()
            ...
            .union( [], { name: name + ' (Compose input union)' } )
          ;
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     A pipelet to group a pipeline into a single pipelet which input operations are
  ...
  Controllet.Input = Pipelet.Input.subclass(
    ...
    */
    ...
    add_branches: function( count ) {
      ...
      this.route_to_destinations( 'add_branches', [ count ] );
  ...
  */
  ...
  function Alter_Output( p, name, query_transform ) {
    ...
    if ( ! query_transform ) {
      // alter() is greedy, need to filter fetched results or update query, tranformed by Alter.Input
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      @short Dispatches source dataflow to a reactive set of branches
      ...
      @description:
        ...
        each branch. When a branch is added in branches, the controller calls branch() to
        create a new pipeline between the dispatcher and the gatherer. When a branch is removed,
        the controller removes the branch pipeline between the dispatcher and the gatherer.
      ...
      @parameters:
        ...
        - branch: function( source, options ), a function that is called when each branch
            ...
            'This' context of branch() is the branch that spawned this pipelet from
            branches.
        ...
        - options: (Object) For maximum performances, e.g. for very large dispachers with
            thousands of simultaneously active branches, one should use options 'loop' or
            'no_encapsulate' along with either 'single' or 'input_output':
      ...
      @throws error if:
        ...
        - a branch is duplicated (use unique() upstream if necessary)
      ...
      @examples:
        ...
        ```
  */
  // Note: Dispatch() is a Greedy Pipelet on "branches" which is unusually instanciated as a composition
  ...
  Greedy.subclass( 'Dispatch', Dispatch, {
    _add: function( new_branches, options ) {
      ...
      new_branches.forEach( function( branch ) {
        ...
        input = requires_pass_through
          ? dispatcher.pass_through( { name: _options.name + '-input-' + id } )
          : dispatcher
        ;
        ...
        if ( is_array( branch ) ) {
          ...
          _options.single
            ...
            && that._error( name, 'option single specified but returned a pipeline with more than one pipelet: ' + get_name( branch._input.source ) )
          ;
        }
    ...
    _remove: function( removed_branches, options ) {
      ...
      removed_branches.forEach( function( branch ) {
        ...
        ( input = branch.input )
          ? branch.input._input.remove_source( that._dispatcher._output, options, unplug_output )
          : unplug_output()
        ;
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      @short Non-ordered set.
      
      @description:
        Base class for stateful pipelets.
        
        This is a @@stateful, @@@synchronous, @@greedy pipelet. Derived classes may be
        @@assynchronous.
  ...
  */
  ...
  Greedy.Build( 'set', Set, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        @short Adds values to the set
        
        @parameters:
        - values (Array of Objects): values to add to set
        - options (Object):
          - locations (Array of Integers): provided by upstream @@order()
            pipelet. These are never forwarded downstream but are used to
            call @@Set.._add_value()
        
        @description:
          Calls @@Set.._add_value() for each individual value in values.
    */
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      @short A set of unique values discarding duplicates.
      
      @description:
        Discards duplicates add and remove operations.
        
        When duplicates are added, they are emitted in bulk in the 'error' flow if option
        verbose is true, with the error code 'DUPLICATE_KEY'. This may be used by a downstream
        pipelet to remove duplicates.
      
      @warnings:
        - Removed and updated values which identity is not found in set are discarded silently.
        
        - If values with the same identity are removed several times, they are removed on the
          first remove, then ignored.
        
        - Updates must preserve values identity, i.e. removed and added values must have the same
          identity, otherwise behavior is unspecified, no error is emitted.
      
      @parameters:
        - values (Array of Objects): initial values of set, duplicates will be discarded if any
        
        - options (Optional Object):
          - verbose (Boolean): emit errors on duplicate adds, default is false.
  ...
  Set.Build( 'unique', Unique, function( Super ) { return {
    _add: function( values, options ) {
      ...
      while ( ++i < l ) {
        ...
        ( v.flow != 'error' && identities[ key ] ? duplicates : identities[ key ] = 1, added )
          .push( v )
        ;
      }
      
      if ( duplicates.length && that._options.versbose ) {
        de&&ug( get_name( that, '_add' ) + 'discard duplicates, identities:', duplicates.map( that.make_key.bind( that ) ), ', key:', that._key );
    ...
    _update: function( updates, options ) {
      ...
      updates = updates
        .filter( function( update ) {
          ...
          return identities[ that.make_key( update[ 0 ] ) ];
        } )
      ;
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      @description:
        ...
        This is an optimization pipelet that is semantically neutral but may improve
        performances significantly, especially between clients and servers but also in
        internal application loops.
        
        It prevents upstream queries and fetch from downstream pipelets to proceeed
        upstream when they do not match query.
        
        Doing so also provides more freedom into the architecture of complex
        applications, that can then be built around a main application loop, e.g.:
        ...
        other internal components dataflows as well as server dataflows. Without
        the use of delivers(), all internal dataflows would be automatically
        subscribed from the server which does not delivers them. This would
        yield unnecessary delays, bandwidth consumption, and server resources
        usage.
      
      @parameters:
        - query (Array of Strings): where each string value specifies a flow attribute
          value that is delivered upstream.
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      @short Auto-increment attribute of adds-only set
   ...
   */
  // ToDo: allow removes by maintaining a hash of assigned ids
  rs.Compose( 'auto_increment', function( source, options ) {
    ...
    var input = source.namespace().map( function( v ) {
      var out = {}
        , ai = v[ attribute ]
      ;
      ...
      return extend_2( out, v )
    }, extend( {}, options, { name: options.name + '_alter' } ) );
    
    // ToDo: Use a set() on the output to prevent re-incrementation through _fetch().
    ...
    function fetch( receiver, query, query_changes, destination ) {
      ...
      function rx( adds, no_more, removes, updates, options, no_source ) {
        ...
        function show( values ) {
          return values && counts ? values.length : values
        }
      } // rx()
    } // fetch()
