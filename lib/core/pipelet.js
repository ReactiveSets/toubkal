  ...
  */
  ...
  Plug.subclass( 'Input', Input, {
    ...
    */
    ...
    // ToDo: remove deprecated, never used notify()
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        @method _add_source_branches( source )
    ...
    */
    _add_source_branches: function( source ) {
      ...
      return this;
    }, // _add_source_branches()
    
    /* ------------------------------------------------------------------------
        @method _remove_source_branches( source )
    ...
    */
    update_upstream_query: function( removes, adds ) {
      var changes = this._update_query( [ removes, adds ] )
        , removes = changes[ 0 ]
        , adds    = changes[ 1 ]
        , source  = this.source
      ;
      ...
      removes.length + adds.length
        && source
        && source.query_update( removes, adds, this )
      ;
    ...
    _update_query: function( changes ) {
      ...
      if ( rl + al ) {
        if ( q = this.query ) {
          ...
          l || q.generate(); // need to generate at least once even if there are no adds
        }
      }
      ...
      function update() {
        ...
        changes = [ removes = q.removes, adds = q.adds ];
        
        l = removes.length + adds.length;
        
        l && q.generate().discard_operations();
      }
    }, // _update_query()
    
    /* ------------------------------------------------------------------------
        fetch_operations( receiver [, query] )
