...
( 'errors', [ './pipelet' ], function( rs ) {
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      Dispatches errors from producers to error subscribers.
      
      This is a singleton, stateless, lazy pipelet with a union input.
      ...
      - attribute  : (optional String), attribute name associated with the error
      - position   : (Integer), of errored value is values (see bellow)
      - values     : (Array of Objects), from operation values or updates
      - engine     : (optional String), identifying the module that emitted the error
      - [engine]   : (optional Object) which attribute name is the same as the value of the
                     engine attribute above. Containing engine-specific description of the
                     error.
  */
  var errors = null;
