...
( 'transforms', [ './group_by' ], function( rs ) {
  ...
  */
  rs.Compose( 'delivers', function( source, flows, options ) {
    if( typeof flows == 'string' ) flows = [ flows ];
  ...
  */
  ...
  function Value_To_Attribute( options ) {
    var content   = safe_attribute( options.content || 'content' )
      ...
      , comma     = ', '
      , key_code  = options.key.map( safe_key_attribute ).join( comma )
      , code      = 'return { ' + key_code + comma + content + ': v'
    ;
    
    code += is_object( defaults ) && Object.keys( defaults ).length
  ...
  */
  function pick_attributes_to_array( attributes ) {
    return is_array( attributes )
      ...
      : is_object( attributes ) ? Object.keys( attributes ).map( attribute_to_array ) : []
    ;
  ...
  rs.Compose( 'pick', function( source, attributes, options ) {
    ...
    function make_transform( attributes, fixed ) {
      fixed = JSON.stringify( typeof fixed == 'string'
        ? { flow: fixed }
