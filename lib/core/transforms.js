  ...
  */
  ...
  function Value_To_Attribute( options ) {
    var content   = safe_attribute( options.content || 'content' )
      ...
      , comma     = ', '
      , key_code  = options.key.map( safe_key_attribute ).join( comma ) // safe
  ...
  /* --------------------------------------------------------------------------
      ...
      bellow will be silently ignored:
        ...
        - (Object): which attribute names are output attribute names
          and attribute values are source attribute names - e.g.
