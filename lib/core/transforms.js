...
( 'transforms', [ './group_by', '../util/cancelable' ], function( rs, cancelable ) {
  ...
  /* --------------------------------------------------------------------------
      ...
      - **transform** (Function or Object):
        ...
        - (Function): ```( value, next, operation, options ) -> cancel```.
          ...
          - **next** (Function): to emit a transformed value or an error,
            see Asynchronous transform() above.
  ...
  */
      ...
      function assynchronous_update( fetch, next ) {
        return function( source_updates, options ) {
          ...
          function update( update, next ) {
            ...
            function cancel() {
              slots && slots.forEach( function( cancel ) {
            ...
            function removed( error, value ) {
              slots && slots.shift(); // remove cancel if asynchronous
            ...
            function added( error, value ) {
              slots && slots.pop(); // remove cancel if asynchronous
  ...
  function Alter_Input( p, name, query_transform, options, input_transactions ) {
    ...
    // ToDo: inputs, provide query_transform, untag and maybe input_transactions as options
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      @short Specifies upstream dataflows that can be subscribed-to of fetched
      ...
      - query:
        ...
        - (Pipelet): where values' attribute ```id``` is a string
          specifying a flow name.
      ...
      It prevents upstream subscriptions and fetch from downstream pipelets
      to proceeed upstream when they do not match query.
      ...
      ```javasacript
        rs
          ...
          .delivers( rs.set( [ { id: 'users' }, { id: 'profiles' }, { id: 'projects' }, { id: 'imagaes' } ] ) )
        ;
      ```
      ### See Also
  ...
  */
  rs.Compose( 'delivers', function( source, flows, options ) {
    flows = is_object( flows ) && flows.api
        // This is a Pipelet, get greedy reactive state
      ? flows.set().a
      
      : // String or Array (exception for anything else)
        ( is_string( flows ) ? [ flows ] : flows )
        .map( function( flow ) { return { id: flow } } )
    ;
    
    return source.pass_through(
      extend( {}, options, {
        name: options.name + ': ' + flows.map( function( _ ) { return _.id } ).join( ', ' ),
        
        query_transform: function( term ) {
          var flow = term.flow;
          
          if ( flows.find( function( _ ) { return _.id == flow } ) ) return term
        }
      } )
    )
  } ); // delivers()
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      @pipelet handle_errors( f, options )
      
      @short Handles errors for asynchronous map transform
      
      @parameters
      - **f** \\<Function>: continuation-passing style, signature:
        ```( ..., ( error, value ) -> undefined ) -> cancel```
        - **...**: parameters gathered from source values, see option
          *parameters* bellow.
        
        - **error** \\<Error>: optional error, if something went wrong
        
        - **value**: emitted value, will be embedded in *emit* option
          property name.
        
        - **cancel** \\<Function>: optional cancel function, if *f*
          is cancelable.
      
      - **options** \\<Object>: optional pipelet map() options plus:
        
        - **key** \\<String []>: @key of emitted values.
        
        - **emit** \\<String>: optional property name for collected values
          from f. If emit option is not defined, all properties from
          emitted value by *f* are shallow copied to emited values.
        
        - **parameters** \\<[]>: optional parameters for f from
          source property names. Defaults provides source values objects
          as first parameter of f. Each parameter is defined by either:
          - \\<String>: source property name, or
          - \\<[]>:
            - 0 \\<String>: source property name
            - 1: default value when source property is falsy.
        
        - **ignore_error_codes** \\<[]>: optional error codes to ignore
          from f ```error.code```.
        
        - **name** \\<String>: optional name for pipelet property in emitted
          error Object.
      
      @emits
      - all source properties.
      
      - \[options.emit]: optional, result value from *f* when no errorr.
      
      - all properties from emmitted values from *f* if ```options.emit```
        is not defined and there are no errors.
      
      - **error** \\<Object>: optional error value, of any:
        
        - **message** \\<String>: human-readable error message.
        
        - **code**: from f error.code, if any.
        
        - **pipelet** \\<String>: this pipelet name, see name option.
        
        - **p** \\<String>: first missing identity property, if any.
      
      @description
      This is a @greedy, @assynchronous, @stateless pipelet.
      
      It maps its parameter function providing the following services:
      - error handling:
        - emits error Object with message, code, and pipelet
        - verifies source values identity presence from options.keys
        - catches synchronous errors
        - forwards upstream errors
        - optionally ignores error codes
      
      - optionally emits property \\[options.emit]
      - builds function parameters with optional default values
  */
  rs.Compose( 'handle_errors', function( source, f, options ) {
    var key                = options.key || options.source_key
      , key_length         = key.length
      , out_emit           = options.emit
      , parameters         = options.parameters
      , ignore_error_codes = options.ignore_error_codes
    ;
    
    if ( parameters ) {
      parameters = Function( [ 'v', 'done' ],
        'return [ ' +
          parameters.map( function( p ) {
            ...
            if ( is_array( p ) ) {
              var or = '||' + RS.Code.safe_value( p[ 1 ] ); // default value
              
              p = p[ 0 ]
            }
            
            return 'v' + RS.Code.safe_dereference( p ) + or
          } ) +
        ', done ]'
      );
    }
    
    return source.map( process, options );
    
    function process( value, emit ) {
      if ( value.error ) return emit( null, value );
      
      var out = extend_2( {}, value )
        , i   = -1
        , p
      ;
      ...
      try {
        while ( ++i < key_length )
          if ( value[ p = key[ i ] ] == ___ ) // null or undefined
            return emit_error( 'Missing identity property', p );
        
        return f.apply( null, parameters ? parameters( value, done ) : [ value, done ] )
      
      } catch ( error ) {
        emit_error( error )
      }
      
      function done( error, emitted ) {
        ! error || ignore_error_codes && ignore_error_codes.indexOf( error.code ) >= 0
          
          ? emit( null, out_emit
              ? ( out[ out_emit ] = emitted, out )
              : extend_2( out, emitted )
            )
          
          : emit_error( error )
      } // done()
      
      function emit_error( error, p ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      - **options** (Object): optional @@pipelet:filter() options:
        ...
        - **pick_key** (Array of Strings): @@key for pipelet pick(),
          will also use pipelet optimize().
  ...
  */
  rs.Compose( 'filter_pick', function( source, parent, expression, options ) {
    var pick               = picker( expression )
      , pick_key           = options.pick_key
      , pick_options       = { name: options.name + '-pick', key: pick_key }
      , inverse_expression = ! options.greedy && picker.inverse_expression( expression )
    ;
    ...
    parent = parent.map( pick, pick_options );
    
    return source.filter( ( pick_key ? parent.optimize() : parent ), options )
  } ); // filter_pick()
