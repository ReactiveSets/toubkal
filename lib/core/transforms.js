...
( 'transforms', [ './group_by', '../util/cancelable' ], function( rs, cancelable ) {
      ...
      function assynchronous_operation( operation, done ) {
        ...
        return function( values, options ) {
          ...
          function no_clone_transform( value, next ) {
            return transform( value, operation, options, next )
          }
          
          function clone_transform( value, next ) {
            return transform( extend_2( value ), operation, options, next )
          }
        }
        ...
        return function( updates, options ) {
          ...
          function update( update, next ) {
            ...
            var remaining = 2
              ...
              , add
              , slots = [
                  transform( update[ 0 ], fetch + 3, removed ),
                  transform( update[ 1 ], fetch + 2, added   )
                ]
            ;
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      @short Groups all source values in one content attribute
      
      @description
      This pipelet is deprecated, use pipelet group() instead. It is
      actually implemented as a composition only using
      ```group( null, options )```.
      ...
      ```
  */
  rs.Compose( 'values_to_attribute', function( source, options ) {
    return source.group( 0, options )
  } );
