...
( 'transforms', [ './group_by' ], function( rs ) {
  ...
  */
  ...
  function Alter( transform, options ) {
    ...
    that._input  = that._input  || new Alter.Input ( that, name, query_transform, options );
    that._output = that._output || new Alter.Output( that, options, query_transform );
    ...
    var code = new Code()
      ._function( 'that.__transform', void 0, [ 'X' ] )
        ._var( vars )
  ...
  function Alter_Output( p, options, query_transform ) {
    ...
    Pipelet.Output.call( that, p, options );
  ...
  */
  ...
  rs.Compose( 'pick', function( source, attributes, options ) {
    ...
    function make_transform( attributes, fixed ) {
      if ( typeof fixed == 'string' ) fixed = { flow: fixed };
