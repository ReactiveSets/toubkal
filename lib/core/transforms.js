...
( 'transforms', [ './group_by' ], function( rs ) {
  ...
  */
  ...
  function Alter( transform, options ) {
    ...
    if ( options.asynchronous ) return assynchronous();
    ...
    function assynchronous() {
      // ToDo: cancel asynchronous cancellable maps on pipelet disconnection from source
      // ToDo: limit should be shared beetween all invocations of cancelable_map(), may even be shareable at a higher level
      that._add    = assynchronous_operation( 'create', emit( '__emit_add'    ) );
      that._remove = assynchronous_operation( 'delete', emit( '__emit_remove' ) );
      that._update = assynchronous_update( '', emit_operations );
      
      // ToDo: implement assynchronous fetch and update_upstream_query
      ...
      function emit( emit_method ) {
        ...
        function done( options ) {
          ...
          function next( values ) {
            that[ emit_method ]( values, options )
          }
        }
      } // emit()
      
      function emit_operations( adds, removes, updates, options ) {
      ...
      function assynchronous_operation( operation, done ) {
        return function( values, options ) {
          ...
          cancelable_map( values, limit, no_clone ? no_clone_transform : clone_transform, done( options ) );
          
          function no_clone_transform( value, next ) {
            return transform( value, operation, mext )
          }
          
          function clone_transform( value, next ) {
            return transform( value = extend_2( value ), operation, next );
          }
        }
        ...
        function get( count, got, next ) {
          ...
          function cancel() {
            queue = queue.filter( function( _ ) {
              ...
              // found
              next( _[ 0 ] ); // may only modify queue on next tick 
      ...
      */
      function cancelable_map( values, limit, f, next, cancel_on_error ) {
        var count    = values.length
          ...
          , out      = []
          , i        = -1
          , slots    = [] // 0: initialiwzed or not-cancellable running; Function: cancellable running; 1: done; 2: cancelled
          , canceled
      ...
      function assynchronous_update( prefix, next ) {
        return function( updates, options ) {
          ...
          // ToDo: cancel map on pipelet disconnection from source
          cancelable_map( updates, limit, update, done );
          
          function done() {
            next( adds, removes, updates, options )
          }
          ...
          function update( update, next ) {
            ...
            var remaining = 2
              ...
              , add
              , slots = [
                  transform( update[ 0 ], prefix + 'update_delete', removed ),
                  transform( update[ 1 ], prefix + 'update_create', added   )
                ]
            ;
      ...
      function assynchronous_fetch( receiver, query, query_changes, destination ) {
        ...
        Input.prototype._fetch.call( that, _receiver, query, query_changes, destination );
        
        function _receiver( values, no_more, operation, options ) {
          if ( operation > 1 ) return assynchronous_update( 'fetch_', emit_operations );
          
          assynchronous_operation( operation ? 'fetch_delete' : 'fetch_create', done )( values );
