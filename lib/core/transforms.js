  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      @description:
        ...
        This is a stateful, synchronous, lazy pipelet.
      
      @parameters:
        - attributes (Array of Strings): attribute names.
        - options (optional Object): options for map()
  */
  rs.Compose( 'pick', function( source, attributes, options ) {
    return source
      .map( pick, extend( { query_transform: identity }, options ) )
    ;
    
    // ToDo: handle aliases (different attribute name for input and output)
    // ToDo: generate code, removing while loop
    function pick( value ) {
      var _value = {}
        , j = -1
        , a
  ...
  /* -------------------------------------------------------------------------------------------
      @pipelet filter_map( parent, flow, attributes, options )
      
      @short Select source values matching parent dataflow values.
      
      @description:
        This is a @@synchronous, @@stateless, @@lazy pipelet.
        
        It performs a kind of stateless and lazy inner join between source and parent,
        where the output only receives unaltered source values.
        
        This is a @@lazy, and optimized version of code following a filter()
        by a map() pattern that is otherwise greedy:
        ...
        source
          .filter(
            parent
              .map( function( value ) {
                return { flow: flow, project_id: value.id };
              } )
          )
        ```
        
        With filter_map() this can be re-written as:
        
        ```javascript
          source.filter_map( parent, flow, { project_id: 'id' } )
        ```
        
        Which is shorter and lazy thanks to the use of a query_transform on map()
        and filter_keys option on the filter() pipelet.
      
      @parameters:
        - parent (Pipelet): dataflow of values used to select source values
        
        - flow:
          - (String): a flow attribute to lookup source
          - (Object): a list of attributes with static values to lookup source
        
        - attributes (Object): which attribute names are source attribute names
            and attribute values are parent attribute names.
        
        - options (optional Object): options for map()
  */
  rs.Compose( 'filter_map', function( source, parent, flow, attributes, options ) {
    attributes = Object.keys( attributes ).map( function( k ) { return [ k, attributes[ k ] ] } );
    
    if ( typeof flow == 'string' ) flow = { flow: flow };
    
    var filter_keys     = attributes.map( function( k ) { return k[ 0 ] } )
      , transform       = attributes.map( to_string )
      , query_transform = attributes.map( swap ).map( to_string )
    ;
    
    transform = Object.keys( flow )
      .map( function( k ) { return k + ': "' + flow[ k ] + '"' } )
      .concat( transform )
    ;
      ...
      .filter(
        ...
        parent
          .map(
            ...
            { query_transform: make_transform( query_transform ) }
          )
        ,
    ...
    function make_transform( attributes ) {
      return new Function( '_', 'return { ' + attributes.join( ', ' ) + ' }' )
    } // make_transform()
  } ); //filter_map()
