...
( 'transforms', [ './group_by' ], function( rs ) {
  ...
  */
  rs.Compose( 'pick', function( source, attributes, options ) {
    ...
    // ToDo: rewrite using filter_pick() generators
    function pick( value ) {
      var _value = {}
        , j = -1
        , a
      ...
      @description:
        ...
        source
          .filter(
            parent
              .map( function( value ) {
                return { flow: 'issues', project_id: value.id };
              } )
          )
        ```
  ...
  */
  rs.Compose( 'filter_pick', function( source, parent, _static, attributes, options ) {
    ...
    var transform       = make_transform( attributes, _static )
      , query_transform = make_transform( attributes.map( swap  ) )
    ;
    ...
    return source
      .filter(
        ...
        parent
          .map( transform, { query_transform: query_transform, name: 'filter_pick#pick-' + options.name } )
        ,
