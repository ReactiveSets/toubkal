...
( 'transforms', [ './group_by' ], function( rs ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      @parameters
      - **attributes**: attributes not matching one of the valid formats
      bellow will be silently ignored:
        - (Array): attribute definitions, each attribute is:
          - (String): the attribute name
          
          - (Array of Strings):
            - **0** (String): output attribute name,
            - **1** (String): source attribute name
            
          e.g. ```[ "id", "user_id" ]``` will emit ```"id"``` attribute which
          value is source ```"user_id"``` attribute.
        
        - (Object): deprecated, which attribute names are output attribute
          names and attribute values are source attribute names - e.g.
          ```{ id: "user_id" }``` will emit ```"id"``` attribute which value
          is source ```"user_id"``` attribute.
          
          This syntax is deprecated as not compatible with @@function:picker()
          expressions.
      
      - **options** (optional Object): options for map() plus:
        - **fixed**:
          - (String): a flow attribute value to add to output
          
          - (Object): attributes with fixed values to add to output
  */
  function pick_attributes_to_array( attributes ) {
    return ( is_array( attributes )
      
         ? attributes.map( to_array_of_2_strings )
         
         : is_object( attributes )
         
           ? Object.keys( attributes ).map( attribute_to_array_of_2_strings )
           
           : []
    
    ).filter( identity );
    
    function to_array_of_2_strings( v ) {
      return is_string( v )
           ? [ v, v ]
           
           : is_array( v ) && v.length == 2 && is_string( v[ 0 ] ) && is_string( v[ 1 ] )
             ? v
             
             : null
    }
    
    function attribute_to_array_of_2_strings( a ) {
      var source_attribute = attributes[ a ];
      
      return is_string( source_attribute )
  ...
  // ToDo: review code generator against code injection vulnerability, search for all 'Function' strings in the project
  rs.Compose( 'pick', function( source, attributes, options ) {
    attributes = pick_attributes_to_array( attributes );
    
    var transform       = make_transform( attributes, options.fixed )
    ...
    function make_transform( attributes, fixed ) {
      var safe_o = 'var o=' + JSON.stringify( is_string( fixed )
        ? { flow: fixed }
        : is_object( fixed ) ? fixed : {}
      ); // safe
      
      var safe_expressions = attributes.map( safe_to_code_string ); // safe
      
      safe_expressions.unshift( safe_o ); // safe
      
      safe_expressions.push( 'return o' ); // safe
      
      return new Function( '_', safe_expressions.join( ';' ) ); // safe
      
      function safe_to_code_string( a ) {
        return 'if(_.hasOwnProperty(' + safe_string( a[ 1 ] ) + '))o' // safe
             + safe_dereference( a[ 0 ] ) // safe
             + '=_' + safe_dereference( a[ 1 ] ) // safe
      } // safe_to_code_string()
    } // make_transform()
  } ); // pick()
  
  /* --------------------------------------------------------------------------
      @pipelet filter_pick( parent, fixed, attributes, options )
      ...
      - **fixed**: for pipelet pick():
        - (String): a flow attribute value for source lookup
        - (Object): a list of attributes with fixed values for source lookup
      
      - **attributes**: for pipelet pick():
        - (Array): attribute definitions, each attribute is:
          - (String): the attribute name
          
          - (Array of Strings): first string is output attribute name,
            second string is source attribute name - e.g.
            [ 'id', 'user_id' ] will emit "id" attribute which value
            is source "user_id" attribute.
      ...
      source
        .filter(
          parent.pick( [ [ "project_id", "id" ] ], { fixed: 'issues' } ),
      ...
      ```
  */
  rs.Compose( 'filter_pick', function( source, parent, fixed, attributes, options ) {
    attributes = pick_attributes_to_array( attributes );
    ...
    return source
      .filter(
        ...
        parent
          .pick( attributes, { fixed: fixed, name: 'filter_pick#pick-' + options.name } )
        ,
