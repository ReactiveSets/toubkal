...
( 'stateful', [ './filter' ], function( rs ) {
  ...
  /* --------------------------------------------------------------------------
      ...
      @parameter options:
        ...
        - pipelet (String): stateful pipelet name, default is "set". other
          possible values are "unique", and "optimize".
  */
  .Compose( 'cache', function( source, options ) {
    var rs      = source.namespace()
      , output  = rs.pass_through( options )
      , pipelet = options.pipelet || 'set'
    ;
  ...
  /* --------------------------------------------------------------------------
      ...
      @parameters:
        ...
        - options (Object): options for dispatch(), plus:
          - pipelet (String): stateful pipelet name for @@cache() pipelet,
            default is "set".
