...
( 'stateful', [ './filter' ], function( rs ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      @short A set discarding @@duplicate @@values and with no @@antistate
      
      @parameters:
      - values (Array of Objects): initial values of set, duplicates will be
        discarded, if any
      
      - options (Object): @@class:Set options plus:
        - **verbose** (Boolean): if ```true``` emit errors on duplicate adds,
          default is ```false```, duplicate adds are discarded silently.
        
        - **key** (Array of String): the list of attributes defining value's
          identities, default is @@upstream @@key or ```[ 'id' ]```
      
      @description:
      Discards values that have the same @@identity\. This implies that the
      set has a proper @@key set. When the key is not properly set, a
      possible symptom is that values' identities are undefined, yielding
      more duplicates than desired.
      
      If option verbose is ```true```, and duplicates are added, they are
      emitted in bulk in the ```"error"``` @@dataflow, with the error code
      ```"DUPLICATE_KEY"```. This may be used by a @@downstream pipelet to
      remove duplicates.
      ...
      - Updates which removed values' identity is not found are emitted
        as adds using their added value, discarding removed value silently
      
      - If values with the same identity are removed several times, they
        are removed on the first remove, then ignored
    ...
    _add: function( values, options ) {
      ...
      while ( ++i < l ) {
        ...
        if ( v.flow != 'error' && identities[ id ] ) {
          // ToDo: allow duplicates to replace existing values if different, using update
