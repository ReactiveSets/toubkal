...
( 'stateful', [ './filter' ], function( rs ) {
  ...
  /* --------------------------------------------------------------------------
      ...
      - options: @@Greedy options:
        ...
        - key (Array of Strings): default is [ 'id' ]
        
        - flush (Boolean): flush transactions at the end of add, remove,
  ...
  */
  .Compose( 'cache', function( source, options ) {
    var rs            = source.namespace()
      ...
      , name          = options.name
      , output        = greedy || rs.pass_through( options_name( 'output' ) )
      , filter        = greedy ? source : source.filter( rs.query_updates( output ).debug( 'query_updates' ), options_name( 'filter' ) )
      , input         = synchronize || synchronizing ? filter.union( [], options_name( 'union' ) ) : filter
    ...
    function options_name( postfix ) {
      return extend( {}, options, { name: name + '-' + postfix } );
    } // options_name()
  ...
  /* --------------------------------------------------------------------------
      ...
      - schema (Pipelet): @@dataflow definitions. Used attributes:
        ...
        - pipelet (String): @@stateful pipelet name, default is
          ```options.pipelet```.
        - debug: use pipelet trace() to debug this cache
          - (true): allow default traces
          - (Object): provide pipelet trace() options
  ...
  */
  .Compose( 'database_cache', function( source, schema, options ) {
    var rs            = source.namespace()
      ...
      , synchronizing = options.synchronizing
      , output_query  = rs.pass_through( options_name( 'query' ) )
      , query
      , output
      , gatherer
    ;
    ...
    function cache( source, options ) {
      ...
      return source
        ...
        .cache( { name: _name, key: this.key || [ 'id' ], pipelet: this.pipelet || pipelet, synchronizing: synchronizing } )
        
        .debug( debug && _name, typeof debug == 'object' ? debug : {} )
        
        .delivers( [ flow ], options_name( 'delivers' ) )
      ;
    } // cache()
  } ) // database_cache()
} );
