...
( 'stateful', [ './filter' ], function( rs ) {
  ...
  */
  .Compose( 'cache', function( source, options ) {
    ...
    // See also bellow disconnection of state and output union()
    options.pipelet && state.remove_source_with( source );
    
    synchronize && fetch_and_apply_differences( synchronize );
    
    if( synchronizing ) {
      var to_synchronizing = fetch_and_apply_differences( synchronizing )
      ...
      to_synchronizing
        ...
        // Disconnect synchronizing's source if cache's source disconnect
        .remove_source_with( source, to_synchronizing )
      ;
    }
    
    remove_branch( output = state.union( [ output ] ) );
    
    // Disconnect pipelet with source disconnection, see above
    options.pipelet && output.remove_source_with( source, state );
