...
( 'stateful', [ './filter' ], function( rs ) {
  ...
  */
  function Set( values, options ) {
    ...
    if ( values ) {
      is_array( values )
        || ( values = [ typeof values == 'object' ? values : { id: values } ] )
      ;
  ...
  /* --------------------------------------------------------------------------
      ...
      @parameters
      - **values** (Array of Objects): initial values of set, duplicates
        will be discarded, if any
      
      - **options** (Object): pipelet set() options, in particular:
        - **key** (Array of String): the set's @@key
      
      @description
      Important considerations:
      - This relies on the set having a proper @@key set. When key does
        not match source values, a possible symptom is that values'
        @@[identities](identity) are undefined, and only one value is
        added or updated into the unique set;
      
      - Updates must preserve values' @@identity, i.e. removed and added
        values must have the same identity, otherwise behavior is
        unspecified, no error is emitted.
      
      Behavior:
      - Added values which have the same identity as an existing value
        but which actual content is different than the existing value
        result in an update of the existing value into the new value;
      
      - Removed values which identity are not found in set are discarded;
      
      - Updates which removed values' identity is not found are emitted
        as adds using their added value, discarding removed value.
  */
  ...
  Set.Build( 'unique', Unique, function( Super ) { Set_prototype = Super; return {
    _add: function( values, options ) {
      ...
      while ( ++i < l ) {
        ...
        if ( identities[ id ] ) {
          // this is a duplicate
          position = that._a_index_of( v );
          old_value = position != -1 && that._get()[ position ];
          
          if ( old_value && ! value_equals( v, old_value ) )
            // new value is different that old value, update it
            updated.push( [ old_value, v ] )
    ...
    _update: function( updates, options ) {
      ...
      updates = updates
        .filter( function( update ) {
          var found = identities[ that._identity( update[ 0 ] ) ];
          
          // Does not verify identity of added value, see documented warning
          found || adds.push( update[ 1 ] );
      ...
      if ( adds.length ) {
        t = new Transaction( 2, options );
        
        that._add( adds, t.next_options() );
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      - **schema** (@@class:Pipelet\): set of @@dataflow definitions for each
        ...
        - **key** (String[]): optional, default is ```[ 'id' ]```
      ...
      - **options** (Object): options for @@pipelet:dispatch() and
        ...
        - **delay_connect**: connect caches to pipelet dispatch() gatherer
          before dispatcher. Default is *true*.
      ...
      This is a @@synchronous, @@stateful, @@lazy, @@singleton pipelet.
      It is also lazy on *schema*.
      ...
      - Pipelet socket_io_synchronizing()
  */
  .Singleton( 'database_cache', function( source, schema, options ) {
    var rs            = source.namespace()
      , name          = options.name
    ...
    output = source.dispatch( schema.filter( query, options_name( 'filter' ) ), cache, options_name( ___, options ) );
    ...
    rs
      ...
      // query updates it would not be part of the same transaction. This will
    ...
    function options_name( _name, options ) {
      return extend( { delay_connect: true }, options, { name: _name ? name + '-' + _name : name } );
    } // options_name()
