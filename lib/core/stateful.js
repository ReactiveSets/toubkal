...
( 'stateful', [ './filter' ], function( rs ) {
  ...
  */
  .Compose( 'cache', function( source, options ) {
    var rs            = source.namespace()
      ...
      , filter        = greedy ? source : source.filter( rs.query_updates( output, options_name( 'query_updates' ) ), options_name( 'filter' ) )
  ...
  */
  .Compose( 'database_cache', function( source, schema, options ) {
    ...
    query = rs
      .query_updates( output_query, options_name( 'query_updates' ) )
