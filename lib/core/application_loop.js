...
( 'application_loop', [ './pipelet' ], function( rs ) {
  ...
  var RS       = rs.RS
    , RS_log   = RS.log
    , log      = RS_log.bind( null, 'application_loop' )
    , picker   = RS.picker
  ...
  /* --------------------------------------------------------------------------
      @pipelet application_loop( components, options )
      
      @short Process application from components and source dataflows
      
      @parameters
      - **components** (@@class:Pipelet\): dataflow of components for the
        current application state. Each component attributes are:
        - **pipelet** (String): the name of the pipelet for this component.
          It is invoked with:
            - **source** (@@class:Pipelet\): *source* filtered by the first
              query.
            
            - first parameter (Object): current component @@value.
            
            - next parameters: *source* filtered by second to last queries.
            
            - no options for now, may be added in future version
        
        - **queries** (Array of Array of Objects): Each Array of Objects
          specifies a query for each pipelet parameter, and where the first
          query is for the pipelet's source.
          
          Each object specifies an OR term for the query. Each OR term has
          attributes which correspond to the following specification:
          - attribute name starts with non-underscore ("_") character:
            - if attribute value starts with a dot ("."), the value is
              picked from the current component attribute path - e.g.
              ```{ id : ".url.project_id" }``` will pick the value
              of *component.url.project_id* and use it for the *id*
              attribute of current query term.
            
            - otherwise value is that of the attribute value - e.g.
              ```{ flow: "projects" }``` which will use the *String*
              "projects" as value for the *flow* attribute of current
              query term.
          
          - attribute name starts with the underscore ("_") character -
            e.g. ```"_url"```, pick attributes(s) from
            component depending on attribute value type:
            - (String): to specify a single attribute from url - e.g.
               ```{ _url: 'project_id' }``` which does
               ```{ project_id: component.url.project_id }```.
        ...
        - **flow** (String): name of output flow or null if this component
          emits nothing.
        ...
        - **delivers* (Array of Strings): if this pipelet delivers more than
          one output flow, these are the name of all delivered flows. These
          will be used by pipelet deliver().
  ...
  */
  .Compose( 'application_loop', { union: true }, function( input, components, options ) {
    var output = input.dispatch( components, start_component, { name: options.name, no_remove_fetch: true } );
    
    // loopback
    output
      .pass_through( { _t_postfix: 'x' } )
    ...
    function start_component( source, options ) {
      ...
      var component  = this
        ...
        , dataflows  = {} // filters on dataflows referenced in queries
      ;
      ...
      function process_queries( query ) {
          ...
          if ( has_pipelet ) {
            ...
            query = query
              .filter( function( term ) {
                if ( term.is_pick ) return 1;
                
                set.push( term );
              } )
            ;
            
            if ( set.length ) query.push( rs.set( set ) );
            
            // log( RS_log.pretty( query.map( function( term ) { return get_name( term ) } ) ) );
            
            query = rs.union( query );
          }
        }
        
        return query ? source.filter( query, { key: [ 'id' ] } ) : source;
        ...
        function compile_term( term ) {
          ...
          for ( p in term ) {
            ...
            if ( p.charAt( 0 ) != '_' ) {
              ...
              if ( c == '.' ) {
                ...
                out = dataflows[ dataflow ] = dataflows[ dataflow ] || source.flow( dataflow );
                
                if ( pick ) {
                  if ( pick != dataflow )
                    // ToDo: emit to global error dataflow
                    log( get_name( that, 'compile_term' ) + 'Error, found term with more than one @dataflow specification:', term );
              ...
              if ( typeof v == 'string' ) {
                ...
                v.forEach( function( term ) { set( term ) } )
              
              } else {
                // Object which attributes are picked from component property by attributes' values
                Object.keys( v ).forEach( function( k ) { set( v[ k ], k ) } )
              }
            }
          }
          
          if ( pick ) {
            out = out.map( picker( out_value ) )
          ...
          function set( a_in, a_out ) {
            if ( p[ a_in ] != null ) out_value[ a_out || a_in ] = p[ a_in ];
          }
        } // compile_term()
      } // process_queries()
    } // start_component()
