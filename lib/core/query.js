...
( 'query', [ './RS' ], function( RS ) {
  ...
  /* -------------------------------------------------------------------------------------------
      de&&ug()
  */
  var de = false, ug = log.bind( null, 'query' );
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      So a query is code expressed with sets of JavaScript objects. With Toubkal, these
  ...
  */
  ...
  /* -----------------------------------------------------------------------------------------
      ...
      Use cases:
        RS Queries are OR-expressions which terms are AND-expresssions as tested by this
        function.
  ...
  */
  Query.least_restrictive_keys = function( e0, e1, keys0, keys1 ) {
    ...
    while ( key = keys[ ++i ] ) {
      if ( e0[ key ] === e1[ key ] ) continue;
      
      // None is least restrictive
      return null;
      
      /*
      
      // ToDo: Query.least_restrictive_keys(): compare expressions that define comparable sets
  ...
  */
  ...
  Query.Evaluation_Context = function( value, attribute ) {
    ...
    that.__ = value[ attribute ];
  };
  
  Loggable.subclass( 'Query.Evaluation_Context', Query.Evaluation_Context, {
  ...
  function get_regexp( r ) {
    ...
    if ( r == ___ ) {
      ...
      this.regexp = toString.call( r ) === '[object RegExp]' ? r : r = new RegExp( r );
    }
  ...
  Operator( 'match', function( text, r ) {
    ...
    r = get_regexp.call( that, r );
    
    that.groups = r.exec( text );
  ...
  Loggable.subclass( 'Query', Query, {
    ...
    */
    and: function( q1 ) {
      ...
      while ( e0 = q0[ ++i0 ] ) {
        ...
        q1_loop: // to continue from inner loop when a term cannot match because an expression is different
        ...
        while ( e1 = q1[ ++i1 ] ) {
          ...
          var k1        = keys1s[ i1 ]
            , k1l       = k1.length
            , different = false        // considered equal until proven different
            , e1_longer = k1l > k0l
            , i         = -1
          ;
          ...
          while ( key = ka[ ++i ] ) {
            if ( key in eb ) {
              ...
              if ( eb[ key ] !== ea[ key ] ) {
                // ToDo: AND complex query expressions here
              ...
              // Expressions can no longer strictly equal
              different = true;
          ...
          if ( different ) {
            // merge different queries, produce a longer query than both
    ...
    */
    generate: function() {
      ...
      for ( var i = -1; ++i < l; ) {
        ...
        for ( var t in p ) {
          ...
          if ( terms_counts[ t ] )
            ++terms_counts[ t ] == 2
              && safe_vars.push( [
                '_' + t, // Code.._var() will use safe_identifier() on this, so this can be considered safe
                'v' + safe_dereference( t )
              ] )
            ;
      ...
      var code = new Code( get_name( that, 'generate' ) )
          ...
          if ( no_and_term ) {
            ...
            ._for( 'var i = -1, l = values.length', '++i < l' )
              ...
              for ( i = -1; ++i < l; ) {
                ...
                for ( var t in p ) {
                  var safe_value = p[ t ]; // made safe bellow
                  
                  if ( typeof safe_value == 'string' ) // only strings may be unsafe in comparison
                    safe_value = safe_string( safe_value )
                  ;
                ...
                code.add( 'if( ' + safe_expression.join( ' && ' ) + ' ) { out.push( v ); continue; }' ); // safe from safe variables
              }
  ...
  Loggable.subclass( 'Query_Tree', Query_Tree, {
      ...
      function add_term( node, term, term_keys ) {
        ...
        while( term_keys.length ) {
          ...
          for( var i = -1; key = keys[ ++i ]; ) {
            ...
            v = term[ key ]; // May be undefined, ToDo: add tests for undefined values
          ...
          node = branch[ v = term[ key ] ] = new Query_Tree_Node(); // v may be undefined
        }
        ...
        if ( false && node.subscribers.indexOf( subscriber ) != -1 )
          throw new Error( get_name( that, 'add' )
    ...
    route: function( operation, values ) {
      ...
      switch( operation ) {
        ...
        case 'clear':
          // Return immediately, all subscribers' source_subscriber_index are cleared by get_all_subscribers()
        return [ [ operation, this.get_all_subscribers() ] ];
      ...
      function route( node, value, add_value ) {
        ...
        while ( ++i < rl ) {
          ...
          if ( subscriber_index ) {
            ...
            subscribers.push( { input: input, v: values = [] } );
            
            // ToDo: add a test that fails if subscriber_index local variable is not assigned bellow:
            subscriber_index = input.source_subscriber_index = subscribers.length; // source subscriber indexes start at 1
          }
        ...
        for ( i = -1; key = keys[ ++i ]; )
          if ( value.hasOwnProperty( key ) ) {
            var v      = value   [ key ] // ToDo: add tests for undefined values
              , branch = branches[ key ]
              , child  = branch[ v ]
            ;
            ...
            if ( child ) {
              ...
              if ( typeof expressions == 'object' && is_array( expressions ) ) {
                // Route on complex expression
                var Query_evaluate = Query.evaluate
                  , expression
                  , j = -1
                ;
    ...
    /* ------------------------------------------------------------------------
        ...
        Returns an Array of all subcribers, at most once.
        
        Each subscriber is an Object:
        - input (Input): input for this subscriber
        
        ToDo: add tests for get_all_subscribers()
    */
    get_all_subscribers: function() {
      var subscribers = [];
      
      get_subscribers( this.top, subscribers );
      
      clear_source_subscriber_indexes( subscribers );
      
      return subscribers;
      
      function get_subscribers( node, subscribers ) {
        ...
        // Get subscribers only once per subscriber pipelet
        for ( i = -1; ++i < rl; ) {
          var input = node_subscribers[ i ];
          
          if ( input.source_subscriber_index ) continue; 
          
          // This pipelet is not yet marked
        ...
        for ( i = -1; key = keys[ ++i ]; ) {
          ...
          for ( var value in branch ) {
            ...
            if ( value != '[...]' ) {
              get_subscribers( expressions, subscribers );
            } else {
              if ( is_array( expressions ) ) {
                var j = -1, expression;
                
                while ( expression = expressions[ ++j ] ) {
                  get_subscribers( expression.child, subscribers );
                }
              }
            }
          } // for all values in branch
        } // for all node keys
      } // get_subscribers()
    }, // get_all_subscribers()
