    ...
    */
    remove: function( q1 ) {
      ...
      if ( q1.length ) {
        ...
        if ( q1.length )
          throw new Query_Error( that._get_name( 'remove' ) + 'expression not found: ' + log.s( q1 ) )
        ;
  ...
  */
  ...
  Loggable.subclass( 'Query_Tree', Query_Tree, {
    ...
    add: function( or_terms, subscriber ) {
      ...
      that.history.push( [ 'add', or_terms, subscriber, new Date() ] );
      ...
      function add_term( node, term, term_keys ) {
        ...
        if ( false && node.subscribers.indexOf( subscriber ) != -1 )
          throw new Error( that._get_name( 'add' )
    ...
    remove: function( or_terms, subscriber ) {
      ...
      that.history.push( [ 'remove', or_terms, subscriber, new Date() ] );
      ...
      function remove_term( node, term, term_keys ) {
        ...
        function fatal( message ) {
          throw new Query_Error( that._get_name( 'remove' ) + message
            + ', key: '       + key
            + ', i: '         + i
            + ', term: '      + pretty( term )
            + ', keys: '      + pretty( keys )
