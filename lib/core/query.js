...
( 'query', [ './RS' ], function( RS ) {
  ...
  */
  ...
  // Array..find() is not available on IE
  function array_find( a, f ) {
    if ( a )
      for ( var i = -1, l = a.length, v; ++i < l; )
        if ( f( v = a[ i ] ) ) return v
  } // array_find()
        ...
        function add_expression_to_branch( branch ) {
          ...
          if ( typeof v == 'object' && v ) { // complex expression
            var expressions = branch[ complex_expression_branch_value ]
              ...
              , expression  = array_find( expressions, has_v )
            ;
            
            expression ||
              expressions.push( expression = { e: v, child: new Query_Tree_Node() } )
            ;
    ...
    remove: function( or_terms, subscriber ) {
      ...
      function remove_term( node, term, term_keys ) {
        ...
        function remove_complex_expression() {
          ...
          if ( ! expression )
            return fatal( 'expression not found: ' + log.s( v ) )
          ;
