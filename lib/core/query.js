    ...
    */
    generate: function() {
      var that             = this
        ...
        , terms_counts     = {}
        , got_terms        = {}
        , safe_vars        = [ 'i=-1', 'l=values.length', 'v', 'out=[]' ]
        , no_AND_term      = false
        , safe_expressions = []
        , i                = -1
        , p
        ...
        ._function( 'this.filter', ___, [ 'values' ] );
          ...
          if ( no_AND_term ) // pass-all
            ...
            for ( i = -1; ++i < l; ) {
              ...
              for ( t in p ) {
                safe_value = p[ t ]; // made safe bellow
                ...
                safe_expression.push(
                  ( terms_counts[ t ] > 1 ? get_term( t ) : get_v() + safe_dereference( t ) )
                  + ' === ' + safe_value
                );
              }
            ...
            ._while( '++i < l' )
              .add( 'if ( ' + safe_expressions.join( ' || ' ) + ' ) out.push( v )' )
            .end()
      ...
      function get_term( t ) {
        var code = got_terms[ t ]
          ? safe_identifier( '_' + t )
          : '( ' + safe_identifier( '_' + t ) + ' = ' + get_v() + safe_dereference( t ) + ' )'
        ;
