...
( 'query', [ './RS' ], function( RS ) {
  ...
  */
  ...
  var fail = {} // a unique reference that cannot be strictly equal to any other value
    , ___ // undefined
  ;
  ...
  */
  ...
  function query_evaluate( value, attribute, expression, state ) {
    var context = new Evaluation_Context( value, attribute );
    
    return context.evaluate( expression, state ) !== fail
  } // Query.evaluate()
  ...
  function Evaluation_Context( value, attribute_name ) {
    ...
    function reset() {
      ...
      return that.__ = has_property.call( value, attribute_name ) ? value[ attribute_name ] : fail
    } // reset()
  ...
  Loggable.subclass( 'Evaluation_Context', Evaluation_Context, {
    ...
    evaluate: function( expression, state ) {
      var that = this
        , left = that.__
      ;
      ...
      if ( is_array( expression ) ) {
        ...
        while ( i < expression_length ) {
          if ( left === fail ) skip = 1; // until the next '||' or 'failed'
          ...
          switch( type ) {
            ...
            case object_s: // subexpression
              if ( ! skip ) left = that.evaluate( first, state )
            break;
                ...
                default:
                  if ( operator = operators[ first ] ) {
                    ...
                    if ( ! literal_parameters ) {
                      ...
                      while ( ++j < l )
                        if ( is_array( v = parameters[ j ] ) ) {
                          parameters[ j ] = left = that.evaluate( v, state );
                    ...
                    if ( stateful ) {
                      ...
                      if ( ! has_property.call( state, 'row' ) ) {
                        /*
                          Do not execute now, as stateful operators operate
                          on the entire result set which will be known once
                          all expressions are processed on all values.
                          
                          Mark the result set as stateful to re-execute later
                          with the row attribute set once all matching rows
                          are collected.
                        */
                        state.stateful = 1;
        ...
        left = that.evaluate_object( expression, state );
      }
    ...
    evaluate_object: function( object, state ) {
      var that = this
        , __   = that.__ // save __
        , left = __
      ;
        ...
        if ( has_property.call( object, a ) ) {
          ...
          switch( class_of( e ) ) {
            ...
            case 'Array':
              ...
              if ( that.evaluate( e, state ) === fail ) left = fail;
            break;
            
            case 'Object':
              that.__ = v;
              
              if ( that.evaluate_object( e, state ) === fail ) left = fail;
            break;
          }
  ...
  Operator
    ...
    // length property, shortcut for [ '.', 'length' ]
    ( 'length', function( left ) {
      return left && typeof left == object_s ? left.length : fail
    } )
    
    // Access value inside Object or Array from relative path to current value
    // [ '.', 'profile', 'age' ]
    ( '.', function( left ) {
      return subscript( left, arguments )
    }, { greedy: true } )
    
    // Access value from absolute path
    // [ '_', 'user', 'profile', 'age' ]
    ( '_', function() {
      return subscript( this.value, arguments )
    }, { greedy: true } )
    
    // Access and change current attribute path and current value
    // [ '__', 'user', 'profile', 'age' ]
    ( '__', function() {
      var that = this
        , a    = arguments
    ...
    */
    ...
    ( 'value', function( left ) {
      return left && typeof left == object_s && typeof left.valueOf == function_s
        ? left.valueOf() : fail
    } )
  ...
  function subscript( v, a ) {
    var a = slice.call( a, 1 ), key;
    
    for ( var i = -1, l = a.length; ++i < l; ) {
      if ( v == ___ || typeof v != object_s || ! has_property.call( v, key = a[ i ] ) )
        return fail
      ;
  ...
  */
  ...
  Stateful_Operator
    ( 'ascending', function( state, attribute_path, left ) {
      return sort_values( 1, state, attribute_path, left )
    }, { attribute_path: true, priority: 0 } )
    
    ( 'descending', function( state, attribute_path, left ) {
      return sort_values( 0, state, attribute_path, left )
    }, { attribute_path: true, priority: 0 } )
    
    ( 'limit', function( state, left, limit ) {
      if ( state.row > limit && ! state.linited ) {
  ...
  function sort_values( ascending, state, dimension, dimensions ) {
    if ( ! state.sorted ) {
      ...
      console.log( 'sort_values():', dimensions );
    ...
    function sorter( a, b ) {
      ...
      function value_of( o ) {
        if ( typeof o.valueOf == function_s ) {
          o = o.valueOf();
    ...
    */
    remove: function( q1 ) {
      ...
      function remove( q0, keys0s, q1, keys1s, removes ) {
        var i0, i1, e0, e1, keys1, l1;
        
        for ( i1 = -1; e1 = q1[ ++i1 ]; ) { // for all terms to remove
          l1 = ( keys1 = keys1s[ i1 ] ).length;
          
          for ( i0 = -1; e0 = q0[ ++i0 ]; )
            if ( keys0s[ i0 ].length == l1 ) {
              
              for ( var k = -1, key; key = keys1[ ++k ]; )
                if ( e0[ key ] !== e1[ key ] )
                  break;
    ...
    and: function( q1 ) {
      ...
      while ( e0 = q0[ ++i0 ] ) {
        ...
        while ( e1 = q1[ ++i1 ] ) {
          ...
          // Let 'ka' be the smallest key, and 'eb' the expression  with the largest key
          e1_longer
            ? ( ka = k0, eb = e1 )
            : ( ka = k1, eb = e0 )
          ;
  ...
  Loggable.subclass( 'Query_Tree', Query_Tree, {
      ...
      function add_term( node, term, term_keys ) {
        ...
        while( term_keys.length ) {
          ...
          for ( i = -1, l = keys.length; ++i < l; )
            ...
            if ( has_property.call( term, key = keys[ i ] ) ) {
    ...
    remove: function( or_terms, subscriber ) {
      var that            = this
        ...
        , history         = [ new Date(), get_name( subscriber ), 'remove', or_terms.slice() ]
        , or_term
      ;
    ...
    route: function( operation, values ) {
      ...
      function route( node, value, add_value ) {
        ...
        for ( i = -1; key = keys[ ++i ]; )
          if ( has_property.call( value, key ) ) {
