...
( 'query', [ './RS' ], function( RS ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      A query is a set of 'OR-AND' expressions, aka or-terms, where each
      object is an operand of the OR-expression and each property of each
      object is an operand of the sub-AND expression.
      ...
      So a query is code expressed with sets of JavaScript objects. With
      Toubkal, these expressions become dataflows, which can enable realtime
      dynamic filters as exposed by filter.js.
      
      Queries are used to retrieve subsets of values from source dataflows
      and to filter emitted operations to downstream pipelets.
      
      Query objects are created using a set of expressions as a parameter,
      ...
      Other performance considerations:
        ...
        operations performance. This is because query trees have to route a
        large number of data events while queries themselves are expected to
        have a much lower rate of change in most applications.
        
        To provide best query syntax while maintaining high performances
        might require to have two syntaxes: one for queries for designers,
        and one for queries trees which is internal to Toubkal's
        implementation.
  */
  ...
  */
  ...
  function least_restrictive_keys( e0, e1, keys0, keys1 ) {
    ...
    while ( key = keys[ ++i ] ) {
      ...
      if ( v0 !== v1 ) {
        t0o = typeof v0 == 'object' && v0;
        t1o = typeof v1 == 'object' && v1;
  ...
  */
  ...
  var Evaluation_Context = Query.Evaluation_Context = function( value, attribute ) {
    var that = this;
    
    Loggable.call( that, attribute );
    ...
    function reset() {
      ...
      return that.__ = value[ attribute ];
    } // reset()
  ...
  Loggable.subclass( 'Evaluation_Context', Evaluation_Context, {
    get_regexp: function( r, options ) {
      try {
        r = r == ___
          ? this.regexp || fail
          : this.regexp = toString.call( r ) === '[object RegExp]' ? r : new RegExp( r, options )
  ...
  Operator( [ 'failed', '||', '&&', 'reset' ] );
  
  Operator( '==', function( a, b ) { return equals( a, b ) ? b : fail; }, { range: { b1i: 1, b2i: 1 } } );
  Operator( '!=', function( a, b ) { return equals( a, b ) ? fail : b; } );
  
  Operator( '>' , function( a, b ) { return a >  b ? b : fail; }, { range: { b2:  Infinity         } } );
  Operator( '>=', function( a, b ) { return a >= b ? b : fail; }, { range: { b2:  Infinity, b1i: 1 } } );
  Operator( '<' , function( a, b ) { return a <  b ? b : fail; }, { range: { b1: -Infinity         } } );
  Operator( '<=', function( a, b ) { return a <= b ? b : fail; }, { range: { b1: -Infinity, b2i: 1 } } );
  ...
  Operator( 'RegExp', function( _, r, options ) {
    return this.get_regexp( r, options );
  } );
  ...
  Operator( 'match', function( text, r ) {
    ...
    that.groups = r === fail ? null : r.exec( text );
    
    return that.groups ? that.groups.index : fail;
  } );
  ...
  /*
      ...
      may fail.
  */
  Operator( 'in_set', function( left, a0 ) {
  ...
  function in_set( set, value ) {
    ...
    while ( ++i < l ) {
      ...
      switch( operator ) {
        case '=='  : if (            left == bound_1         ) r = bound_1; break;
        case '>='  : if ( bound_1 <= left                    ) r = bound_1; break;
        case '>'   : if ( bound_1 <  left                    ) r = bound_1; break;
        case '<'   : if (                    left <  bound_2 ) r = bound_2; break;
        case '<='  : if (                    left <= bound_2 ) r = bound_2; break;
        
        case '<<'  : if ( bound_1 <  left && left <  bound_2 ) r = bound_2; break;
        case '<=<' : if ( bound_1 <= left && left <  bound_2 ) r = bound_2; break;
        case '<=<=': if ( bound_1 <= left && left <= bound_2 ) r = bound_2; break;
        case '<<=' : if ( bound_1 <  left && left <= bound_2 ) r = bound_2; break;
      }
      ...
      if ( left > bound_2 ) break; // assumes non-intersecting and sorted ranges
    }
  ...
  // range operators
  Operator( '<=<' , function( v, a, b ) { return a <= v && v <  b || fail; }, { range: { b1i: 1         } } );
  Operator( '<=<=', function( v, a, b ) { return a <= v && v <= b || fail; }, { range: { b1i: 1, b2i: 1 } } );
  Operator( '<<',   function( v, a, b ) { return a <  v && v <  b || fail; }, { range: {                } } );
  Operator( '<<=',  function( v, a, b ) { return a <  v && v <= b || fail; }, { range: {         b2i: 1 } } );
  ...
  Loggable.subclass( 'Query', Query, {
    ...
    */
    ...
    /* -----------------------------------------------------------------------------------------
        ...
        Description
        Should work with complex expressions
        
        ToDo: add automated tests for differences on queries with complex expressions
    */
        ...
        loop1: for ( i1 = -1; keys1 = keys1s[ ++i1 ]; )
          if ( l0 == keys1.length ) {
            ...
            for ( j = -1; j < l0; ) {
              ...
              if ( v0 !== v1
                && ! ( typeof v0 == 'object' && typeof v1 == 'object' && equals( v0, v1 ) )
        ...
        while ( e1 = q1[ ++i1 ] ) {
          ...
          while ( key = ka[ ++i ] ) {
            if ( key in eb ) {
              ...
              if ( t0 !== t1 ) {
                var t0no = typeof t0 != 'object', t1no = typeof t1 != 'object';
    ...
    add: function( or_terms, subscriber ) {
      ...
      function add_term( node, term, term_keys ) {
        ...
        while( term_keys.length ) {
          ...
          for ( i = -1; key = keys[ ++i ]; ) // ToDo: allow falsy key
        ...
        function add_expression_to_branch( branch ) {
          ...
          if ( typeof v == 'object' && v ) { // complex expression
            var expressions = branch[ complex_expression_branch_value ]
                            = branch[ complex_expression_branch_value ] || []
              , position    = expressions.indexOf( has_v )
            ;
            
            return expressions[ position < 0
              ? expressions.push( { e: v, child: new Query_Tree_Node() } ) - 1
              : position
            ].child;
          }
          
          return branch[ v ] || ( branch[ v ] = new Query_Tree_Node() )
          
          function has_v( expression ) {
            return equals( expression.e, v )
          } // has_v()
    ...
    remove: function( or_terms, subscriber ) {
      ...
      function remove_term( node, term, term_keys ) {
        ...
        if ( term_keys.length ) {
          ...
          for ( i = -1; key = keys[ ++i ]; ) { // ToDo: allow falsy keys
            // Lookup this node key in the term
            if ( ! term.hasOwnProperty( key ) ) continue; // Not found, keep looking
        ...
        function remove_complex_expression() {
          var expressions = branch[ complex_expression_branch_value ]
            , i           = expression ? expressions.indexOf( has_v ) : -1
          ;
          
          if ( i < 0 ) return fatal( 'expression not found: ' + log.s( v ) );
          
          // history.push( 'removing complex expression: ' + log.s( v ) + ', for key: ' + key + ', of term: ' + log.s( term ) );
          
          if ( remove_term( expression[ i ].child, term, term_keys ) ) {
            // no-more subscribers for this expression, remove it from expression
