  ...
  */
  ...
  Loggable.subclass( 'Query', Query, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        @method Query..generate( transform )
        
        @short Generate filter() function from query.
        
        @parameters
        - **transform** (Function): optional @@method:Plug.._transform() to
        transform values before filter.
        ...
        - synchronous and stateless if both transform() and filter()
        are stateless and synchronous;
        - assynchronous and stateless if transform() is assynchronous
        and both transform() and filter() are stateless;
        - assynchronous and stateful if transform() or filter() is
        stateful.
    */
    generate: function( transform ) {
      var that             = this
        , q                = that.query
        , l                = q.length
        , terms_counts     = {}
      ...
      code = new Code( get_name( that, 'generate' ) )
        ._function( 'this.filter', ___, [ 'values' ] );
          ...
          if ( transform ) // always call it if stateful even if nul-filter
            code.add( 'values = transform( values )' );
