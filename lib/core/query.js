  ...
  Loggable.subclass( 'Query', Query, {
    ...
    */
    discard_operations: function() {
      var changes = [ this.removes, this.adds ];
    ...
    */
    add: function( q1 ) {
      ...
      // ToDo: use that instead of this, assign keys1s = q1.keys right away to test if adding a Query, don't test is_array()
    ...
    */
    differences: function( q1 ) {
      ...
      // ToDo: a bug is suspected here where an exception occured in Query_Tree..remove() (undefined node in remove_term) after differences returned removed terms from this method that were not added before
    ...
    add: function( or_terms, subscriber ) {
      ...
      function add_term( node, term, term_keys ) {
        ...
        if ( false && node.subscribers.indexOf( subscriber ) != -1 )
          throw new Error( that._get_name( 'Query_Tree..add' )
            + 'or terms:' + log.s( or_terms )
            + ', adding term:' + log.s( term )
            + ' twice for the same subscriber: ' + subscriber._get_name() )
          ;
    ...
    remove: function( or_terms, subscriber ) {
      ...
      function remove_term( node, term, term_keys ) {
        ...
        if ( term_keys.length ) {
          ...
          index == -1
            && fatal( 'Cannot find subscriber (' + subscriber._get_name() + ') in tree where it was expected' )
          ;
        ...
        function fatal( message ) {
          throw new Query_Error(  that._get_name( 'Query_Tree..remove' ) + message
            + ', key: '       + key
            + ', i: '         + i
            + ', term: '      + log.s( term )
            + ', keys: '      + log.s( keys )
            + ', term_keys: ' + log.s( term_keys )
            + ', branch: '    + log.s( branch )
