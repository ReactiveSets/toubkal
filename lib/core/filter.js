...
( 'filter', [ './pipelet' ], function( rs ) {
  ...
  */
  ...
  function Downstream_Plug( pipelet, options ) {
    Input.call( this, pipelet, options )
  }
  
  Input.subclass( 'Downstream_Plug', Downstream_Plug, function( Super ) { return {
    _upstream_query_changes: function( changes, destination ) {
      // update downstream query to get optimized changes
      changes = this._update_query( changes, destination, future_query_s );
      ...
      if ( changes_count( changes ) )
        ...
        changes = this.pipelet._get_upstream_query_changes( future_query_s )
      ;
    ...
    _fetch: function( receiver, query, query_changes, destination ) {
      var that           = this
        ...
        , filter_query   = pipelet._filter_query
      ...
      if ( query_changes ) {
        var removes          = query_changes[ 0 ]
          ...
          , al               = adds.length
          , upstream_changes = query_changes
          , output_query
          , ___
        ;
        
        if ( al && ( output_query = pipelet._output_query ) ) output_query._add( adds );
        
        if ( al + removes.length ) {
          upstream_changes = that._upstream_query_changes( query_changes, destination );
          
          if( ! changes_count( upstream_changes ) ) upstream_changes = ___;
        }
      }
      ...
      Plug_p._fetch.call(
        ...
        query_changes,    // changes to apply to query when fetch completes
        destination       // act as a controllet, not processing operations, ToDo: implement in Plug.._fetch()
      );
    ...
    update_upstream_query: function( changes, destination ) {
      ...
      if ( removes.length + al ) {
        ...
        Plug_p.update_upstream_query.call(
          that,
          changes,     // applied to query with _update_query(), possibly delayed if there are fetches in progress
          destination, // for _update_query()
          that._upstream_query_changes( changes, destination ), // upstream changes to apply immediately
          destination // act as a controllet, not processing operations, ToDo: implement in Plug..update_upstream_query()
        );
      }
    } // Downstream_Plug..update_upstream_query()
  } } ); // Downstream_Plug() instance methods
  
  Pipelet.Build( 'filter', Filter, function( Super ) { return {
    /* ------------------------------------------------------------------------
        @method Filter.._get_upstream_query_changes( query_name )
        
        @short Get query differences between upstream and filter AND downstream
        
        @parameters
        - query_name (String): optional name of attribute for downstream query,
          default is ```"query"```
        ...
        ToDo: move this as Downstream_Plug.._query_changes_transform( changes, query_name )
    */
    _get_upstream_query_changes: function( query_name ) {
      var that             = this
        , name             = '_get_upstream_query_changes'
        , downstream_query = that._downstream[ query_name || query_s ]
        ,   upstream_query = that._input[ query_name || query_s ] // ToDo: should this use query_name also?
    ...
    _init_filter_query: function( filter ) {
      ...
      input[ query_s ]                      = new Query( [], name + 'input'  );
      input[ future_query_s ]               = new Query( [], name + 'input'  );
      ...
      var downstream = that._downstream = output.source
        = new Downstream_Plug( that, { name: get_name( that ) + '-downstream' } )
      ;
      ...
      function update_query( options ) {
        ...
        if( count ) {
          changes = that._get_upstream_query_changes( future_query_s );
          
          input._update_query( changes, ___, future_query_s );
