    ...
    */
    ...
    _init_filter_query: function( filter ) {
      ...
      function update_query( options ) {
        ...
        // ToDo: updating input query should be done after/while updating subscription
        count = changes[ 0 ].length + changes[ 1 ].length;
        ...
        if( count || _t && _t.forks ) {
          ...
          input._update_subscription( count ? input._update_query( that._get_upstream_query_changes() ) : changes, options );
        }
        
        // de&&ug( that._get_name( 'update_query' ) + 'input query:', input.query.query );
      } // update_query()
        ...
        _fetch: function( receiver, query, query_changes, destination ) {
          ...
          if ( query_changes ) {
            ...
            if ( al + removes.length ) {
              ...
              // ToDo: what happens if there are concurrent fetches or if their receivers are called out-of-order, are query_changes still valid?
