  ...
  */
  ...
  Input.subclass( 'Downstream_Plug', Downstream_Plug, function( Super ) { return {
    _upstream_query_changes: function( changes ) {
      ...
      changes = Super._upstream_query_changes.call( this, changes );
      ...
      if ( changes_count( changes ) )
        ...
        // ToDo: Return undefined if there are no changes
      ;
    ...
    _fetch: function( receiver, query, query_changes, destination ) {
      ...
      if ( query_changes ) {
        ...
        if ( al + removes.length ) {
          upstream_changes = that._upstream_query_changes( query_changes );
      ...
      Plug_p._fetch.call(
        ...
        query_changes,    // changes to apply to query when fetch completes, ignored because _query_update() is undefined on input
    ...
    update_upstream_query: function( changes, destination ) {
      ...
      if ( removes.length + al ) {
        ...
        // ToDo: do not submit upstream changes if there are none
        // ToDo: DRY with _fetch() into _upstream_query_changes()
        
        // Calculate changes of upstream query then apply these upstream, if any
        Plug_p.update_upstream_query.call(
          that,
          changes,     // ignored because upstream changes are defined bellow
          destination,
          that._upstream_query_changes( changes ), // upstream changes to apply immediately
