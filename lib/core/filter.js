...
( 'filter', [ './pipelet' ], function( rs ) {
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     ToDo: filter(): allow assynchronous filtering when filter provides a callback.
  */
    ...
    switch ( typeof filter ) {
      ...
      case "object":
        if ( filter._is_a && filter._is_a( 'Pipelet' )
          || Object.prototype.toString.call( filter ) === '[object Array]'
        ) {
  ...
  Filter.Output = Pipelet.Output.subclass(
    ...
    _fetch: function( receiver, query ) {
      ...
      return p._input.fetch_source( receiver, q.query );
    }, // _fetch()
    ...
    */
    ...
    _add_query_source: function( filter, options ) {
      ...
      if ( ! query_controller ) {
        ...
        // ToDo: query controller should become a Greedy Input.
        this._query_controller = query_controller
          = new Pipelet( { name: options.name + ' Query Controller' } )
        ;
        
        if ( filter_keys ) {
          de&&ug( this._get_name( '_add_query_source' ) + 'filter keys:', filter_keys );
          ...
          query_controller._input.no_fetch = false; // we want to fetch right away on _add_source() bellow
        }
      ...
      query_controller._input.add_source( filter._output || filter ); // filter may be an Array of Objects
      
      return this;
      
      function add( or_terms, options ) {
        de&&ug( this._get_name( '_add_query_source()#add' ) + 'or_terms:', or_terms );
      ...
      function remove( or_terms, options ) {
        de&&ug( this._get_name( '_add_query_source()#remove' ) + 'or_terms:', or_terms );
    ...
    _update_query: function( options ) {
      ...
      if ( filter_query.adds.length || filter_query.removes.length ) {
        ...
        de&&ug( '_update_query(), adds:', adds, '- removes:', removes );
    ...
    // ToDo: filter(), _update will mostly no longer being called, because Set does not implement update
    // This code is no-longer tested in the test suite
    /*
  ...
  */
  rs.Compose( 'flow', { single: true }, function( source, flow_name, options ) {
    return source
      .filter( [ { flow: flow_name } ], { key: options.key, name: 'flow#' + flow_name } )
    ;
  } );
