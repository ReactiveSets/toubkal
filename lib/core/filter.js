...
( 'filter', [ './pipelet' ], function( rs ) {
  ...
  */
  ...
  Input.subclass( 'Downstream_Plug', Downstream_Plug, {
    ...
    _fetch: function( receiver, query, query_changes, destination ) {
      ...
      pipelet._input._fetch( rx, query, query_changes, destination ); // act as a controllet plug
    ...
    update_upstream_query: function( changes, destination ) {
      ...
      if ( removes.length + al ) {
        if ( al && ( output_query = pipelet._output_query ) ) {
        ...
        changes_count( changes )
          && pipelet._input.update_upstream_query( changes, destination ) // act as a controllet, not processing operations
        ;
      }
