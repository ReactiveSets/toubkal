      ...
      that._output.source = {
        ...
        _fetch: function( receiver, query, query_changes, destination ) {
          ...
          if ( query_changes ) {
            ...
            // ToDo; implement update query in update_query_changes()
            if ( al + removes.length ) {
              // rx = update_query_changes;
              query_changes = that._update_query( query_changes, destination );
              
              if ( query_changes[ 0 ].length + query_changes[ 1 ].length ) 
                query_changes = input._update_query( query_changes, destination )
              ;
            }
          }
          ...
          function update_query_changes( adds, no_more, removes, updates, options ) {
            ...
            if ( no_more ) { // ToDo: implement update query on update_query_changes()
