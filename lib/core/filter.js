    ...
    */
    ...
    _init_filter_query: function( filter ) {
      ...
      that._query_controller = that._add_input(
        ...
        // ToDo: make this a Filter.Query_Controller Plug
        {
          ...
          // ToDo: add tests for Filter.Query_Controller..update_upstream_query()
          update_upstream_query: function( changes, destination ) {
            var query_controller = that._query_controller
              , filter_keys      = that._options.filter_keys
              , adds             = changes[ 1 ]
              , filter_adds      = []
            ;
            
            if ( filter_keys && adds.length ) {
              ...
              // get additional filters from query controller
          ...
          } // Filter.Query_Controller..update_upstream_query()
        }
      );
      ...
      that._output.source = {
        ...
        _fetch: function( receiver, query, query_changes, destination ) {
          ...
          if ( query_changes ) {
            query_changes = [ // !! do not use query_changes.map()
              q_and( query_changes[ 0 ] ),
              q_and( query_changes[ 1 ] )
            ];
          }
          
          query = q_and( query );
          
          // ToDo: we may have to update Filter.Query_Controller query on receiver
          input._fetch( receiver, query, query_changes, destination ); // act as a controllet plug
        ...
        update_upstream_query: function( changes, destination ) {
          ...
          if ( split_and_count() ) {
            that._query_controller.pipelet.update_upstream_query( changes, destination );
            
            var changes = that
              ._downstream_query
              
              // use Query..update( changes )
              .add   ( adds )
              .remove( removes )
            ...
            split_and_count()
              ...
              && input.update_upstream_query( that._get_upstream_query_changes(), destination ) // act as a controllet, not processing operations
            ;
          }
          
          function split_and_count() {
            removes = changes[ 0 ];
            adds    = changes[ 1 ];
            
            return removes.length + adds.length;
          }
