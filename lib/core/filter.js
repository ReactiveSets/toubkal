  ...
  */
  ...
  Input.subclass( 'Downstream_Plug', Downstream_Plug, function( Super ) { return {
    _upstream_query_changes: function( changes ) {
      ...
      changes = this._update_query( changes );
    ...
    _fetch: function( receiver, query, query_changes, destination ) {
      ...
      if ( query_changes ) {
        ...
        if ( al + removes.length ) {
          query_changes = that._upstream_query_changes( query_changes );
          ...
          that._update_query( [ adds, removes ] );
          
          // ToDo: fix for concurrent fetches' receivers called out-of-order, see Plug
      ...
      that.source._fetch( rx, query, query_changes, destination ); // act as a controllet plug
      ...
      function update_query_changes( adds, no_more, removes, updates, options ) {
        no_more
          && that._update_query( _query_changes )
        ;
    ...
    update_upstream_query: function( changes, destination ) {
      ...
      if ( removes.length + al ) {
        ...
        changes = that._upstream_query_changes( changes, destination );
        
        changes_count( changes )
          && that.source.update_upstream_query( changes, destination ) // act as a controllet, not processing operations
        ;
      }
  ...
  Pipelet.Build( 'filter', Filter, function( Super ) { return {
    /* ------------------------------------------------------------------------
        ...
        We have 3 queries:
        - ```this._downstream_query```: from Downstream_Plug
    ...
    */
    ...
    _init_filter_query: function( filter ) {
      ...
      var downstream_plug = output.source
        = new Downstream_Plug( that, { name: get_name( that ) + '-output_source' } )
      ;
      
      downstream_plug.source = input;
      downstream_plug.query = that._downstream_query;
    }, // _init_filter_query()
