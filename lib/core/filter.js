    ...
    */
    ...
    _init_filter_query: function( filter ) {
      ...
      that._query_controller = that._add_input(
        ...
        options.name + ' Query Controller',
        
        {
          _add   : add,
          _remove: remove,
          _update: update // Pipelet.prototype._update
        }
      );
    ...
    _update_query: function( options ) {
      ...
      if ( filter_query.adds.length || filter_query.removes.length ) {
        ...
        var input  = this._input
          , source = input.source
          , o      = this._options.sync ? extend_2( {}, options ) : {} // ToDo: need to implement input transactions tag
        ;
