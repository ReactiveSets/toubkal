...
( 'filter', [ './pipelet' ], function( rs ) {
  ...
  */
  function Filter( filter, options ) {
    ...
    that._output || ( that._output = new Filter.Output( that, options && options.name ) );
    
    // ToDo: either move this to Filter.Output() or an Filter.Input()
    ...
    function ( Super ) {
      return {
        ...
        update_upstream_query: function( changes, destination ) {
          ...
          // ToDo: overload that.source instead of calling _update_query() which breaks encapsulation, this will be done when the new fetch is available
          changes = that._update_query( changes, destination );
          
          removes = changes[ 0 ];
          adds    = changes[ 1 ];
          
          if ( removes.length + adds.length == 0 ) return;
