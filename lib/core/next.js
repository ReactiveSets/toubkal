...
( 'next', [ './pipelet' ], function( rs ) {
  ...
  /* --------------------------------------------------------------------------
      ...
      @parameters
      - **trigger** (@@class:Pipelet()): a dataflow of events (adds only)
      
      - **f( source_value, trigger_value )** (Function): altering a next
        source value and a new trigger value using this next source value.
        Both the source_value and trigger_value are shallow-cloned Objects.
      
      - **options** (Object): Compose options:
        - fork_tag (String): output fork tag to allow handling of concurrent
          output transactions typically between the trigger dataflow and the
          source dataflow.
      
      @source a dataflow with a single current value which is the last value
      that will be updated by f() on triggers.
      
      @examples
      - Auto-Incrementing and assigning an issue number:
        ```javascript
      ...
      @description
      On trigger additions, calls f() to update source dataflow (downstream)
      and alter trigger add.
      
      On errors from source, next() will revert changes to resolve conflicts
      and generate new updates downstream using f() to generate new values.
      To allow source error recovery, source dataflow values must have a
      flow attribute. Likewise to allow trigger values error recovery,
      trigger values must have a flow attribute set no-later than when
      calling f().
      
      Error recovery:
      - On source duplicates:
      - On trigger values duplicates:
        - remove previous trigger value, using key that must not include the
          attribute set by f()
        - add a new next trigger value by calling f() to replce the removed
          trigger value
  */
  rs.Compose( 'next', function( source, trigger, f, options ) {
    ...
    var source_out = source.unique( [], { untag: fetch_tag } );
