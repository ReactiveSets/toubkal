...
( 'next', [ './pipelet' ], function( rs ) {
  ...
  */
  rs.Compose( 'next', function( source, trigger, f, options ) {
    ...
    var source_out = source.unique( [], { silent: true, tag: fetch_tag } );
    
    var fetched = trigger
      .fetch( source_out, function() {} )
      //.trace( 'fetched source_out' )
      .map( function( trigger ) {
        var values = trigger.values
          , source_value = values && values[ 0 ]
        ;
    ...
    return rs
      .union( [
        ...
        fetched.map( function( _ ) { return _.new_trigger } ).delay( 30 )
      ], { tag: fetch_tag } )
