...
( 'next', [ './pipelet' ], function( rs ) {
  ...
  */
  Compose( 'next', function( source, trigger, f, options ) {
    ...
    trigger
      // ToDo: replace alter() by for_each() once implemented
      .alter( function( trigger_value, i, trigger_values, options, caller ) {
        if ( caller !== 'add' ) return;
        
        var source_value = source.fetch_all()[ 0 ];
        
        source_value && next_trigger( source_value, trigger_value );
      } )
    ;
    
    return output;
    
    function next_trigger( source_value, trigger_value ) {
