  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     - options (Object): Compose options:
       - fork_tag (String): output fork tag to allow handling of concurent output transactions
  ...
  */
  rs.Compose( 'next', function( source, trigger, f, options ) {
    ...
    source
      ...
      .alter( function( error, operation ) {
        if ( operation == 'add' ) {
    ...
    source = source.unique( [], { silent: true } );
    ...
    trigger
      ...
      .map( function( trigger ) {
        ...
        source_value
          && next_trigger( source_value, trigger.source, trigger.options )
        ;
      } )
    ;
    ...
    // ToDo: implement next_trigger() as a pipelet
    function next_trigger( source_value, trigger_value, options ) {
      ...
      output._transaction( 3, options, function( t ) {
        this._remove( [ source_value ]  , t.next().get_emit_options() );
        this._add( [ next_source_value ], t.next().get_emit_options() );
