  ...
  function terminate_tagged_transaction( options, branches, new_ids ) {
    ...
    if ( t && ! t.more && last_fork_tag( t ) )
      ...
      for ( id in branches )
        new_ids[ id ]
          || ( out = branches[ id ][ 1 ] )
            && out._output.emit( 'add', [], options ) // ToDo: make this work also when out is a union
  ...
  Greedy.subclass( 'Dispatch', Dispatch, {
    _add: function( new_branches, options ) {
      ...
      function add_branch( branch ) {
        ...
        // ToDo: improve legibility, splitting in 2 expressions:
        output = ( branches[ id ] = input_output
          ? branch
          : [ requires_pass_through ? input : branch, branch ]
        )[ 1 ];
        
        output && output.through( gatherer, delay_connect ? null : options ); // ToDo: never use null for options
