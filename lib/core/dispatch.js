...
( 'dispatch', [ './pipelet' ], function( rs ) {
  ...
  /* --------------------------------------------------------------------------
      ...
      @description:
      Dispatch is a high-level building block for managing dataflows
      @@[concurency]concurent with Toubkal. It is also useful for
      many non-concurent design patterns.
      ...
      Dispatch currently ignores @@update operations on branches (B), which
      prevents tearing-down updated branches. Use pipelet split_updates() to
      force branch tear-down and reset on updates.
  ...
  */
  ...
  function Dispatch( source, branches, branch, options ) {
    var that            = this
      ...
      , gatherer        = that._gatherer = rs.union( [], { name: name + 'gatherer', tag: branches_tag } ) // ToDo: add key as options.key or source._key
      , dispatcher
    ;
    ...
    that._branches     = {};
    that._transactions = [];
    that._branches_tag = branches_tag;
  ...
  function terminate_tagged_transaction( options, branches, new_ids, gatherer ) {
    var t, id, branch, count = 0;
    
    // Terminate tagged transaction as many times as there were branches before
    if(  options
      && ( t = options._t )
      && ! t.more
      && last_fork_tag( t )
      ...
      for( id in branches )
        new_ids[ id ] || branches[ id ][ 1 ] && ++count
    ;
    
    count && log( 'terminate_tagged_transaction()', count, options, new_ids );
    
    // ToDo: Consider a method that removes n inputs from a transaction with one function call:
    // count && gatherer._output.terminate_transaction_on_branches( options, count )
    while( count-- ) gatherer.__emit( 'add', [], options );
  ...
  Greedy.subclass( 'Dispatch', Dispatch, {
    _add: function( new_branches, options ) {
      var that                  = this
        ...
        , name                  = '_add'
        , branches_tag          = that._branches_tag
      ;
      
      if( branches_tag )
        options = enforce_tag_on_many( new_branches, options, branches_tag, that._transactions )
      ;
      
      new_branches.forEach( add_branch );
      
      // Terminate tagged transactions as many times as there were branches before
      terminate_tagged_transaction( options, branches, new_ids, gatherer );
      ...
      function add_branch( branch ) {
        ...
        output && output.through( gatherer, delay_connect ? null : options );
        
        if( delay_connect ) dispatcher.through( input, options );
    ...
    _remove: function( removed_branches, options ) {
      ...
      if( branches_tag )
        options = enforce_tag_on_many( removed_branches, options, branches_tag, that._transactions )
      ;
      
      removed_branches.forEach( remove_branch );
      
      // If this is the end of a tagged transaction, terminate it on all remaining branches
      terminate_tagged_transaction( options, branches, {}, gatherer );
      ...
      function remove_branch( branch ) {
        ...
        ( input = branch[ 0 ] )
          ? input._input.remove_source( dispatcher_output, options, unplug_output )
          : unplug_output()
        ;
        ...
        function unplug_output() {
          ...
          ( output = branch[ 1 ] )
            && gatherer_input.remove_source( output._output, options )
          ;
        } // unplug_output()
      } // remove_branch()
    }, // _remove()
