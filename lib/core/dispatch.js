  ...
  */
  ...
  function Dispatch( source, branches, branch, options ) {
    var that            = this
      ...
      , tag             = options.inner_tag || ( options.name + '#' + ++dispatch_counter )
      , gatherer        = that._gatherer = rs.union( [], { name: name + 'gatherer', tag: tag } ) // ToDo: add key as options.key or source._key
      , dispatcher
    ;
  ...
  function terminate_tagged_transaction( options, branches, new_ids, gatherer ) {
    ...
    if(  options
      ...
      for( id in branches )
        ...
        new_ids[ id ] || branches[ id ][ 1 ] && gatherer.__emit( 'add', [], options );
  ...
  Greedy.subclass( 'Dispatch', Dispatch, {
    _add: function( new_branches, options ) {
      ...
      options = enforce_tag_on_many( new_branches, options, that._tag, that._transactions );
    ...
    _remove: function( removed_branches, options ) {
      ...
      options = enforce_tag_on_many( removed_branches, options, that._tag, that._transactions );
