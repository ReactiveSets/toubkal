  ...
  Greedy.subclass( 'Dispatch', Dispatch, {
    _add: function( new_branches, options ) {
      var that                  = this
        ...
        , transaction_branches  = _t && that.transaction_branches
        , ___
      ;
      ...
      // ToDo: if more than one new branch, create a transaction for new_branches.length operations, use it in add_branch()
      
      // ToDo: transform into loop to reduce call stack depth:
      new_branches.forEach( add_branch );
      
      if ( _t && ! _t.more && transaction_branches && transaction_branches.length ) {
      ...
      function add_branch( branch ) {
        ...
        if ( transaction_branches && transaction_branches.length ) {
          ...
          //de&&ug( get_name( that, name ) + 'options with transaction_branches:', pretty( options ) )
        }
        
        output && output.through( gatherer, delay_connect ? ___ : options );
        
        if ( delay_connect )
          dispatcher.through( input, options );
    ...
    _remove: function( removed_branches, options ) {
      ...
      // ToDo: create a transaction if many branches are removed
