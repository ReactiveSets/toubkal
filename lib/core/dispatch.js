...
( 'dispatch', [ './pipelet' ], function( rs ) {
  ...
  */
  ...
  function terminate_tagged_transaction( options, branches, new_ids, gatherer ) {
    ...
    count && de&&ug( 'terminate_tagged_transaction()', count, options, new_ids );
  ...
  Greedy.subclass( 'Dispatch', Dispatch, {
    _add: function( new_branches, options ) {
      ...
      if ( branches_tag ) {
        ...
        // de&&ug( get_name( that, name ) + 'enfore tag on many:', options )
      }
    ...
    _remove: function( removed_branches, options ) {
      ...
      function remove_branch( branch ) {
        ...
        unplug_output( null, unplug_input );
        
        // Unplug input from dispatcher
        function unplug_input( error, then ) {
          de&&ug( get_name( that, name ) + 'remove input for branch', id );
        ...
        function unplug_output( error, then ) {
