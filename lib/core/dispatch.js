  ...
  /* -------------------------------------------------------------------------------------------
      ...
      @parameters:
        ...
        - branch: function( source, options ), a function that is called when each branch
            ...
            To cleanup resources on branch removal, set a one-time event listener on
            ```source._input``` for event ```"remove_source"```:
            
            ```javascript
              source._input.once( "remove_source", function( dispatcher, options ) {
              } );
            ```
            When using options ```"single"``` or  ```"input_output"```, monitor the
            input pipelet, as seen in examples.
      ...
      - Load and hot-reload server javascript modules having database access, cleanup on removal:
        ...
        - Method Event_Emitter..once() on for evemt ```"remove_source"``` emitted by method Input..remove_source()
      
      - A module for the example above joining ```"users"``` and ```"profiles"``` dataflows:
      
        ```javascript
          module.exports = function processor( source, module, options ) {
