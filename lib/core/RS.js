...
( 'RS', [ 'javascript', 'escape', 'console_logger', 'lazy_logger', 'extend', 'subclass', 'loggable', 'code', 'event_emitter', 'value_equals', 'object_diff', 'timestamp_string', 'picker' ].map( function( d ) { return '../util/' + d } ).concat(
[ [ 'uuid', 'node-uuid' ] ] ),
function(  javascript,   escape,   Console_Logger,   Lazy_Logger,   extend,   subclass,   Loggable,   Code,   Event_Emitter,   value_equals,   object_diff,   timestamp_string,   picker, uuid ) {
  'use strict';
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      @returns
      - \\<Strings []>:
        - Empty if stack trace is not available.
