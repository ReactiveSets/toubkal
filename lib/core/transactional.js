...
( 'transactional', [ './pipelet' ], function( rs ) {
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      @description:
        ...
        This is a transactional, stateless, lazy pipelet. It is synchronous when fetching the
        store dataflow is synchronous, or assynchronous when fetching is assynchronous.
      ...
      @parameters:
        ...
        - fetch_query (Function): called to build fetch query, signature is:
  ...
  */
  ...
  Pipelet.Build( 'fetch', Fetch, {
    _fetch_values: function( operation, values, done ) {
      var that        = this
        , store       = that._store
      ...
      function fetch_value( value, position ) {
        var query = fetch_query( value )
          , fetched_values = []
        ;
        
        store._fetch( query, fetched );
    ...
    _add: function( values, options ) {
      ...
      that._fetch_values( 'create', values, emit );
    ...
    _remove: function( values, options ) {
      ...
      that._fetch_values( 'delete', values, emit );
    ...
    _update: function( updates, options ) {
      ...
      that._fetch_values( 'update_previous', updates.map( get_remove ), emit );
