...
( 'transactional', [ './pipelet' ], function( rs ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      - fetch_query (Function): optional, called to build fetch query,
        ...
        returning undefined. Signature is ```( value, operation ) -> query```
        
        - value (Object): source event
        - operation (String): corresponding to stict transactional semantics:
          - "create": value of created object
          - "delete": value of deleted object
          - "update_previous": previous value of an updated object
          - "update_new": new value of an updated object
        - query (Array of Objects): the query to fetch ```store```
      ...
      - values (Array of Objects): fetched values from the store dataflow
        with query
      - operation (String): corresponding to stict transactional semantics,
        see fetch_query parameter above for details.
      ...
      previous and new values with the same @@identity.
  */
  ...
  Greedy.Build( 'fetch', Fetch, {
    _fetch_values: function( operation, values, options, done ) {
      ...
      function fetch_value( value, position ) {
        var query = fetch_query && fetch_query( value, operation )
          , fetched_values = []
        ;
  ...
  */
    ...
    _add: function( values, options ) {
      ...
      if ( v ) {
        ...
        if ( values.length > 1 ) { // concatenates all adds together, all removes together, all updates together
          adds    = adds    ? slice.call( adds    ) : [];
          removes = removes ? slice.call( removes ) : [];
          updates = updates ? slice.call( updates ) : [];
  ...
  /* --------------------------------------------------------------------------
      ...
      @short Update fetched values or create not found values in a transaction
      ...
      - update (Function): optional, called to calculate new values from
        ...
        - fetched_value (Object): value fetched, null if no value fetched
        - source_value (Object): source value of upstream pipelet fetch()
        
        If there are no ```fetched_value```, a new value is created
        (emit @@add), otherwise all fetched values are updated (emit
        @@update).
        
        If ```update()``` returns falsy no value is created or updated for
        the current fetched value.
      ...
      - ToDo: rename update_fetched_or_create
  */
  rs.Compose( 'update_fetched', function( source, update, options ) {
    ...
    function update_or_create( fetched ) {
      ...
      if( values.length )
        ...
        return { updates: values.reduce( _update, [] ) }
      ;
      
      // Found no value, this is a creation
      if ( update ) source = update( null, source );
      
      if ( source ) return { adds: [ source ] };
      
      function _update( out, fetched_value ) {
        var value = update
          ? update(     fetched_value, source )
          : extend( {}, fetched_value, source )
        ;
        
        value && out.push( [ fetched_value, value ] );  
      ...
      socket.io server:
        ```javascript
          ...
          rs
            ...
            // Filter for hard reconnections, i.e. state changes for which 
            // connection identity has changed
        ...
        Note that the above is already implemented in pipelet cache()
  ...
  */
  .Compose( 'fetched_differences', function( source, options ) {
    ...
    function A_minus_B( _ ) {
      var A       = _.source.values // first  fetched values
      ...
      A.forEach( function( value ) {
        A_kv[ that._make_key( value ) ] = value;
      } );
      ...
      B.forEach( function( b_value ) {
        var identity = that._make_key( b_value )
