...
( 'transactional', [ './pipelet' ], function( rs ) {
  ...
  var RS              = rs.RS
    ...
    , log             = RS.log.bind( null, transactional_s )
    , Pipelet         = RS.Pipelet
    , Greedy          = RS.Greedy
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      @parameters
      - **store** (Pipelet): the dataflow from which incomming values
        are fetched
      ...
      - **fetch_query**:
        ...
        - (Function): called to build fetch query, Signature is:
          ...
          - **operation** (String): corresponding to strict transactional
            semantics:
            - ```"create"```: value is from a source add operation
            - ```"delete"```: value is from a source remove operation
            - ```"update_previous"```: value is the old value of an
              update operation
          
          - **query** (Array of Objects): the query to fetch *store*, if
            null or undefined, all values are fetched from store.
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      This is a *@@synchronous, @@stateless, @@greedy pipelet. It is
      @@transactional unless option *transactional* is falsy.
  ...
  */
  function Emit_Operations( options ) {
    var that          = this
      , transactional = options[ transactional_s ]
    ;
    
    if ( transactional == null || transactional ) {
      options = extend_2( {}, options )
      options[ transactional_s ] = 1
      transactional = 1;
  ...
  Pipelet.Build( 'emit_operations', Emit_Operations, {
    _add: function( values, options ) {
      var v       = values[ 0 ]
        , tid
        , adds
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      **emit_transactions**.
      ...
      - Pipelet emit_operations()
  */
  .Compose( 'fetched_transaction', function( source, processor, options ) {
    return source
      ...
      .map( function( fetched ) {
        var values      = fetched.values
          ...
          , updates     = transaction.updates
          , _on         = {}
          , all_events  = [ 'add', 'remove', 'update' ]
        ;
        
        on( 'all', identity );
        
        if ( !errors.length ) {
          processor( transaction, values, error, on )
          
          if ( adds    ) transaction.adds    = adds   .map( process( 1, _on[ 'add'    ] ) );
          if ( removes ) transaction.removes = removes.map( process( 0, _on[ 'remove' ] ) );
          if ( updates ) transaction.updates = updates.map( process_update );
        ...
        function on( event_name, f ) {
          is_array( event_name )
            ? event_name.forEach( r_on )
            : event_name == 'all'
            ? all_events.forEach( r_on )
            : _on[ event_name ] = f
          ;
          
          function r_on( event_name ) { on( event_name, f ) }
        } // on()
        
        function process( is_add, on ) {
          return function( value, i ) {
            return on( value, i, is_add, 0 )
          }
        }
        
        function map_update( update, i ) {
          return update.map( function( value, is_add ) {
            return _on[ 'update' ]( value, i, is_add, 1 )
          } )
        }
        
        function error( message, args ) {
          errors.push( [ message, [].slice.call( args ) ], [].slice.call( arguments, 2 ) )
        }
      } )
    ;
  } ) // fetched_transaction()
  ...
  .Compose( 'validate_transactions', function( source, store, schema, options ) {
    ...
    return source
      ...
      .fetched_transaction( function( transaction, schema, error, on ) {
        ...
        function add_to_query( value ) {
          ...
          if ( model )
            query.push( model.pick( value ) );
          else
            error( 'model not found for flow ' + flow, arguments )
