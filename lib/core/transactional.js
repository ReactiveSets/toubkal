...
( 'transactional', [ './pipelet' ], function( rs ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      @@assynchronous if fetching is assynchronous. It never subscribes
  ...
  */
  ...
  rs.Compose( 'update_fetched', function( source, update, options ) {
    ...
    return source
      ...
      .union( [
        source.flat_map( removed_values, fork_tag ).revert(),
        source.flat_map(   added_values, fork_tag )
      ] )
      
      .pass_through( { key: options.key, tag: tag } )
    ;
