  ...
  */
  rs.Compose( 'update_fetched', function( source, update, options ) {
    ...
    function added_values( fetched ) {
      ...
      if( values.length ) {
        var _update = update
          ? function( fetched_value ) { return [ fetched_value, update( fetched_value, source ) ] }
          : function( fetched_value ) { return [ fetched_value, extend( {}, fetched_value, source ) ] }
        ;
        
        // Update list of values
        return { updates: values.map( _update ) };
      }
