...
( 'operations', [ './pipelet' ], function( rs ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      @short Split @@update @@operations into @@remove plus @@add
      
      @deprecated, use pipelet adds() and pipelet removes() instead.
      
      @examples:
      - Process all add operations:
        ```javascript
          source
            .split_updates() // no-longer required, adds() now splits updates
            .adds()
      ...
      Pipelet @@pipelet:optimize() would do the opposite, recombining
