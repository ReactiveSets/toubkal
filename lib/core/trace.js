...
( 'trace', [ './pipelet' ], function( rs ) {
  ...
  /* --------------------------------------------------------------------------
      ...
      @short Traces for debugging pipelet @@operation\s
      
      @parameters
      - **name** (String): a name for all traces.
      
      - **options** (Object): optional attributes:
        - **filter_map** (Function): optional, allows to reduce the amount
          of traces by filtering values, as well as transforming (map)
          displayed values. Signature is ```filter_map( value ) -> value```
          where *value* is an *Object* value to filter and map. If the
          returned value is *falsy*, it is not displayed. If there are no
          values displayed after filtering all values of an @@operation, no
          trace is displayed for this operation.
        
        - **counts** (Boolean): default *false*, only provide counts of
          values from @@operation\s instead of full content. Counts are
          evaluated after applying *filter_map*.
  ...
  */
      ...
      if ( values ) {
        ...
        if ( filter_map && l ) {
          values = values
            ...
            .map( is_update
              ? function( update ) {
                  update = update.map( filter_map );
                  
                  if ( update[ 0 ] || update[ 1 ] ) return update
                }
        ...
        object.values
          ...
          : pick ? values.map( is_update
              ? function( update ) { return update.map( pick ) }
              : pick
            )
  ...
  /* --------------------------------------------------------------------------
      ...
      @short Conditional @@pipelet:trace()
