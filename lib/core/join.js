...
( 'join', [ './order' ], function( rs ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     ToDo: add tests for join conditions involving multiple attributes
  */
        ...
        if ( matches ) {
          ...
          while ( ++j < ml ) {
            ...
            if ( other_all // The other half join requires all
              ...
              && first_seen( match_keys, match ) // ToDo: add anti-state tests
              && ( ! _locations // source set was empty, all matches had previously no match
                ...
                || source._locate( [ match ], { all: true                } )[ 0 ].all_matches.length
                  == ( l == 1 ? 1 :
                   // ToDo: add test where the length bellow yields more than 1
      ...
      while ( ++i < l ) { // all locations
        ...
        if ( matches ) {
          ...
          while ( ++j < ml ) {
            ...
            if ( other_all
              && first_seen( match_keys, match ) // ToDo: add full-join anti-state tests
