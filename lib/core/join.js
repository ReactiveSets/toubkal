/*  join.js
    ...
    For these reasons, RS comes with joins that can be used both server-side, for complex
    ...
    The other significant difficulty is that RS is an incremental database, and that
...
( function( exports ) {
  ...
  var rs      = exports.rs || ( module.exports = require( './order.js' ) )
    , RS      = rs.RS
    , extend  = RS.extend
    , Code    = RS.Code
    , Pipelet = RS.Pipelet
  ...
  */
  Pipelet.Compose( 'join', function( a, b, on, merge, options ) {
    ...
    return rs.union( [ _a, _b ], extend( {}, options, { name: name + '_union' } ) );
  } ); // join()
