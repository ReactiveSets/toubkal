      ...
      while ( ++i < l ) { // all locations
        ...
        if ( matches ) {
          ...
          while ( ++j < ml ) {
            ...
            if ( other_all ) {
              // The other half join requires all
              
              // Find out if we need to remove merge_match_only before adding back merged value and match
              
              if ( ! _locations // source set was empty, all matches had previously no match
                   // ToDo: use _locations and input source.a instead of source._locate
                || source._locate( [ match ], { all: true } )[ 0 ].all_matches.length == 1 // previously no match
              ) {
      ...
      while ( ++i < l ) { // all locations
        ...
        if ( matches ) {
          ...
          while ( ++j < ml ) {
            ...
            // de&&ug( get_name( this ) + 'removed merge:', _merge );
