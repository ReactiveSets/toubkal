  ...
  /* --------------------------------------------------------------------------
      ...
      - options: (Object):
        ...
        - concurrent (Object): if ```a``` and ```b``` are @@concurrent,
          ...
          ```javascript
            options = { concurrent: { tag_name: true } };
          ```
      ...
      joined output.
  */
  // ToDo: make no_filter the default behavior, using option filter instead
  rs.Compose( 'join', function( a, b, on, merge, options ) {
    typeof merge === 'function' ||
      fatal( 'expected merge() to be a function, got a(n) ' + type + ', merge: ' + log.s( merge ) )
    ;
    ...
    if ( ! options.no_filter && ! left ) {
      ...
      // ToDo: join(), add test with an update on b where the condition does not change, verify that join output is properly updated for other changes
      
      if( ! Object.keys( concurrent || {} ).length ) {
        // a and b are not concurrently tagged upstream
        b = b.pass_through( { fork_tag: fork_tag = name + '-tag' } );
    ...
    var options_a = extend( {}, options, { name: name + '-left' , all: left , other_all: right } )
      ...
      , options_u = extend( {}, options, { name: name + '-union', tag: fork_tag, concurrent: concurrent } )
    ...
    if ( transform ) {
      output.source = {
        _fetch: function( receiver ) {
          that._input._fetch( rx );
          
          function rx( adds, no_more ) {
            if ( adds.length ) adds = transform( adds );
            
            receiver( adds, no_more );
          }
        },
