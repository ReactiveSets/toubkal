  ...
  */
  ...
  Pipelet.Compose( 'join', function( a, b, on, merge, options ) {
    ...
    var name = options.name
      ...
      , a_order = a.order( b_parsed.organizer, options_a )
      , b_order = b.order( a_parsed.organizer, options_b )
      
      // half-joins do not require their source ordered
      // this only allows to guaranty that order is updated before processing adds in half-joins.
      // ToDo: verify that this is necessary
      , _a = a_order.half_join( 0, b_order, a_parsed, merge, output, options_a )
      , _b = b_order.half_join( 1, a_order, b_parsed, merge, output, options_b )
  ...
  /* -------------------------------------------------------------------------------------------
     half_join( position, order, parsed_conditions, merge, output, options )
  */
  function Half_Join( position, order, parsed_conditions, merge, output, options ) {
    Greedy.call( this, options );
    
    var half_join = this;
    
    half_join._position         = position;
    half_join._half_join_order  = order;
    half_join._half_join_merge  = merge;
    half_join._full_join_output = output;
    half_join._renames          = parsed_conditions.renames;
    
    half_join._make_locate( parsed_conditions.attributes );
    
    // ToDo: move all fetch code to Half_Join.Output
    half_join._output._fetch = pick_fetch();
    ...
    function non_matching_fetch( receiver, query ) {
      ...
      half_join._input.fetch_source( non_matching );
      
      function non_matching( values, no_more ) {
        var u
          , merge     = half_join._half_join_merge
          , renames   = half_join._renames
          , position  = half_join._position
          , locations = half_join._locate( values )
          , l         = locations.length
          , i         = -1
          , out       = []
        ;
    ...
    function left_fetch( receiver, query ) {
      ...
      half_join._input.fetch_source( matching );
      ...
      function matching( values, no_more ) {
        var u
          ...
          , renames   = half_join._renames
          , position  = half_join._position
          , all       = half_join._options.all
          , locations = half_join._locate( values, { all: true } )
          , l         = locations.length
          , i         = -1
          , out       = []
        ;
  ...
  Greedy.Build( 'half_join', Half_Join, {
    _make_locate: function( attributes ) {
      ...
      var locate = new Code( '_locate' )
        ...
        ._function( 'this._locate', null, [ 'values', 'options' ] )
          ._var( 'v', 'out = []', 'l = values.length', 'i = -1', 'order = this._half_join_order' )
          
          .unrolled_while( 'out.push( ' + attributes, ', ' + attributes, ')' )
          
          .add( 'return order.locate( out.sort( order.sorter ), options )' )
        .end()
      ;
    ...
    // ToDo: Add tests for non-generated locate()
    _locate: function( values, options ) {
      var order = this._half_join_order
      
      // Shallow copy values before sort
      
      return order.locate( values.slice( 0 ).sort( order.sorter ), options ); 
    }, // _locate()
        ...
        if ( matches ) {
          ...
          while ( ++j < ml ) {
            ...
            if ( other_all ) {
              ...
              // ToDo: find another algorythm using input state instead of output state
            ...
            var _merge = position ? merge( match, v ) : merge( v, match );
