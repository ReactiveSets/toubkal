  ...
  */
  ...
  Pipelet.Compose( 'join', function( a, b, on, merge, options ) {
    ...
    var name = options.name
      ...
      , _a = a_order.half_join( 0, b_order, a_parsed.attributes, a_parsed.renames, merge, output, options_a )
      , _b = b_order.half_join( 1, a_order, b_parsed.attributes, b_parsed.renames, merge, output, options_b )
  ...
  /* -------------------------------------------------------------------------------------------
     half_join( position, order, attributes, renames, merge, output, options )
  */
  function Half_Join( position, order, attributes, renames, merge, output, options ) {
    ...
    if ( position ) {
      ...
      if ( this._options.all ) {
        ...
        this._output.__fetch = function( receiver, query ) {
          var filter = query && ( new Query( query ).generate().filter );
          
          Greedy.Output.prototype._fetch.call( this, non_matching );
    ...
    this._make_locate_and_fetch( position, attributes, renames, fetch_non_matching );
    
    return this;
  } // Half_Join()
  
  Greedy.Build( 'half_join', Half_Join, {
    _make_locate_and_fetch: function( position, attributes, renames, fetch_non_matching ) {
      var u;
