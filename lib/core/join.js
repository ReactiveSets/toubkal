  ...
  */
  rs.Compose( 'join', function( a, b, on, merge, options ) {
    ...
    if ( ! options.no_filter && ! left ) {
      ...
      b = b
        .pass_through( { fork_tag: fork_tag = 'join' } )
      ;
    ...
    var options_a = extend( {}, options, { name: name + '_left' , all: left , other_all: right } )
      , options_b = extend( {}, options, { name: name + '_right', all: right, other_all: left  } )
      
      , a_locator = a
        //.trace( name + '-a' )
        .join_locator( b_parsed, extend( {}, options, { name: name + '_left_locator'  } ) )
        
      , b_locator = b
        //.trace( name + '-b' )
        .join_locator( a_parsed, extend( {}, options, { name: name + '_right_locator' } ) )
