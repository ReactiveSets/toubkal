...
( 'join', [ './order' ], function( rs ) {
  ...
  */
  ...
  Pipelet.Compose( 'join', function( a, b, on, merge, options ) {
    ...
    var name = options.name
      ...
      , _a = a_locator
        ...
        .half_join( 0, b_locator, merge, output, options_a )
        
      , _b = b_locator
        //.trace( name + ' right half' )
        .half_join( 1, a_locator, merge, output, options_b )
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     - parsed_conditions (Object):
       - organizer (Array of Objects): for order()
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     half_join( position, locator, merge, output, options )
  */
  function Half_Join( position, locator, merge, output, options ) {
      ...
      while ( ++i < l ) { // all locations
        ...
        if ( matches ) {
          ...
          while ( ++j < ml ) {
            ...
            if ( other_all ) {
              // There are locations in options (i.e. not the first insert) and the other half join requires all
              
              // try to add merge_match_only to output
              var merge_match_only = position ? merge( match ) : merge( u, match );
              
              // ToDo: use _locations and input state instead of output state
              if ( ! _locations || output.index_of( merge_match_only ) != -1 ) {
      ...
      while ( ++i < l ) { // all locations
        ...
        if ( matches ) {
          ...
          while ( ++j < ml ) {
            ...
            // de&&ug( get_name( this ) + 'removed merge:', _merge, ', result:', output.a, ', key:', output._key );
            ...
            if ( other_all ) {
              ...
              // de&&ug( get_name( this ) + 'merge_match_only: ', merge_match_only );
              
              if ( output.a.filter( is_merge_match_only_in ).length == 0 ) {
                // de&&ug( get_name( this ) + 'adding back merge_match_only:', merge_match_only );
