      ...
      var transform = new Code( 'join..__transform(), position: ' + position + ', all: ' + all )
        ._function( 'this.__transform', u, [ 'values', 'options', 'operation' ] )
          ...
          ._var(
            ...
            , '_operation = add ? "__emit_add" : "__emit_remove"'
          ...
          ._while( '++i < l' ) // all locations
            ...
            ._if( 'matches' )
              ...
              ._while( '++j < ml' )
                ;( other_all ) && transform
                ._if( 'add' )
                  // remove merge_match_only from output if present
                  
                  .add( 'position = output.index_of( _v = ' + merge_match_only + ' )' )
                  
                  ._if( 'position != -1' )
                    .add( 'this.__emit_remove( [ _v ], options )' )
                  .end()
                .end()
                
                ;transform
                .add( 'this[ _operation ]( [ ' + merge + ' ], options )' )
                
                ;( other_all ) && transform
                ._if( 'remove' )
                  // try to add merge_match_only to output
                  
                  .add( '_v = ' + merge_match_only )
                  
                  ._if( 'output.a.filter( is_v_in ).length == 0' )
                    .add( 'this.__emit_add( [ _v ], options )' )
                  .end()
                .end()
                
              ;transform
              .end() // while there are matches
              
            ;all && transform
            ._else() // no match
              .add( 'this[ _operation ]( [ ' + merge_without_match + ' ], options )' )
              
            ;transform
            .end()
          .end() // while there are locations
          
          .add( 'return []' )
          
          .add( 'function is_v_in( v ) { var __v = {}, p; for( p in _v ) if ( v.hasOwnProperty( p ) ) __v[ p ] = v[ p ]; return equals( _v, __v ) }' )
