        ...
        if ( that.get( source ) ) {
          ...
          // } else {
            ...
            var transactions = that.transactions
              ...
              , input_name   = input._get_name ? input._get_name() : container.input._get_name()
            ;
            
            // ToDo: the transaction should be tagged if it has several sources, consider throwing, also consider a warning when adding
            log( '!Warning - Input_Transaction..remove():',
              '\n  Waiting for more outputs to terminate on untagged transaction.' +
              '\n' +
              '\n  This may cause unterminated transactions, interruption of dataflows, or errors.' +
              '\n' +
              '\n  To fix this issue:' +
              '\n    - If this is due to a diamond graph:' +
              '\n      - Add a fork tag at the start of the diamond,' +
              '\n      - and add the same tag at this input: ' + input_name +
              '\n' +
              '\n    - If this is from a dispatch loop connecting "internal" pipelines, either:',
              '\n      - Rename transactions at all upstream outputs',
              '\n      - Remove one or more pipelines from the loop',
              '\n' +
              '\n  Context:',
              '\n    - Terminating input  :', [ input_name ],
              '\n    - Removed output     :', [ source._get_name() ],
              '\n    - Waiting for outputs:',
                Object.keys( transactions )
                  .filter( function( _ ) { return transactions[ _ ] } )
                  .map( function( _ ) { return transactions[ _ ]._get_name() } )
                ,
              '\n    - Transaction object :', _t,
              '\n    - Upstream branches  :', container.branches,
              '\n    - Input_Transaction  :', that
            )
          }
