...
( 'transactions', [ './RS' ], function( RS ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     ToDo: IO_Transactions(): remove container parameter, only used as a name, allow one IO_Transactions() to be referenced by more than one container input in particular
  */
    ...
    remove_source: function( source ) {
      ...
      for ( tid in that.transactions ) {
        if ( ( input_transaction = that.get_from_tid( tid ) )
          ...
          RS.log( 'notice:', that._get_name( 'remove_source' ) + ', remove input_transaction:', input_transaction );
