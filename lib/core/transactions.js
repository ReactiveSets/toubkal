  ...
  */
  function Transaction( count, options, output, fork ) {
    ...
    if ( ( t && t.more ) || count > 1 || fork ) {
      ...
      // ToDo: implement deep copy in RS.deep_copy()
      if ( fork ) {
        o = extend_2( {}, options );
  ...
  Loggable.subclass( 'IO_Transactions', IO_Transactions, {
    ...
    */
    has: function( io_transaction ) {
      io_transaction || fatal( this, 'is null or undefined' );
      
      var t = this.transactions[ io_transaction.tid || fatal( this, 'has no tid' ) ];
      
      t && t !== io_transaction && fatal( this, 'does not match found io_transaction' );
      
      return t;
      
      function fatal( that, message ) {
  ...
  Loggable.subclass( 'IO_Transaction', IO_Transaction, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       ...
       Returns
       - undefined: if transactions was never added to this
       - transactions if transactions is already added in this.
       - null if transactions was added then removed from this.
       
       Parameters:
       - transactions (IO_Transactions): to add to this.
    */
    get: function( transactions ) {
      return this.transactions[ transactions.id ];
    }, // IO_Transaction.get()
    
    /* ------------------------------------------------------------------------
       add( transactions )
       
       Adds transactions counterpart to this transaction
       
       Parameters:
       - transactions (IO_Transactions): to add to this.
       
       Throws an Error if transactions was already added, even if removed.
    */
    add: function( transactions ) {
      var that = this
      ...
      these_transactions[ id ] = transactions;
    ...
    /* ------------------------------------------------------------------------
       ...
       Removes transactions counterpart from this transaction
       
       Parameters:
       - transactions (IO_Transactions): to remove from this.
       
       Throws an Error if transaction was already removed or was never added
       to this, or does not match transactions.
    */
    remove: function( transactions ) {
      ...
      t !== null         || fatal( 'already terminated' );
      t                  || fatal( 'not found' );
      t === transactions || fatal( 'does not match transactions' );
  ...
  */
  ...
  IO_Transactions.subclass( 'Input_Transactions', Input_Transactions, function( Super ) { return {
    /* ------------------------------------------------------------------------
       ...
       Sets transaction tag from an upstream query tree and / or the number of
       concurrent branches.
       
       Parameters:
       - tag (optional String): transaction tag from an upstream query tree
    ...
    */
    add: function( tid, source, forks ) {
      var tag = this.tag, transaction;
      
      // if ( ! ( tag && forks ) ) return null; // There are no concurrent transactions at this input, not added
    ...
    /* ------------------------------------------------------------------------
       ...
       - Object: object that was returned by this.add() with additional more
         flag set to true if there are more outputs emitting operations that
         have not yet removed for this transaction.
    */
    ...
    /* ------------------------------------------------------------------------
       ...
       Called by:
       - Input.._transactions_remove_source() by Input..remove_source() when
    ...
    */
    remove_source: function( source ) {
      ...
      source.remove_source( that );
      
      return Object.keys( that.transactions )
        
        .map( function( input_transaction ) {
          if ( input_transaction.remove( source ).is_empty() ) {
            // This input transaction terminates at this input.
            ...
            // ToDo: handle fork
            that.remove( input.transaction );
            
            return input_transaction.tid;
          }
        } )
      ;
  ...
  function Input_Transaction( destination, tid, forks ) {
    ...
    // this input
    this.tagged = forks && forks.indexOf( destination.tag ) != -1;
  } // Input_Transaction()
  
  IO_Transaction.subclass( 'Input_Transaction', Input_Transaction, function( Super ) {
    return {
      // ToDo: document Input_Transaction..remove()
      remove: function( source ) {
        var that = this;
        
        ++that.terminated_count; // One more has terminated
        
        de&&ug( that._get_name( 'remove' ), { terminated_count: that.terminated_count } );
  ...
  IO_Transaction.subclass( 'Output_Transaction', Output_Transaction, {
    ...
    add_destination: function( input_transactions, forks ) {
      ...
      if ( this.get( input_transactions ) ) return this;
      
      var s = input_transactions.add( this.tid, this.container, forks );
    ...
    remove_destination: function( input_transactions ) {
      de&&ug( this._get_name( 'remove_destination' ), input_transactions._get_name() );
      
      var _t = input_transactions.remove( this.tid, this.container );
      
      if ( _t ) {
        this.remove( input_transactions );
    ...
    // ToDo: Document and test Output_Transaction..get_terminated_destinations()
    // Returns inputs transactions for which transactions were terminated
    get_terminated_destinations: function() {
      var that = this
        , terminated_inputs = []
      ;
      ...
      if ( that.count ) {
        de&&ug( get_name(), { count: that.count } );
        ...
        Object.keys( that.transactions ).forEach( function( id ) {
          var input_transactions = that.transactions[ id ];
          
          // Remove source from input.transactions
          // ToDo: fix and finalize the following code:
          that.remove_destination( input_transactions ) // returns 'more'
            || terminated_inputs.push( input_transactions )
          ;
      ...
      function get_name() {
        return that._get_name( 'get_terminated_destinations' );
      }
    } // Output_Transaction..get_terminated_destinations()
  ...
  IO_Transactions.subclass( 'Output_Transactions', Output_Transactions, {
    /* ------------------------------------------------------------------------
       get_concurrent_options( input_transactions, [ options ] )
       
       Get output options for an input_transactions from options, with 'more' possibly added
       ...
       Parameters:
       - input_transactions (Input_Transactions): input transactions to get options for
       ...
       - options (Object): output options with more possibly added to
         transaction Object
    */
    get_concurrent_options: function( input_transactions, o ) {
      // ToDo: change method name to/and memorize transactions even when no input_transactions tags, to allow closing of all transactions.
      // ToDo: also needs to create an Output_Transaction if there are fork tags in the transaction
      //if ( ! input_transactions.tag ) return o; // There are no concurrent transaction at the target input
      
      var _t, tid, transaction;
      
      if ( o && ( _t = o._t ) && ( tid = _t.tid ) ) {
        // ToDo: add tests
        
        // Get or create output transaction for transaction_id
        transaction = this.get_transaction( tid );
        
        // Add destination input_transactions if not already added
        transaction.add_destination( input_transactions, _t.forks );
        
        // ToDo: optimize to not add then remove immediatly when no more
        if ( ! _t.more ) {
          ...
          if ( transaction.remove_destination( input_transactions ) ) {
            ...
            // ToDo: use a deep clone version or extend()
            o = extend_2( {}, o );
            
            ( o._t = extend_2( {}, _t ) ).more = true; // Don't close this transaction just yet
          }
      ...
      return o;
    }, // Output_Transactions..get_concurrent_options()
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        If options indicate the end of a transaction for which an output
        transaction exists, terminate this transaction and return terminated
        input transactions.
        
        Returns:
        - (Array): input transactions for which the transaction terminates
        
        ToDo: add tests for get_terminated_input_transactions()
    */
    get_terminated_input_transactions: function( options ) {
      var _t, transaction;
      
      if ( options
        && ( _t = options._t )
        && ! _t.more
        && ( transaction = this.get_from_tid( _t.tid ) )
      ) {
        return this
          .remove( transaction )
          .get_terminated_destinations()
        ;
      }
      
      return [];
    }, // Output_Transactions..get_terminated_input_transactions()
    
    /* ------------------------------------------------------------------------
       remove_source( tid, source )
       
       Removes source for a transaction id.
       
       If the transaction becomes empty, remove it from this.transactions.
    ...
    */
    remove_source: function( tid, source ) {
      ...
      return this;
    } // Output_Transactions..remove_source()
  } ); // Output_Transactions
