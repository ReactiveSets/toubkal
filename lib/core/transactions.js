  ...
  */
  ...
  IO_Transactions.subclass( 'Input_Transactions', Input_Transactions, function( Super ) { return {
    /* ------------------------------------------------------------------------
       ...
       Sets transaction tag from an upstream query tree.
    ...
    */
    set_branches: function( count ) {
      ...
      // ToDo: if there are an ongoing transactions, some may terminate if count is decreased and 
    ...
    get_terminated_destinations: function() {
      ...
      if ( that.count ) {
        ...
        for ( var id in that.transactions ) {
          ...
          input_transactions && ( // input_transactions not null, i.e. not removed yet
  ...
  IO_Transactions.subclass( 'Output_Transactions', Output_Transactions, {
    ...
    */
    get_options: function( input_transactions, options ) {
      var that = this, _t, tid, transaction;
      ...
      if ( options && ( _t = options._t ) && ( tid = _t.id ) ) {
        ...
        if ( ! _t.more && ( transaction = that.get_from_tid( tid ) ) && transaction.get( input_transactions ) ) {
          ...
          transaction.is_empty() && that.remove( transaction );
        }
      }
