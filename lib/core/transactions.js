...
( 'transactions', [ './RS' ], function( RS ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Parameters:
     - input: reference to container
     
     Input Transactions are held in a Dictionary of source transactions by transaction_id.
     
     An input transaction is a Dictionary of Objects indexed by Output_Transactions..id.
     These objects have  attributes:
       ...
       ToDo: the object above should be the Output_Transaction itself, position is no-longer
       used because it could not work reliably to help remove this Input_Transactions from
       the source Output_Transaction.
     
     A special object for each transaction, name '_' holds additional meta-data about the
     transaction:
       - count: initialized to 1, the number of output sources that have started but not
         terminated this transaction, i.e. the number of ongoing chunks of the transaction
         from inputs
       
       - terminated_count: initialized to 0, the total number of sources that have terminated
         the transaction
       
       - tagged (Boolean): true if this input tag is found in forks, this transaction is
         tagged and will have to terminate at all branches before terminating at this input
     
     ToDo: encapsulate the above into an Input_Transaction object
  */
  function Input_Transactions( input ) {
    this.input = input;
    this.transactions = Dictionary();
    this.tag = null;
    this.branches = 0;
    ...
    */
    remove: function( tid, source ) {
      ...
      if ( transaction ) {
        ...
        if ( s ) {
          transaction[ source.id ] = null; // marks this branch as terminated
    ...
    remove_source: function( source ) {
      ...
      for ( transaction_id in transactions ) {
        ...
        for ( var i = 0, s; s = transaction[ i ]; ) {
          ...
          de&&ug( name, { removing_source: source._get_name(), from_transaction: transaction_id } );
          
          transaction.splice( i, 1 );
          
          source.remove_source( transaction_id, this );
  ...
  Root.subclass( Output_Transaction, {
    ...
    add_destination: function( input_transactions, forks ) {
      ...
      if ( s ) {
        ...
        */
        ...
        // ToDo; remove assignement to s.position
        s.position = inputs.length;
        
        // ToDo: inputs[ input_transactions.id ] = input_transactions;
        inputs.push( input_transactions );
        
        de&&ug( this._get_name( 'add_destination' ), { input_added_to_transaction: tid } );
      }
    ...
    get_terminated_destinations: function() {
      var inputs = this.inputs
        , id = this.id
        , name = de && this._get_name( 'get_terminated_destinations' )
        , terminated_inputs = []
      ;
      ...
      if ( inputs.length ) {
        ...
        while ( input_transactions = inputs[ ++i ] ) {
          ...
          var s = input_transactions.remove( id, this.source ); // ToDo: consider using this instead of this.source
          ...
          log( 'exception Output_Transaction..terminate()' );
    ...
    /* ------------------------------------------------------------------------
       remove_source( input_transactions )
       
       Removes input_transactions from this output transaction
       
       Parameters:
       - input_transactions (Input_Transactions): to remove from this.
    */
    remove_source: function( input_transactions ) {
      // ToDo: this.inputs[ input_transactions.id ] = null;
      
      var position = this.inputs.indexOf( input_transactions );
      
      if ( position == -1 ) throw new Error( this._get_name( 'remove_source' ) + ' input not found' );
  ...
  var Output_Transactions_id = 2000; // Start at 2000 to ease traces
  ...
  Root.subclass( Output_Transactions, {
    ...
    */
    get_concurrent_options: function( input, o ) {
      // ToDo: change method name to/and memorize transactions even when no input tags, to allow closing of all transactions.
    ...
    /* ------------------------------------------------------------------------
       get_transaction: function( transaction_id )
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        If options indicate the end of a transaction, terminate this
        transaction and return terminated inputs.
    ...
    */
    get_terminated_input_transactions: function( options ) {
      ...
      if ( _t && ! _t.more ) {
        ...
        // ToDo: should we take forks in consideration here?
        
        if ( transaction ) {
          delete this.transactions[ tid ];
          
          return transaction.get_terminated_destinations();
        }
      }
      
      return [];
    }, // get_terminated_input_transactions()
