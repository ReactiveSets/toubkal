      ...
      */
      remove: function( source ) {
        ...
        if ( that.get( source ) ) {
          ...
          if ( that.count == 0 ) {
            ...
            if ( not_tagged || that.terminated_count >= that.container.branches ) { // ToDo: fix encapsulation
              ...
              _t.more ||
                error( [
                  '\n  Already terminated transaction terminates a second time.'
                ] )
              ;
            ...
            error( [
              ...
              '\n      - Rename transactions at all upstream outputs using pipelet option _t_postfix' +
        ...
        function error( message ) {
          ...
          message.unshift( '!Error - ' + get_name() + ':\n' );
          
          message = message.concat( [
            '\n\n  Context:' +
            '\n    - Terminating input  :', [ get_input_name() ],
            '\n    - Removed output     :', [ source._get_name() ],
            '\n    - Waiting for outputs:',
              Object.keys( transactions )
                .map   ( function( _ ) { return transactions[ _ ] && transactions[ _ ]._get_name() } )
                .filter( Boolean )
              ,
            '\n    - Transaction object :', _t,
            '\n    - Upstream branches  :', that.container.branches,
            '\n    - Input_Transaction  :', that
          ] );
          
          // ToDo: send warning to global log dataflow
          log.apply( null, message.concat( get_context() ) );
        } // warn()
