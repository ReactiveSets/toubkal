...
( 'transactions', [ './RS' ], function( RS ) {
  ...
  */
  function Transaction( count, options, output, fork ) {
    ...
    this.updated      = [];     // Values updated but which emission is defered
  ...
  Event_Emitter.subclass( 'Transaction', Transaction, {
    ...
    get_options: function() {
      if ( this.closed ) {
        throw new Error ( this._get_name( 'get_options' )
    ...
    */
      ...
      if ( removed.length ) {
        ...
        this.removed = []; // _clear before __emit() to allow __emit() to set or unset more
      ...
      if ( updated.length ) {
        ...
        this.updated = []; // _clear before __emit() to allow __emit() to set or unset more
        
        this._emit( 'update', updated, this.get_emit_options() );
      }
      
      if ( added.length || this.need_close ) {
        de&&ug( name, '__emit added' );
        
        this.added = []; // _clear before __emit() to force to unset more
  ...
  */
  ...
  IO_Transactions.subclass( 'Input_Transactions', Input_Transactions, function( Super ) { return {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        @manual internal
    */
    remove_source: function( output_transactions ) {
      ...
      for ( tid in that.transactions ) {
        ...
        if ( input_transaction && input_transaction.get( output_transactions ) ) {
          ...
          _t = output_transactions.remove_destination( output_transactions.get_from_tid( tid ), that );
  ...
  function Output_Transaction( input, source, tid ) {
    IO_Transaction.call( this, source, tid );
    
    // ToDo: remove input parameter and instance attribute
    this.input = input;
  }
  ...
  IO_Transaction.subclass( 'Output_Transaction', Output_Transaction, {
    ...
    /* ------------------------------------------------------------------------
        ...
        - (Array of Objects): terminated destinations with transaction objects.
          ...
          - input: opaque reference to target input
    ...
    */
    terminate: function() {
      ...
      return that.map( function( input_transactions ) {
        ...
        //de&&ug( that._get_name( 'terminate' ) + 'input_transactions:', input_transactions );
        
        return _t.more
          ? null // more expected, this input does not terminate
          : { _t: _t, input: input_transactions.input }
        ;
  ...
  IO_Transactions.subclass( 'Output_Transactions', Output_Transactions, {
    /* ------------------------------------------------------------------------
       get_options( input, input_transactions, [ options ] )
       ...
       Parameters:
       - input (Object): input, opaque reference, to return with terminate()
    ...
    */
    get_options: function( input, input_transactions, options ) {
      ...
      if ( options && ( _t = options._t ) && ( tid = _t.id ) ) {
        ...
        if ( _t.more || forks ) {
          ...
          output_transaction || ( output_transaction = that.get_transaction( input, tid ) );
        ...
        if ( ! _t.more && output_transaction && output_transaction.get( input_transactions ) ) {
          ...
          var t = that.remove_destination( output_transaction, input_transactions );
    ...
    /* ------------------------------------------------------------------------
       get_transaction: function( input, tid )
       
       Get or create an output transaction for transaction_id
       
       Parameters:
       - input: opaque reference to return with terminate()
    ...
    */
    get_transaction: function( input, tid ) {
      return this.get_from_tid( tid ) || this.add( new Output_Transaction( input, this, tid ) );
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       remove_destination( output_transaction, input_transactions )
       
       If output transaction becomes empty, remove it from this.transactions.
    ...
    */
    remove_destination: function( output_transaction, input_transactions ) {
      var that = this, _t;
      
      if ( output_transaction ) {
        de&&ug( that._get_name( 'remove_destination' ) );
        
        _t = output_transaction.remove_destination( input_transactions );
        
        // Add test for non-empty output_transaction
