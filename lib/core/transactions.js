    ...
    */
    get_count: function() {
      ...
      if( tids.length != count )
        throw new Error( get_name( that ) + ', bad count: ' + count + ', tids: ' + tids );
      ;
    ...
    */
    get_transaction: function( count, options, output, fork ) {
      ...
      if( options && ( _t = options._t ) ) {
        // There is an upstrean transaction
        tid = _t.id;
        
        if( t = transactions[ tid ] ) {
          // This is a continuation of an upstream transaction
          de&&ug( name + 'continue upstream transaction, tid:', tid );
        ...
        // This is a new upstream transaction
        de&&ug( name + 'create from upstream transaction, tid:', tid );
        
        return record_tid.call( new_transaction(), tid );
      }
      
      de&&ug( name + 'No upstream transaction, count:', count );
      
      return new_transaction().on( 'tid', record_tid );
      ...
      function record_tid( tid ) {
        ...
        return ( transactions[ tid ] = this ).on( 'ended', on_transaction_ended );
      }
      
      // Remove tid from transactions when transaction has ended, emit event "no_transactions"
      function on_transaction_ended() {
        de&&ug( name + 'on_transaction_ended, tid: %s, count: %d', tid, that.count );
  ...
  Event_Emitter.subclass( 'Transaction', Transaction, {
    ...
    get_options: function() {
      ...
      if( that.closed ) {
        ...
        ug( '!Warning:', error, error.stack )
      }
      
      //de&&ug( _get_name(), { more: more } );
      
      options.more = more; // ToDo: needs to be redefined with the role of get_options()
      
      return options;
      
      function _get_name() { return get_name( that, 'get_options' ) }
    ...
    get_emit_options: function() {
      var that    = this
        ...
        , fork    = that.fork
        //, de      = true
        , ug      = de && log.bind( null, get_name( that, 'get_emit_options' ) )
      ;
  ...
  Loggable.subclass( 'IO_Transaction', IO_Transaction, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        @throws
        - (Error): io_transactions was already added, even if removed.
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        @throws
        - (Error): io_transactions was already removed or was never
    ...
    */
    error: function( method, other, message ) {
      ...
      if( warned > 9 ) throw new Error( 'too many errors on this transaction' );
  ...
  IO_Transaction.subclass( 'Output_Transaction', Output_Transaction, {
    ...
    */
    add_destination: function( input, forks ) {
      ...
      /*
        ...
        This can be taken care of Pipelet._add_input() with a warning and/or
    ...
    */
    ...
    remove_destination: function( input ) {
      ...
      // ToDo: remove input instead of input_transactions
      // _t && that.remove( input );
      _t && that.remove( input_transactions ); // Note: caller has responsibility to remove this when empty()
    ...
    */
    terminate: function() {
      ...
      return that.map( function( input ) {
        ...
        //de&&ug( get_name( that, 'terminate' ) + 'input_transactions:', input.transactions );
  ...
  */
  function options_add_fork_tag( options, fork_tag ) {
    ...
    if ( ! t || options_last_fork_tag( t ) !== fork_tag ) {
      ...
      t = new Transaction( 1, options, null, fork_tag );
