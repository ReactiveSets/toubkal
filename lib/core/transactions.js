  ...
  Loggable.subclass( 'IO_Transaction', IO_Transaction, {
    ...
    */
    remove: function( io_transactions ) {
      ...
      if ( t !== io_transactions ) return error( 'Does not match io_transactions, expected: ' + get_name( t ) );
  ...
  IO_Transactions.subclass( 'Input_Transactions', Input_Transactions, function( Super ) { return {
    ...
    */
    terminate_transaction: function( transaction, _t ) {
      ...
      // ToDo: this does not work, some transactions cannot be closed this way, in dispatch remove branch, branches may have unterminated transactions
      // but nothing should be emitted necessarily
      this.input.listen( 0, [], { _t: _t } );
      ...
      add_branches: function( branches ) {
        ...
        if( that.concurrent ) {
          ...
          if( that.terminated_count >= that.branches ) {
            ...
            log( get_name( that, 'add_branches' ) + 'terminating transaction, count:', branches );
            
            that
              .map( function( output_transactions ) {
                Super.remove.call( that, output_transactions );
                
                // ToDo: Fix, this does not work if input_transactions is a from a controllet such as a union
