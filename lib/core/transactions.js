...
*/
  ...
  */
  ...
  IO_Transactions.subclass( 'Input_Transactions', Input_Transactions, function( Super ) { return {
    ...
    */
    add: function( tid, source, forks ) {
      ...
      if ( transaction = that.get_from_tid( tid ) ) {
        ...
        transaction = Super.add.call( that, new Input_Transaction( that, tid, forks ) );
      }
  ...
  function Input_Transaction( destination, tid, forks ) {
    ...
    if ( _.tagged = !! l ) {
      ...
      if ( ( input_tag = destination.tag )
        ...
        de&&ug( get_name( _ ) + ', removed fork tag "' + input_tag + '", remaining forks:', forks );
      }
    }
  ...
  IO_Transaction.subclass( 'Output_Transaction', Output_Transaction, {
    ...
    remove_destination: function( input ) {
      ...
      var _t = input_transactions.remove( that.tid, that.container ); // ToDo: fix encapsulation for this.container
    ...
    get_options: function( input, options ) {
      ...
      if ( options && ( _t = options._t ) && ( tid = _t.id ) ) {
        ...
        if ( forks
          ...
          de&&ug( _get_name() + ', removed fork tag "' + input_tag + '", remaining forks:', _t.forks );
        }
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      @class_method Options.last_fork_tag( options )
      
      @short Get last fork tag in transaction options, if any
      
      @returns
      - (falsy): there are no options, transaction, or fork tags
