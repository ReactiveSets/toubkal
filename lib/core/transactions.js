...
( 'transactions', [ './RS' ], function( RS ) {
  ...
  */
  ...
  IO_Transactions.subclass( 'Input_Transactions', Input_Transactions, function( Super ) { return {
    ...
    */
    set_tag: function( tag ) {
      ...
      de&&ug( this._get_name( 'set_tag' ) + ', tag:', tag )
  ...
  IO_Transaction.subclass( 'Output_Transaction', Output_Transaction, {
    ...
    /* ------------------------------------------------------------------------
        get_terminated_destinations( options )
        
        Terminate this transaction and return terminated input transactions.
        
        Returns:
        - (Array of Objects): destinations with options for which the
          transaction terminates. Each object has attributes:
          - input_transactions (Input_Transactions): terminating input
            transactions
          - options (Object): options to terminate this transaction at this
            input.
        
        ToDo: add tests for get_terminated_destinations()
        ToDo: find a better name because this method has side effects
    */
    get_terminated_destinations: function( options ) {
      var that = this
        , terminated_inputs = []
        , input_transactions
        , _t
      ...
      if ( that.count ) {
        ...
        for ( var id in that.transactions ) {
          input_transactions = that.transactions[ id ];
          
          // Remove source from input.transactions
          if ( input_transactions ) {
            // input_transactions not null, i.e. not removed yet
            
            ( _t = that.remove_destination( input_transactions ) )
              && _t.more
              || terminate_destination( options, _t )
            ;
          } else {
            // ToDo: throw new Error( get_name() + input_transactions + ' input_transactions, id:', id );
            
            de&&ug( get_name() + ', ' + input_transactions + ' input_transactions, id:', id );
          }
      ...
      function terminate_destination( options, _t ) {
        // ToDo: DRY code, duplicated in Output_Transactions..get_options()
        // ToDo: consider moving this code to Input_Transactions
        if ( options ) { // ToDo: only clone when value of _t != options._t
        ...
        terminated_inputs
          .push( { input_transactions: input_transactions, options: options } )
        ;
      } // terminate_destination()
  ...
  IO_Transactions.subclass( 'Output_Transactions', Output_Transactions, {
    ...
    */
    get_options: function( input_transactions, options ) {
      var that = this, _t, tid, forks, input_tag, l, cloned, transaction;
      ...
      if ( options && ( _t = options._t ) && ( tid = _t.id ) ) {
        ...
        if ( ! _t.more && ( transaction = that.get_from_tid( tid ) ) && transaction.get( input_transactions ) ) {
          ...
          var t = transaction.remove_destination( input_transactions );
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        Returns:
        - (Array of Objects): input transactions and options for which the
          transaction terminates. Each object has attributes:
          - input_transactions (Input_Transactions): terminating input
            transactions
          - options (Object): options to terminate this transaction at this
        ...
        ToDo: find a better name because this method has side effects by removing an ouput transaction
    */
