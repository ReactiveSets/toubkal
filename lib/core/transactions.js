...
*/
  ...
  */
    ...
    add: function( tid, source, forks ) {
      ...
      if( transaction ) {
        ...
        transaction = Super.add.call( that,
          new Input_Transaction( that, tid, _t, is_concurrent )
        );
      }
  ...
  function Input_Transaction( destination, tid, _t, concurrent ) {
    var that = this;
    
    IO_Transaction.call( that, destination, tid );
      ...
      remove: function( source ) {
        ...
        if( that.get( source ) ) {
          ...
          if( that.count ) {
            ...
            if( not_concurrent || that.terminated_count >= that.container.branches ) { // ToDo: fix encapsulation
