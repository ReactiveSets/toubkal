    ...
    */
    ...
    /* ------------------------------------------------------------------------
       ...
       - _t (Object): returned by Input_Transaction..remove():
         - null: if this transaction is not currently in progress from source
         - { more: more (Boolean) }: where more can be:
           - true: more operations are expected for this transaction from other
             branches
           - false: all branches have terminated this transaction
    */
  ...
  IO_Transaction.subclass( 'Input_Transaction', Input_Transaction, function( Super ) {
    return {
      /* ------------------------------------------------------------------------
         ...
         - _t (Object): { more: more (Boolean) }: where more can be:
           - true: more operations are expected for this transaction from other
             branches
           - false: all branches have terminated this transaction
      */
  ...
  IO_Transaction.subclass( 'Output_Transaction', Output_Transaction, {
    ...
    get_terminated_destinations: function() {
      ...
      if ( that.count ) {
        ...
        for ( var id in that.transactions ) {
          var input_transactions = that.transactions[ id ];
          
          de&&( input_transactions || ug( get_name() + ', null or undefined input_transactions:', input_transactions + ', id:', id ) );
          
          // Remove source from input.transactions
          // ToDo: fix and finalize the following code:
          input_transactions && ( // input_transactions not removed yet
    ...
    */
    get_options: function( input_transactions, options ) {
      ...
      if ( options && ( _t = options._t ) && ( tid = _t.id ) ) {
        if ( _t.more || _t.forks ) {
          de&&ug( get_name( this ) + ', add destination' );
          
          this
            // Get or create output transaction for tid
            .get_transaction( tid )
            // Add destination input_transactions if not already added
            .add_destination( input_transactions, _t.forks )
          ;
        }
        
        if ( ! _t.more && ( transaction = this.get_from_tid( tid ) ) && transaction.get( input_transactions ) ) {
          de&&ug( get_name( this ) + ', remove destination' );
          ...
          if ( transaction.remove_destination( input_transactions ) ) {
            ...
            ( options._t = extend_2( {}, _t ) ).more = true; // Don't close this transaction just yet
          }
    ...
    */
    get_terminated_input_transactions: function( options ) {
      ...
      if ( options
        && ( _t = options._t )
        && ! _t.more
        && ( transaction = this.get_from_tid( _t.id ) )
      ) {
        return this
          .remove( transaction )
