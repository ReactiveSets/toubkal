  ...
  Loggable.subclass( 'IO_Transaction', IO_Transaction, {
    ...
    */
    error: function( method, other, message ) {
      ...
      if( warned ) {
        ...
        message.push(
          ...
          '\n    - Ongoing ' + other_type + 's:',
            that.filter_map( io_transactions_name ),
      ...
      function io_transactions_name( io_transactions ) {
        return get_name( io_transactions )
      }
  ...
  IO_Transaction.subclass( 'Output_Transaction', Output_Transaction, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        @description
        This method is called by @@Output_Transactions..move_to().
    */
  ...
  IO_Transactions.subclass( 'Output_Transactions', Output_Transactions, {
    ...
    */
    move_to: function( to, input, tid ) {
      ...
      if ( from ) {
        log( get_name( that, 'move_to' ) + 'from:', from );
        
        from
          .move_to( to.get_transaction( tid ), input, tid )
          .is_empty() && that.remove( from )
    ...
    */
    ...
    /* ------------------------------------------------------------------------
        ...
        transaction. Should not have attribute truly, otherwise terminate()
        does not terminate the transaction and returns an empty Array.
        ...
        @description
        If transaction_object indicates the end of a transaction for which
        an output transaction exists, terminate this transaction and return
        terminated input transactions with termination transaction object.
