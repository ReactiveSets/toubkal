    ...
    _add: function( values, options ) {
      ...
      if ( ! _t || ! _t.more ) {
        ...
        if ( l = values.length ) {
          ...
          /*
            !!!Warning: Set new state after calling that.__emit_xxx().
            
            If during an emission, a pipelet is added downstream
            synchronously, it will fetch the current state then receive
            this operation. Therefore fetch must retrieve the state
            before the operation started.
            
            This can happen typically with dispatch() when the branches
            dataflow is also consummed by a branched dataflow.
            
            There is possibly a better way to deal with this issue:
            - make Output _fetch assynchronous while emitting, after
              emission, call any waiting fetch. If this is implemented
              we will need to move the state change before the emission
              because the operation ariving after fetch should get the
              final state and will not receive the operation.
              
              This could be done with a locking mechanism on the output.
              
              This could be a better option because retriving a state
              that changes after synchronously is not optimal.
          */
          that._last = [ v ]
