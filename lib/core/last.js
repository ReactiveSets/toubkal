/*  last.js
    
    Copyright (c) 2013-2016, Reactive Sets

    This program is free software: you can redistribute it and/or modify
    it under the terms of the GNU Affero General Public License as
    published by the Free Software Foundation, either version 3 of the
    License, or (at your option) any later version.

    This program is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU Affero General Public License for more details.

    You should have received a copy of the GNU Affero General Public License
    along with this program.  If not, see <http://www.gnu.org/licenses/>.
    
    ---
    
    A set with a single value of the last value added to the source.
*/
( this.undefine || require( 'undefine' )( module, require ) )()
( 'last', [ './pipelet' ], function( rs ) {
  'use strict';
  
  var RS         = rs.RS
    , Greedy     = RS.Greedy
    , Query      = RS.Query
  ;
  
  /* -------------------------------------------------------------------------------------------
     de&&ug()
  */
  var de = false, ug = RS.log.bind( null, 'last' );
  
  /* -------------------------------------------------------------------------------------------
      last( options )
      
      Forwards downstream the last value from its upstream dataflow, emitting updates with
      the previous value and new current last value.
      
      "last value" means the last value received from the last received added values chunk.
      
      First emission is an "add" operation, subsequent emissions are "update" operations.
      
      This is a stateful, synchronous, greedy pipelet. Current state has zero or one value.
      
      Note: this does not provide the last value based on some order.
      
      Limitation. This pipelet is stateful but only memorizes the last value from the last add.
      This is very efficient at the cost of the following limitation:
        
        If the last value is removed from upstream, it will remain downstream until a new add
        comes from upstream.
      
      Parameters:
      ----------
      - options (Object): pipelet options:
        - name (String): debugging name
      
      Example:
      -------
      - provide last event from a stream of add-only parsed url events:
      
        rs
          .url_events() // emits a stream of add operations with a "url" attribute
          .url_parse()  // parses "url" attribute into url components
          .last()       // provides the set containing the last parsed url
        ;
      
      ToDo: add tests for last()
  */
  function Last( options ) {
    var that = this;
    
    that._output || ( that._output = new Last.Output( that, options.name ) );
    
    Greedy.call( that, options );
    
    // State
    that._last = null;
  } // Last()
  
  Last.Output = Greedy.Output.subclass(
    'Last.Output',
    
    function( p, name ) { Greedy.Output.call( this, p, name ) }, {
    
    fetch_unfiltered: function( receiver ) {
      var last = this.pipelet._last;
      
      receiver( last ? [ last ] : [], true );
    } // _fetch()
  } );
  
  Greedy.Build( 'last', Last, {
    _add: function( values, options ) {
      var l = values.length, _t = options && options._t;
      
      if ( _t && _t.more ) return this; // more chunks comming, wait for last chunk
      
      if ( l ) {
        var last = this._last
          , v = this._last = values[ l - 1 ]
        ;
        
        if ( last ) {
          this.__emit_update( [ [ last, v ] ], options );
        } else {
          this.__emit_add( [ v ], options );
        }
      }
      
      return this;
    }, // _add()
    
    _remove: function() { return this }
  } ); // Last instance methods
  
  /* --------------------------------------------------------------------------
     module exports
  */
  RS.add_exports( { 'Last': Last } );
  
  de&&ug( "module loaded" );
  
  return rs;
} ); // last.js
