...
( 'operations_optimizer', [ './pipelet' ], function( rs ) {
  ...
  /* -------------------------------------------------------------------------------------------
     de&&ug()
  */
  var de = true, ug = log.bind( null, 'operations_optimizer' );
  
  /* -------------------------------------------------------------------------------------------
     operations_optimizer( options )
  */
  function Operations_Optimizer( options ) {
    this._output || ( this._output = new Operations_Optimizer.Output( this, 'operations_optimizer_out' ) );
      ...
      return {
        ...
        */
        emit: function( event_name, values, options ) {
          ...
          if ( operations > 1 ) t = new Transaction( operations, options );
