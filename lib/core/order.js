...
( 'order', [ './pipelet' ], function( rs ) {
  ...
  /* -------------------------------------------------------------------------------------------
     de&&ug()
  */
  var de = true, ug = log.bind( null, 'order' );
  ...
  Set.Build( 'order', Order, {
    ...
    /* --------------------------------------------------------------------------------------------
       ...
       - options (Object): optional parameters:
         ...
         - all           (Boolean): return one or more found value for each value. Used by
                                    join(), all matching values are returned in all_matches[]
                                    attribute of returned locations.
         
         - insert_before (Boolean): returned insert location is before the last match if true,
                                    default is insert after.
       ...
       Other attributes used internaly by() locate and that are not part of the API, meaning that
       these may change without an upgrade of major version number:
       ...
       - order         (Int): returned by sorter indicating relative position vs current guess
    ...
    */
    locate: function( values, options ) {
      ...
      return locate( this.a, this.sorter, locations, 1, n, exact, all, de );
      
      function locate( a, sorter, locations, begin, end, exact, all, de ) {
        ...
        de&&ug( get_name() + "name: " + _options.name + ", locations: (" + locations.length + ") " + log.s( locations, [ 'position', 'found', 'insert', 'all_matches' ] ) );
