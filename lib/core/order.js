...
( 'order', [ './stateful' ], function( rs ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      @pipelet order( organizer, options )
      
      @short Emits ordered operations
      
      @parameters
      - **organizer**:
        - (Function): function to sort source values:
          ```( a, b ) -> Number```
          
          Where a and b are *Object* values from the set to organize.
          
          And where the returned number should be:
          - 0: when a equals b
          - < 0: when a < b
          - \\> 0: when a \\> b
          
          If a and b where numbers, the operation ```a - b``` would produce
          the desired result.
        ...
        - (Array): attributes to sort source values. Each attribute refers
          to an attribute that must be present in all source values and that
          will be used to sort the source.
          
          Attributes are evaluated in the order provided by the array, the
          first being the most significant and the last being the least
          significant.
          
          Attributes are *Objects* with the following attributes:
          - **id** (String) the name of the attribute in source values.
          
          - **descending** (Bollean): optional, specifying the sort order.
            If true the sort order will be descending for this attribute,
            if false or undefined the sort order will be ascending.
        
        - (@@class:Pipelet\): to allow the dynamic change of sort Attributes.
          In which case the corresponding set must be a set of attributes as
          defined above.
      
      - **options** (Object): optional
        - **key** (Array of Strings): list of attribute names used to
          uniquelly identify values default is ```[ 'id' ]```.
        
        - **insert_before** (Boolean): insert new values before matching
          values if true, default is false, i.e. insert after.
        
        - **name** (String): debugging name for this instance.
      
      @description
      This is a @@stateful, @@greedy, @@synchronous pipelet.
      
      Order a source set using an organizer which can be a function, an
      Array of attributes, or a Pipelet allowing for dynamic changes of
      the organizer.
      
      Emitted operations have additional option *locations* or *moves*:
      - **locations* (Array of Numbers): each number represents the
        location in the set where each added of removed value was
        inserted or removed.
      
      - **moves** (Array of Objects): when the operation is an
        @@update, for each update a move Object has attributes:
        - **from** (Number): the location of the value before the update.
        - **to** (Number): the location of the value after the update.
      
      If the downstream pipelet is an @@class:Ordered\, it will be
      ordered by this pipelet using operation options *locations* and
      *moves*.
  */
  function Order( organizer, options ) {
    ...
    that._sorter         = null; // sorter function generated by _generate_code()
    that._sorter_waiters = [];
    that._organizer      = organizer;
    
    // ToDo: add tests for organizer changes
    organizer._output
      ? that._add_input(
          organizer,
    ...
    function generate_on_complete( _, options ) {
      ...
      _t && _t.more || that._generate_code( options );
    } // generate_on_complete()
  } // Order()
  
  Set.Build( 'order', Order, {
    _generate_code: function( options ) {
      var that      = this
        , name      = '_generate_code'
        , organizer = that._organizer
      ;
      
      de&&ug( get_name( that, name ) + 'complete, re-generate code' );
      
      switch( typeof organizer ) {
        case 'function': // organizer is the sorter
        return that._sort( organizer );
        ...
        default:
          ...
          that._error( name, 'missing organizer function or Array or Set' );
        return
      }
      ...
      function generate( organizer ) {
        ...
        return that._sort( eval( code.get() ), options );
      } // generate()
    }, // _generate_code()
    
    _wait_for_sorter: function( handler ) {
      this._sorter_waiters.push( handler );
      
      de&&ug( '_wait_for_sorter(), _sorter_waiters: ' + this._sorter_waiters.length );
      
      return this;
    }, // _wait_for_sorter()
    
    _sort: function( sorter, options ) {
      var that = this
        , a    = that.a
      ;
      ...
      if ( a ) {
        ...
        if ( l ) {
          ...
          updates.length && that.__emit_update( updates, extend( options, { moves: moves } ) );
        }
      } // if ( a )
      
      var waiters = that._sorter_waiters
        , l       = waiters.length
      ;
      
      if ( l ) {
        // de&&ug( get_name( that, '_sort' ) + 'sorter is ready, calling ' + l + ' waiters' );
    ...
    /* --------------------------------------------------------------------------------------------
       ...
       Note: the behavior of _locate() is part of the API, currently used by other pipelets such as
       join(). Supported API attributes are documented bellow in addition to unsuported attributes
       used by _locate() internally.
       ...
       - values (Array of values): sorted values to locate, if value is an Array it is assumed to
         ...
         to calling _locate() using this._sorter()
       ...
       Other locations attributes used internaly by _locate() that are not part of the API, meaning
    ...
    */
    _locate: function( values, options ) {
      ...
      for ( var i = -1; ++i < n; guess += step ) {
        ...
        if ( i === 0 ) {
          locate( state, that._sorter, locations, 0, 1, exact, all );
      ...
      function locate( state, sorter, locations, begin, end, exact, all, de ) {
        ...
        while( some_not_located ) {
          ...
          for ( var i = begin - 1; ++i < end; ) {
            ...
            if ( ( location.order = sorter( state[ guess ], o ) ) === 0 ) {
              ...
              if ( exact ) {
                var id        = that._identity( o )
                  , _identity = that._identity
                ;
              }
              
              if ( insert_before ) {
                while ( true ) {
                  if ( exact && location.found === u && id == _identity( state[ insert ] ) )
                    location.found = insert // first position where there is both an sorter match and an id match
                  ;
        ...
        de&&ug( _get_name() + locations.length + " locations: " + log.s( locations, [ 'position', 'found', 'insert', 'all_matches' ] ) );
    ...
    _add: function( values, options ) {
      ...
      if ( ! sorter )
        return that._wait_for_sorter( function() { that._add( values, options ) } )
      ;
      ...
      if ( a.length === 0 ) {
        ...
        var locations = that._locate( values ) // we're not looking for exact id matches but we might have to
        ...
        for ( var i = locations.length; i; ) {
          ...
          // ToDo: if there is an exact id match this could be an add waiting for a remove
    ...
    _remove: function( values, options ) {
      ...
      if ( ! sorter )
        return that._wait_for_sorter( function() { that._remove( values ) } )
      ;
      ...
      var locations = that._locate( values, { exact: true } )
        , _locations = options.locations = []
        , a = that.a, u
      ;
    ...
    _update: function( updates, options ) {
      ...
      if ( ! sorter )
        return that._wait_for_sorter( function() { that._update( updates ) } )
      ;
      ...
      // de&&ug( get_name( that, '_update' ) + 'updates:', updates );
      
      options = options_forward( options ) || {};
      
      var locations = that._locate( updates, { exact: true } )
        , moves = options.moves = []
        , a = that.a, u
      ;
      ...
      for ( var i = locations.length; i; ) {
        ...
        if ( f === u ) {
          ...
          if ( ! sorter( l.o, l.update ) ) {
            ...
            l = that._locate( [ l.update ] )[ 0 ]; // locate updated object
            ...
            /* Update other locations if affected by the move.
              ...
              2012/12/30 20:41:20.518 - rs, Order._locate(), name: books_ordered_by_descending_year, locations: [
  ...
  */
  ...
  function assert_ordered_options( that, method, attribute, options ) {
    if ( ! ( options && options[ attribute ] ) ) {
      ...
      throw new Error(
          get_name( that, method )
        + 'missing ' + attribute
        + ' from ordered source: ' + get_name( source )
      );
    }
