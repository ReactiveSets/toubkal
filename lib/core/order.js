  ...
  */
  function Order( organizer, options ) {
    ...
    organizer._output
      ? that._add_input(
          ...
          that._options.name + '-organizer',
          
          {
            _add   : generate_on_complete,
            _remove: generate_on_complete,
            _update: generate_on_complete
          }
        )
      : that.generate_code()
    ;
    
    function generate_on_complete( _, options ) {
      var _t = options && options._t;
      
      _t && _t.more || that.generate_code( options );
  ...
  Set.Build( 'order', Order, {
    generate_code: function( options ) {
      ...
      de&&ug( that._get_name( 'generate_code' ) + 'complete, re-generate code' );
      
      switch( typeof organizer ) {
        case 'function': // organizer is the sorter
        return that.sort( organizer );
        ...
        default: that.error( 'generate_code', 'missing organizer function or Array or Set' );
      }
          ...
          ._function( 'sorter', null, [ 'a', 'b' ] )
            ...
            for ( var i = -1; ++i < l; ) {
              var d            = organizer[ i ]
                ...
                , _safe_id     = safe_dereference( d.id )
                , inferior
                , superior
              ;
                  ...
                  if ( no_undefined && no_null )
                    if ( l == 1 )
                      code.add( d.descending
                        ? 'return b' + _safe_id + ' - a' + _safe_id
                        : 'return a' + _safe_id + ' - b' + _safe_id
                      );
                    
                    else
                      code.add( d.descending
                        ? 'if ( x = b' + _safe_id + ' - a' + _safe_id + ' ) return x'
                        : 'if ( x = a' + _safe_id + ' - b' + _safe_id + ' ) return x'
                      );
                  
                  else {
                    code
                      ._if( '( x = a' + _safe_id + ' ) !== ( y = b' + _safe_id + ' )' );
    ...
    */
    locate: function( values, options ) {
      ...
      for ( var i = -1; ++i < n; guess += step ) {
        ...
        if ( i === 0 ) {
          locate( state, that.sorter, locations, 0, 1, exact, all );
      ...
      return locate( state, that.sorter, locations, 1, n, exact, all, de );
        ...
        while( some_not_located ) {
          ...
          for ( var i = begin - 1; ++i < end; ) {
            ...
            if ( ( location.order = sorter( state[ guess ], o ) ) === 0 ) {
              ...
              if ( exact ) {
                var key      = that._make_key( o )
                  , make_key = that._make_key
                ;
              }
    ...
    _add: function( values, options ) {
      ...
      if ( ! sorter )
        return that.wait_for_sorter( function() { that._add( values, options ) } )
      ;
      ...
      if ( a.length === 0 ) {
        ...
        that.a = values.slice( 0 );
      } else {
        var locations = that.locate( values ) // we're not looking for exact key matches but we might have to
      ...
      return that.__emit_add( values, options );
    }, // _add()
    ...
    _remove: function( values, options ) {
      ...
      if ( ! sorter )
        return that.wait_for_sorter( function() { that._remove( values ) } )
      ;
      ...
      var locations = that.locate( values, { exact: true } )
        , _locations = options.locations = []
        , a = that.a, u
      ;
      ...
      return that.__emit_remove( values, options );
    }, // _remove()
    ...
    _update: function( updates, options ) {
      ...
      if ( ! sorter )
        return that.wait_for_sorter( function() { that._update( updates ) } )
      ;
      
      updates.sort( function( a, b ) { return sorter( a[ 0 ], b[ 0 ] ) } );
      
      // de&&ug( that._get_name( '_update' ) + 'updates:', updates );
      
      options = Options.forward( options );
      
      var locations = that.locate( updates, { exact: true } )
        , moves = options.moves = []
        , a = that.a, u
      ;
      ...
      locations.sort( function( a, b ) {
        return a.found > b.found ? 1 : a.found < b.found ? -1 : 0
      } );
      ...
      for ( var i = locations.length; i; ) {
        ...
        if ( f === u ) {
          ...
          if ( ! sorter( l.o, l.update ) ) {
            ...
            l = that.locate( [ l.update ] )[ 0 ]; // locate updated object
  ...
  */
  ...
  Set.Build( 'ordered', Ordered, {
    ...
    update_from_to: function( updates, options ) {
      ...
      if ( that.update_ )
        update_ = function( at, v ) { that.update_( at, v ) }
      ;
    ...
    _add: function( added, options ) {
      ...
      added.length && that.a.length && assert_ordered_options( that, '_add', 'locations', options );
