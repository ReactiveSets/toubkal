  ...
  */
  ...
  Pipelet.build( 'form_display_errors', Form_Display_Errors, {
    transform: function( values, options, caller ) {
      var form        = values[ 0 ]
        , form_node   = this.form
        , classes     = this.options.css_classes
        , css_error   = classes && classes.error
        , css_success = classes && classes.success
      ;
      ...
      if( caller === 'add' ) {
        for( var spans = form_node.getElementsByTagName( 'span' ), i = spans.length; i; ) {
          ...
          if( css_success ) container.setAttribute( 'class', css_success );
        }
      }
      ...
      for( var i  = len; i; ) {
        ...
        if( css_error ) {
          field_container.hasAttribute( 'class' ) ? field_container.className += ' ' + css_error : field_container.setAttribute( 'class', css_error );
        }
      }
  ...
  */
  ...
  Ordered.subclass( Form_Process_Fields, {
    ...
    draw_form_field: function( container, field ) {
      ...
      container_style && container.setAttribute( 'class', container_style );
      ...
      if( field_node ) {
        field_node.id   = field.id;
        field_node.name = field.name;
        
        field_style && field_node.setAttribute( 'class', field_style );
      ...
      if( field_label ) {
        ...
        label_style && label.setAttribute( 'class', label_style );
    ...
    event_handler: function() {
      ...
      for( var i = -1; ++i < len; ) {
        var element = elements[ i ], field = fields[ i ];
        ...
        switch( element.type ) {
          ...
          case 'hidden':
            if ( typeof field.value === 'object' && field.value && field.value.type == 'UUID' ) {
              element.value = object[ element.name ] = uuid.v4();
  ...
  */
  ...
  Ordered.build( 'form_process', Form_Process, {
    ...
    draw: function() {
      ...
      if( style && style.submit_button ) button.setAttribute( 'class', classes.submit_button );
