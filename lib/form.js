    ...
    transform: function( values, options, caller ) {
      ...
      if( caller === "add" ) {
        for( var spans = form_node.getElementsByTagName( 'span' ), i = spans.length; i; ) {
          ...
          if( css_error ) {
            container.setAttribute( 'class', css_success );
          } else {
      ...
      for( var i  = len; i; ) {
        ...
        if( css_error ) {
          field_container.setAttribute( 'class', css_error );
        } else {
          field_container.style.color = '#ff0000';
        }
      }
    ...
    event_handler: function() {
      ...
      for( var i = -1; ++i < len; ) {
        ...
        switch( element.type ) {
          ...
          case 'hidden':
            if ( typeof element.value == 'object' && element.value && element.value.type == 'UUID' ) {
