  ...
  */
  ...
  Pipelet.build( 'form_display_errors', Form_Display_Errors, {
    transform: function( values, options, caller ) {
      ...
      // init error messages
      for( var spans = this.form.getElementsByTagName( 'span' ), i = spans.length; i; ) spans[ --i ].innerText = '';
      
      if( ! object.errors ) return values;
  ...
  */
  ...
  Ordered.subclass( Form_Process_Fields, {
    insert_: function( at, v ) {
      ...
      if( v.type !== 'hidden' ) div.appendChild( document.createElement( 'span' ) );
