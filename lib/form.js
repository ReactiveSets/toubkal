  ...
  */
  ...
  Pipelet.build( 'form_display_errors', Form_Display_Errors, {
    transform: function( values, options, caller ) {
      ...
      if( ! object || ! object.errors ) return values;
      ...
      // init the previous
      for( var spans = form.getElementsByTagName( 'span' ), i = spans.length; i; ) spans[ --i ].innerText = '';
      
      for( var i  = len; i; ) {
        var error = errors[ --i ]
          , field_container = form[ error.field_name ].parentNode
          , span = field_container.getElementsByTagName( 'span' )[ 0 ] || document.createElement( 'span' )
        ;
        
        span.style.display = 'block';
        span.innerText = error.message;
        
        field_container.appendChild( span );
      }
