...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     Form_Process_Fields()
  */
  function Form_Process_Fields( dom_node, form_fields, options ) {
  ...
  Ordered.subclass( Form_Process_Fields, {
    ...
    event_handler: function() {
      ...
      for( var i = -1; ++i < len; ) {
        ...
        switch( element.type ) {
          case 'checkbox' :
            ...
            if( element.checked ) object[ element.name ].push( to_value( element.value ) );
          break;
          
          case 'radio' :
            if( element.checked ) object[ element.name ] = to_value( element.value );
          break;
          
          default:
            object[ element.name ] = to_value( element.value );
        }
      }
      ...
      if( elements[ 'uuid' ].value === '' ) {
        ...
        this.emit_add( [ object ] );
      } else {
        this.emit_update( [ [ this.form_value, object ] ] );
      }
  ...
  */
  function Form_Process( dom_node, model, form_fields, options ) {
    ...
    if( form_fields instanceof Array ) {
      ...
      this.fields = new Form_Process_Fields( dom_node, form_fields, options );
    }
