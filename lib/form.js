...
( function( exports ) {
  ...
  */
  ...
  Pipelet.build( 'form_display_errors', Form_Display_Errors, {
    ...
    update: function( updates ) {
      de&&ug( "Form_Display_Errors..update(), updates: " + log.s( updates ) );
      return this;
    } // update()
  ...
  */
  ...
  Pipelet.build( 'form_validate', Form_Validate, {
    transform: function( values, options, caller ) {
      ...
      if( fields instanceof Array ) {
        ...
        fields.fetch_all(
          function( fields ) {
            errors = that.validate( fields, values[ 0 ] );
      ...
      function _emit() {
        switch( caller ) {
          case 'add'    : that.emit_add   ( errors.length ? errors : values ); break;
          case 'remove' : that.emit_remove( errors.length ? errors : values ); break;
        }
      } // _emit()
    }, // transform()
    
    // process and validate form values; return errors
    validate: function( fields, object ) {
      ...
      for( var i = -1; ++i < len; ) {
        ...
        if( ! r.valide ) errors.push( { field_name: name, message: r.message } );
      }
  ...
  function validate_text( value, field ) {
    var result = { valide: true, message: 'Valide' };
    
    if( field.mandatory && value === '' ) {
      result = { valide: false, message: field.label + ' is not valid' };
    }
  ...
  function validate_email( value, field ) {
    ...
    if( field.mandatory ) {
      if( value === '' ) {
        result = { valide: false, message: 'Email is mandatory' };
      } else if( ! regex.test( value ) ) {
        result = { valide: false, message: 'Email is not valid' };
      }
  ...
  function validate_phone( value, field ) {
    ...
    if( field.mandatory ) {
      if( value === '' ) {
        result = { valide: false, message: 'Phone Number is mandatory' };
      } else if( ! regex.test( value ) ) {
        result = { valide: false, message: 'Phone Number is not valid' };
      }
    } else if( value !== '' && ! regex.test( value ) ) {
      result = { valide: false, message: 'Phone Number is not valid' };
    }
  ...
  function validate_radio( value, field ) {
    var result = { valide: true, message: '' };
    
    if( field.mandatory && ( ! value || value === '' ) ) {
      result = { valide: false, message: field.label + ' is mandatory' };
    }
