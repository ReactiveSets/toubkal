...
( function( exports ) {
  ...
  */
  ...
  function Filter( filter, options ) {
    ...
    this._lazy = true; // ToDo: try to find a better way for detecting the _lazy state, this should work once query AND is fully implemented
    ...
    switch ( typeof filter ) {
      ...
      case "object":
        if ( filter instanceof Pipelet || filter instanceof Array ) {
          // ToDo: implement redirection from sources to destinations, check Union for that
  ...
  Pipelet.Build( 'filter', Filter, {
    ...
    __update_upstream_query: function( removes, adds ) {
      if ( this._filter ) return this; // Using filter function does not allow to _update upstream query, but _fetch() + __transform() filter input data
      /*
        ...
        Upstream query tree can be updated using Super.__update_upstream_query( removes, adds )
      ...
      */
      ...
      if ( destination_query.adds.length || destination_query.removes.length ) {
        ...
        destination_query.clear_operations(); // We do not yet know how to _update incrementally
        ...
        if ( differences[ 0 ].length || differences[ 1 ].length ) {
          ...
          Super.__update_upstream_query.apply( this, differences );
        }
      }
    ...
    _add_query_source: function( filter, options ) {
      ...
      p._add_source( filter );
      
      return this;
      
      function add( or_terms, options ) {
        de&&ug( '_add_query_source()#_add(), or_terms: ' + log.s( or_terms ) );
      ...
      function remove( or_terms, options ) {
        de&&ug( '_add_query_source()#_remove(), or_terms: ' + log.s( or_terms ) );
    ...
    _update_query: function( options ) {
      ...
      function query_tree_update_and_fetch_adds( transaction ) {
        ...
        source._query_tree_update( removes, adds, that );
