  ...
  */
  ...
  Pipelet.Build( 'filter', Filter, {
    fetch: function( receiver, query ) {
      if ( this._filter ) return Super.fetch.call( this, receiver, query );
      
      // Filter is using a Query
      var q = this._query;
      
      if ( query ) q = new Query( q.query ).and( query ).generate(); // ToDo: add test
      
      var filter = q.filter;
      
      return this._fetch_source( filter_rx_data, q.query );
      
      function filter_rx_data( values, no_more ) {
        receiver( values && values.length ? filter( values ) : [], no_more );
      } // filter_rx_data()
    }, // fetch()
    
    _update_upstream_query: function( adds, removes ) {
      // ToDo: implement AND query from downstream
      // Need 3 queries: input, control, and output! We'll optimize this later this week
    ...
    _add_query_source: function( filter, options ) {
      ...
      if ( ! p ) {
        ...
        q = this._query = new Query( [] ).generate(); // nul filter, ToDo: this should become the controller query
