  ...
  */
  function Filter( filter, options ) {
    ...
    switch ( typeof filter ) {
      case "function":
        this._filter = parse_filter( filter );
  ...
  Pipelet.Build( 'filter', Filter, function( Super ) { return {
    _fetch: function( receiver, query ) {
      if ( this._filter ) return Super._fetch.call( this, receiver, query ); // filter locally using this.__transform()
      ...
      return this.__fetch_source( receiver, q.query );
    }, // _fetch()
    
    __update_upstream_query: function( removes, adds ) {
      if ( this._filter ) {
        // ToDo: Using filter a function, _update upstream query directly with downstream changes
        // will then be filtered by __transform()
        
        // return Super.__update_upstream_query.call( this, removes, adds );
        return this;
      }
      
      // var name = de && this._get_name( '__update_upstream_query' );
      
      if ( adds.length ) {
        var filter_keys = this._options.filter_keys;
        
        if ( filter_keys ) {
          // update filter query using filter keys
          // Example of filter keys:
          //   [ 'flow' ]
          var query_controller = this._query_controller, filter_adds = [];
          ...
          query_controller.__update_upstream_query( [], filter_adds );
          ...
          query_controller._query.discard_optimized();
        }
      }
      ...
      if ( downstream_query.adds.length || downstream_query.removes.length ) {
        ...
        Super.__update_upstream_query.apply( this, this._get_upstream_query_changes() );
    ...
    _add_query_source: function( filter, options ) {
      ...
      if ( ! query_controller ) {
        ...
        if ( filter_keys ) {
          ...
          // Do not request any filter for now, wait for __update_upstream_query()
          q = null;
        } else {
        ...
        query_controller._query = q;
        
        this._query            = new Query( [] ).generate(); // nul filter
    ...
    _update_query: function( options ) {
      ...
      if ( filter_query.adds.length || filter_query.removes.length ) {
        ...
        if ( source && removes.length ) {
          ...
          source._transactional_fetch( this, removes, o, query_tree_update_and_fetch_adds );
        } else {
      ...
      function query_tree_update_and_fetch_adds( transaction ) {
        // ToDo: updating upstream queries through source._query_tree_update() must be synchronized with source._fetch()
        
        // Update upstream query trees and queries
        Super.__update_upstream_query.apply( that, changes );
        
        if ( source && adds.length ) {
          o.transaction = transaction;
          
          source._transactional_fetch( that, adds, o );
        }
      } // query_tree_update_and_fetch_adds()
    }, // _update_query()
      ...
      switch( typeof filter ) {
        ...
        case 'function':
          this._update = function( updates ) {
            ...
            return this.__emit_operations( added, removed, updated, XS.only_more( options ) );
          };
