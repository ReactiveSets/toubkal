  ...
  */
  ...
  Pipelet.Build( 'filter', Filter, function( Super ) { return {
    _fetch: function( receiver, query ) {
      if ( this.pipelet._filter ) return Output.prototype._fetch.call( this, receiver, query ); // filter locally using this.__transform()
      
      // Filter is using a Query in upstream pipelet
      var q = this.pipelet._query;
    ...
    _update_query: function( options ) {
      ...
      if ( filter_query.adds.length || filter_query.removes.length ) {
        ...
        if ( source && removes.length ) {
          ...
          source._output._transactional_fetch( this, removes, o, query_tree_update_and_fetch_adds );
        } else {
      ...
      function query_tree_update_and_fetch_adds( transaction ) {
        // ToDo: updating upstream queries through source._query_update() must be synchronized with source._fetch()
        ...
        if ( source && adds.length ) {
          ...
          source._output._transactional_fetch( that, adds, o );
        }
      } // query_tree_update_and_fetch_adds()
    }, // _update_query()
