...
( function( exports ) {
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Parameters:
       - filter: can be a Function, an Array or a Pipelet:
           ...
           (Array of Objects): the filter is a static query.
           
           (Pipelet): the filter is a query dataflow.
  ...
  function Filter( filter, options ) {
    ...
    switch ( typeof filter ) {
      ...
      case "object":
        ...
        // The query will be used by the upstream pipelet to filter dataflows
        // therefore this pipelet becomes a pass-through pipelet, therefore
        // transform should not attempt to filter anything
        this.transform = function( objects ) { return objects };
        
        if ( filter instanceof Pipelet ) {
          // This is a query dataflow
          
          this._delayed_constructor = true;
          
          filter.fetch_all( function( filter ) {
            that._query = new Query( filter );
            
            that._delayed_constructor = false;
            
            if ( that._delayed_subscribe ) {
              that._delayed_subscribe = false;
              
              that.source.subscribe( that._query, that );
            }
          } );
          
          // ToDo: refactor to use on( 'add' ), on( 'remove' ) ..
          filter.on( 'change',function( operation, values, options ) {
            var diff = [];
            
            switch ( operation ) {
              case 'add':
                that._query.or( values, diff );
                
                that.emit_add( that.source.subscribe( diff, that ) );
              break;
              
              case 'remove':
                that._query.and_not( values, diff );
                
                // ToDo: fetch upstream data to remove
              break;
          ...
          this._query = new Query( filter );
          
          break;
        }
      // pass-though 
      default:
        throw new error( "filter(), filter should be a function or an Query Array" );
    }
  ...
  */
  XS.Compose( 'flow', function( source, flow_name, options ) {
    // ToDo replace by static query:
    // return source.filter( [ { flow: flow_name } ] );
    
    var filter = new Code()
      ._function( 'filter', '', [ 'v' ] )
        .add( "return '" + flow_name + "' == v.flow" )
      .end( 'Filter for flow ' + flow_name )
    ;
