     ...
     Parameters:
       - filter: can be a Function, an Array or a Pipelet:
           ...
           (Pipelet): the filter is a query dataflow. This allows dynamic filters from dataflows.
             states = set( [ { country: 'USA', state 'Michigan' } ] );
  ...
  function Filter( filter, options ) {
    ...
    function update_query_filter( options ) {
      filter.fetch_all( function( filter ) {
        var old_transform = that.transform
        ...
        that._fetch_source( function( values, no_more ) {
          ...
          if ( no_more ) {
            ...
            var al = added.length && 1, rl = removed.length && 1, l = al + rl;
    ...
    function transform_from_query( query ) {
      var t = that.transform = new Query( query ).generate().filter;
      
      // ToDo: The following code is required for update, which might be removedin the future
