...
( function( exports ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
     Pipelet.methods( methods )
     
     Usage:
       Pipelet.methods( function( Super, Class ) {
         // Private class variable commun to all methods
         
         return {
           // A simple methods without traces
           __transform: function( values ) {
             return return values;
           },
           
           // A method with private locals that can be set by a 
           _transaction: with_privates( function() {
             var d;
             
             function f( count, options, f ) {
               var transactions = this._transactions;
               
               d&&d( { count: count, options: options } );
               
               // .. more code 
             }
             
             f.set_private = function( p ) { d = p.debug };
  ...
  */
  function With_Privates() {}
  
  function with_privates( fn ) {
    return subclass( With_Privates, fn );
  }
  
  Pipelet.methods = function( methods ) {
    if ( typeof methods == 'function' ) {
      methods = methods( this._super, this._class );
    }
    ...
    Object.keys( methods ).forEach( function( method_name ) {
      ...
      if ( method instanceof With_Privates ) {
        var set_private = method.set_private;
        
        if ( typeof set_private == 'function' ) {
          that.set_privates[ method_name ] = set_private;
          
          // Set initial debug value for this method
          de && set_private( { debug: function( o ) {
            log( class_name, instance_name, method_name, o );
          } } );
        }
