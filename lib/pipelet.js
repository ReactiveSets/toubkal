...
( function( exports ) {
  ...
  extend( p, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       ...
       Parameters:
         source: (Pipelet) the source pipelet or any other object to connect to.
    ...
    _add_source: function( p ) {
      ...
      if ( s ) {
        if ( s instanceof Union ) {
          ...
          this.source = new Union( this, { _implicit: true }, s, p );
        }
      } else {
        this.source = p;
      }
    ...
    if ( pipelet === u ) {
      pipelet = ( function() {
        ...
        return function pipelet() {
          ...
          return new constructor_apply( a ).add_source( this );
        } // pipelet()
      } )();
    }
  ...
  */
  ...
  Pipelet.add( 'fork', function( options ) {
    var fork = new Fork( this, options );
    
    for( var i = 0; ++i < arguments.length; ) {
      arguments[ i ].add_source( fork );
    }
    
    return fork.add_source( this );
  } ) // Pipelet..fork()
  
  Pipelet.subclass( Fork, {
    /* ------------------------------------------------------------------------
       emit_add( added [, options ] )
