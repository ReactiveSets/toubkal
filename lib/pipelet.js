...
( function( exports ) {
  ...
  extend( p, {
    ...
    */
    emit: function( event_name, values, options ) {
      var u, events = this.events, listeners, l, a = slice.call( arguments, 1 ), name;
      
      de&&( name = 'emit( "' + event_name + '" )' + ( this.options.name ? ', name: ' + this.options.name : '' ) );
      
      switch( event_name ) {
        case 'clear':
          a = [ options = values ]; values = u;
        // fall-through
  ...
  */
  ...
  Pipelet.subclass( Query_Tree, {
    ...
    route: function( node, operation, values, options ) {
      var top = this.top;
      ...
      function emit( node, operation, options ) {
        var keys = node.keys, key
        ...
        /* ToDo: handle no-( more option ):
             If no-more is specified and we have aleady sent to one recepiant
             something with more, then this recipient needs to be notified of
             no-more. Problem: what if there are mulitple transactions going on
             from multiple sources? We might have to add transaction ids with
             more / no-more.. Transaction id could be the model name if only
             one transaction is authorized by model at a time. Transaction id
             could also be optional, using model by default, using none if no
             model, or could be added dynamically when needed, such as here.
        */
        if ( l = recipients.length ) {
          var recipients_values = node.recipients_values;
          
          for ( i = -1; ++i < l; ) {
            recipients[ i ].emit( operation, recipients_values, options );
          }
        ...
        for ( i = -1; key = keys[ ++i ]; ) {
          ...
          for ( var child in branch ) emit( child, operation, options )
        }
      } // emit()
    } // route()
