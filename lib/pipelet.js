...
( function( exports ) {
  ...
  */
  function get_transaction( transactions, count, options, pipelet ) {
    var t, tid;
    
    if ( options ) {
      if ( t = options.__t ) {
        // this is a nested transaction within this pipelet
        t.count += count; // add count operations to this transaction at this pipelet
        
        de&&ug( 'get_transaction(), adding ' + count + ' operations to inner nested transaction, new total:' + t.count );
        
        return t;
      }
      
      if ( ( tid = options.transaction_id ) && ( t = transactions[ tid ] ) ) {
        // this is a nested transaction from an upstream pipelet for which we have already a local transaction
        // ToDo: add xs_core test
        t.count += count; // add count operations to this transaction at this pipelet
        
        de&&ug( 'get_transaction(), adding ' + count + ' operations to upstream nested transaction, new total:' + t.count );
        
        return t;
      }
    }
    
    return new Transaction( transactions, count, options, pipelet, tid );
  } // get_transaction()
  
  function Transaction( transactions, count, options, pipelet, tid ) {
    var o;
    ...
    if ( tid ) {
      ...
      if ( options.more || count > 1 ) {
        ...
        transactions[ tid ] = this; // Store upstream transaction at this pipelet
        
        o = extend_2( {}, options );
        
        de&&ug( 'new Transaction(): count: ' + count + ', transaction from upstream pipelet, options: ' + log.s( o ) );
      } else {
        // This is already the end of the upstream transaction, and we have only one operation to emit
        // ToDo: add xs_core test
        // There is therefore no need for a complete transaction
        de&&ug( 'new Transaction(), end of upstream transaction, tid: ' + tid );
        
        transactions = null;
        
        o = options; // no need to make a copy, will be used with no change
      }
    } else if ( count < 2 ) {
      // There is no upstream transaction, and only one operation to emit,
      // There is therefore no need for a complete transaction with tid
      de&&ug( 'new Transaction(), only one operation' );
      ...
      de&&ug( 'new Transaction(): count: ' + count + ', new transaction, options: ' + log.s( o ) );
    }
    ...
    this.name           = pipelet ? ', name: ' + pipelet.get_name() : '';
  ...
  extend_2( Transaction.prototype, {
    ...
    get_emit_options: function() {
      if ( this.closed )
        throw new Error ( 'Transaction..get_emit_options(): this transaction is already closed' );
      
      var tid
        , more = this.count > 0 || this.removed.length > 0 || this.added.length > 0
        ...
        if ( more ) {
          ...
          if ( options ) {
            ...
            if ( options.transaction_id === tid ) {
              ...
              de&&ug( 'Transaction..get_emit_options()' + this.name + ', new tid: ' + tid );
            }
          } else {
            this.emit_options = options = { more: this.need_close = true, transaction_id: tid = uuid_v4() };
            
            de&&ug( 'Transaction..get_emit_options()' + this.name + ', new options: ' + tid );
          }
        } else {
          ...
          de&&ug( 'Transaction..get_emit_options()' + this.name + ', use source options: ' + log.s( options ) );
        }
      }
    ...
    next: function() {
      ...
      throw new Error( 'Transaction count negative: ' + this.count + this.name );
    }, // Transaction..next()
    ...
    terminate: function() {
      ...
      if ( transactions ) {
        var tid = this.emit_options.transaction_id;
    ...
    */
      ...
      if ( this.count || this.state == 1 ) return this;
      ...
      if ( removed.length ) {
        de&&ug( 'Transaction..end()' + this.name + ', emit removed' );
      ...
      if ( added.length || this.need_close ) {
        de&&ug( 'Transaction..end()' + this.name + ', emit added' );
  ...
  Transaction.prototype.emit_nothing = Transaction.prototype.next;
      ...
      switch( operation ) {
        case 'update':
          ...
          for ( i = -1, rl = recipients.length; ++i < rl; ) {
            ...
            var t = get_transaction( {}, operations, options );
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    _transaction: function( count, options, f ) {
      ...
      var t = get_transaction( this._transactions, count, options, this );
    ...
    notify: function( transaction, options ) {
      return this._transaction( transaction.length, options, function( t, l ) {
        for ( var i = -1, a; ++i < l; ) {
          ...
          this[ a.action ]( a.objects, t.next().get_options() );
        }
      } );
    }, // notify()
    ...
    */
    update: function( _updates, options ) {
      ...
      return this._transaction( _l ? 2: 0, options, function( t ) {
        ...
        this.remove( [ update[ 0 ] ], options );
        this.add   ( [ update[ 1 ] ], t.next().get_options() );
      } );
    }, // update()
