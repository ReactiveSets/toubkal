  ...
  */
  ...
  Pipelet.subclass( Query_Tree, {
    ...
    add: function( or_terms, options ) {
      var top = this.top
        , recipient = options.recipient // All OR-terms MUST have one recipient and share this recipient
      ...
      function add_term( node, term, term_keys ) {
        while( term_keys.length ) {
          ...
          for( var i = -1; key = keys[ ++i ]; ) {
            ...
            // Add value to this node, then keep adding remaining keys for this term
            branch = branches[ key ];
          ...
          if ( v === u ) {
            ...
            // Retreive the first key from term keys, add it to this node keys
          ...
          // Create a new branch for this value
