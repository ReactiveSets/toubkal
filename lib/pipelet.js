...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     This class provides an arithmetic on queries. It is used by fetch to retrieve a subset of
     values from a source dataflow.
     
     A query is a Set representing an 'OR-AND' expression, where each record is an operand of
     the OR expression and each attribute of each record is an operand of the sub-AND
     expression.
     ...
     Example: The set
       ...
       Will be interpreted as the query: values where
           ( model === 'users' AND  id === 1092 )
         OR
           ( model === 'stores' AND id === 231 )
     
       Which is also equivalent to a where SQL clause:
     ...
     generate() generating the filter() method that filters an Array of values.
     
     A query object can be "ORed" with an other query object to result in the ORed combination
     of both queries. This is useful to merge a number of branches and build an optimized query
     for all these branches.
     
     A query object can be "ANDed" with an other query object to result in the ANDed combination
     of both queries. This is useful to restrict one query by another one when two filters are
     connected one downstream of the other. A use case if for combining an authorization query
     with a user query requesting a subset of what this user is authorized to request.
  ...
  extend( Query.prototype, {
    ...
    */
    or: function( q1 ) {
      ...
      for ( var i1 = -1; i1 < l1; ) {
        var e1 = q1[ i1 ]
          , keys1 = Object.keys( e1 ), kl1 = keys1.length
          , added = false
        ;
        
        for ( var i0 = -1; i0 < l0; ) {
          var e0 = q0[ i0 ]
            , keys0 = Object.keys( e0 ), kl0 = keys0.length
            , key, kl, e, p
          ;
        ...
        // Add e1 into result query if was not added already
    ...
    */
    and: function( q1 ) {
      var q0 = this.query, l0 = q0.length, l1 = q1.length;
