...
( function( exports ) {
  ...
  */
  ...
  extend( p, {
    /* ------------------------------------------------------------------------
       ...
       Parameter:
         receiver: function that will be called for each chunk of data.
           
           Each chunk is delivered in a array of values.
    ...
    */
    fetch: function( receiver ) {
      ...
      return this._fetch_source( function( values ) {
        if ( values.length ) {
          values = that.transform( values );
        
          values.length && receiver( values );
        } else {
          receiver( [] );
        }
      } );
    ...
    /* ------------------------------------------------------------------------
       ...
       Parameter:
         receiver: function that will be called for each chunk of data from
           ...
           Each chunk is delivered as a array of values.
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       ...
       Parameter:
         receiver: optional function that will be called once all chunks are
           fetched from the source as an array of objects.
           
           The function must be provided if the source is not in the same
    ...
    */
    fetch_all: function( receiver ) {
      ...
      if ( this.fetch === Pipelet.prototype.fetch ) {
        ...
        this._fetch_source_all( function( values ) {
        ...
        this.fetch( function( values ) {
          if ( out ) throw new Error( "Pipelet..fetch_all(): received extra chunck after last" );
    ...
    */
    update: function( updated, options ) {
      ...
      previous = this.transform( previous, options );
      modified = this.transform( modified, options );
      
      if ( previous.length === l && modified.length === l ) {
        var out = [];
        ...
        this.emit_update( out, options );
      } else {
        this.emit_remove( previous, options );
        this.emit_add   ( modified, options );
      }
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       ...
       Parameters:
         ...
         - options: (Object):
           - _source: the actual source pipelet to set. If provided,
                      _add_source() is called directly, and no fetch() or
                      _add_destination() is performed. This option is used
                      by _add_destination() to prevent an infinite loop while
                      creating a fork().
    */
    add_source: function( source, options ) {
      if ( options ) {
        var s = options._source;
        
        if ( s ) return this._add_source( s );
      }
    ...
    _add_source: function( p ) {
      ...
      if ( s ) {
        if ( s instanceof Union ) {
          ...
          this.source = this.union( [ s, p ], { _destination: this } );
        }
      } else {
        this.source = p;
      }
    ...
    _add_destination: function( p ) {
      ...
      if ( d ) {
        if ( d instanceof Fork ) {
          ...
          this.destination = this.fork( [ d, p ], { _source: this } );
        }
      } else {
        this.destination = p;
      }
    ...
    */
    remove_source: function( s ) {
      if ( s ) {
        ...
        this.clear(); // ToDo: this does not work if there are multiple sources from a Union
      }
  ...
  */
  Pipelet.set_default_options = function( source, parameters, f ) {
    ...
    if ( pipelet === u ) {
      pipelet = ( function() {
        ...
        return function pipelet() {
          ...
          a = Pipelet.set_default_options( this, a, constructor );
  ...
  */
  function Compose( name, composition, context ) {
    return Pipelet.add( name, function() {
      var a = Pipelet.set_default_options( this, arguments, composition );
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Parameters:
       ...
       - options      : (Object):
         - _source    : (Pipelet) the true source of destinations which should reference the
                        source of the fork itself. If not present, the source of destination
                        pipelets is this one. This option is used by Pipelet._add_destination()
                        when creating a fork and should not be used or overloaded by other
                        pipelets.
  */
  function Fork( destinations, options ) {
    var s = Pipelet.call( this, options ).options._source;
  ...
  */
  ...
  Pipelet.add( 'union', function( sources, options ) {
    if ( sources ) {
