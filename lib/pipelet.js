...
( function( exports ) {
  ...
  */
  Pipelet.set_default_options = function( f, source, parameters ) {
    ...
    var options = { key: source.key || [ 'id' ] } // Default options
      , l = parameters.length
      , u
    ;
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     ! Important Limitation: This will work only on sets with no remove or update operation.
       A remove will not be able to find the original auto-incremented.
     
     ! Important warning: add() operations should be sent to the source pipelet connected to
       auto_increment() and not to auto_increment(), otherwise there would be no
       auto-incrementation. This is because of a limitation of Compose that returns the last
       pipelet in the composition.
  ...
  Compose( 'auto_increment', function( source, options ) {
    ...
    return source
      ...
      .set() // prevents re-incrementation through fetch()
    ;
  } ); // auto_increment()
