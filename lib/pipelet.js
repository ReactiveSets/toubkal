...
( function( exports ) {
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    _get_name: function( method ) {
      return this.options.name + ( method ? '.' + method + '(), ' : '' );
    }, // _get_name()
    ...
    */
    _transactional_fetch: function( receiver_pipelet, query, options, next ) {
      ...
      function receiver( values, no_more ) {
        if ( ended ) {
          this._error( '_transactional_fetch', 'already ended, no_more received twice from fetch()' );
        }
    ...
    */
    fetch_all: function( receiver, query ) {
      ...
      if ( out === u && receiver === u ) this._error( 'fetch_all', 'is asynchronous and no receiver function was provided' );
      ...
      function rx_concat( values, no_more ) {
        if ( out ) that._error( 'fetch_all', 'received extra chunck after no_more' );
    ...
    */
    add_source: function( source ) {
      ...
      this
        ._add_source            ( source )
        ._add_source_destination( source ) // ToDo: this should only be adding the destination, not updating the query tree
      ;
    ...
    */
    _remove_source: function( p ) {
      if ( this._source !== p ) this._error( 'remove_source', 'this._source is not ' + p._get_name() );
    ...
    /* ------------------------------------------------------------------------
       ...
       Derived pipelets can use this method to redirect source's destination
       to another pipelet as done by Controller.
    */
    ...
    */
    _add_destination: function( destination, query ) {
      ...
      if ( i != -1 ) this._error( '_add_destination', 'already added, destination: ' + destination._get_name() );
    ...
    */
    _remove_destination: function( destination, query ) {
      ...
      if ( i == -1 ) this._error( '_remove_destination', 'not found, destination: ' + destination._get_name() );
    ...
    /* ------------------------------------------------------------------------
       ...
       pipelet than this as done by Controller pipelets.
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     The first implemented controller is Union, the next candidates are Filter and Pass_Through.
  */
