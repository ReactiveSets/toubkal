...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     Transaction( transactions, count, options )
  */
  function Transaction( transactions, count, options ) {
    var tid = options && options.transaction_id;
    
    if ( tid ) {
      // there is an existing transaction
      var t = transactions[ tid ];
      
      if ( t ) {
        // this transaction is already created at this pipelet (i.e. present in transactions)
        
        t.count += count; // add count operations to this transaction at this pipelet
        
        de&&ug( 'new Transaction(), adding ' + count + ' operations, new total:' + t.count );
        
        return t;
      }
      
      // This transaction is not already created at this point
      if ( ! options.more && count < 2 ) {
        // This is already the end of the upstream transaction, and we have only one operation to submit
        // There is therefore no need to create a new transaction
        
        de&&ug( 'new Transaction(), end of upstream transaction ' + tid );
        
        transactions = null;
        
        return init_transaction( this );
      }
      
      // Need to create a new transaction with this tid
    } else {//if ( count > 0 ) {
      // There is no upstream transaction
      // There is more than one item, therefore we need to create a new transaction
      tid = uuid_v4();
      
    } /*else {
      // There is no upstream transaction, and only one operation to emit,
      // There is therefore no need for a transaction
      transactions = null;
      
      return init_transaction( this );
    }*/
    
    // Create a transaction at this point with tid, for count operations
    options = options ? extend_2( {}, options ) : {};
    options.more = true;
    options.transaction_id = tid;
    
    init_transaction( this );
    
    de&&ug( 'new Transaction(): count: ' + count + ', options: ' + log.s( options ) );
    
    // Store new transaction at this pipelet
    return transactions[ tid ] = this;
    
    function init_transaction( that ) {
  ...
  extend_2( Transaction.prototype, {
    ...
    next_options: function() {
      var count = --this.count, options = this.o;
      
      if ( count > 0 ) return options;
      
      if ( count < 0 )
        throw new Error( 'Transaction count negative: ' + count + ', options: ' + log.s( options ) );
      
      // count == 0
      
      if ( this.transactions ) {
        // There was an ongoing transaction
        options.more = false;
      }
    ...
    terminate: function() {
      ...
      if ( transactions ) {
        var id = this.o.transaction_id;
        
        de&&ug( 'Transaction..terminate(), id: ' + id );
    ...
    // should be called after next_options()
    packetize_add: function( values ) {
      var added = this.added;
      
      de&&ug( 'Transaction..packetize_add(), values: ' + values.length );
      
      push.apply( added, values );
      
      return this;
      //return added;
    }, // packetize_add()
    
    get_add_packet: function() {
      // should be called after next_options() that will decrement count to zero when the transaction is done
      ...
      de&&ug( 'Transaction..get_add_packet(), values: ' + added.length );
      
      return added;
    }, // get_add_packet()
    
    // should be called after next_options()
    packetize_remove: function( values ) {
      var removed = this.removed;
      
      de&&ug( 'Transaction..packetize_remove(), values: ' + values.length );
      
      push.apply( removed, values );
      
      return this;
    }, // packetize_remove()
    
    get_remove_packet: function() {
      // should be called after next_options() that will decrement count to zero when the transaction is done
      if ( this.count ) return [];
      ...
      de&&ug( 'Transaction..get_remove_packet(), values: ' + removed.length );
      
      return removed;
    } // get_remove_packet()
  } ); // Transaction instance methods
  
  XS.Transaction = Transaction;
  ...
  */
  function only_more( options ) {
    return options && ( options.more || options.transaction_id )
      ? { more: !!options.more, transaction_id: options.transaction_id || uuid_v4() }
      : { more: false }
    ;
  } // only_more()
  ...
  */
  ...
  function Query_Tree_Node() {
    ...
    // all recipient pipelets
    this.recipients = [];
    
    // all recipients values as they are accumulated
    this.recipients_values = [];
  ...
  var Query_Tree_Prototype = {
    init_query_tree: function() {
      ...
      this.source_recipient_index = 0; // incomming recipient index in current operation
    ...
    query_tree_emit: function( operation, values, options ) {
      ...
      //  p: recipient pipelet
      //  v: values to emit
      var recipients = [], i, p, v, rl;
      
      switch( operation ) {
        case 'update':
          for ( i = -1; v = values[ ++i ]; ) {
            route_update( this, more && transaction, transaction_id, top, recipients, v[ 0 ], i, 0 ); // route remove
            route_update( this, more && transaction, transaction_id, top, recipients, v[ 1 ], i, 1 ); // route add
          }
          
          more || this._end_transaction( transaction, transaction_id, options );
          
          // emit operations to all recipients
          for ( i = -1, rl = recipients.length; ++i < rl; ) {
            var r = recipients[ i ], p = r.p, added = [], removed = [], updated = [];
            ...
            removed.length && p.remove( removed, t.next_options() );
            added  .length && p.add   ( added  , t.next_options() );
            updated.length && p.update( updated, t.next_options() );
          }
        return this;
        
        case 'clear':
          route_clear( top, recipients );
          
          more || this._end_transaction( transaction, transaction_id, options );
          
          // emit clear to all recipients
          for ( i = -1, rl = recipients.length; ++i < rl; ) {
            p = recipients[ i ];
      ...
      //   source_recipient_index used to prevent sending duplicate values to each pipelets.
      ...
      for ( i = -1; v = values[ ++i ]; ) {
        route_value( this, more && transaction, transaction_id, top, recipients, v, [ 0 ] );
      }
      
      more || this._end_transaction( transaction, transaction_id, options );
      
      // Emit accumulated values in recipients[] from previous loop
      recipients_emit( recipients );
      
      return this;
      
      // Emit accumulated values to recipients
      function recipients_emit( recipients ) {
        var i, l, r, p;
        
        for ( i = -1, l = recipients.length; ++i < l; ) {
          r = recipients[ i ], p = r.p;
          
          p.source_recipient_index = 0; // we are done sending data for this operation to this pipelet
      ...
      function route_value( that, transaction, transaction_id, node, recipients, value, pushed ) {
        ...
        for( i = -1; key = keys[ ++i ]; ) {
          if ( ( v = value[ key ] ) !== u && ( child = branches[ key ][ v ] ) ) {
            ...
            route_value( that, transaction, transaction_id, child, recipients, value, pushed );
          }
      ...
      function route_update( that, transaction, transaction_id, node, recipients, value, position, offset ) {
        var keys            = node.keys, key
          , branches        = node.branches
          , node_recipients = node.recipients
        ...
        // Push value into this node's recipients at position and offset
        // Except if this value was already pushed to some recipients, preventing duplicates
        // This also guaranties that operations are sent in the same order received although this is not a requirement
        // Duplicates may happen on queries with an intersection
        for( i = -1; ++i < rl; ) {
          ...
          if ( recipient_index ) {
            ...
            v = recipients[ recipient_index - 1 ].v;
          } else {
            // This pipelet does not have any update yet
            recipients.push( { p: p, v: v = [] } );
            
            p.source_recipient_index = recipients.length; // source recipient indexes start at 1
        ...
        for( i = -1; key = keys[ ++i ]; ) {
          if ( ( v = value[ key ] ) !== u && ( child = branches[ key ][ v ] ) ) {
            // We have a value for this term and this value has a child node, descend
            route_update( that, transaction, transaction_id, child, recipients, value, position, offset );
          }
          // Keep looking for other possible matches
        } // for all term keys
      } // route_update()
      
      function route_clear( node, recipients ) {
        ...
        for( i = -1; key = keys[ ++i ]; ) {
          ...
          for ( var value in branch ) route_clear( branch[ value ], recipients );
        }
      } // route_clear()
    }, // query_tree_emit()
    ...
    _end_transaction: function( transaction, transaction_id, options ) {
      ...
      if ( l ) {
        ...
        // as marked by a non-zero source_recipient_index
        for ( var i = -1, p; ++i < l; ) {
          p = transaction[ i ];
          ...
          if ( p.source_recipient_index ) continue; // the transaction will terminate in the recipients emit
  ...
  */
  ...
  function Pipelet( options ) {
    ...
    this._transactions = new Dictionary();
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       _transaction_start( count, options )
    */
    _transaction_start: function( count, options ) {
      if ( count == 0 ) {
        var more = options && options.more;
        
        de&&ug( '_transaction_start(), name: ' + this.options.name + ', count is zero, more: ' + more );
        
        if ( more ) return; // do not emit anything cause there is more to come
        
        // This could be the end of a transaction, must emit
        this.emit_add( [], options );
        
        return;
      }
      
      return new Transaction( this._transactions, count, options );
    }, // _transaction_start()
    
    /* ------------------------------------------------------------------------
       _transaction_end( count, options )
    */
    _transaction_end: function( t ) {
      var options = t.get_options();
      
      if ( options.more || ! t.done() || t.terminated() ) return this;
      
      var removed = t.get_remove_packet()
        , added   = t.get_add_packet()
        , rl = removed.length
        , al = added.length
      ;
      ...
      if ( rl ) {
        de&&ug( '_transaction_end(), name: ' + this.options.name + ', emit removes: ' + rl );
        
        if ( al ) options.more = true;
        
        this.emit_remove( removed, options );
        
        if ( al ) options.more = false;
      }
      
      if ( al || ! rl ) {
        de&&ug( '_transaction_end(), name: ' + this.options.name + ', emit adds: ' + al );
    ...
    _transaction_emit_add: function( transaction, values, emit_now ) {
      var options = transaction.next_options();
      
      if ( values.length ) {
        if ( emit_now ) {
          de&&ug( '_transaction_emit_add(), name: ' + this.options.name + ', values: ' + values.length + ', options: ' + log.s( options ) );
          
          return this.emit_add( values, options );
        }
        
        transaction.packetize_add( values );
      }
      
      return this;
    }, // _transaction_emit_add()
    
    _transaction_emit_remove: function( transaction, values, emit_now ) {
      var options = transaction.next_options();
      
      if ( values.length ) {
        if ( emit_now ) {
          de&&ug( '_transaction_emit_remove(), name: ' + this.options.name + ', values: ' + values.length + ', options: ' + log.s( options ) );
          
          return this.emit_remove( values, options );
        }
        
        transaction.packetize_remove( values );
      }
      
      return this;
    }, // _transaction_emit_remove()
    ...
    */
    notify: function( transaction, options ) {
      var l = transaction.length
        , t = this._transaction_start( l, options )
        , i, a
      ;
      ...
      for ( i = -1; ++i < l; ) {
        ...
        this[ a.action ]( a.objects, t.next_options() );
      }
      
      return this._transaction_end( t );
    }, // notify()
    ...
    */
    update: function( updates, options ) {
      ...
      de&&ug( 'update(), name: ' + this.options.name + ', updates: ' + l );
      
      if ( l ) {
        // ToDo: add test for nested transactions
        var t = this._transaction_start( 2, options )
          , i = -1, update
        ;
        ...
        this.remove( [ update[ 0 ] ], options );
        this.add   ( [ update[ 1 ] ], t.next_options() );
        
        this._transaction_end( t );
      }
    ...
    */
    emit: function( event_name, values, options ) {
      var name = this.options.name
        , more = this.more = options && options.more || false
        , tid  = options && options.transaction_id
      ;
      
      name = name ? ', name: ' + name : '';
      
      de&&ug(
          'emit( "' + event_name + '" )' + name
      ...
      try {
        ...
        exception( e, 'emit( "' + event_name + '" )' + name )
      }
      
      var events = this.events;
      
      emit_event( event_name, [ values, options ] );
      
      more || emit_event( 'complete', [ options ] );
      
      return this;
      
      function emit_event( event_name, a ) {
        var listeners, l;
        
        if ( ( listeners = events[ event_name ] ) && ( l = listeners.length ) ) {
          de&&ug( 'emit_event( ' + event_name + ' )' + name + ', listeners count: ' + l );
          ...
          try {
            ...
            exception( e, 'emit_event( ' + event_name + ', ' + log.s( a ) + ' )' );
          }
        }
      } // emit_event()
    }, // emit()
    ...
    */
    emit_operations: function( added, removed, updates, options ) {
      ...
      var l = operations.length
        , _more = options && options.more || false
      ;
      ...
      if ( l ) {
        ...
        if ( l > 1 ) {
          ...
          var options_more = more( options, l )
            , l1 = l -1
          ;
          
          for ( i = -1; ++i < l1; ) emit( options_more );
          
          options = no_more( options_more ); // get transaction id from options_more
        } else {
          i = 0
        }
    ...
    /* ------------------------------------------------------------------------
       ...
       This will listen to all operations emited by this pipelet regardless of
       queries to individual recipients. This means that a listener may have to
       perform any required filtering prior to interpreting the events.
       ...
       Parameters:
         - event_name: (String) the name of the event.
             ...
             'update', and 'clear' operations simultaneously.
    ...
    */
    ...
    // ToDo: deprecated on_change(), use on( 'change', listener, that )
    // on_change: function( listener, that ) {
    //   throw new error( "on_change() is deprecated, use on( 'change', listener, that ); comment out to keep using it" );
    // }, // on_change()
    ...
    */
    _subscribe: function( destination, query ) {
      var that = this
        , options_more = more() // ToDo: provide a count
      ;
      
      this.fetch( function( values, no_more ) {
         if ( no_more ) {
           destination.add( values || [], no_more( options ) );
           
           // This replaces _add_destination()
           that.query_tree_add( query, destination );
         } else if ( values && values.length ) {
           destination.add( values, options_more );
         }
      }, query );
       ...
       Parameters:
         ...
         - options: (Object):
           - lazy: (Bollean) if true, do not fetch() and add() values from
               ...
               to the first destination while creating a fork() for a second
               destination.
    ...
    add_source: function( source, options ) {
      ...
      if ( ! ( options && options.lazy ) ) {
        ...
        if ( source.fetch ) {
          ...
          // ToDo: if we keep this code, will need to handle more and transaction id as in _subscribe()
    ...
    _add_destination: function( destination ) {
      ...
      if ( d ) {
        if ( ! ( d instanceof Fork ) ) {
          ...
          this.destination = u;
          d = ( this.fork( u, { lazy: true } ) )._add_destination( d );
        }
  ...
  */
  Pipelet.set_default_options = function( f, source, parameters, defaults ) {
    ...
    var options = extend_2( { key: source.key || [ 'id' ] }, defaults ) // Default options
      , l = parameters.length
      , u
    ;
  ...
  */
  function Alter( transform, options ) {
    ...
    // Generate code for this.transform()
    var code = new Code()
  ...
  */
  ...
  Pipelet.build( 'set', Set, {
    ...
    */
    ...
    add: function( values, options ) {
      var l = values.length
        , transaction = this._transaction_start( l, options );
      ;
      ...
      while( ++i < l ) this._add_value( transaction, values[ i ] );
      
      return this._transaction_end( transaction );
    }, // add()
    ...
    _add_value: function( transaction, value, emit_now ) {
      if ( value ) {
        ...
        return this._transaction_emit_add( transaction, [ value ], emit_now );
      }
    ...
    _add_values: function( transaction, values, emit_now ) {
      ...
      return this._transaction_emit_add( transaction, added, emit_now );
    }, // _add_values()
    ...
    */
    ...
    remove: function( values, options ) {
      ...
      for ( var i = -1; ++i < l; ) {
        ...
        if ( p == -1 ) {
          ...
          this._remove_value( transaction, v );
        }
      }
    ...
    _remove_value: function( transaction, value, emit_now ) {
      ...
      return this._transaction_emit_remove( transaction, removed, emit_now );
    }, // _remove_value()
    ...
    */
    /*
    // ToDo: rewrite to work with _add_value() and _remove_value(), until then, use Pipelet..update()
    update: function( updates, options ) {
      ...
      if ( added.length ) {
        if ( updated.length ) {
          var options_more = more( options );
          
          this.emit_add   ( added  , options_more );
          this.emit_update( updated, no_more( options_more ) );
        } else {
          this.emit_add( added, options );
        }
      } else if ( updated.length ) {
        this.emit_update( updated, options );
      }
