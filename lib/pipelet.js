  ...
  */
  ...
  var Query_Tree_Prototype = {
    init_query_tree: function() {
      ...
      this.transactions           = create( null ); // ongoing transactions
      this.source_transactions    = create( null ); // incomming transactions by source
    ...
    new_query_tree_node: function() {
      return {
        branches         : create( null ), // hashed by terms' keys, each branch is the hashed by terms' values to lead to it's sub-node 
        keys             : [], // all the term keys for the above branches
        recipients       : [], // all recipients
        recipients_values: [], // all recipients values as they are accumulated
        transaction_ids  : create( null ) // Of transactions in progress (not completed)
      };
    }, // new_query_tree_node()
    ...
    query_tree_add: function( or_terms, recipient ) {
      ...
      function add_term( node, term, term_keys ) {
        while( term_keys.length ) {
          ...
          branch = branches[ key ] = create( null ); // allows to use 'in' without testing for hasOwnProperty()
    ...
    // ToDo: handle more and transaction_id
    query_tree_emit: function( operation, values, options ) {
      ...
      switch( operation ) {
        case 'update':
          ...
          !more && end_transaction( this, transaction, transaction_id, options );
          ...
          for ( i = -1, rl = recipients.length; ++i < rl; ) {
            ...
            removed.length && p.remove( removed, added.length || updated.length ? options_more : options );
        ...
        for( i = -1; ++i < rl; ) {
          ...
          if ( recipient_index ) {
            ...
            if ( transaction ) {
              ...
              add_destination_to_transaction( that, transaction, p, transaction_id );
            }
          }
        }
      ...
      function add_destination_to_transaction( that, transaction, p, transaction_id ) {  
        ...
        if ( source_transaction ) {
          // lookup source_transaction to find out if it has 'that' source
          for ( j = -1, sl = source_transaction.length; ++j < sl; ) {
            if ( source_transaction[ j ].source === that ) return; // already added
          }
          ...
          // There may be more that one source emiting data to the same desination during the same transaction if the
          // dataflow graph is not a strict tree but has loops or forks followed by unions.
          source_transaction = source_transactions[ transaction_id ] = [];
        }
        
        // Memorize the position at that source for this transaction to allow fast removal from that source
        source_transaction.push( { source: that, position: transaction.length } );
        
        // Add destination into the list of pipelets that have received data for this transaction and that source
        transaction.push( p );
      } // add_destination_to_transaction()
      
      function end_transaction( that, transaction, transaction_id, options ) {
        if ( ! transaction ) return; // this is not a transaction
        
        delete that.transactions[ transaction_id ]; // was the reference to transaction
        ...
        if ( l ) {
          ...
          for ( var i = -1, p; ++i < l; ) {
            ...
            for ( j = -1; ++j < sl; ) {
              if ( source_transaction[ j ].source === that ) {
            ...
            if ( p.source_recipient_index ) continue; // the transaction will terminate in the recipients loop bellow
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    emit: function( event_name, values, options ) {
      ...
      de&&ug( name = 'emit( "' + event_name + '" )' + ( this.options.name ? ', name: ' + this.options.name : '' ) );
    ...
    */
    _subscribe: function( destination, query ) {
      var that = this, options = more();
      
      this.fetch( function( values, no_more ) {
         if ( no_more ) {
           destination.add( values || [], { more: false, transaction_id: more.transaction_id } );
