  ...
  extend( Query.prototype, {
    ...
    /* -----------------------------------------------------------------------------------------
       ...
       Algorithm:
         ...
         2/ To perform pi AND pj, lookup properties of pi and pj, pick the one that has the
            ...
            - if one property of pa does not have the same value in pb, then the result is
              always false, and no term is produced
            - if all properties of pa have the same value in pb, then produce pb because pb
              is more restrictive than pa
    ...
    */
    and: function( q1 ) {
      ...
      // All variables with a trailing 0 correspond to the current query
      // All variables with a trailing 1 correspond to q1, the parameter query
      var q0 = this.query, result = new Query( [] ), i0 = -1, i1 = -1, p0, p1, keys0s = [], keys1s = [];
      ...
      for ( i0 = -1; p0 = q0[ ++i0 ]; ) {
        ...
        for ( i1 = -1; p1 = q1[ ++i1 ]; ) {
          // Perform p0 AND p1
          var k1 = keys1s[ i1 ], k1l = k1.length, ka, pb;
          
          // Let ka be the smallest key, let pb be the property with the largest key
          if ( k0 < k1 ) {
            ka = k0; pb = p1;
          } else {
            ka = k1; pb = p0;
