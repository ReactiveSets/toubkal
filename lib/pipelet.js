...
( function( exports ) {
  ...
  */
    ...
    get_transaction: function( count, options, pipelet ) {
      ...
      if ( options ) {
        ...
        if ( t = options._t ) {
          ...
          // ToDo: remove the following 2 lines once transaction object is implemented
      ...
      // Nothing more expected from upstream or count <= 1, no need to memorize this transaction in this.transactions
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Parameters:
     - count (Integer): the number of operations expected
     - options (optional Object): upstream pipelet options
     - pipelet (optional Pipelet): pipelet to emit operations to
     ...
     Diamond-graph issue:
       ...
       terminate these indenpendent transactions, a transaction tag is
       added before the diamond to all transactions entering the diamond.
       
       Transporting this tag militates for a transaction Object in options
       that would embed the transaction id, the more flag, diamond tags and
       possibly more transaction attributes to come, e.g. options: {
       ...
       If there are multiple diamonds nested in a graph, multiple diamond
       tags could be added, then removed by recombining unions.
       
       Example: a diamond with 3 branches tagged 'map'
       
         var fork = source.diamond( 'map' )
           , branch_1 = fork...
           , branch_2 = fork...
           , branch_3 = fork...
         ;
         
         xs.union( [ branch_1, branch_2, branch_3 ], { diamond: 'map' } );
       
       Union removes the 'map' diamond tag from transactions' tags added by
       diamond(). If a single operation has no transaction, diamond() makes
       a transaction with that single operation with the tag to allow union()
       to wait for all branches to complete that transaction.
  ...
  */
  function Transaction( count, options, pipelet ) {
    ...
    if ( ( options && options.more ) || ( t && t.more ) || count > 1 ) {
  ...
  Event_Emitter.subclass( Transaction, {
    ...
    set_source_options: function( options ) {
      // t / this.source_t: source transaction Object
      ...
      this.source_more = ( ( options && options.more ) || t.more ) || false;
      ...
      if ( this.need_close ) {
        ...
        if ( more ) {
          ...
          if ( options ) {
            ...
            t.more = options.more = this.need_close = true;
            
            if ( /* options.transaction_id === tid || */ t.id === tid ) {
              // transaction id is not set (undefined)
              tid = t.id = /* options.transaction_id = */ uuid_v4();
            ...
            this.emit_options = options = { more: true /*, transaction_id: tid */, _t: { id: tid, more: true } };
  ...
  function options_forward( options ) {
    ...
    if ( options ) {
      ...
      if ( t ) {
        ...
        // ToDo: remove once transactions are always transported in _t
        more = options.more;
        tid = options.transaction_id;
      }
      
      if ( more ) o.more = true; // ToDo: remove once transactions are always transported in _t
      
      if ( tid ) {
        o.transaction_id = tid; // ToDo: remove once transactions are always transported in _t
      } else if ( more ) {
  ...
  */
  function Input( p, name ) {
    ...
    /*
      Incomming transactions by transaction_id and source
      
      source_transactions: (Dictionary) of source transactions by
        transaction_id.
        
        Where a source transaction is an Array of Objects with attributes:
  ...
  Plug.subclass( Input, {
    ...
    source_transactions_add: function( transaction_id, source ) {
      ...
      */
      ...
      if ( source_transaction ) {
        ...
        // ToDo: accelerate lookup using a hash key that could be a uuid for the source output: source_transaction[ source.id ]
        for ( j = -1, sl = source_transaction.length; ++j < sl; ) {
          if ( source_transaction[ j ].source === source ) return null; // already added for this source
        }
      ...
      source_transaction.push( source = { source: source } );
      
      return source;
    }, // source_transactions_add()
    
    /* ------------------------------------------------------------------------
       source_transactions_remove( transaction_id, source )
       
       Removes a source from a transaction at this input.
       
       Parameters:
       - transaction_id (String): id of the transaction to remove source from
       - source (Output): the source being removed.
       
       Returns:
       - null: this source was not found
       - Object: objact that was returned by source_transactions_add()
    */
    source_transactions_remove: function( transaction_id, source ) {
      var source_transactions = this.source_transactions
        , source_transaction = source_transactions[ transaction_id ]
      ;
      
      if ( source_transaction ) {
        var sl = source_transaction.length
          , i = -1
        ;
        
        de&&ug( 'source_transaction_remove(), transaction id: ' + transaction_id + ', source_transaction length: ' + sl );
        
        while ( ++i < sl ) {
          if ( source_transaction[ i ].source === source ) {
            source_transaction.splice( i, 1 );
            
            if ( sl == 1 ) {
              // This was the last one for this transaction
              delete source_transactions[ transaction_id ];
            }
      ...
      return null;
    }, // source_transactions_remove()
    
    /* ------------------------------------------------------------------------
       source_transactions_remove_source( source )
       
       Removes a source output from all source transactions, if any. For each
       transaction removed, source.transaction_remove_source() is called with
       the following parameters:
       - transaction_id (String): transaction id for source
       - Object: the object returned by source_transactions_add()
       ...
       ToDo: add tests for source_transactions_remove_source()
    */
    source_transactions_remove_source: function( source ) {
      var source_transactions = this.source_transactions
        , transaction_id, source_transaction
        , name
      ;
      
      if ( de ) name = this._get_name( 'source_transactions_remove_source' );
      
      for ( transaction_id in source_transactions ) {
        source_transaction = source_transactions[ transaction_id ];
        
        for ( var i = 0, s; s = source_transaction[ i ]; ) {
          if ( source !== s.source ) {
            i += 1;
          ...
          source.transaction_remove_source( transaction_id, s );
          ...
          // ToDo: we need more than a transaction id, we need the whole transaction Object
          this.add( [], { transaction_id: transaction_id, _t: { id: transaction_id } } );
        } // For all sources of source transaction
        
        if ( source_transaction.length == 0 ) {
          delete source_transactions[ transaction_id ];
        }
      } // For all source transactions
      
      return this;
    }, // source_transactions_remove_source()
    ...
    */
    _add_source: function( source, options ) {
      ...
      var output = source._output;
      
      if ( output ) {
        output.__add_destination( this );
        
        this.__add_source_query( output ); // May be overloaded (Controllet)
      }
      
      this
        // ToDo: replace source by output to get direct access to output from source
        .__add_source              ( source )
    ...
    */
    _remove_source: function( source, options ) {
      ...
      if ( output ) {
        this.source_transactions_remove_source( output );
        
        this.__remove_source_query( output );
    ...
    /* ------------------------------------------------------------------------
       ...
       Sets the source pipelet for this input plug.
    ...
    */
    _insert_source_union: function( name ) {
      ...
      if ( source ) {
        this.pipelet._remove_source( source, no_fetch );
      } else {
        source = xs;
      }
      ...
      return xs
        ...
        ._add_destination( this.pipelet, no_fetch )
      ;
    ...
    */
    __fetch_source_destination: function( source, options ) {
      ...
      var q = this.query, output = source._output, p = this.pipelet;
      
      if ( output ) {
        output._transactional_fetch( p, q && q.query, options );
      } else {
        ...
        p[ ( options && options.operation ) || '_add' ]( source, options_forward( options ) );
      }
    ...
    */
    ...
    update_upstream_query: function( removes, adds ) {
      ...
      if ( removes.length || adds.length ) {
        ...
        source && source._output.query_update( removes, adds, this );
  ...
  if ( typeof require === 'function' ) require( './query.js' );
  ...
  */
  function Output( pipelet, name ) {
    ...
    // Destination, aka downstream, inputs
    this.destinations = [];
    
    // Output transactions
    this.transactions = Dictionary(); // ongoing output transactions
  ...
  Plug.subclass( Output, {
    ...
    */
    _add_destination: function( destination, options ) {
      ...
      destination._add_source( this.pipelet, options );
    ...
    */
    _remove_destination: function( destination, options ) {
      destination._remove_source( this.pipelet, options );
    ...
    */
    _fetch: function( receiver, query ) {
      ...
      return p._input.__fetch_source( rx, query );
    ...
    /* ------------------------------------------------------------------------
       _transactional_fetch( receiver_pipelet, query [, options [, next ] ] )
    ...
    */
    _transactional_fetch: function( receiver_pipelet, query, options, next ) {
      ...
      function receiver( values, no_more ) {
        ...
        if ( values.length || o.transaction_id || ( o._t && o._t.id ) ) {
    ...
    */
    ...
    query_update: function( removes, adds, input ) {
      ...
      return this.pipelet.__update_upstream_query( removes, adds );
    }, // query_update()
    
    emit: function( operation, values, options ) {
      var subscribers_operations = this.tree.route( operation, values )
        , t, more, transaction_id, transaction
        , subscribers_operation
      ...
      if ( options ) {
        ...
        if ( transaction_id ) {
          transaction = transactions[ transaction_id ] || ( transactions[ transaction_id ] = [] );
          /* transaction is an Array of destination inputs for which this upstream transaction is in progress */
        }
      }
      ...
      */
      ...
      for ( var i = -1; subscribers_operation = subscribers_operations[ ++i ]; ) {
        var operation = subscribers_operation[ 0 ]
          , subscribers = subscribers_operation[ 1 ]
        ;
        
        var j = -1, r, input, t, o, clear = operation == 'clear';
        ...
        while ( r = subscribers[ ++j ] ) {
          ...
          if ( t ) {
            // This is an update splited in 2 or 3 operations 
            // ToDo: add tests
            
            // get or create transaction
            var _transaction = t.t;
            
            if ( ! _transaction ) {
              // Create the transaction
              if ( more ) {
                // there is an ongoing transaction with more operations to come after the current operation
                this.add_destination_to_transaction( transaction, input, transaction_id );
              }
              
              // ToDo: emit()#_update(): test transaction w/ nesting
              _transaction = t.t = new Transaction( t.count, options );
              
              /*
                Since this is a new Transaction, it cannot be seen at the
                destination input from any other source, therefore it does not
                need to be added using this.add_destination_to_transaction()
                if it would come back through a graph loop, it would then be
                added at the desintation input by
                this.add_destination_to_transaction()
              */
            }
            
            o = _transaction.next().get_emit_options();
          } else {
            if ( more ) {
              // there is an ongoing transaction with more operations to come after the current operation
              de&&ug( this._get_name( 'emit' ) + 'add destination to transaction ' + transaction_id );
          ...
          if ( clear ) {
            ...
            // ToDo: should we emit when r.v.length == 0 and more?
            input[ operation ]( r.v, o );
          }
          
          if ( o && o.more ) continue; // there are more emissions comming either from this update or from upstream
          
          // This was the last emission for this input and transaction, if any
          if ( transaction ) {
            this.remove_destination_from_transaction( transaction, input, transaction_id );
          }
        } // for all subscribers
      ...
      if ( transaction && transaction.length ) {
        this.end_source_transaction( transaction, transaction_id, options );
      }
    ...
    add_destination_to_transaction: function( transaction, input, transaction_id ) {
      // Graph loops, resulting in reentrance of transaction functions are taken care
      // of by testing of the source is already added in a source transaction
      // This also takes care of diamond issues in the graph, allowing the same transaction
      // to have multiple sources.
      
      var source = input.source_transactions_add( transaction_id, this );
      /* source: (optional Object), null if already added. Attributes:
         - source: (Output) this
      */
      
      if ( source ) {
        source.position = transaction.length; // the position of input in transaction[] bellow
        
        transaction.push( input );
        
        de&&ug( this._get_name( 'add_destination_to_transaction' )
          + 'input added to transaction ' + transaction_id
          + ' at position ' + source.position
        );
      }
      
      return this;
    }, // add_destination_to_transaction()
    
    // ToDo: document remove_destination_from_transaction()
    remove_destination_from_transaction: function( transaction, input, transaction_id ) {
      de&&ug( this._get_name( 'remove_destination_from_transaction' ) + transaction_id );
      
      var source = input.source_transactions_remove( transaction_id, this );
      
      if ( source ) transaction.splice( source.position, 1 );
      
      return this;
    }, // remove_destination_from_transaction()
    
    // ToDo: Document end_source_transaction()
    end_source_transaction: function( transaction, transaction_id, options ) {
      // ToDo: provide tests for transaction termination
      
      de&&ug( this._get_name( 'end_source_transaction' ) + transaction_id );
      
      delete this.transactions[ transaction_id ]; // was the reference of transaction at this source
      ...
      while ( input = transaction[ ++i ] ) {
        // Remove this source from input.source_transactions
        if ( input.source_transactions_remove( transaction_id, this ) ) {
          de&&ug( this._get_name( 'end_source_transaction' ) + 'emit end of transaction ' + transaction_id );
        ...
        // There is a bug in either this.add_destination_to_transaction()
        exception( // Does not throw but reports exception, ToDo: make this an error trace
          ...
          'emit()/end_source_transaction()'
        );
      } // for all destination inputs for which this transaction was started
      
      return this;
    }, // end_source_transaction()
    
    // ToDo: docuement transaction_remove_source()
    // Called from Input when removing a source to a destination input
    transaction_remove_source: function( transaction_id, source ) {
      var transaction = source.source.transactions[ transaction_id ];
      
      transaction.splice( source.position, 1 );
      
      if ( transaction.length == 0 ) delete source.transactions[ transaction_id ];
      
      return this;
    }, // transaction_remove_source()
  ...
  */
  ...
  Event_Emitter.subclass( Pipelet, {
    /* ------------------------------------------------------------------------
       _get_name( [ method_name ] )
    ...
    */
    ...
    // ToDo: remove bridge functions when _input and _output refactoring is complete
    _fetch_all: function( receiver, query ) {
    ...
    _add_destination: function( destination, options ) {
      this._output._add_destination( destination, options );
      
      return this;
    },
    
    _remove_destination: function( destination, options ) {
      this._output._remove_destination( destination, options );
    ...
    __update_upstream_query: function( removes, adds ) {
      this._input.update_upstream_query( removes, adds );
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     Pipelet.set_default_options( f, source, parameters, defaults )
     ...
     Parameters:
     - f          : (function) Pipelet constructor or composition function
  ...
  * /
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     The it redirects methods that tie pipelets together so that all subsequent method
     invocations are done directly to/from input or output pipelets with no overhead.
     
     However, as users may still hold references to the encapsulated pipelet, other
     ...
     of these methods as applying to the input or output of a pipelet. But this semantic
     is being clarified by the use of _input and _output plugs, which should eventually
     remove the need for methods redirections.
     ...
     _add_source                    none         Calls _input._add_source()
     _remove_source                 none         Calls _input._remove_source()
     ...
     _add_destination               none         Calls _output._add_destination()
     _remove_destination            none         Calls _output._remove_destination()
     
     _fetch_all                     none         Calls _output.fetch_all()
  */
  ...
  Pipelet.Build( 'encapsulate', Encapsulate, {
    ...
    _on: function( event_name, listener, that, once ) {
      this._output_pipelet._on( event_name, listener, that, once );
  ...
  */
  ...
  Controllet.Input = Pipelet.Input.subclass(
    ...
    _is_lazy: function() {
      var i = -1, destinations = this.pipelet._output.destinations, input;
    ...
    // Route __add_source_query() to all destinations's inputs
    __add_source_query: function( output ) {
      // Add to that source this destinations and queries
      var i = -1, destinations = this.pipelet._output.destinations, input, name;
      
      de&&ug( get_name( this ) + 'output: ' + output._get_name() );
      
      while( input = destinations[ ++i ] ) {
        de&&ug( name + 'destination input: ' + input._get_name() );
        
        input.__add_source_query( output );
      }
    ...
    __remove_source_query: function( output, options ) {
      ...
      de&&ug( get_name( this ) + 'output: ' + output._get_name() + ', options: ' + log.s( options ) );
      
      while( input = destinations[ ++i ] ) {
        de&&ug( name + 'destination: ' + input._get_name() );
        
        input.__remove_source_query( output, options );
      }
    ...
    __fetch_source_destination: function( source, options ) {
      ...
      while( input = destinations[ ++i ] ) {
        ...
        input.__fetch_source_destination( source, options );
      }
  ...
  */
  function Union( sources, options ) {
    this._input  || ( this._input  = new Union.Input ( this ) );
    this._output || ( this._output = new Union.Output( this ) );
    ...
    if ( sources ) {
      for( var i = -1, input = this._input; ++i < sources.length; ) {
        var p = sources[ i ];
        
        if ( p && p instanceof Pipelet ) input._add_source( p );
      }
    }
  } // Union()
  ...
  Union.Input = Controllet.Input.subclass(
    function( p, name ) { Controllet.Input.call( this, p, name ) }, {
    ...
    */
    _fetch: function( receiver, query ) {
      ...
      function resume_all_sources() {
        for ( var i = -1, l = sources.length; ++i < l; ) {
          sources[ i ]._output._resume_destination( p );
        }
      } // resume_all_sources()
    ...
    query_update: function( removes, adds, input ) {
      ...
      if ( rl || al ) {
        ...
        while ( ++i < l ) sources[ i ]._output.query_update( removes, adds, input );
      }
  ...
  */
  ...
  extend_2( Dispatch.prototype, {
    ...
    create_branches: function( new_branches, options ) {
      ...
      if ( options ) {
        var t = options._t
          , tid = t ? t.id : options.transaction_id
        ;
    ...
    remove_branches: function( removed_branches, options ) {
      ...
      if ( options && options.more ) {
        var t = options._t
          , tid = this.remove_branches_tid = t ? t.id : options.transaction_id
        ;
  ...
  */
  function Greedy( options ) {
    ...
    this._input.query = Query.pass_all;
  ...
  */
  ...
  Pipelet.Build( 'delay', Delay, {
    ...
    _output_query_update: function() {
      var that = this;
      
      setTimeout( function() {
        Pipelet.Output.prototype.query_update.apply( that, arguments )
      }, this.delay );
