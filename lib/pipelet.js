...
( function( exports ) {
  ...
  */
  ...
  var Query_Tree_Prototype = {
    ...
    query_tree_emit: function( operation, values, options ) {
      ...
      for ( i = -1; v = values[ ++i ]; ) route_value( this, top, recipients, v, [] );
      ...
      function route_value( that, node, recipients, value, pushed ) {
        ...
        for( i = -1; ++i < rl; ) {
          ...
          if ( recipient_index ) { // defined and not-zero
            ...
            if ( transaction && ! p.source_transactions[ transaction_id ] ) {
              // This transaction has not been started yet at this destination pipelet
              
              // Memorize the position in transaction to allow fast removal from source
              p.source_transactions[ transaction_id ] = { source: that, position: transaction.length };
              
              // Add this destination into the list of pipelets that have received data for this transaction
              transaction.push( p );
            }
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Parameters:
       - transform: (Function) a function which signature is:
           ...
           ToDo: allow transform to be an object with parameters and code for a inlining
  ...
  */
  function Alter( transform, options ) {
    ...
    var vars = [ 'i = -1', 'l = values.length', 'r = []', 't = transform' ];
    
    var while_body = 'r.push(t(' + parameters + '));';
  ...
  */
  Compose( 'set_flow', function( source, flow, options ) {
    ...
    return source.alter( new Function( ['v'], "v.flow = '" + flow + "'" ), options );
  } );
