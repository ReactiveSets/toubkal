  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    emit: function( event_name, values, options ) {
      ...
      de&&ug(
          this._get_name() + 'emit( "' + event_name + '" )'
    ...
    */
    fetch: function( receiver, query ) {
      ...
      // source immediatly.
      // ToDo: consider deprecating _lazy, which could become equivalent to a null this._query 
      this._lazy = false;
    ...
    */
    _fetch_source: function( receiver, _query ) {
      ...
      de&&ug( name( this ) + 'query: ' + log.s( query ) + ', keys: ' + log.s( keys ) );
      ...
      function name( that ) {
        return name = that._get_name() + '._fetch_source(), ';
      } // name()
    }, // _fetch_source()
    ...
    */
    fetch_all: function( receiver, query ) {
      ...
      if ( out === u && receiver === u ) throw new Error( name() + "is asynchronous and no receiver function was provided" );
      ...
      function rx_concat( values, no_more ) {
        if ( out ) throw new Error( name() + "received extra chunck after no_more" );
    ...
    */
    add_source: function( source, options ) {
      ...
      var that = this, q = this._query, connected, t, o;
      ...
      if ( q ) {
        ...
        // _query is null
        // This pipelet is not interested in anything for now from this source, it is still lazy
        // ToDo: source has no reference to this, this could be a problem for disconnecting pipelets?
        return this;
      }
      ...
      if ( ! ( options && options.lazy ) ) {
        ...
        // ToDo: how about fetching such as source? Do we still need to support this? What's the use case?
        this.add( source );
      } else {
    ...
    */
    _add_destination: function( destination, query ) {
      // ToDo: _add_destination(), if the query is the nul query there will be no reference from the source to this. This could be an issue for cleaning up references on disconnections
    ...
    _update_upstream_query: function( adds, removes ) {
      ...
      if ( removes.length || adds.length ) {
        if ( de ) {
          var name        = this._get_name() + '._update_upstream_query(), '
            , source_name = ' source: ' + ( source ? source._get_name() : 'none' )
          ;
          
          ug( name + 'update' + source_name );
        }
    ...
    */
    remove_source: function( source, options ) {
      ...
      q && source._remove_destination && source._remove_destination( this, q.query ); // ToDo: test proper query removal
  ...
  Pipelet.Build( 'union', Union, {
    ...
    */
    ...
    _query_tree_update: function( adds, removes, destination ) {
      ...
      // ToDo: DRY duplicated code from Pipelet.._update_upstream_query()
      if ( q ) {
        var query = q.query;
  ...
  */
  ...
  extend_2( Dispatch.prototype, {
    create_branches: function( new_branches, options ) {
      var name = de && ( ( this.options.name || 'dispatch' ) + '.create_branches(), ' );
      ...
      for ( var i = -1, l = new_branches.length; ++i < l; ) {
        ...
        de&&ug( name + 'gatherer: '
          + log.s( { _lazy: gatherer._lazy, _query: gatherer._query && gatherer._query.query } )
        );
  ...
  */
  ...
  Pipelet.Build( 'trace', Trace, {
    ...
    _fetch_source: function( receiver, query ) {
      ...
      function rx( values, no_more ) {
        that.log( name, { values: values, no_more: no_more } );
