...
/* -------------------------------------------------------------------------------------------
   ...
   application using this dataflow programming model where events are handled at a lower level
   by the XS framework itself.
   ...
   Remove Operation, and Objects Identity:
   --------------------------------------
   To remove objects from a set, objects need to have a unique identity accross the set
   and time.
   
   Object identity attributes are defined by the set's key which by default has the single
   attribute 'id'. So by default all objects of a set must define a unique 'id' field.
   ...
   This requirement for unique identity also applies to update operations but not to add
   operations on imutable objects.
