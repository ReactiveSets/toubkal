...
( function( exports ) {
  ...
  */
  function Transaction( transactions, count, options ) {
    // Hidden class: count(Int), o (Object), transactions (Object)
    ...
    if ( tid ) {
      ...
      if ( t ) {
        // this transaction is already created at this point (i.e. present in transactions)
        
        t.count += count; // add count operations to this transaction at this point
    ...
    this.count        = count;
    this.o            = options = options ? extend_2( {}, options ) : {};
    this.transactions = transactions;
    
    options.more = true;
    options.transaction_id = tid;
    
    de&&ug( 'new Transaction(): options: ' + log.s( options ) );
    
    // Strore new transaction at this point
    return transactions[ tid ] = this;
    
    function no_transaction( that ) {
      that.count        = count;
      that.o            = options || {};
  ...
  extend_2( Transaction.prototype, {
    ...
    next_options: function() {
      ...
      if ( count > 0 ) return this.o;
      
      if ( count < 0 )
        throw new Error( 'Transaction count negative: ' + count + ', options: ' + log.s( this.o ) );
      ...
      if ( this.transactions ) {
        ...
        delete this.transactions[ o.transaction_id ];
      }
      ...
      switch( operation ) {
        case 'update':
          ...
          for ( i = -1, rl = recipients.length; ++i < rl; ) {
            ...
            var operations = 0, options_more;
            
            if ( removed.length ) operations += 1;
            if ( added  .length ) operations += 1;
            if ( updated.length ) operations += 1;
            
            if ( operations > 1 ) options_more = more( options );
            
            removed.length && p.remove( removed, operations > 1 ? options_more : options );
            added  .length && p.add   ( added  , updated.length ? options_more : options );
            updated.length && p.update( updated, operations > 1 ? no_more( options_more ) : options );
            /*
            var t = new Transaction( this._transactions, operations, options );
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    notify: function( transaction, options ) {
      ...
      if ( l > 1 ) {
        var l1 = l - 1, options_more = more( options, l );
        
        for ( i = -1; ++i < l1; ) {
          a = transaction[ i ];
          
          this[ a.action ]( a.objects, options_more );
        }
    ...
    */
    update: function( updates, options ) {
      var l = updates.length, update, i = -1, options_more;
      
      if ( l ) {
        options_more = more( options, 2 * l );
        ...
        this.remove( [ update[ 0 ] ], options_more );
        this.add   ( [ update[ 1 ] ], no_more( options ) ); // last operation does not have more
      }
  ...
  Pipelet.build( 'set', Set, {
    ...
    */
    ...
    _transaction_start: function( count, options ) {
      if ( count == 0 ) {
        ...
        de&&ug( 'Set.._transaction_start(), count is zero, more: ' + more );
    ...
    */
    ...
    _add_value: function( transaction, value ) {
      ...
      de&&ug( 'Set.._add_value(), name: ' + this.options.name + ', options: ' + log.s( options ) );
      
      this.a.push( value );
      
      return this.emit_add( [ value ], options );
    }, // _add_value()
    
    _add_values: function( transaction, values ) {
      var options = transaction.next_options();
      
      de&&ug( 'Set.._add_values(): values: ' + values.length + ', options: ' + log.s( options ) );
    ...
    _no_value: function( transaction ) {
      var options = transaction.next_options();
      
      de&&ug( 'Set.._no_value(): options: ' + log.s( options ) );
      
      // Emit if this is the end and there is no more data comming from upstream
      options.more || this.emit_add( [], options ); // ToDo: add test for _no_value
    ...
    */
    ...
    _remove_value: function( transaction, values ) {
      if ( values ) {
        values instanceof Array || ( values = [ values ] );
        
        de&&ug( '_remove_value(): values: ' + values.length + ', transaction count: ' + ( transaction.count - 1 ) ); // display count after emit()
        ...
        for ( var i = -1, l = values.length, removed = []; ++i < l; ) {
          ...
          if ( p === -1 ) {
            // This should not happen since the anti-state should have tested in add()
            throw new Error( 'Set.._remove_value(), value not found' );
          } else {
        ...
        return this.emit_remove( removed, --transaction.count ? transaction.more : transaction.done );
      } else {
        this._no_value( transaction );
      }
  ...
  */
  ...
  Compose( 'auto_increment', function( source, options ) {
    ...
    // encapsulate()
    var output = input.set( [], extend_2( options, { lazy: false } ) );
