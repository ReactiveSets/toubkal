...
( function( exports ) {
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    _update: function( _updates, options ) {
      ...
      de&&ug( this._get_name( '_update' ) + 'name: ' + this.options.name + ', updates: ' + _l );
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       source._add_destination( pipelet ).
    ...
    */
    _add_destination: function( pipelet ) {
      de&&ug( this._get_name( '_add_destination' ) + 'pipelet: ' + pipelet._get_name() );
      
      return pipelet._add_source( this );
    }, // _add_destination()
    
    /* ------------------------------------------------------------------------
       source._remove_destination( pipelet ).
       ...
       Parameters:
         - pipelet: (Pipelet) the destination pipelet to _remove_destination
         - options:
           - no_fetch: do not _fetch the source to remove
    */
    _remove_destination: function( pipelet, options ) {
      return pipelet._remove_source( this, options );
    }, // _remove_destination()
    ...
    /* ------------------------------------------------------------------------
       ...
       Parameters:
         - source: (Object) to add as a source to this pipelet.
             ...
             If source is a Pipelet or an Object providing a _fetch() function
             ...
             _fetch() function, it's content is only added to the current
             pipelet using this._add().
    */
    _add_source: function( source ) {
      // Add the source first so that, if a downsteam pipelet trys to _fetch()
      // before this pipelet has finished fetching, that a new _fetch be
      // requested to this source.
      return source.is_void ? this : this
        .__add_source              ( source ) // ToDo: consider merging this and next
        .__add_source_destination  ( source )
        .__add_source_query        ( source )
        .__fetch_source_destination( source )
      ;
    }, // _add_source()
    ...
    /* ------------------------------------------------------------------------
       ...
       Calls low-level __remove_source_destination() and __remove_source() to
    ...
    */
    _remove_source: function( source, options ) {
      return source.is_void ? this : this
        ...
        .__remove_source_destination( source ) // ToDo: consider merging this and next
    ...
    */
    __add_source: function( source ) {
      ...
      if ( s ) {
        ...
        de&&ug( this._get_name( '__add_source' ) + 'insert ' + union._get_name() );
        
        // Do not update s' query tree because Union is just a controllet
        
        this
          .__remove_source            ( s )
          .__remove_source_destination( s )
          .__add_source               ( union )
          .__add_source_destination   ( union )
        ;
        
        union
          .__add_source            ( s )
          .__add_source_destination( s )
        ;
        
        this.__add_source = function( source ) {
          union.__add_source( source );
          
          return this;
        };
        
        this.__remove_source = function( source ) {
          union.__remove_source( source );
          
          return this;
        };
        
        return this.__add_source( source );
      } else {
        this._source = source;
      }
  ...
  * /
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     __init_query_tree              null         Called by Pipelet()
  ...
  */
  ...
  Pipelet.Build( 'encapsulate', Encapsulate, {
    ...
    _add_destination: function( pipelet, options ) {
      return this._output._add_destination( pipelet, options );
    },
    
    _remove_destination: function( pipelet, options ) {
      return this._output._remove_destination( pipelet, options );
    },
    ...
    _remove_source: function( source ) {
      this._input._remove_source( source );
     ...
     Example:
       ...
       update that same database. In this example, branches are socket.io clients:
         ...
         function client_dataflow( source, options ) {
           return source
             ._add_destination( this.socket ) // _add_destination dispatcher's source into socket.io client socket
           ;
         }
  ...
  */
  ...
  Pipelet.Build( 'set', Set, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
      __update_upstream_query( adds, removes )
      ...
      propagate upstream. Sets therefore _fetch all they can regarless of
