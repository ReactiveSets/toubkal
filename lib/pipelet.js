...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     new Transaction( count [, options [, pipelet ]] )
     ...
     - options (optional Object): upstream pipelet options:
       - _t (optional Object): a transaction Object:
     ...
     - pipelet (optional Pipelet): pipelet to emit operations to. When not provided, the
       transaction will emit events 'add' and 'remove' instead of emitting to the pippelet
       directly, see Emitted Events bellow.
     ...
     - 'add'   : ( values, options ),
       When no pipelet is provided and an 'add' is emited
     
     - 'remove': ( values, options ),
       When no pipelet is provided and a 'remove' is emited
  ...
  */
  ...
  Event_Emitter.subclass( Transaction, {
    ...
    set_source_options: function( options ) {
      ...
      if ( t && t.forks ) this.need_close = true; // Forces a closing emit()
    ...
    _emit: function( operation, values, options ) {
      var pipelet = this.pipelet;
      
      if ( pipelet ) {
        this.pipelet.__emit( operation, values, options );
      } else {
  ...
  Plug.subclass( Input, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       ...
       Parameters:
       - source: (Array of Objects or Pipelet) to add as a source to this
         Input.
    ...
    if ( ! pipelet ) {
      pipelet = ( function() {
        ...
        return function pipelet() {
          ...
          // ToDo: should use this._output plug in input._add_source()
  ...
  function Union( sources, options ) {
    ...
    if ( sources ) {
      for( var i = -1; ++i < sources.length; ) {
        ...
        if ( p && p._output ) _input._add_source( p );
      }
    }
  } // Union()
