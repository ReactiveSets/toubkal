  ...
  */
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       source._add_destination( destination, options )
    ...
    */
    _add_destination: function( destination, options ) {
      ...
      return destination._add_source( this, options );
    }, // _add_destination()
    
    /* ------------------------------------------------------------------------
       source._remove_destination( destination, options )
    ...
    */
    _remove_destination: function( destination, options ) {
      return destination._remove_source( this, options );
    }, // _remove_destination()
    ...
    */
    _add_source: function( source, options ) {
      ...
      this
        .__add_source              ( source ) // !! May return added union as source of this
    ...
    */
    __add_source: function( source ) {
      ...
      if ( s ) {
        ...
        // ToDo: Wouldn't it be better to integrate Union in inputs just as Query Trees have replaced Fork?
        var union = s.union( [], { name: this._get_name() + ' (union)' } )
          , no_fetch = { no_fetch: true } // Will just swicth the connexion with the union
        ;
        
        de&&ug( this._get_name( '__add_source' ) + 'insert ' + union._get_name() );
        
        this
          ._remove_source( s    , no_fetch )
          ._add_source   ( union, no_fetch )
        ;
        
        this.__add_source = function( source ) {
          // !! Returning the union itself will force _add_source() to complete with union
          return union.__add_source( source ); // Encapsulation: this is a friend of this union
        };
        
        var _remove_source = this._remove_source;
        
        this._remove_source = function( source ) {
          if ( source === union ) {
            _remove_source.call( this, source );
          } else {
        ...
        return this.__add_source( source ); // !! returns union
      } else {
        this._source = source;
      }
