...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     Transaction( transactions, count, options [, pipelet ] )
  */
  function Transaction( transactions, count, options, pipelet ) {
    var tid = options && options.transaction_id, o;
    ...
    if ( tid ) {
      ...
      // ToDo: build tid only when and if needed
      tid = uuid_v4();
    ...
    o = options ? extend_2( {}, options ) : {};
    o.more = true;
    
    if ( tid ) transactions[ o.transaction_id = tid ] = this; // Store new transaction at this pipelet
    
    de&&ug( 'new Transaction(): count: ' + count + ', options: ' + log.s( o ) );
    ...
    function init_transaction( that ) {
      ...
      that.name           = pipelet ? ', name: ' + pipelet.get_name() : '';
  ...
  extend_2( Transaction.prototype, {
    ...
    emit: function( operation, values, options ) {
      ...
      de&&ug( 'Transaction..emit( "' + operation + '" )' + this.name + ', options: ' + log.s( options ) );
      
      if ( ( this.need_close = options.more ) && options.transaction_id === tid ) {
        tid = options.transaction_id = uuid_v4();
        
        transactions[ tid ] = this;
        
        de&&ug( 'Transaction..emit()' + this.name + ', new tid: ' + tid );
      }
      
      this.pipelet.emit( operation, values, options );
      
      return this;
    }, // emit()
    
    emit_add: function( values, emit_now ) {
      var options = this.next_options(), l = values.length;
    ...
    next_options: function() {
      ...
      if ( count < 0 )
        throw new Error( 'Transaction count negative: ' + count + this.name + ', options: ' + log.s( options ) );
    ...
    terminate: function() {
      ...
      if ( transactions ) {
        var tid = this.o.transaction_id;
        
        de&&ug( 'Transaction..terminate()' + this.name + ', id: ' + tid );
    ...
    */
      ...
      var options = this.o, o = options || {};
      
      if ( o.more || this.count || this.state == 1 ) return this;
      
      var removed    = this.removed, rl = removed.length
        , added      = this.added  , al = added.length
        , p          = this.pipelet
        , name       = this.name
      ...
      var need_close = this.need_close;
      
      if ( rl ) {
        de&&ug( 'Transaction..end()' + name + ', emit removes: ' + rl + ', need_close: ' + need_close );
        
        if ( al ) {
          options.more = true;
          
          this.emit( 'remove', removed, options );
          
          options.more = false
          
        } else if ( need_close ) {
          p.emit( 'remove', removed, options );
          
          need_close = false;
          
        } else {
          p.emit( 'remove', removed, this.source_options );
        }
      }
      
      if ( al || need_close ) {
        de&&ug( 'Transaction..end()' + name + ', emit adds: ' + al + ', need_close: ' + need_close );
        
        if ( need_close ) {
          p.emit( 'add', added, options );
          
          need_close = false;
        } else {
          p.emit_add( added, this.source_options );
        }
      }
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    _transaction: function( count, options, f ) {
      ...
      var t = new Transaction( this._transactions, count, options, this );
    ...
    */
    emit: function( event_name, values, options ) {
      ...
      de&&ug(
        ...
        + ( options && options.more !== u ? ', more: ' + options.more : '' )
