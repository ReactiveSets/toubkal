  ...
  extend_2( Query.prototype, {
    ...
    */
    ...
    /* -----------------------------------------------------------------------------------------
      ...
      Removes or-terms from query. These or-terms should exist in the original query or an
      exception is triggered. In the future it might be possible to remove non-existant terms
      when we implement an operator 'not'.
    ...
    */
    remove: function( q1 ) {
      ...
      var q0 = this.query
        , i0 = -1, i1 = -1, e0, e1, keys0s = this.keys, u
      ;
      
      while ( e1 = q1[ ++i1 ] ) { // for all terms to remove
        var keys1 = Object.keys( e1 ), l1 = keys1.length;
        
        for ( i0 = -1; e0 = q0[ ++i0 ]; ) {
          var keys0 = keys0s[ i0 ];
          
          if ( l1 != keys0.length ) continue;
          
          for ( var k1 = -1, key1; key1 = keys1[ ++k1 ]; ) if ( e0[ key1 ] !== e1[ key1 ] ) break;
          
          if ( key1 === u ) { // all terms matched
            keys0s.splice( i0, 1 );
            q0.splice( i0--, 1 );
        ...
        if ( ! e0 ) throw new Error( 'Query..remove() could not find term to remove: ' + log.s( e1 ) );
      }
