  ...
  var Query_Tree_Prototype = {
    ...
    query_tree_emit: function( operation, values, options ) {
      ...
      switch( operation ) {
        case 'update':
          for ( i = -1; v = values[ ++i ]; ) {
            route_update( top, recipients, v[ 0 ], i, 0 ); // route remove
            route_update( top, recipients, v[ 1 ], i, 1 ); // route add
          }
      ...
      for ( i = -1; v = values[ ++i ]; ) route_value( this, top, recipients, v, [ 0 ] );
      ...
      function route_value( that, node, recipients, value, pushed ) {
          ...
          if ( recipient_index ) { // defined and not-zero
            ...
            if ( transaction ) {
              ...
              if ( source_transaction ) {
                ...
                for ( var j = -1, sl = source_transaction.length; ++j < sl && source_transaction[ j ].source !== that; );
                
                if ( j == sl ) {
                  // ToDo: make sure this gets tested as well as every branch of this complex section of code
                  
                  // Note: this code is duplicated bellow. Since this part is seldom executed, duplication may be desirable
                  ...
                  // Add this destination into the list of pipelets that have received data for this transaction and this source
                  transaction.push( p );
                }
              } else {
                // This transaction has not been started yet at this destination pipelet
                
                // Memorize the position in transaction to allow fast removal from this source
                ...
                source_transactions[ transaction_id ] = [ { source: that, position: transaction.length } ];
        ...
        for( i = -1; key = keys[ ++i ]; ) {
          if ( ( v = value[ key ] ) !== u && ( child = branches[ key ][ v ] ) ) {
            ...
            route_value( that, child, recipients, value, pushed );
          }
      ...
      function route_update( node, recipients, value, position, offset ) {
        var keys            = node.keys, key
          , branches        = node.branches
          , node_recipients = node.recipients
        ...
        for( i = -1; ++i < rl; ) {
          ...
          if ( recipient_index ) { // defined and not-zero
            // This pipelet already has at least one update value
          ...
          if ( v[ position ] ) {
            v[ position ][ offset ] = value; // duplicates are copied at the same position and offset, preventing duplicates
          } else if ( offset ) {
        ...
        for( i = -1; key = keys[ ++i ]; ) {
          if ( ( v = value[ key ] ) !== u && ( child = branches[ key ][ v ] ) ) {
            // We have a value for this term and this value has a child node, descend
            route_update( child, recipients, value, position, offset );
          }
          // Keep looking for other possible matches
        } // for all term keys
      } // route_update()
