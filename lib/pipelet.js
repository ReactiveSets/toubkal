...
( function( exports ) {
  ...
  */
  ...
  Root.subclass( Source_Transactions, {
    /* ------------------------------------------------------------------------
    ...
    remove_source: function( source ) {
      ...
      for ( transaction_id in transactions ) {
        ...
        for ( var i = 0, s; s = transaction[ i ]; ) {
          ...
          source.transactions.remove_source( transaction_id, s );
  ...
  Root.subclass( Output_Transaction, {
    ...
    add_destination: function( input ) {
      ...
      var id = this.id
        , source = input.source_transactions.add( id, this.source )
      ;
      ...
      */
      ...
      if ( source ) {
        ...
        source.position = inputs.length; // the position of input in inputs[] bellow
    ...
    remove_destination: function( input ) {
      ...
      var source = input.source_transactions.remove( id, this.source );
      
      if ( source ) this.inputs.splice( source.position, 1 );
    ...
    // ToDo: Document end_source_transaction()
      ...
      if ( inputs.length ) {
        ...
        while ( input = inputs[ ++i ] ) {
          // Remove source from input.source_transactions
          if ( input.source_transactions.remove( id, source ) ) {
            de&&ug( this._get_name( 'end' ) + 'emit end of transaction ' + id );
    ...
    remove_source: function( source ) {
      transaction.inputs.splice( source.position, 1 );
  ...
  Root.subclass( Output_Transactions, {
    ...
    */
    get: function( transaction_id ) {
      ...
      return transactions[ transaction_id ] || ( transactions[ transaction_id ] = new Output_Transaction( transaction_id, this.source ) );
    }, // get()
    
    end: function( transaction_id, options ) {
      var transactions = this.transactions
        , transaction  = transactions[ transaction_id ]
      ;
  ...
  */
  ...
  Plug.subclass( Output, {
    ...
    */
    ...
    // Route and emit using query tree, managing output transactions
    _emit: function( operation, values, options ) {
      ...
      // Get more and output transaction
      if ( t ) {
        more = t.more;
        
        if ( transaction_id = t.id ) {
          transaction = transactions.get( transaction_id );
          // transaction manages destination inputs for which this output transaction is in progress
        }
      }
      ...
      */
      ...
      for ( var i = -1; subscribers_operation = subscribers_operations[ ++i ]; ) {
        ...
        while ( r = subscribers[ ++j ] ) {
          ...
          if ( t ) {
            ...
            if ( ! _transaction ) {
              ...
              if ( more ) {
                ...
                transaction.add_destination( input ); // adds input only the first time
              }
          ...
          transaction && transaction.remove_destination( input );
        } // for all subscribers
      } // for all subscribers_operations
      
      if ( more ) return this;
      
      // No more, end transaction, if any
      transaction && transactions.end( transaction_id, options );
