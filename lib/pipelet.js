  ...
  Pipelet.subclass( Query_Tree, {
    new_node: function() {
      return {
        ...
        recipients: []  // all recepients for this node
      };
    }, // new_node()
    ...
    route: function( node, operation, values, options ) {
      ...
      for ( var i = -1, v; v = values[ ++i ]; ) {
        route_value( top, v, operation, options );
      }
      
      return this;
      
      function route_value( node, value, operation, options ) {
        var recipients = node.recipients, recipient
          , keys = node.keys, key
        ...
        // Emit this operation / value to this node's recipients 
        // ToDo: handle no-( more option )
        for ( i = -1; recipient = recipients[ ++i ]; ) {
          recipient.emit( operation, values, options );
        }
        
        // Lookup child nodes for possible additional recipients
        for ( i = -1; key = keys[ ++j ]; ) {
          if ( ( v = value[ key ] ) !== u && ( child = branches[ key ][ v ] ) ) {
            // We have a value for this term and this value has a child node, descend
            route_value( child, value, operation, options );
          }
