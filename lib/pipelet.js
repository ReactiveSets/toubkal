...
( function( exports ) {
    ...
    query_tree_remove: function( or_terms, subscriber ) {
      ...
      function remove_term( node, term, term_keys ) {
        ...
        if ( term_keys.length ) {
          ...
          if ( index == -1 ) {
            throw new Error(
              'Query_tree..remove(), Cannot find subscriber (' + subscriber._get_name() + ') in tree where it was expected'
              + ', or_terms: ' + log.s( or_terms )
              + ', at position: ' + position
              + ', this name: ' + that._get_name()
            );
          }
  ...
  */
  function Pipelet( options ) {
    ...
    this._query_controller = null; // a controller pipelet for dynamic queries
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    emit: function( event_name, values, options ) {
      ...
      // ToDo: consider removing permanetly this try block
      //try {
    ...
    */
    _fetch_source: function( receiver, _query ) {
      // ToDo: use query parameter to fetch using this._query when query is not provided
      var s = this._source, q = this._query, query = q.query, ql = query.length, name = de && ( this._get_name() + '._fetch_source(), ' );
      
      if ( s && ql ) { // there is a source and the query is not the nul query
        if ( 1 == ql && 0 == q.keys[ 0 ].length ) {
        ...
        de&&ug( name + 'no source or nul query' );
    ...
    */
    plug: function( pipelet, options ) {
      ...
      de&&ug( this._get_name() + '.plug(), ' + log.s( { pipelet: pipelet._get_name(), options: options } ) );
    ...
    */
    add_source: function( source, options ) {
      ...
      var that = this, q = this._query.query, connected, t, o;
      ...
      // ToDo: add_source() and _update_query(): guaranty synchronization between fetch() and _query_tree_update()
      source._add_destination && source._add_destination( this, q );
      ...
      if ( ! ( options && options.lazy ) ) {
        ...
        if ( source.fetch ) {
          ...
          // !!! use source.fetch(), not this._fetch_source() because derived pipelets, such as Union
          // overload fetch() but not _fetch_source(), ToDo: test and document
          source.fetch( function( values, no_more ) {
            if ( no_more ) {
              that.add( values, t && t.next().get_emit_options() );
              
              t && that._transactions.end_transaction( t );
              ...
              if ( ! t ) { // ToDo: add_source(): test transaction
                t = that._transactions.get_transaction( 2, {}, that );
        ...
        // specify filters for a future fetch()
      }
    ...
    */
    _add_destination: function( destination, query ) {
      // ToDo: _add_destination(), if the query is the nul query there will be no reference from the source to this. This could pose issues for cleaning up references on disconnections
      return this._query_tree_update( query, [], destination );
    }, // _add_destination()
    
    // ToDo: document add_query_source()
    add_query_source: function( filter, options ) {
      ...
      function add( or_terms, options ) {
        de&&ug( 'add_query_source()..add(), or_terms: ' + log.s( or_terms ) );
      ...
      function remove( or_terms, options ) {
        de&&ug( 'add_query_source()..remove(), or_terms: ' + log.s( or_terms ) );
    ...
    _query_tree_update: function( adds, removes, destination ) {
      ...
      de&&ug( this._get_name() + '._query_tree_update(), ' + log.s( { adds: adds, removes: removes, destination: destination._get_name() } ) );
      
      rl && this.query_tree_remove( removes, destination );
      al && this.query_tree_add   ( adds   , destination );
      
      // ToDo: this is a workaround to avoid propagating pass-all queries upstream when there is a local query
      // This will be removed once and() is implemented in pipelets and the default query becomes the nul query
      if ( 1 == al && 0 == Object.keys( adds   [ 0 ] ).length ) adds    = [];
      if ( 1 == rl && 0 == Object.keys( removes[ 0 ] ).length ) removes = [];
      
      return this._update_upstream_query( adds, removes );
    }, // _query_tree_update()
    
    _update_upstream_query: function( adds, removes ) {
      var rl = removes.length, al = adds.length;
      ...
      if ( q === Query.pass_all ) {
        // ToDo: this is a workaround until the default query becomes the nul query
        de&&ug( this._get_name() + '._update_upstream_query(), replace pass_all query by nul query, source: ' + source._get_name() );
        
        // replace default pass_all query by nul query
        q = this._query = new Query( [] ).generate();
        
        // And remove pass-all query from upstream query tree as it must have been setup by add_source()
        source && source.query_tree_remove( [ {} ], this );
      }
      ...
      if ( removes.length || adds.length ) {
        de&&ug( this._get_name() + '._update_upstream_query(), update source: ' + source._get_name() );
        
        q.generate();
        
        source && source._query_tree_update( adds, removes, this );
    ...
    _update_query: function( options ) {
      ...
      var old_transform = q.filter
        , new_transform = q.generate().filter
        , source = this._source
      ;
      ...
      if ( source ) {
        ...
        // ToDo: optimization: fetch should specify a query to fetch less data in client / server communications
        
        // Remove, using previous upstream queries
        this._lazy || ( removes.length && source.fetch( remover, removes ) );
        
        // Update upstream query trees and queries
        source._query_tree_update( adds, removes, this );
        
        // Add using new upstream queries
        this._lazy || ( adds.length    && source.fetch( adder  , adds    ) );
      }
      
      return this;
      
      function remover( values, no_more ) {
        // ToDo: remove transforms, once fetch implements queries as values should be the list of values to be removed
        var t0 = old_transform, t1 = new_transform, removed = [], v, V;
        
        for( var i = -1, l = values.length; ++i < l; ) {
          V = [ v = values[ i ] ];
          
          t0( V ).length && ! t1( V ).length && removed.push( v ); // v no longer passes filter
        }
        
        var rl = removed.length;
        
        // process removed in a transaction
        de&&ug( 'Query changed, fetched source, removed: ' + rl );
        
        // ToDo: _update_query(): add tests for transactions
      ...
      function adder( values, no_more ) {
        // ToDo: remove transforms, once fetch implements queries as values should be the list of values to be added
        var t0 = old_transform, t1 = new_transform, added = [], v, V;
        
        for( var i = -1, l = values.length; ++i < l; ) {
          V = [ v = values[ i ] ];
          
          ! t0( V ).length && t1( V ).length && added.push( v ); // v now passes filter
        }
        
        var al = added.length;
        
        // process added and removed in a transaction
        ...
        if ( no_more ) {
          ...
          if ( ! t ) { // ToDo: _update_query(): add test for transaction
            t = that._transactions.get_transaction( 2, options, that );
    ...
    */
    remove_source: function( source, options ) {
      ...
      source._remove_destination && source._remove_destination( this, this._query.query ); // ToDo: test proper query removal
      
      this._remove_source( source );
      
      if ( ! ( options && options.no_fetch ) && typeof source.fetch == 'function' ) {
        var that = this;
        // ToDo: remove_source(), handle transaction
        source.fetch( function( values, no_more ) {
  ...
  * /
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     query_tree_add        this --> output  ToDo: Eventually should be null
     query_tree_remove     this --> output  ToDo: Eventually should be null
  ...
  */
  ...
  Pipelet.Build( 'encapsulate', Encapsulate, {
    ...
    query_tree_add: function( removes, destination ) {
      this._output.query_tree_add( removes, destination );
      
      return this;
    },
    
    query_tree_remove: function( removes, destination ) {
      this._output.query_tree_remove( removes, destination );
      
      return this;
    },
    
    _query_tree_update: function( adds, removes, destination ) {
      this._output._query_tree_update( adds, removes, destination );
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     ToDo: provide shortcuts for query tree and fetch
  */
  ...
  Pipelet.Build( 'union', Union, {
    ...
    */
    fetch: function( receiver, query ) {
      ...
      // After a first fetch, if any other source is added, fetch from that source.
      this._lazy = false;
    ...
    query_tree_add: function( adds, destination ) {
      var sources = this._sources, l = sources.length, i = -1;
      
      while ( ++i < l ) sources[ i ].query_tree_add( adds, destination );
    ...
    query_tree_remove: function( removes, destination ) {
      ...
      while ( ++i < l ) sources[ i ].query_tree_remove( removes, destination );
      
      return this;
    }, // query_tree_remove()
    
    _query_tree_update: function( adds, removes, destination ) {
      var q = this._query, query = q.query, sources = this._sources, l = sources.length, i = -1;
      
      // ToDo: remove duplicated code from Pipelet._update_upstream_query()
      if ( query.length == 1 && Object.keys( query[ 0 ] ).length == 0 ) {
        q = this._query = new Query( [] ); // nul query
      }
      
      adds   .length && q.add   ( adds );
      removes.length && q.remove( removes );
      
      ( q.adds.length + q.removes.length ) && q.generate().clear_operations();
      
      de&&ug( this._get_name() + '._query_tree_update(): query: ' + log.s( q.query ) );
      
      while ( ++i < l ) sources[ i ]._query_tree_update( adds, removes, destination );
  ...
  extend_2( Dispatch.prototype, {
    create_branches: function( new_branches, options ) {
      ...
      for ( var i = -1, l = new_branches.length; ++i < l; ) {
        ...
        de&&ug( 'Dispatch..create_branches(), gatherer: ' + log.s( { _lazy: gatherer._lazy, _query: gatherer._query.query } )  )
  ...
  */
  function Set( a, options ) {
    ...
    this._query = new Query( [ {} ] ).generate(); // pass-all ToDo: this is in view of future Pipelet default which will be the nul query
  ...
  */
  ...
  Pipelet.Build( 'trace', Trace, {
    ...
    add_source: function( source, options ) {
      this.log( 'add_source()', { source: source._get_name(), options: options } );
      
      return Pipelet.prototype.add_source.call( this, source, options );
    }, // add_source()
    
    _fetch_source: function( receiver, query ) {
      var u, s = this._source, that = this;
      
      this.log( '_fetch_source()', { source: s._get_name(), query: this._query.query } );
      
      if ( s ) {
        Pipelet.prototype._fetch_source.call( this, rx, query );
      } else {
        this.log( '_fetch_source(), no source', { values: [], no_more: true } );
      ...
      function rx( values, no_more ) {
        that.log( '_fetch_source()', { values: values, no_more: no_more } );
    ...
    query_tree_remove: function( removes, destination ) {
      this.log( 'query_tree_remove()', { removes : removes, destination: destination._get_name() } );
      
      return Pipelet.prototype.query_tree_remove.call( this, removes, destination );
    }, // query_tree_remove()
    
    _query_tree_update: function( adds, removes, destination ) {
      this.log( '_query_tree_update()', { adds: adds, removes : removes, destination: destination._get_name() } );
      
      return Pipelet.prototype._query_tree_update.call( this, adds, removes, destination );
    }, // _query_tree_update()
