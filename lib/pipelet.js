...
( function( exports ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
     Trace( realm, emit [, query ] )
     
     Returns a trace() Function Object to emit traces from a realm.
     
     Trace() can be invoqued with or without new.
     ...
     - emit (Function): to emit traces, signature is:
       ...
       - _realm   (String): this trace's realm (the first parameter of Trace()
     ...
     - trace( ... ) (Function Object) which is invoqued to attempt the emission
       ...
       - query_default(): Sets the query to its default value which specifies
         that this will emit traces for any instance and any method at level 5
         (Notice).
     ...
     Trace Levels:
       These are defined per RFC 5424 (syslog) severity levels with some
       extended levels:
     ...
     Trace Query:
       ...
       - _level (Integer): the level abvove which traces are consumed. If not
         specified the default is 5 (Notice).
       ...
       - _name   (String): the name of the instance for which traces are
         consumed. If not specifie, the expresion stands for "For Any Instance"
       
       - _method (String): the name of the method for which traces are consumed.
         If not specified, the expression means "For Any Method".
     ...
     - values ...: All other parameters are trace values defined as JavaScript
       ...
       Any attribute name that is already used in the trace Object will have
       its value override the previous value so care should be taken in
       choosing attribute names, at least to avoid collisions, one should
       avoid names starting with '_' and the attribute 'flow' should be
       reserved.
     ...
     Invocation of trace()
       ...
       This would emit a trace such as:
         {
           ...
           _realm    : 'order',                   // the realm used to invoque Trace()
       ...
       true if the trace query accepts traces at or above level 6 for the
  ...
  */
  function Trace( realm, emit, query ) {
    ...
    function trace() {
      ...
      while ( ++i < l ) {
        ...
        switch( ++_gathered ) {
          ...
          case 3: // method
            ...
            if ( instance_levels === ___ ) {
              instance_levels = _methods_instance_levels[ Trace.any_method ];
              
              if ( instance_levels === ___ ) return reset();
            }
            
            if ( no_trace_for_instance( instance_levels, _trace._name ) ) return reset();
          ...
          default:
            ...
            _gathered > 4 && ( trailer += '_' + _gathered - 3 );
      ...
      function no_trace_for_instance( instance_levels, instance_name ) {
        ...
        if ( typeof level != 'number' ) {
          level = instance_levels[ Trace.any_instance ];
    ...
    function query_default() {
      ...
      _instance_levels[ Trace.any_instance ] = default_trace_level;
      
      _methods_instance_levels[ any_method ] = {};
      
      _methods_instance_levels[ any_method ][ Trace.any_instance ] = default_trace_level;
    ...
    function query_add( query ) {
      ...
      query
        .forEach( function( term ) {
          ...
          if ( flow && flow != 'trace' ) return; // This term is not for a trace dataflow
          // flow is 'trace' or not defined
          
          var _realm = term.realm;
          
          if ( _realm && _realm != realm ) return; // There is a realm and it does not match this trace realm
          ...
          var name = term.name || Trace.any_instance;
          
          _instance_levels[ name ] = level;
          
          var method = term.method || Trace.any_method;
  ...
  Trace.any_instance        = 'Any Instance';
  Trace.any_method          = 'Any Method';
  Trace.default_trace_level = 5;
           ...
           __transform: function( values ) {
             ...
             *-/
             ...
             /* The above deserves some explanations.
                ...
                For more details on trace() see the documentation for Trace().
