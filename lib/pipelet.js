...
( function( exports ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
     Pipelet.methods( [ methods [, Super ] ] )
     
     Usage:
       Pipelet.methods( function( Super, trace ) {
         // Private class attributes common to all methods
         
         return {
           __transform: function( values ) {
             var _trace = this.__make_trace( trace, '__transform', 6 );
             
             _trace() && trace( { values: values.length } );
             
             // The following trace generates a trace level 6 (Informational), for this instance, for
             // method '__transform', and values
             trace( 6, this, '__transform', { values: values.length } );
             
             /* This emits a trace value which would look like:
                  {
                    ...
                    _pipelet  : 'order',                   // pipelet name
                    _name     : 'books',                   // pipelet instance name
                    _method   : '__transform',             // from the trace function
                    values    : 5                          // from trace( ..., { values: values.length } )
                  }
                
                When directed to console.log(), this trace would be displayed as:
                
                  2014/04/21 21:32:12.038 - 3, books( order ).__transform(), values: 5
                
                These traces can be consumed by a downstream trace dataflow consumer.
                
                The trace dataflow can be filtered to not only restrict the amount of trace
                displayed but also the amount of traces actually generated.
                
                To accomplish this, use on of the following conditional forms which are
                all semantically equivalent but execute faster if the trace is filtered
                by a downstream trace consumer:
             *-/
             
             trace >= 6 && trace( 6, this ) && trace( '__transform' ) && trace( { values: values.length } );
             ...
             *-/
             ...
             trace( 6, this, '__transform' ) && trace( { values: values.length } );
             
             trace( 6 ) && trace( this, '__transform' ) && trace( { values: values.length } );
             
             trace( 6 ) && trace( this ) && trace( '__transform' ) && trace( { values: values.length } );
             
             // Note that 'trace( 6 )' and 'trace > 6 && trace( 6 )' are semantically
             // equivalent but the later is faster when the trace is not consumed which
             // could be most of the time
             
             // Also note that trace > 0 and trace( 0 ) will always return true and stand
             // for traces always emited.
           ...
           *-/
           __make_trace: function( trace, method, level ) {
             return function( level, object ) {
               return trace > ( level || level = 6 )
                 && trace( level, this, method, object )
               ;
             }
           }
         } // methods
  ...
  */
  ...
  /* --------------------------------------------------------------------------
     trace_level, per RFC 5424 severity levels:
     ...
     8: Verbose debugging (not specified in RFC 5424):
          Additional information for developpers to diagnose hard issues.
  ...
  */
  ...
  /* --------------------------------------------------------------------------
     ...
     Returns a trace() function. Can be invoqued with or without new.
     
     Parameters:
       - realm (String): a domain name for traces emitted by this trace()
       - query: (Optional Array of Objects) initial query
  */
  function Trace( realm, query ) {
    var undefined
      , _trace
      
      , trace_level = 0
      
      , _instance_levels         = { 'Any Instance': 5 }
      , _methods_instance_levels = { 'Any Method': { 'Any Instance': 5 } }
      
      , gathered
    ;
    
    trace.valueOf = function() { return trace_level };
    ...
    trace.set_level = function( level ) {
      trace_level = level;
      
      return this;
    }; // set_level()
    
    trace.set_query = function( query ) {
      ...
      query
        .forEach( function( term ) {
          ...
          var level = term.level || 5; // Notice
          
          level > trace_level && trace_level = level;
          
          var name = term.name || 'Any Instance';
          
          _instance_levels[ name ] = level;
          
          var method = term.method || 'Any Method';
          
          method = _methods_instance_levels[ method ] || ( _methods_instance_levels[ method ] = {} );
    ...
    trace = function() {
      ...
      while ( ++i < l ) {
        ...
        switch( ++gathered ) {
          case 1: // level
            if ( trace_level < value ) return reset();
            
            _trace._level = value;
          break;
          
          case 2: // name
            if ( typeof value._get_name == 'function' ) value = value._get_name();
            
            if ( ! check_instance_name( _instance_levels, value ) ) return reset();
          ...
          case 3: // method
            ...
            if ( instance_levels === undefined ) {
              instance_levels = _methods_instance_levels[ 'Any Method' ];
              
              if ( instance_levels === undefined ) return reset();
            }
            
            if ( ! check_instance_name( instance_levels, _trace._name ) ) return reset();
            
            _trace._method = value;
          break;
          
          default:
            var trailer = '';
            
            gathered > 4 && ( trailer += '_' + gathered - 3 );
      ...
      function check_instance_name( instance_levels, instance_name ) {
        if ( ! instance_levels ) return false;
        
        var level = instance_levels[ instance_name ];
        
        if ( typeof level != 'number' ) {
          level = instance_levels[ 'Any Instance' ];
          
          if ( typeof level != 'number' ) return false;
        }
        
        return _trace._level >= level;
      } // check_instance_name()
  ...
  Pipelet.methods = function( methods, Super ) {
    if ( ! methods ) return this;
    
    if ( typeof methods == 'function' ) {
      methods = methods( Super, this.prototype );
    }
    ...
    this._methods 
    this._set_locales = null;
    
    var set_locales = methods.set_locales;
    
    if ( typeof set_locales == 'function' ) {
      this._set_locales = set_locales;
      
      var trace;
            
      var trace_reset = function() {
        var trace = {
          flow: '',
          'number': 0,
          'pipelet': null,
      ...
      debug = function() {
        ...
        while ( ++i < arguments.length ) {
          ...
          if ( type == 'object' ) {
            if ( argument instanceof Pipelet ) {
              type = 'pipelet';
            }
          }
          
          if ( typeof debug_parameters[ type ] == 'undefined' ) {
            debug_reset();
          }
          
          debug_parameters[ type ] = argument;
        }
      ...
      debug.toString = function() { return 3; } // 3 is the debug level
