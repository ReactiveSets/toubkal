...
( function( exports ) {
  ...
  */
  function Query( query ) {
    ...
    // Cache keys for query
    this.keys = query_get_keys( query );
  ...
  extend_2( Query.prototype, {
    ...
    */
    or: function( q1 ) {
      ...
      var q0 = this.query, result = []
        , i0 = -1, i1 = -1, e0, e1, keys0s = this.keys
      ;
      ...
      while ( e1 = q1[ ++i1 ] ) {
        ...
        while ( e0 = q0[ ++i0 ] ) {
          var keys0 = keys0s[ i0 ], keys, e
            , i = -1, key, less_restrictive = true
          ;
          
          // Based on the number of properties, determine which expression may be less restrictive, OR-wise than the other
    ...
    /* -----------------------------------------------------------------------------------------
      ...
      Exemples:
        new Query( [ { id: 1 }, { id: 2 } ] ).and_not( [ { id: 2 } ] )
        
          --> [ { id: 1 } ]
        
        new Query( [ { id: 1 }, { id: 2 } ] ).and_not( [ { id: 3 } ] )
        
          --> exception
        
        new Query( [ { id: 1 }, { id: 2 } ] ).and_not( [ { id: 2, name: 'test' } ] )
    ...
    */
    and_not: function( q1 ) {
      ...
      while ( e1 = q1[ ++i1 ] ) { // for all terms to remove
        ...
        if ( ! e0 ) throw new Error( 'Query..and_not() could not find term to remove: ' + log.s( e1 ) );
      }
    ...
    */
    and: function( q1 ) {
      ...
      // All variables with a trailing 0 correspond to the current query
      // All variables with a trailing 1 correspond to q1, the parameter query
      var u, q0 = this.query, result = new Query( [] ), i0 = -1, i1 = -1, p0, p1
        , keys0s = this.keys, keys1s = query_get_keys( q1 )
      ;
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     upstream using Query..and() and Query..or(). This is particularly useful for authorization
