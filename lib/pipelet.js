...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     A dataflow is a flow of operations which define the state of a set of JavaScript objects.
     Over time, the state of the set is modified as objects are added, removed or updated.
     
     Pipelets are typically assembled in a pipe fashion to describe applications e.g.:
       var new_york_city_sales_by_year = sales
         .filter( { city: "New York" } )
         .aggregate( [ { id: sales } ], [ { id: year } ] )
       ;
       
       Where:
         - the 'sales' pipelet defines a source dataflow providing sales objects,
         ...
         - the aggregate() pipelet delivers a dataflow of New York city sales by year which
           output reference is kept in the variable 'new_york_city_sales_by_year'.
         
       Each time sales are added to the sales dataflow, the 'new_york_city_sales_by_year'
     ...
     These four Operations are typically not used by application developpers who only need to
     assemble pipelets in order to describe applications as shown in the above example.
     
     On the other hand pipelet developpers must define these four operations by overloading all
     or some of these in derived classes of the Pipelet base class.
     ...
     in XS projects with different skills sets:
     ...
     The remaining of this section can be ignored by application developpers but must be
     mastered by pipelet developpers.
     ...
     should be propagated by all pipelets and may serve two possible purposes:
     ...
     executed before its corresponding add.
     ...
     will fetch their source content. This can be achieved by deriving from 'Set', the base
     stateful pipelet.
  ...
  */
  function Query_Tree( options ) {
    this.top = this.new_node()
    
    return Pipelet.call( this, options );
  } // Query_Tree()
  
  Pipelet.subclass( Query_Tree, {
    new_node: function() {
      return {
      ...
      function add_term( node, term, term_keys ) {
        while( term_keys.length ) {
          ...
          for( var i = -1; key = keys[ ++i ]; ) {
            ...
            node = branch[ v ] || ( branch[ v ] = that.new_node() );
          ...
          node = branch[ v = term[ key ] ] = that.new_node();
        }
    ...
    route: function( operation, values, options ) {
      var top = this.top;
      ...
      function route_value( node, value ) {
      ...
      function emit( node, operation, no_more, transaction_id, options ) {
        ...
        // Emit this operation routed values to this node's recipients
        if ( no_more ) {
        ...
        function _emit( operation, values, options ) {
          ...
          for ( var i = -1; ++i < rl; )
            recipients[ i ][ operation ]( values, options );
