...
( function( exports ) {
  ...
  */
  ...
  var Query_Tree_Prototype = {
    ...
    // ToDo: handle update and clear operations.
    ...
    query_tree_emit: function( operation, values, options ) {
      ...
      for ( var i = -1, v; v = values[ ++i ]; ) pushed = [], emit_value( top, recipients, v );
      ...
      function emit_value( node, recipients, value ) {
        ...
        if ( rl ) {
          ...
          // for this operation
          while( ++i < rl ) {
            var p = node.recipients[ i ];
            
            // ToDo: eliminate this loop by referencing each recipient values directly in node.recipients.
            for( var j = -1, l = recipients.length; ++j < l; ) {
  ...
  */
  function Dispatch( source, branches, branch_dataflow, options ) {
    ...
    var that = this
      , dispatcher = this.dispatcher = xs.pass_through( options )
      , gatherer   = this.gatherer   = xs.union( options )
      , _add_destination = dispatcher._add_destination
    ;
    
    dispatcher._add_destination = function( destination ) {
      // This only works if branch_dataflow() immediately calls _add_destination()
      // ToDo: find another solution because this will stop working on lazy connection
      // comming from clients.
      ...
      that.current_branch.input = destination;
      
      return _add_destination.call( dispatcher, destination );
    };
  ...
  extend( Dispatch.prototype, {
    ...
    create_branch: function( branch ) {
      ...
      destination && ( this.current_branch.output = destination ).plug( this.gatherer );
      
      // Current branch should no longer be modified, set it to null
      // This forces an exception in path_through._add_destination() in case there is a bug reusing this current branch
