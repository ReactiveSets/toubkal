      ...
      if ( source ) {
        ...
        if ( ! this._lazy ) {
          ...
          source.fetch( function( values, no_more ) {
            ...
            var al = added.length, rl = removed.length;
            
            if ( al || rl ) {
              // process added and removed in a transaction
              de&&ug( 'Query changed, fetched source, added: ' + al + ', removed: ' + rl );
              ...
              if ( no_more ) {
                var last_options = t.next().get_emit_options();
                
                rl && that.remove( removed, al ? o : last_options );
                al && that.add   ( added  ,          last_options );
                
                that._transactions.end_transaction( t );
              } else {
                rl && that.remove( removed, o );
                al && that.add   ( added  , o );
              }
            }
          } );
