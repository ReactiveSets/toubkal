...
( function( exports ) {
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    _transaction_start: function( count, options ) {
      if ( count == 0 ) {
        ...
        de&&ug( 'Pipelet.._transaction_start(), count is zero, more: ' + more );
    ...
    */
    notify: function( transaction, options ) {
      ...
      var t = this._transaction_start( l, options );
    ...
    */
    update: function( updates, options ) {
      ...
      de&&ug( 'Pipelet..update(), updates: ' + l );
      
      if ( l ) {
        var t = this._transaction_start( 2, options )
          , options_more = t.next_options()
          , i = -1, update
        ;
  ...
  */
  function Set( a, options ) {
    ...
    this._added = []; // to group _add_value()
  ...
  Pipelet.build( 'set', Set, {
    ...
    */
      ...
      if ( value ) {
        ...
        if ( b.length ) {
          ...
          if ( p != -1 ) {
            ...
            return this._no_value( transaction );
          }
        }
        ...
        if ( emit_now ) return this.emit_add( [ value ], options );
        
        this._added.push( value );
        
        if ( ! transaction.count ) {
          this.emit_add( this._added, options );
    ...
    // ToDo: test _add_values()
    _add_values: function( transaction, values, emit_now ) {
      var i = -1, v, b = this.b, added = [], l;
      
      if ( values && ( l = values.length ) ) {
        while( b.length && ++i < l ) {
          // There are values in the antistate b, waiting for an add or
          // update, or conflict resolution
          var p = this._index_of( v = values[ i ] );
        ...
        if ( added.length ) {
          ...
          if ( emit_now ) return this.emit_add( added, options );
          
          this._added.push.apply( this._added, added );
    ...
    _remove_value: function( transaction, value ) {
      if ( value ) {
        ...
        if ( p === -1 ) {
          ...
          de&&ug( 'Set.._remove_value(), options: ' + log.s( options ) ); // display count after emit()
          
          return this.emit_remove( [ value ], options );
        }
      }
    ...
    // ToDo: _remove_values(): add tests
    _remove_values: function( transaction, values ) {
      if ( values && values.length ) {
        ...
        if ( removed.length ) {
          ...
          de&&ug( 'Set.._remove_values(): values: ' + values.length + ', options: ' + log.s( options ) ); // display count after emit()
    ...
    _no_value: function( transaction ) {
      ...
      if ( ( transaction.count == 0 && this._added.length ) || ! options.more ) {
        this.emit_add( this._added, options ); // ToDo: add test for _no_value()
