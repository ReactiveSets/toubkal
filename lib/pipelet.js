...
( function( exports ) {
  ...
  */
  ...
  Pipelet.build( 'set', Set, {
    ...
    */
    // ToDo: allow synchronous mode to regroup operations after _add_value(), _remove_value(), _update_value()
    add: function( values, options ) {
      var l = values.length
      ...
      if ( values && values.length ) {
        for ( var i = -1, l = values.length, removed = []; ++i < l; ) {
          ...
          if ( p === -1 ) {
            ...
            throw new Error( 'Set.._remove_values(), value not found' );
          }
    ...
    */
    // ToDo: rewrite to work with _add_value() and _remove_value()
    update: function( updates, options ) {
