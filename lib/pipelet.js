...
( function( exports ) {
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    add_source: function( source ) {
      ...
      return source.is_void ? this : this
        ._add_source              ( source ) // ToDo: consider merging this and next
        ._add_source_destination  ( source )
        ._add_source_query        ( source )
        ._fetch_source_destination( source )
      ;
    }, // add_source()
    ...
    /* ------------------------------------------------------------------------
       ...
       ToDo: tests for remove_source and unplug
    */
    remove_source: function( source, options ) {
      return source.is_void ? this : this
        ._fetch_source_destination ( source, extend_2( { operation: 'remove' }, options ) )
        ._remove_source_query      ( source )
        ._remove_source_destination( source ) // ToDo: consider merging this and next
    ...
    */
    _add_source: function( source ) {
      ...
      if ( s ) {
        ...
        // Need to insert a union because there is already one source
        var union = xs.union( [], { name: this._get_name() + ' (union)' } );
        
        de&&ug( '_add_source(), insert ' + union._get_name() );
        
        // Do not update s' query tree because Union is just a controllet
        
        this
          ._remove_source            ( s )
          ._remove_source_destination( s )
          ._add_source               ( union )
          ._add_source_destination   ( union ) 
        ;
        
        union
          ._add_source            ( s )
          ._add_source_destination( s )
        ;
        
        this._add_source = function( source ) {
          union._add_source( source );
          
          return this;
        };
        
        this._remove_source = function( source ) {
          union._remove_source( source );
    ...
    */
    _add_source_destination: function( source ) {
      source._add_destination && source._add_destination( this );
    ...
    */
    _add_source_query: function( source ) {
      var q = this._query;
    ...
    /* ------------------------------------------------------------------------
       ...
       Parameters:
         source (Pipelet): to add query and destination to
    */
    _remove_source_query: function( source ) {
      var q = this._query;
    ...
    /* ------------------------------------------------------------------------
       ...
       be overloaded to allow redirections of the fetch output other pipelets
       than this and as done by controllets.
       
       Parameters:
         - source
           - implementing _transactional_fetch():
               the source pipelet to fetch from for a destination of this
               pipelet
    ...
    */
    _fetch_source_destination: function( source, options ) {
      ...
      if ( source._transactional_fetch ) {
        ...
        // This source does not provide a fetch method, it should be an Array
        ...
        this[ ( options && options.operation ) || 'add' ]( source, only_more( options ) );
      }
  ...
  * /
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     _add_source_query              null         Called by add_source()
     _remove_source_query           null         Called by remove_source()
  ...
  */
  ...
  Pipelet.Build( 'encapsulate', Encapsulate, {
    ...
    _add_destination: function( destination ) {
      this._output._add_destination( destination );
      
      return this;
    },
    
    _remove_destination: function( destination ) {
      this._output._remove_destination( destination );
    ...
    _add_source_query          : null,
    _remove_source_query       : null,
  ...
  */
  ...
  Pipelet.subclass( Controllet, function( Super ) {
    return {
      ...
      // Route _add_source_query() to all destinations
      _add_source_query: function( source ) {
        ...
        while( d = destinations[ ++i ] ) {
          ...
          d._add_source_query( source );
        }
        
        return this;
        
        function get_name( that ) {
          return name = that._get_name( '_add_source_query' );
        }
      }, // Controllet.._add_source_query()
      
      // Route _remove_source_query() to all destinations
      _remove_source_query: function( source, options ) {
        ...
        while( d = destinations[ ++i ] ) {
          ...
          d._remove_source_query( source, options );
        }
        
        return this;
        
        function get_name( that ) {
          return name = that._get_name( '_remove_source_query' );
        }
      }, // Controllet.._remove_source_query()
