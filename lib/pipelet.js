...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Always return an Object with attributes:
       ...
       - transaction_id: from options.transaction_id
        
     Optional Parameter:
       - options: (Object)
       
     ToDo: add tests for XS.only_more()
  */
  function only_more( options ) {
    return options && ( options.more || options.transaction_id )
      ? { more: true, transaction_id: options.transaction_id }
      : { more: false }
    ;
  } // only_more(()
      ...
      switch( operation ) {
        case 'update':
          ...
          for ( i = -1, rl = recipients.length; ++i < rl; ) {
            ...
            // ToDo: handle more
            removed.length && p.remove( removed, options );
            added  .length && p.add   ( added  , options );
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    plug: function( pipelet, options ) {
      if ( pipelet._lazy ) options = extend( { lazy: true }, options );
    ...
    if ( pipelet === u ) {
      pipelet = ( function() {
        ...
        return function pipelet() {
          ...
          if ( that._lazy ) options = extend( { lazy: true }, options );
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     XS.Compose( 'aggregate_from', function( source, from, measures, dimensions, options ) {
       ...
       return source.encapsulate( input, output, XS.extend( { lazy: false }, options ) );
     } )
  ...
  */
  ...
  Compose( 'auto_increment', function( source, options ) {
    ...
    return source.encapsulate( input, output, extend( { lazy: false }, options ) );
  } ); // auto_increment()
