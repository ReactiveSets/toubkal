...
( function( exports ) {
  ...
  */
  function only_more( options ) {
    return options && ( options.more || options.transaction_id )
      ? { more: !!options.more, transaction_id: options.transaction_id || XS.uuid_v4() }
      : { more: false }
    ;
  } // only_more()
  ...
  */
  ...
  var Query_Tree_Prototype = {
    init_query_tree: function() {
      // Initialize all pipelet properties here to avoid modifying v8 hidden classes
      ...
      switch( operation ) {
        case 'update':
          ...
          for ( i = -1, rl = recipients.length; ++i < rl; ) {
            ...
            updated.length && p.update( updated,                                 options );
          }
        return this;
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    notify: function( transaction, options ) {
      ...
      if ( l > 1 ) {
        var l1 = l - 1, options_more = more( options );
      ...
      this[ a.action ]( a.objects, no_more( options_more ) );
    ...
    */
    update: function( updates, options ) {
      var l = updates.length, update, i = -1;
      ...
      if ( l ) {
        ...
        this.remove( [ update[ 0 ] ], options_more );
        this.add   ( [ update[ 1 ] ], options      ); // last operation does not have more
      }
    ...
    */
    emit: function( event_name, values, options ) {
      ...
      de&&ug(
          ( name = 'emit( "' + event_name + '" )' + ( this.options.name ? ', name: ' + this.options.name : '' ) )
  ...
  */
  ...
  Pipelet.build( 'set', Set, {
    ...
    */
    ...
    _operation_start: function( count, options ) {
      if ( count == 0 ) {
      ...
      if ( count > 1 ) options = no_more( options_more = more( options ) );
      
      return { count: count, done: options, more: options_more };
    }, // _operation_start()
    ...
    */
    add: function( values, options ) {
      var l = values.length
        , operation = this._operation_start( l, options );
      ;
      ...
      while( b.length && ++i < l ) {
        ...
        if ( p == -1 ) {
          this._add_value( operation, v );
        } else {
          // Remove this add from the anti-state
          b.splice( p, 1 );
          
          this._no_value( operation );
        }
      }
      
      while( ++i < l ) this._add_value( operation, values[ i ] );
      
      return this;
    }, // add()
    
    _add_value: function( operation, values ) {
      de&&ug( 'Set.._add_value(): values: ' + log.s( values ) );
      
      if ( values ) {
        var options = --operation.count ? operation.more : operation.done;
        
        if ( values instanceof Array ) {
          push.apply( this.a, values )
          
          this.emit_add( values, options );
        } else {
          this.a.push( values );
    ...
    _no_value: function( operation ) {
      de&&ug( 'Set.._no_value(): operation: ' + log.s( operation ) );
      
      var done = operation.done;
      
      // Emit if this is the end and there is no more data comming from upstream
      --operation.count || ( done && done.more ) || this.emit_add( [], done ); // ToDo: add test for _no_value
    ...
    */
    remove: function( values, options ) {
      ...
      for ( var i = -1; ++i < l; ) {
        ...
        if ( p === -1 ) {
          ...
          this._remove_value( operation, v );
        }
      }
    ...
    _remove_value: function( operation, values ) {
      de&&ug( '_operation_emit_remove(): values: ' + log.s( values ) );
      ...
      if ( values ) {
        ...
        return this.emit_remove( removed, --operation.count ? operation.more : operation.done );
      } else {
        this._operation_no_value( operation );
      }
    }, // _remove_value()
  ...
  */
  ...
  Compose( 'auto_increment', function( source, options ) {
    ...
    // encapsulate()
    var output = input.set( { lazy: true } );
