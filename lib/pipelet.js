  ...
  extend( p, {
    ...
    */
    update: function( updated, options ) {
      var l = updated.length, removed, added, more_options = more( options );
      ...
      removed.length && this.emit_remove( removed, added_length ? more_options : options );
      added_length   && this.emit_add   ( added  , options );
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       ...
       Emit a 'standard' event. These events are not propagated downstream and
       therefore must be listened directly on the pipelet.
    ...
    */
    emit: function( event_name ) {
      var listeners = ( this.listeners || ( this.listeners = {} ) )[ event_name ];
      
      var l = listeners.length, a = slice.call( arguments, 1 );
      
      de&&ug( 'emit ' + event_name + ', listeners: ' + l );
    ...
    */
    ...
    on: function( event_name, handler, that ) {
      ...
      (    ( this.listeners || ( this.listeners = {} ) )[ event_name ]
        || ( this.listeners[ event_name ] = [] )
      ).push( { handler: handler, that: that || this } );
  ...
  */
  ...
  Pipelet.build( 'trace', Trace, {
    log: function( method, object ) {
      var s = object ? ', ' + JSON.stringify( object, void 0, '  ' ) : '';
      
      log( 'xs ' + this.name + '.' + method + s );
    ...
    _fetch_source: function( receiver ) {
      ...
      if ( s ) {
        ...
        this.log( 'fetch(), no source', { values: 'undefined', no_more: true } );
  ...
  */
  ...
  Pipelet.build( 'delay', Delay, {
    _fetch_source: function( receiver ) {
      ...
      setTimeout( function() {
        Pipelet.prototype._fetch_source.call( that, receiver )
      }, this.timeout );
    ...
    remove: function( values, options ) {
      ...
      return setTimeout( function() {
        this.emit_remove( values, options )
      }, this.timeout() )
    }, // remove()
