  ...
  Output.subclass( Query_Tree_Output, {
      ...
      */
      ...
      for ( var i = -1; subscribers_operation = subscribers_operations[ ++i ]; ) {
        ...
        while ( r = subscribers[ ++j ] ) {
          ...
          // this was the last emission for this transaction, if any
          if ( transaction ) {
            // ToDo: end_transaction() should not emit anything and should not terminate at the destination
            // until all sources are done with this transaction
    ...
    add_destination_to_transaction: function( transaction, input, transaction_id ) {
      ...
      /* 
         ...
         Example graph:
           ...
           source
             .tag_transactions( 'diamond-1' )
             .fork( [ ... ] )
             .union( { transaction_tag: 'diamond-1' } )
           ;
         
         The union would remove the 'diamond-1' tag from the transaction's
         tags.
      ...
      */
      ...
      var source_transactions = input.source_transactions
      ...
      */
      ...
      if ( source_transaction ) {
        ...
        for ( j = -1, sl = source_transaction.length; ++j < sl; ) {
          if ( source_transaction[ j ].source === this ) return this; // already added, also prevents graph loops issues
        }
      ...
      // Add input into the list of inputs that have received data for this transaction and source
    ...
    end_source_transaction: function( transaction, transaction_id, options ) {
      ...
      while ( input = transaction[ ++i ] ) {
        ...
        de&&ug( 'end_source_transaction(), transaction id: ' + transaction_id + ', source_transaction length: ' + sl );
        
        for ( j = -1; ++j < sl; ) {
          if ( source_transaction[ j ].source === this ) {
            source_transaction.splice( j, 1 );
        ...
        if ( not_found ) {
          // There is a bug in either this.add_destination_to_transaction() or this.emit()
    ...
    // ToDo: complete implementation of end_source_transactions() and call on Input..__remove_source()
    end_source_transactions: function( source ) {
      // ToDo: move to Input
      var input = this.pipelet_input
        , source_transactions = input.source_transactions
      ...
      for ( transaction_id in source_transactions ) {
        ...
        if ( source != source_transaction.source ) continue;
        ...
        source._add( [], { transaction_id: transaction_id } );
      }
      
      return this;
    }, // end_source_transactions()
