  ...
  */
  ...
  var Query_Tree_Prototype = {
    ...
    query_tree_add: function( or_terms, recipient ) {
      ...
      function add_term( node, term, term_keys ) {
        while( term_keys.length ) {
          ...
          branch = branches[ key ] = Object.create( null ); // allows to use 'in' without testing for hasOwnProperty()
    ...
    query_tree_emit: function( operation, values, options ) {
      ...
      for ( i = -1; v = values[ ++i ]; ) route_value( top, recipients, v, [] );
      ...
      function route_value( node, recipients, value, pushed ) {
        ...
        for( i = -1; key = keys[ ++i ]; ) {
          if ( ( v = value[ key ] ) !== u && ( child = branches[ key ][ v ] ) ) {
            ...
            route_value( child, recipients, value, pushed );
          }
