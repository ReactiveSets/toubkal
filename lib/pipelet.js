...
/* -------------------------------------------------------------------------------------------
   ...
   Many Sets per Pipelet, and the 'model' Attribute:
   ------------------------------------------------
   ...
   To differentiate between sets the 'model' attribute is used, while the model() pipelet
   allows to filter operations for a single model.
   ...
   Stateless pipelets managing many sets simultaneously, may use the 'model' attribute
   ...
   there are sets. Each set is associated with a 'model' and a key that allows to manage these
   antistates.
...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     A query is a set (an Array of Objects) representing an 'OR-AND' expression, where each
     object is an operand of the OR expression and each property of each object is an operand
     of the sub-AND expression.
     ...
     Example: The set
       ...
       Will be interpreted as the query: Objects where
         ( model === 'users' &&  id === 1092 ) || ( model === 'stores' && id === 231 )
     
       Which is also equivalent to an SQL-where clause:
         ( model = 'users' AND id = 1092 ) OR ( model = 'stores' AND id = 231 )
  ...
  */
  ...
  extend( Query.prototype, {
    /* -----------------------------------------------------------------------------------------
       ...
       Example:
         new Query( [ { model: 'store' } ] ).or( [ { model: 'user' } ] )
         
         results in the query:
           [ { model: 'store' }, { model: 'user' } ]
       
       Optimized Example:
         new Query( [ { model: 'store', id: 1465 } ] ).or( [ { model: 'store' } ] )
    ...
    */
    ...
    /* -----------------------------------------------------------------------------------------
       ...
       Example:
         new Query( [ { model: 'store', id: 1465 }, { model: 'store', id: 3678 } ] )
           .and( [ { id: 3678 } ] )
        ...
        Generates code such as:
          this.filter = function( values ) {
            ...
            for( var i = -1, l = values.length; ++i < l;  ) {
              ...
              if( _model === "store" ) { out.push( v ); continue; };
              if( _model === "user" && v.id === 231 ) { out.push( v ); continue; };
            } 
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     terms such as these queries:
       [ { model: 'user', id: 5 }, { model: 'store', id: 8 } ] for user_5
       
       [ { model: 'user', id: 7 }, { model: 'store', id: 8 } ] for user_7
       
       [ { model: 'user', id: 12 }, { model: 'store', id: 8 } ] for user_12
       
       [ { model: 'user', id: 3 }, { model: 'store', id: 2 } ] for user_3
       
     In this case, instead of evaluating the two queries sequentially, the Query_Tree allows to
     evaluate the model term first for both queries simultaneously, then branch out to two
  ...
  */
  ...
  var Query_Tree_Prototype = {
    ...
    query_tree_add: function( or_terms, options ) {
      var top = this.query_tree_top
        , recipient = options && options.recipient // All OR-terms MUST have one recipient and share this recipient
        , or_term, i = -1, that = this;
      ;
    ...
    query_tree_remove: function( or_terms, options ) {
      var top = this.query_tree_top
        , recipient = options && options.recipient // All OR-terms MUST have one recipient and share this recipient
        , or_term, i = -1, that = this;
      ;
    ...
    query_tree_emit: function( operation, values, options ) {
      ...
      for ( var i = -1, v; v = values[ ++i ]; ) emit_value( top, v );
      ...
      // Emit accumulated node.recipients_values from previous loop
      emit( top, operation, no_more, transaction_id, options );
      
      return this;
      
      // Route value, recursively
      function emit_value( node, value ) {
        var keys = node.keys, key
          , branches = node.branches
          , i = -1, v, child
        ;
        
        // Accumulate value into this node's recipients_values if there are recipients
        node.recipients.length && node.recipients_values.push( value );
  ...
  extend( p, Query_Tree_Prototype, {
    /* ------------------------------------------------------------------------
       ...
       Optional Parameters:
         - query: (Query)
    */
    fetch: function( receiver, query, recipient ) {
      var that = this;
    ...
    */
    _fetch_source: function( receiver, query, recipient ) {
      var u, s = this.source;
      
      if ( s ) {
        s.fetch( receiver, query, recipient );
      } else {
    ...
    if ( pipelet === u ) {
      pipelet = ( function() {
        ...
        return function pipelet() {
          ...
          // ToDo: add query option for fetch()
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Parameters:
       ...
       - context / constructor : used to call composition, if this is a function it is used
           as a class constructor which instance is the context. The constructor is created
           using the same parameters as the composition function.
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     ToDo: implement optimized dispatching using a xs.query_tree() instead of xs.fork().
  ...
  Pipelet.build( 'dispatch', Dispatch, {
    ...
    remove_branch: function( id ) {
      ...
      if ( branch ) {
        ...
        branch.destination && branch.destination.unplug( this.gatherer );
        
        branch.source && this.dispatcher.unplug( branch.source );
    ...
    add: function( values, options ) {
      this.dispatcher.add( values, options );
      
      return this;
    }, // add()
    
    remove: function( values, options ) {
      this.dispatcher.remove( values, options );
      
      return this;
    }, // remove()
    
    update: function( updates, options ) {
      this.dispatcher.update( updates, options );
      
      return this;
    }, // update()
    
    clear: function( options ) {
      this.dispatcher.clear( options );
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Parameters:
       ...
       - options: optional object to provide options for Set and parent class Pipelet:
         ...
         - auto_increment: (String or Boolean) the name of the field auto-incremented for each
             add operation. If set to true, the field is set to 'id'. Auto increment will
             prevent the set from being charding.
         ...
         - set_model: (String) the name of the model to set for objects of this set. 
             When this option is provided, all values added or removed are altered to add the
             attrinute 'model' with this string.
  */
  function Set( a, options ) {
    ...
    this.set_model = options.set_model;
    ...
    */
    add: function( values, options ) {
      ...
      if ( this.set_model ) {
        ...
        for ( i = -1; ++i < l; ) values[ i ].model = model;
      }
  ...
  */
  eval( XS.export_code( 'XS', [
    'xs', 'Compose', 'Pipelet', 'Fork', 'Union', 'Dispatch', 'Alter', 'Set', 'Trace', 'Delay', 'Query', 'Query_Tree'
  ] ) );
