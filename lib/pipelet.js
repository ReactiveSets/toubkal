...
( function( exports ) {
  ...
  extend( p, {
    ...
    */
    emit_operations: function( added, removed, updates, options ) {
      ...
      added   && added  .length && operations.push( { name: 'add'   , values: added   } );
      removed && removed.length && operations.push( { name: 'remove', values: removed } ); 
      updates && updates.length && operations.push( { name: 'update', values: updates } );
      
      var more = options && options.more || false
        , l = operations.length
        , i
        , that = this
      ;
      ...
      if ( l ) {
        if ( l > 1 ) {
          ...
          for ( i = -1, l1 = l -1; ++i < l1; ) emit( options_more );
        } else {
          i = 0
        }
      ...
      function emit( options ) {
        ...
        switch( operation.name ) {
          case 'add'   : return that.emit_add   ( operation.values, options ); 
          case 'remove': return that.emit_remove( operation.values, options );
          case 'update': return that.emit_update( operation.values, options );
          
          default:
            throw new Error( 'Pipelet..emit_operations(), invalid operation name: ' + name );
        }
      } // emit()
    }, // emit_operations()
  ...
  */
  ...
  /* --------------------------------------------------------------------------
     module exports
  */
  eval( XS.export_code( 'XS', [ 'xs', 'Compose', 'Pipelet', 'Fork', 'Union', 'Alter', 'Set', 'Trace', 'Delay' ] ) );
