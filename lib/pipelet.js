  ...
  */
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    ...
    _update_filter: function( options ) {
      var q = this._query
        , adds = q.adds, removes = q.removes
      ;
      
      if ( adds.length + removes.length == 0 ) return this;
      ...
      if ( source ) {
        de&&ug( 'Filter_Query.._update_filter(), adds: ' + log.s( adds ) + ', removes: ' + log.s( removes ) );
        
        // update source query tree
        source._query_tree_update( adds, removes, this );
        
        if ( ! this._lazy ) {
          // ToDo: update downstrem pipelets contents
          var that = this, added = [], removed = [];
          ...
          source.fetch( function( values, no_more ) {
            ...
            if ( no_more ) {
              ...
              // ToDo: stop caching added and removed and process emits in a bigger transaction
              var al = added.length && 1, rl = removed.length && 1;
              
              de&&ug( 'Filter_Query changed, fetched source, added: ' + added.length + ', removed: ' + removed.length );
      ...
      // Now that operations have been applied upstream of the pipelet, we can discard these
      q.clear_operations();
