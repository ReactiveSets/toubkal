...
( function( exports ) {
  ...
  extend( p, {
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       ...
       must be listened directly on the pipelet using on( event_name, handler )
       .
    ...
    */
    emit: function( event_name, values, options ) {
      var events = this.events, listeners, a = slice.call( arguments, 1 ), name;
      ...
      switch( event_name ) {
        ...
        case 'add': case 'remove': case 'update':
          var more = this.more = options && options.more || false
            , listeners = events.complete
            , no_complete = more || ! listeners
            , d = this.destination
          ;
          ...
          de&&ug( name
            ...
            + ', more: ' + more
            + ( ! no_complete ? ', complete listeners: ' + listeners.length : '' )
          );
      ...
      de&&ug( name + ', listeners: ' + l );
      
      try {
        for( var i = -1; ++i < l; ) {
          var listener = listeners[ i ];
          
          listener.handler.apply( listener.that, a );
        }
      } catch( e ) {
        exception( e )
      }
    ...
    */
    ...
    /* ------------------------------------------------------------------------
       on( event_name, handler [, that] )
       on_change( handler [, that] )
       
       Set a 'standard' event listener. This event handling method 
       ...
       Parameters:
         - event_name: (String) the name of the event.
             ...
             The event handler is then called with the following signatures:
               handler( 'add'   , values , options )
               handler( 'remove', values , options )
               handler( 'update', updates, options )
               handler( 'clear'          , options )
             
         - handler: (Function) will be called with the parameters emited by
             emit(), see emit() for more information.
             
         - that: (Object) optional, the context to call the handler, if not
             specified the context is this pipelet instance or in the case
             of the event 'change', the context of the pipelet created by
             on().
    */
    on: function( event_name, handler, that ) {
      if ( event_name == 'change' ) return this.on_change( handler, that );
      
      var events = this.events
        , listeners = events[ event_name ] || ( events[ event_name ] = [] )
      ;
      
      listeners.push( { handler: handler, that: that || this } );
    ...
    on_change: function( handler, that ) {
      de&&ug( 'on_change()'
        + ', name: "' + ( this.options.name || '' ) + '"'
      );
      ...
      p.add = function( values, options ) {
        handler.call( that, 'add', values, options );
        
        return this;
      };
      
      p.remove = function( values, options ) {
        handler.call( that, 'remove', values, options );
        
        return this;
      };
      
      p.update = function( updates, options ) {
        handler.call( that, 'update', updates, options );
        
        return this;
      };
      
      p.clear = function( options ) {
        handler.call( that, 'clear', options );
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     source.map_reduce( branches, compose [, options ] )
     
     Maps a source data-flow over a number of branches, then reduces the output of these
     branches to produce 'this' destination:
     
               Map              Reduce
                |                  |
                |---> branch 0 --->|
                |                  |
     source --->|---> branch i --->|---> destination
                |                  |
                |---> branch n --->|
                |                  |
            xs.fork()         xs.union()
     
     Map is implemented using xs.fork(), while Reduce uses xs.union().
     
     Applications:
       - Send a server data-flow to a number of clients connecting to the server with some
         socket protocol such as WebSockets, socket.io, or plain Unix sockets. Client dataflows
         are then collected back to the server and sent to 'this' destination.
         
       - Distribute a workload horizontally over multiple processes, servers, or worker
         threads using a Map/Reduce pattern.
      
     Parameters:
       - branches: (Pipelet) controls branches creation and removal. Each add operation spawns
           the creation of a new branch, while remove operations remove branches. Each branch
           has attributes:
             - id: (scalar) unique identifier for the branch
             - any other attribute required by compose()
         
       - compose: function( source, branch, options ), a function that is called when each
           branch is created and which is supposed to process the data-flow coming from the
           source. The composition returns its destination or undefined if it has no
           destination.
           
           The compose function will be called with the following parameters:
             - source : (Pipelet) the source for this branch
             - branch : (Object) the branch that spawned this pipelet from branches
             - options: (Object) the options of this map_reduce pipelet
             
           'This' context of compose is the map_reduce pipelet instance.
     ...
     Example:
       ...
       this example, branches are socket.io clients:
         ...
         var clients = xs
           .set( [ { ip_address: '0.0.0.0', port: 80 } ] )
           .http_servers()
           .socket_io_servers()
         ;
         ...
         xs.database()
           ...
           .map_reduce( clients, create_client )
         ...
         function create_client( source, client, options ) {
  ...
  */
  function Map_Reduce( branches, compose, options ) {
    Pipelet.call( this, extend( { name: 'map_reduce' }, options ) );
    ...
    this.map    = xs.fork ( options )
    this.reduce = xs.union( options )
    
    var that = this, map = this.map;
    
    map._add_destination = function( d ) {
      ...
      return Fork.prototype._add_destination.call( map, d );
    };
  ...
  Pipelet.build( 'map_reduce', Map_Reduce, {
    branches_change: function( operation, branches, options ) {
      switch( operation ) {
        ...
        // provide the branch parameter to compose() but we do not
    ...
    create_branch: function( branch ) {
      ...
      if ( this.branches[ id ] ) throw new Error( 'error, Map_Reduce()..create_branch(), branch: ' + id + ', already exists' );
      
      de&&ug( 'Map_Reduce()..create_branch(), id: ' + id );
      
      this.current_branch = this.branches[ id ] = {};
      
      var destination = this.compose( this.map, branch, this.options );
      
      if ( destination ) {
        this.current_branch.destination = destination;
        
        destination.plug( this.reduce );
      }
    ...
    remove_branch: function( id ) {
      ...
      if ( branch ) {
        de&&ug( 'Map_Reduce()..remove_branch(), id: ' + id );
        ...
        log( 'error, Map_Reduce..remove_branch(), branch: ' + id + ', does not exist' );
      }
    ...
    fetch: function( receiver ) {
      this.reduce.fetch( receiver );
      
      return this;
    }, // fetch()
    
    _add_source: function( s ) {
      this.map._add_source( s );
      
      return this;
    }, // _add_destination()
    
    _remove_source: function( s ) {
      this.map._remove_source( s );
      
      return this;
    }, // _remove_destination()
    
    _add_destination: function( d ) {
      this.reduce._add_destination( d );
      
      return this;
    }, // _add_destination()
    
    _remove_destination: function( d ) {
      this.reduce._remove_destination( d );
      
      return this;
    } // _remove_destination()
  } ); // Map_Reduce instance methods
  ...
  */
  ...
  /* --------------------------------------------------------------------------
     module exports
  */
  eval( XS.export_code( 'XS', [ 'xs', 'Compose', 'Pipelet', 'Fork', 'Union', 'Map_Reduce', 'Alter', 'Set', 'Trace', 'Delay' ] ) );
