...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Roadmap:
       ...
       become the highest priority once basic routing and authorizations are finalized.
       
       The highest priority for a new operator should be to implement the 'greater than'
       operator, to be able to fetch data past a certain date, or after a certain monotically
       increasing identifier. This might be required to implement replication, and recovering
       after a reconnection without requesting to retransmit every data previously received.
     ...
     Performance considerations:
       Query Tree performance is considered more important than Query operations performance.
       This is because query trees have to route a large number of data events while queries
       themselves should have a much lower rate of change in most applications.
  ...
  */
  function Query( query ) {
    ...
    this.optimized = []; // expressions optimized-out in ..and()
    this.optimized_keys = [];
  ...
  extend_2( Query.prototype, {
    ...
    */
    add: function( q1 ) {
      ...
      while ( e1 = q1[ ++i1 ] ) {
        ...
        while ( e0 = q0[ ++i0 ] ) {
          ...
          if ( less_restrictive ) {
            if ( keys === keys1 ) { // e1 is the less restrictive query
              ...
              continue; // there could be more expression in q0 more restrictive than e1
            }
            
            // e1 is either already in query or more restrictive than e0
            this.optimized.push( e1 );
            this.optimized_keys.push( keys1 );
            
            added = true;
            
            break; // if q0 is already optimized there should not be another expression more restrictive than e1
          }
    ...
    /* -----------------------------------------------------------------------------------------
      Query..remove( or_terms )
      
      Removes or-terms from query.
      
      These or-terms should have been added previously with add() or an exception is triggered.
      
      If or-terms have been optimized-out of the result query, these can still be removed
      ...
      Exemples:
        ...
        new Query( [ { flow: "user", id: 1 } ] )
          ...
          .remove( [ flow: "user", id: 1 ] ) // no excpetion triggered
          
      ToDo: Query..remove(), when removing expression, if other expressions were optimized-out
        by the removed expression, these optimized-out expressions should be added back to the
        query 
    */
    remove: function( q1 ) {
      var keys1s;
      ...
      remove( this.optimized, this.optimized_keys, q1, keys1s               );
  ...
  extend( p, Query_Tree_Prototype, {
    ...
    */
    _fetch_source: function( receiver ) {
      var s = this.source, q = this._query, query = q.query;
