...
( function( exports ) {
    ...
    if ( typeof f === 'function' && f.length !== u ) {
      if ( l === f.length ) {
        ...
        if ( typeof last !== 'object' ) {
          throw new Error( 'Pipelet.set_default_options(), expected last parameter to be an object but found ' + typeof last );
        }
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     !!! Important Warning:
     ---------------------
       The composition MUST have an "option" parameter, otherwise difficult to debug exceptions
       will occur such as '... has no method ...'. Because minification can optimize the options
       parameter out, programmers MUST use the options parameter in at least one function call,
       typically to pass the options to another pipelet. The example bellow will not have this
       issue because the options parameter is used in the filter() and aggregate() pipelets.
  ...
  */
  function Dispatch( source, branches, branch_dataflow, options ) {
    ...
    var that = this
      , dispatcher = this.dispatcher = xs.fork ( options )
     ...
     Parameters:
       ...
       - options: optional object to provide options for Set and parent class Pipelet:
         ...
         - auto_increment: (String or Boolean) the name of the field auto-incremented for each
             add operation. If set to true, the field is set to 'id'.
         ...
         - set_flow: (String) the name of the flow to set for objects of this set. 
             When this option is provided, all values added or removed are altered to add the
             attrinute 'flow' with this string.
     
     ToDo:
       - extract auto_increment as a stateless pipelet, this will remove the overhead for all
         other sets while providing a cleaner solution.
       - extract set_flow as a stateless pipelet for the same reasons
       - as a general rule, anything that can be implemented as a pipelet should be, unless
         there are significant performance issues. Options should be reserved for things that
         cannot be implemented as pipelet or when significant performance issues require merging
         features into the same pipelet.
  */
  function Set( a, options ) {
    ...
    if ( this.auto_increment = options.auto_increment ) {
      // auto_increment is true or an attribute name string
      if ( this.auto_increment === true ) this.auto_increment = this.key[ 0 ] || 'id';
      
      this.auto_increment_value = this.options.auto_increment_start || 0;
    }
    
    this.set_flow = options.set_flow;
  ...
  */
  extend( Set.prototype, {
    ...
    */
    add: function( values, options ) {
      ...
      if ( this.set_flow ) {
        var flow = this.set_flow;
        
        for ( i = -1; ++i < l; ) values[ i ].flow = flow;
      }
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     ToDo: add tests
     ToDo: make this update method the default for the Pipelet class as this is the semantically
       corect sollution for updates that should work in all cases. Any other implementation of
       update should be implemented only for optimization purposes
  */
  ...
  Set.build( 'unique_set', Unique_Set, {
    add: function( values, options ) {
      ...
      return added.length ? Set.prototype.add.call( this, added, options ) : this;
    }, // add()
  ...
  */
  eval( XS.export_code( 'XS', [
    'xs', 'Compose', 'Pipelet', 'Fork', 'Union', 'Dispatch', 'Alter', 'Set', 'Unique_Set', 'Trace', 'Delay', 'Query', 'Query_Tree'
  ] ) );
