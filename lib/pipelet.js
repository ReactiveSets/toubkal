...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     The QueryTree implementation can reduce this evaluation time when the queries have common
     ...
     In this case, instead of evaluating the two queries sequentially, the QueryTree allows to
     ...
     id 8 and 2:
      ...
      QueryTree will always find the list of all recipients after evaluating two terms, versus
      3 * n terms for the trivial dispatcher.
      
      Finding the full list of recipients using the QueryTree completes in O( t ) time vs
      ...
      Obviously the second is user-scallable while the first is not.
  ...
  */
  function QueryTree( options ) {
    return Pipelet.call( this, options );
  } // QueryTree()
  
  Pipelet.subclass( QueryTree, {
  } ); //  QueryTree instance methods
  ...
  */
  ...
  /* --------------------------------------------------------------------------
     module exports
  */
  eval( XS.export_code( 'XS', [ 'xs', 'Compose', 'Pipelet', 'Fork', 'Union', 'Dispatch', 'Alter', 'Set', 'Trace', 'Delay', 'Query' ] ) );
