...
( function( exports ) {
  ...
  extend( p, Query_Tree_Prototype, {
    /* ------------------------------------------------------------------------
       ...
       E.g.:
         ...
         this._get_name( 'fetch' ) --> "set.fetch(), "
    */
    _get_name: function( name ) {
      return this.options.name + ( name ? '.' + name + '(), ' : '' );
    }, // _get_name()
       ...
       Parameters:
         ...
         - f (Function): transaction processor, signature:
             f( transaction, count ):
               ...
               - transaction (Transaction) : to emit operations to
    ...
    */
    fetch: function( receiver, query ) {
      ...
      return this._fetch_source( rx, query );
    ...
    /* ------------------------------------------------------------------------
       ...
       Parameters:
         ...
         - options (Optional Object): options for the transactional fetch
             - more (Boolean): true if this fetch is part of a larger unfinished
                 transaction.
                 
             - transaction_id (String): transaction id of a larger transaction
                 this fetch is part of.
                 
             - operation (String): "add" or "remove", default is "add".
                 
                 This option is deleted after use.
             
             - count (Integer): the number of fetch operations after this one.
         ...
         - next (Optional Function): callback right after calling fetch to allow
             other fetch operations in the same transaction to proceed
    ...
    */
    _transactional_fetch: function( receiver_pipelet, query, options, next ) {
      ...
      function receiver( values, no_more ) {
        if ( ended ) {
          throw new Error( this._get_name( '_transactional_fetch' ) + 'already ended, no_more received twice from fetch()' );
        }
        ...
        receiver_pipelet[ operation ]( values, o );
      } // receiver()
    ...
    /* ------------------------------------------------------------------------
       ...
       Parameters:
         ...
         - query (optional Array of Objects): see fetch() for definition
    */
    _fetch_source: function( receiver, _query ) {
      var s = this._source
        , q = this._query
        , name
      ;
      ...
      function get_name( that ) {
        return name || ( name = that._get_name( '_fetch_source' ) );
      } // get_name()
    }, // _fetch_source()
    ...
    */
    fetch_all: function( receiver, query ) {
      ...
      if ( this.fetch === Pipelet.prototype.fetch ) {
        ...
        this._fetch_source_all( rx_all, query );
      } else {
        var chunks = [];
        
        this.fetch( rx_concat, query );
      }
      
      if ( out === u && receiver === u ) throw new Error( get_name() + "is asynchronous and no receiver function was provided" );
      ...
      function rx_concat( values, no_more ) {
        if ( out ) throw new Error( get_name() + "received extra chunck after no_more" );
      ...
      function get_name() {
        return name || ( name = that._get_name( 'fetch_all' ) );
      } // name()
    }, // fetch_all()
    
    _fetch_source_all: function( receiver, query ) {
      var chunks = [], out, u;
      
      this._fetch_source( rx_concat, query );
      
      if ( out === u && receiver === u ) throw new Error( "Pipelet..fetch_source_all() is asynchronous and no receiver function was provided" );
      
      return out;
      
      // ToDo: dry code with fetch_all()#rx_concat()
      function rx_concat( values, no_more ) {
        if ( out ) throw new Error( "Pipelet..fetch_source_all(): received extra chunck after no_more" );
    ...
    /* ------------------------------------------------------------------------
       ...
       ToDo: the name 'plug' might intuitively be understood as 'pluging this pipelet into a source (a socket)', which is what add_source() does, choose alternative names
         rename into add_destination()
    */
    plug: function( pipelet ) {
      de&&ug( this._get_name( 'plug' ) + 'pipelet: ' + pipelet._get_name() );
      
      return pipelet.add_source( this );
    }, // plug()
    ...
    /* ------------------------------------------------------------------------
       ...
       ToDo: rename into remove_destination()
    */
    ...
    /* ------------------------------------------------------------------------
       ...
       The content of the source is fetched and added to this pipelet unless
       the pipelet has no query (this._query is null)
       ...
       Parameters:
         - source: (Object) to add as a source to this pipelet.
             ...
             the content is fetched from the source and added to this pipelet
             using this.add().
    ...
    */
    add_source: function( source ) {
      ...
      // Add the source first so that, if a downsteam pipelet trys to fetch()
      ...
      if ( this._is_lazy() ) return this;
      
      // Add data from source
      if ( source._transactional_fetch ) {
        de&&ug( this._get_name( 'add_source' ) + 'fetching source: ' + source._get_name() );
        
        this._fetch_source_destination( source );
      } else {
        // This source does not provide a fetch method, it could be:
        // - An Array of JavaScript Objects
        // - any other value that this pipelet accepts in add()
        //
        // Array sources are used for static sets as input control ports
        //
        // These sources cannot be filtered or redirected
        //
        // ToDo: make source an output port to allow filtering
        //
        this.add( source );
      }
    ...
    /* ------------------------------------------------------------------------
       ...
       Calls low-level _remove_source_destination() and _remove_source() to
       remove the bidirectional link between the two pipelets.
    ...
    */
    remove_source: function( source, options ) {
      ...
      if ( ! ( options && options.no_fetch ) && source._transactional_fetch ) {
        this._fetch_source_destination( source, { operation: 'remove' } );
      }
    ...
    /* ------------------------------------------------------------------------
       ...
       Fetch a source for this pipelet destinations.
       
       This is a low-level function that should not be called directly but can
       be overloaded to allow redirections of the fetch output to another
       pipelet than this.
       
       This is done by Union, will be done by Filter and eventually all
       Controller Pipelets.
       
       Parameters:
         - source (Pipelet): the source pipelet to fetch from for a destination
           of this pipelet
    ...
    */
    _fetch_source_destination: function( source, options ) {
      source._transactional_fetch( this, this._query && this._query.query, options );
    ...
    */
    _add_source: function( source ) {
      ...
      if ( s ) {
        ...
        this._source = s = xs
          .union()
          ._add_destination( this, q && q.query ) // ToDo: allow redirections
          ._add_source( s )
        ;
    ...
    /* ------------------------------------------------------------------------
       ...
       The default implementation adds this as a destination to source using
       this._query.
       
       Derived pipelet can use this method to redirect source's destination
       to another pipelet as Union does.
    */
    _add_source_destination: function( source ) {
      var q = this._query;
      
      source._add_destination && source._add_destination( this, q ? q.query : [] );
    ...
    /* ------------------------------------------------------------------------
       ...
       Adds a subscription query for a destination.
    ...
    */
    _add_destination: function( destination, query ) {
      // ToDo: manage _destinations either in this or through query tree
      return this._query_tree_update( [], query, destination );
    }, // _add_destination()
    ...
    */
    ...
    // ToDo: document _query_tree_update() and _update_upstream_query()
    _query_tree_update: function( removes, adds, destination ) {
      // ToDo: guaranty synchronization between fetch() and _query_tree_update()
      ...
      rl && this.query_tree_remove( removes, destination );
      al && this.query_tree_add   ( adds   , destination );
      
      return this._update_upstream_query( removes, adds );
    }, // _query_tree_update()
    
    _update_upstream_query: function( removes, adds ) {
      var rl = removes.length, al = adds.length;
      
      if ( rl + al == 0 ) return this;
      ...
      if ( removes.length || adds.length ) {
        de&&ug( this._get_name( '_update_upstream_query' )
    ...
    /* ------------------------------------------------------------------------
       ...
       The default implementation removes this as a destination to source using
       this._query.
       
       See _add_source_destination() for more information.
    */
    _remove_source_destination: function( source ) {
      // disconnect from upstream source pipelet
      var q = this._query;
      
      source._remove_destination && source._remove_destination( this, q ? q.query : [] ); // ToDo: test proper query removal
  ...
  * /
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     _transform                     null         Called by add(), remove(), update(), fetch() receiver
     ...
     _add_source_destination        null         Called by add_source()
     _remove_source_destination     null         Called by remove_source()
     _fetch_source_destination      null         Called by add_source() and remove_source()
     ...
     query_tree_add                 null         Called by _query_tree_update()
     query_tree_remove              null         Called by _query_tree_update()
     _update_upstream_query         null         Called by _query_tree_update()
  */
  ...
  Pipelet.Build( 'encapsulate', Encapsulate, {
    ...
    fetch: function( receiver, query ) {
      this._output.fetch( receiver, query );
      
      return this;
    },
    
    fetch_all: function( receiver, query ) {
      this._output.fetch_all( receiver, query );
    ...
    _remove_destination: function( pipelet, query ) {
      this._output._remove_destination( pipelet, query );
    ...
    _update_upstream_query     : null,
    _transform                 : null,
    _add_source_destination    : null,
    _remove_source_destination : null,
    _fetch_source_destination  : null,
    query_tree_add             : null,
    query_tree_remove          : null,
    emit_add                   : null,
    emit_remove                : null,
    emit_update                : null,
    emit_clear                 : null,
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Exemple:
       ...
       xs.loop( tmp ).plug( tmp );
       
     ToDo: Make Pass_Through a Controller
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Base class for Controller Pipelets.
     
     A Controller controls the communication between it's source and it's destination(s) by
     updating its source Query_Tree outputs to point directly at destinations' input ports.
     
     Therefore a Controller pipelet does not process data operation events such as add, remove,
     update, clear and does not need input or output ports.
     
     Doing so allows controllers to get out of the data event path, improving performances, both
     CPU-wise and memory-wise because the controller's does not need a Query_Tree. The source
     Query_Tree is more fully used as it handles all destination pipelets of its downstream
     controllers, resulting in better overall performances.
     
     Controllers can be chained, redirecting traffic between a source and destinations accross
     several controllers:
     ...
     Eventually, Controllers could run on separate servers while controlling data event traffic
     between other groups of servers.
     
     The first implemented controller is Union, the next candidate is Filter, at least for the
     non-functional version, using queries.
  */
  ...
  Pipelet.subclass( Controller, function( Super ) {
    return {
      // Keep track of all destinations
      _add_destination: function( destination, query ) {
        var destinations = this._destinations;
        
        destinations.indexOf( destination ) == -1 && destinations.push( destination );
        
        return Super._add_destination.call( this, destination, query );
      }, // Controller.._add_destination()
      ...
      _remove_destination: function( destination, query ) {
        ...
        return Super._remove_destination.call( this, destination, query );
      }, // Controller.._remove_destination()
      ...
      // Route _add_source_destination() to all destinations
      _add_source_destination: function( source ) {
        ...
        while( d = destinations[ ++i ] ) {
          ...
          d._add_source_destination( source );
        }
        
        return this;
        
        function get_name( that ) {
          return name || ( name = that._get_name( '_add_source_destination' ) );
        }
      }, // Controller.._add_source_destination()
      
      // Route _remove_source_destination() to all destinations
      _remove_source_destination: function( source, options ) {
        ...
        while( d = destinations[ ++i ] ) {
          ...
          d._remove_source_destination( source );
        }
        
        return this;
        
        function get_name( that ) {
          return name || ( name = that._get_name( '_remove_source_destination' ) );
        }
      }, // Controller.._remove_source_destination()
      
      // Route _fetch_source_destination() to non-lazy destinations
      _fetch_source_destination: function( source, options ) {
        var i = -1, destinations = this._destinations, d, name;
        ...
        while( d = destinations[ ++i ] ) {
          // Fetch only for non-lazy destinations
          
          if ( d._is_lazy() ) continue;
          
          de&&ug( name + 'destination: ' + d._get_name() );
          
          d._fetch_source_destination( source, options );
        }
        
        return this;
        
        function get_name( that ) {
          return name || ( name = that._get_name( '_fetch_source_destination' ) );
        }
      }, // Controller.._fetch_source_destination()
      
      // Route _query_tree_update to source
      _query_tree_update: function( removes, adds, destination ) {
        var rl = removes.length, al = adds.length;
        ...
        if ( rl || al ) {
          ...
          source._query_tree_update && soure._query_tree_update( removes, adds, destination );
        }
        
        return this;
      } //  Controller.._query_tree_update()
    }; // Controller instance methods
  } ); // Controller methods() maker
      ...
      function fetch( source ) {
        ...
        function rx( values, no_more ) {
          if ( no_more ) {
            ...
            if ( complete ) {
              throw new Error( this._get_name( 'fetch' )
  ...
  */
  function Dispatch( source, branches, branch_dataflow, options ) {
    ...
    branches // ToDo: implement branches using a proper pipelet port instead of on() + fetch(), see filter.js for example
  ...
  extend_2( Dispatch.prototype, {
    _get_name: function( name ) {
      return ( this.options.name || 'dispatch' ) + ( name ? '.' + name + '(), ' : '' );
    }, // _get_name()
    ...
    create_branches: function( new_branches, options ) {
      ...
      for ( var i = -1, l = new_branches.length; ++i < l; ) {
        ...
        // ToDo: test initial fetch on output from gatherer
  ...
  */
  function Alter( transform, options ) {
    ...
    this._query = new Query( [ {} ] ).generate(); // pass-all
  ...
  Pipelet.Build( 'alter', Alter, {
    // ToDo: we probably don't need this fetch(), looks like the same as Pipelet.prototype.fetch()
    fetch: function( receiver, query ) {
      if ( query ) {
        var filter = new Query( query ).generate().filter;
        
        return Pipelet.prototype.fetch.call( this, rx );
      }
      
      return Pipelet.prototype.fetch.call( this, receiver );
    ...
    _update_upstream_query: function() {
      // Prevent upstream propagation
      ...
      // ToDo: implement option to provide a function for update_upstream_query()
      return this;
    } // _update_upstream_query()
  } );
  ...
  */
  ...
  Pipelet.Build( 'set', Set, {
    /* ------------------------------------------------------------------------
       ...
       See Pipelet.fetch() for receiver and query documentation.
    */
    fetch: function( receiver, query ) {
      ...
      if ( query ) {
        de&&ug( 'Set ' + this._get_name( 'fetch' ) + 'query: ' + log.s( query ) + ', values: ' + a.length );
        
        a = new Query( query ).generate().filter( a );
        
        de&&ug( 'Set ' + this._get_name( 'fetch' ) + 'filtered values: ' + a.length );
      }
    ...
    /* ------------------------------------------------------------------------
      _update_upstream_query( adds, removes )
      
      Pipelet default behavior: Updates upstream query and propagate uptream.
      
      This behavior: to not do anything, do not update query and do not
      propagate upstream. Sets therefore fetch all they can regarless of
      downstream pipelet needs.
      
      Filtering content stored in set must therefore be specified upstream
      of the set.
    */
    _update_upstream_query: function() {
      // Do not update this._query, do not propagate upstream
  ...
  */
  ...
  Pipelet.Build( 'trace', Trace, {
    ...
    add_source: function( source ) {
      this.log( 'add_source', { source: source._get_name() } );
      
      return Pipelet.prototype.add_source.call( this, source );
    }, // add_source()
    
    _fetch_source: function( receiver, query ) {
      var u, s = this._source, that = this, name = '_fetch_source';
      
      this.log( name, { source: s._get_name(), query: this._query && this._query.query } );
      
      if ( s ) {
        Pipelet.prototype._fetch_source.call( this, rx, query );
      } else {
  ...
  */
  ...
  Pipelet.Build( 'delay', Delay, {
    _fetch_source: function( receiver, query ) {
      ...
      setTimeout( function() {
        Pipelet.prototype._fetch_source.call( that, _receiver, query )
      }, delay );
