...
( function( exports ) {
  ...
  var log                 = RS.log
    ...
    , Code                = RS.Code
    , Query               = RS.Query
    , Query_Tree          = RS.Query_Tree
    , Event_Emitter       = RS.Event_Emitter
    , Transaction         = RS.Transaction
    , Transactions        = RS.Transactions
    , Input_Transactions  = RS.Input_Transactions
    , Output_Transactions = RS.Output_Transactions
    , Options             = RS.Options
  ...
  Plug.subclass( Input, {
    ...
    */
    ...
    _fetch_source_all: function( receiver, query ) {
      ...
      if ( out === u && receiver === u ) this.error( '_fetch_source_all', 'is asynchronous and no receiver function was provided' );
      
      return out;
      
      function rx_concat( values, no_more ) {
        if ( out ) that.error( '_fetch_source_all', 'received extra chunck after no_more' );
  ...
  Plug.subclass( Output, {
    ...
    */
    _fetch: function( receiver, query ) {
      ...
      p._input._fetch_source( rx, query );
    ...
    */
    fetch_all: function( receiver, query ) {
      ...
      if ( this._fetch === Output.prototype._fetch ) {
        ...
        p._input._fetch_source_all( rx_all, query );
      } else {
        var chunks = [];
  ...
  */
  Pipelet.Build = function( name, constructor, methods, pipelet ) {
    ...
    if ( ! pipelet ) {
      pipelet = ( function() {
        var constructor_apply = RS.make_constructor_apply( constructor );
  ...
  */
  var Compose = Pipelet.Compose = function ( name, composition ) {
    var constructor_apply = RS.make_constructor_apply( composition );
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     updating its sources' Query_Tree outputs to point directly at destinations' input ports.
  ...
  */
  function Dispatch( source, branches, branch_dataflow, options ) {
    ...
    var that = this
      ...
      , gatherer   = this.gatherer   = rs.union( options, { name: 'gatherer (union)' } )
    ;
  ...
  */
  function Trace( name, options ) {
    ...
    input._fetch_source = this.__input_fetch_source;
  ...
  Pipelet.Build( 'trace', Trace, {
    ...
    __input_fetch_source: function( receiver, query ) {
      var u, s = this.source, that = this, name = "_fetch_source", p = this.pipelet;
      
      p.log( name, { source: s._get_name(), query: this.pipelet._query && this.pipelet._query.query } );
      
      if ( s ) {
        Pipelet.Input.prototype._fetch_source.call( this, rx, query );
      } else {
  ...
  */
  function Delay( delay, options ) {
    ...
    this._input._fetch_source = this.__input_fetch_source;
  ...
  Pipelet.Build( 'delay', Delay, {
    __input_fetch_source: function( receiver, query ) {
      ...
      setTimeout( function() {
        Pipelet.Input.prototype._fetch_source.call( that, _receiver, query )
      }, delay );
  ...
  */
  RS.add_exports( {
    'Plug'             : Plug,
    'Pipelet'          : Pipelet,
    'rs'               : rs,  // ToDo: remove rs from the list once all modules load it directly from rs = require( lib/pipelet.js ) and get RS as rs.RS.
  ...
  typeof module == "object" ? module.exports = rs : exports.rs = rs;
