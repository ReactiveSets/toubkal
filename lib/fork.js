  ...
  */
  ...
  extend( p, {
    /* ------------------------------------------------------------------------
       ...
       This method must be defined by subclasses, or an exception will be
       triggered.
    */
    get: function() {
      var s = this.source;
    ...
    */
    set_source: function( source ) {
      ...
      if ( s ) {
        var r = s._remove_destination;
        
        if ( r ) {
          // disconnect from upstream source pipelet
          // r( this )
          s._remove_destination( this );
          // JHR, fixed, used to with _remove_destination() complaining about
          // the fact that it's "this" is null (that is what happens in nodejs
          // apparently, in the browser I think "this" would have been the
          // window object, not sure).I assumed the idea was to call 
          // _remove_destination only if it is defined. But...
          // Question: why would a source not define its _remove_destination()?
          // Answer 1 : because it does not derives from a class that does.
          // Question 2: why? don't all source derives from a class that do
          // define _remove_destination()?
          // Answer 2: yet, they do
          // Question 3: so, _remove_destination( is always defined, right?
          // Answer 3: yes, that's right!
          // Question 4: so, if _remove_destiation is always define, what is
          // the point in checking for its existance?
          // Answer 4: well, I can see that you are missing the point.
          // JHR: yes, I am missing the point, please explain it to me, is
          // for legacy reasons? or smething else? :)
        }
      ...
      if ( source && ! source.is_void ) {
        ...
        if ( source instanceof Pipelet ) {
          ...
          // source.fetch( function( values ) { that.add( values ) } );
        } else {
          this.add( source );
        }
      }
