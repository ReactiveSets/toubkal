...
undefine()( '', [ '../core/pipelet' ], function( rs ) {
  ...
  */
  rs.Compose( 'bootstrap_photo_album', function( source, photos_matrix_node, carousel_node, options ) {
    ...
    options = extend_2( { play: true, download: false, auto_start: false }, options );
    ...
    thumbnails_events = thumbnails_events
      ...
      .bootstrap_photos_matrix( photos_matrix_node, extend_2( { input_events: _carousel_events }, options ) )
    ...
    carousel_events = _carousel
      ...
      .bootstrap_carousel( carousel_node, extend_2( { input_events: thumbnails_events }, options ) )
  ...
  */
  function Bootstrap_Photos_Matrix( photos_matrix_node, options ) {
    ...
    Load_Images.call( that, options );
    
    // global options for css classes
    that._css_classes = extend_2( { images: 'img-responsive img-thumbnail' }, options.css_classes );
    
    that._thumbnails_count = 0;
    
    that._set_photos_matrix_node( photos_matrix_node );
    
    input_events && that._add_input(
      input_events,
      
      RS.Greedy.Input,
      
      options.name + '_input_events',
  ...
  Load_Images.Build( 'bootstrap_photos_matrix', Bootstrap_Photos_Matrix, {
    _set_photos_matrix_node: function( photos_matrix_node ) {
      ...
      function onclick( e ) {
        ...
        that.__emit_add( [ extend_2( { index: index }, that.a[ index ] ) ] );
  ...
  /* -------------------------------------------------------------------------------------------
     module exports
  */
  RS.Bootstrap_Photos_Matrix = Bootstrap_Photos_Matrix;
