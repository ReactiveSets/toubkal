...
*/
undefine()( 'bootstrap_carousel', [ '../core/pipelet' ], function( rs ) {
  'use strict';
  ...
  /* -------------------------------------------------------------------------------------------
     de&&ug()
  */
  var de = false, ug = de && log.bind( null, 'bootstrap carousel' );
  ...
  */
  Load_Images.Build( 'bootstrap_carousel', Bootstrap_Carousel, {
    ...
    _set_events_handlers: function() {
      ...
      input_events && input_events._output.on( 'add', function( values ) { move_to( values[ 0 ].index ) }, this );
    ...
    // display or hide controls depending on the number of the Carousel items
    _display_controls: function( stat  ) {
      var $controls = this._$controls;
      
      if( $controls ) {
        stat ? remove_class( $controls, 'hidden' ) : add_class( $controls, 'hidden' );
      }
    ...
    _add_value: function( transaction, value ) {
      ...
      this._display_controls( $inner.childNodes.length > 1 );
      
      this._start_carousel && this._output.fetch_all( inistialize_carousel );
      ...
      function inistialize_carousel( values ) {
        ...
        de&&ug( '_inistialize_carousel(), options:', that._carousel_options, '- auto_start:', that._options.auto_start );
    ...
    _remove_value: function( transaction, value ) {
      ...
      for( $item = $inner.childNodes[ 0 ]
        ; ( $image = $item.getElementsByTagName( 'img' )[ 0 ] ) && $image.getAttribute( 'rs_uri' ) !== value.uri
      ...
      if( $item ) {
        ...
        // if the removed item is active, display the next item
        if( has_class( $item, 'active' ) ) add_class( $item.nextSibling || $inner.childNodes[ 0 ], 'active' );
  ...
  /* --------------------------------------------------------------------------
     module exports
  */
  RS.add_exports( { 'Bootstrap_Carousel': Bootstrap_Carousel } );
