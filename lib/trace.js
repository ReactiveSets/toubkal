  ...
  */
  function Trace_Domain( realm, emit, query ) {
    ...
    function trace() {
      ...
      while ( ++i < l ) {
        ...
        switch( ++_gathered ) {
          ...
          case 3: // method
            var instance_levels
              =  _methods_instance_levels[ value ]
              || _methods_instance_levels[ Trace_Domain.any_other_method ]
            ;
    ...
    function query_default() {
      ...
      _instance_levels
        [ Trace_Domain.any_other_instance ]
        = _trace_level
      ;
      
      _methods_instance_levels
        [ Trace_Domain.any_other_method ]
        = {}
      ;
    ...
    function query_add( query ) {
      ...
      query
        .forEach( function( term ) {
          ...
          var level = term._level || Trace_Domain.default_trace_level;
          
          level > _trace_level && ( _trace_level = level );
          
          var name = term._name || Trace_Domain.any_other_instance;
          
          if ( ! _instance_levels[ name ] || level > _instance_levels[ name ] ) {
          ...
          if ( ! method[ name ] || level > method[ name ] ) {
            method[ name ] = level;
          }
        } )
      ;
