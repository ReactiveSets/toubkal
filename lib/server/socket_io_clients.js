...
( function( exports ) {
  ...
  */
          ...
          function on_rs_connection( _connection ) {
            ...
            if ( id ) {
              ...
              if ( connection = connections[ id ] ) {
                ...
                de&&ug( 'socket_io_server(), accept rs_connection as a reconnection: ' + log.s( _connection ) );
                ...
                de&&ug( 'socket_io_server(), connection id not found in rs_connection: ' + log.s( _connection ) );
              }
            } else {
              de&&ug( 'socket_io_server(), received rs_connection request: ' + log.s( _connection ) );
            }
            ...
            if ( connection ) {
              ...
              connections[ id ] = connection = { id: id, state: 'connected', socket: new RS.Socket_IO_Crossover( socket, { name: id } ) };
  ...
  /* --------------------------------------------------------------------------
     module exports
  */
  eval( RS.export_code( 'RS', [ 'Socket_IO_Clients' ] ) );
