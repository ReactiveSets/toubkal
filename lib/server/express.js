...
rs
  /* --------------------------------------------------------------------------
      @pipelet express_application( session_options )
      ...
      rs.express_application( session_options );
      ```
      
      @description
      This is a namespace-scoped singleton with one element.
  ...
  */
  .Singleton( 'express_application', function( source, session_options, options ) {
    let rs          = source.namespace()
      , application = express()
    ;
    ...
    return rs.set( [ { id: 1, application, session_options } ] );
  } ) // express_application()
;
