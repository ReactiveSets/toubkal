...
function Express_Session_Store( store_pipelet ) {
  ...
  // ToDo: controllets, unions cannot be used as objects to call operations, improve this API
  this.store_output = store_pipelet._output;
  
  if ( store_pipelet._input._is_a( 'Controllet.Input' ) ) {
    this.store = new RS.Pipelet( { name: 'express_session_store', key: store_pipelet._key } );
    this.store
      .through( store_pipelet )
    ;
  } else {
...
subclass( Store, Express_Session_Store, {
  get: function( id, done ) {
    if ( id ) {
      this.store_output.fetch_all( fetched, [ { id: id } ] );
    } else {
