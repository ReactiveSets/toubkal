...
Set.build( 'uglify', Uglify, {
  uglify: function() {
    ...
    this._fetch_source_all( function( files ) {
      ...
      if ( previous ) {
        Set.prototype.remove.call( that, [
          { name: name      , content: previous.code },
          { name: source_map, content: previous.map, mime_type: 'application/json' }
        ], { more: source_length !== 0 } );
        
        that.uglified = u;
      }
      
      if ( source_length ) {
        Set.prototype.add.call( that, [
          { name: name      , content: content.code },
          { name: source_map, content: content.map, mime_type: 'application/json' }
        ] );
        
        that.uglified = content;
      }
    } );
