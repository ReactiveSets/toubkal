    ...
    this._fetch_source_all( function( files ) {
      ...
      try {
        if ( source_length ) {
          ...
          code_sha1 = crypto.createHash( 'sha1' ).update( content.code );
          
          code_etag = code_sha1.digest( 'base64' );
          
          map_sha1 = crypto.createHash( 'sha1' ).update( content.map );
          
          map_etag = map_sha1.digest( 'base64' );
        ...
        if ( source_length ) {
          Set.prototype.add.call( that, that.uglified = [
            { name: name      , content: content.code, sha1: code_sha1, etag: code_etag },
            { name: source_map, content: content.map , sha1: map_sha1 , etag: map_etag , mime_type: 'application/json' }
          ] );
        }
      } catch( e ) {
