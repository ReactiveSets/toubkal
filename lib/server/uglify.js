...
/* -------------------------------------------------------------------------------------------
   de&&ug()
*/
var de = false, ug = RS.log.bind( null, 'uglify' );
...
Set.Build( 'uglify', Uglify, function( Super ) { return {
  _uglify: function( options ) {
    ...
    // Use _input.fetch_source instead of _fetch because files to uglify are from the source while
    // uglify() adds uglified and source map files.
    
    this._input.fetch_source( function( files, no_more ) {
      try {
