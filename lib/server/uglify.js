...
function Uglify( location, options ) {
  var u, options = Set.call( this, [], options ).options;
...
Set.Build( 'uglify', Uglify, {
  uglify: function() {
    var that = this, filenames = [], source_length = 0, ast;
    ...
    // Use _fetch_source instead of _fetch because files to uglify are from the source while
    // uglify() adds uglified and source map files.
    
    // ToDo: keep parsed ast for each file and re-parse only files which have changed
    this._fetch_source( function( files, no_more ) {
      try {
        ...
        // ToDo: use transaction or update()
        if ( previous ) {
