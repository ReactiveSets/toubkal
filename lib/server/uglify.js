...
function Uglify( location, options ) {
  var ___, options = Set.call( this, [], options )._options;
  
  // Keep function arguments by default, which is the opposite as UglifyJS default
  if ( options.keep_fargs === ___ ) options.keep_fargs = true;
...
Set.Build( 'uglify', Uglify, function( Super ) { return {
  _uglify: function( options ) {
    ...
    if ( t && t.more ) return this;
    
    var ___, that = this, filepaths = [], source_length = 0, ast;
    ...
    this._input.__fetch_source( function( files, no_more ) {
      try {
        ...
        options = this._end_transaction_options();
        ...
        if ( ast ) {
          ...
          if ( previous ) {
            Super._update.call( that, [ previous, that.uglified ], options );
          } else {
            Super._add.call( that, that.uglified, options );
          }
        } else {
          that.uglified = ___;
          
          if ( previous ) {
            Super._remove.call( that, previous, options );
          } else {
            // Nothing to emit, transaction should be closed
            if ( options._t && ! options._t.more ) {
              Super._add.call( that, [], options );
            }
          }
        }
      } catch( e ) {
  ...
  _start_transaction_options: function( options ) {
    var t = this._t;
    ...
    this._t = { transaction: t, options: options };
    
    return options;
  }, // _start_transaction_options()
  
  _end_transaction_options: function() {
    var transaction = this._t.transaction
      , options = transaction.next().get_emit_options()
    ;
    ...
    return options;
  }, // _end_transaction_options()
  
  _add: function( files, options ) {
    Super._add.call( this, files, this._start_transaction_options( options ) );
    
    return this._uglify( options );
  }, // _add()
  
  _remove: function( files, options ) {
    Super._remove.call( this, files, this._start_transaction_options( options ) );
