  ...
  rs.Compose( 'url_parse', function( source, options ) {
    ...
    function url_parse( value ) {
      ...
      if( protocol || rest.match( /^\/\/[^@\/]+@[^@\/]+/ ) ) {
        var slashes = value.slashes = rest.slice( 0, 2 ) === '//';
      ...
      if( host_end_index !== -1 ) {
        value.host = rest.slice( 0, host_end_index );
        
        rest = rest.slice( host_end_index );
      } else {
        value.host = rest;
      ...
      var hash_index   = rest.indexOf( "#" )
        , hash         = hash_index != -1 ? rest.slice(  hash_index  ) : ''
        , qs_index     = hash.indexOf( '?' )
        , query_string = qs_index   != -1 ? hash.slice( qs_index + 1 ) : ''
      ;
      
      // parse query string into an object
      if( parse_query_string ) value.query = qs_index != -1 ? parse_qs( query_string ) : {};
      
      if( qs_index   != -1 ) hash = hash.slice( 0, qs_index   );
      if( hash_index != -1 ) rest = rest.slice( 0, hash_index );
      ...
      function parse_host( host ) {
        ...
        if( at != -1 ) {
          out.auth = host.slice( 0, at );
          host     = host.slice( at + 1 ); // drop the @
        }
