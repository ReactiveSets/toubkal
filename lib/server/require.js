...
/* ----------------------------------------------------------------------------
    ...
    @short Resolves node modules ```"name"``` attribute using require.resolve()
...
rs.Compose( 'require_resolve', function( source, options ) {
  ...
  function resolve( module ) {
    ...
    try {
      ...
      log( 'require_resolve(), cannot resolve:', name, '- error:', e );
...
/* ----------------------------------------------------------------------------
    ...
    - **options** (Object): typically from dispatch, may include a @@transaction
      object, passed to initialize pipeline as third parameter.
...
rs.Compose( 'require_pipeline', function( source, module, options ) {
  var name     = 'require_pipeline(),'
    , path     = module.path
    , resolved
  ;
  
  log( name, path, 'options:', options );
  ...
  try {
    ...
    log( name, 'failed to load pipeline module:', path, ', error:', e, e.stack );
  }
  
  function clear_require_cache() {
    log( name, 'clear_require_cache(), path:', path );
