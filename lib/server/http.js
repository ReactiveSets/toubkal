...
function HTTP_Listen( listen_trigger, options ) {
  ...
  listen_trigger.greedy( options )._output.on( 'complete', function() {
    this.triggered = true;
...
/* --------------------------------------------------------------------------
    ...
    @parameters:
    - http_servers (Pipelet): output of pipelet http_servers().
    ...
    ### Source File attributes (all optional except path or uri)
    ...
    - get_content (Function): retrieves unavailable content asynchronously: function( error, content )
    - redirect      (Object):
