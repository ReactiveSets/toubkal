...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Get express module if not already loaded
  */
  var require_express = ( function() {
    var express;
    
    return function() {
      return express ? express : express = require( 'express' );
    }
  } )();
  ...
  */
  function HTTP_Server_Handler( endpoint ) {
    ...
    if ( e ) {
      ...
      // ToDo: handle virtual hosts
      this.listener = app;
      
      // Configure all middlewares
      for ( var i = -1, m; m = middlewares[ ++i ]; ) {
        // ToDo: test environement to configure middleware conditionally
        typeof m.configure == 'function' && m.configure( app );
      }
      
      // Add routes for all middlewares
      for ( i = -1; m = middlewares[ ++i ]; ) {
        typeof m.route     == 'function' && m.route    ( app );
      }
    }
  ...
  */
  ...
  Set.Build( 'http_servers', HTTP_Servers, {
    _add: function( end_points ) {
      ...
      return Set.prototype._add.call( this, servers );
    }, // add()
  ...
  */
  function Serve( http_servers, options ) {
    ...
    http_servers.serve_http_servers( this.handler, this, options );
