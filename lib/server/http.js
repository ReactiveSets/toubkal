  ...
  */
  ...
  Pipelet.build( 'serve', Serve, {
    handler: function( request, response, that ) {
      ...
      if ( file ) {
        var u, content = file.content, mime_type = file.mime_type;
    ...
    add: function( files ) {
      for ( var i = -1, l = files.length; ++i < l; ) {
        ...
        file.mime_type || ( file.mime_type = mime.lookup( location ) );
        
        this.files[ location ] = file;
        
        de&&ug( 'Serve.add(), location: ' + location + ', mime type: ' + file.mime_type );
      }
