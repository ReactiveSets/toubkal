    ...
    function Server( filter ) {
      ...
      this._listeners = {
        ...
        'request': function( request, response ) {
          filter( request ) && that.emit( 'request', request, response );
        },
      ...
      listen: function( http_server ) {
        ...
        this
          .on( 'newListener', function( event ) {
            ...
            de&&ug( name + 'newListener: ' + event );
            
            var listener = _listeners[ event ];
            
            listener && http_server.on( event, listeners[ event ] = listener );
          } ) // on newListener
          ...
          .on( 'removeListener', function( event ) {
            ...
            if ( listener ) {
              ...
              de&&ug( name + 'removeListener: ' + event + ', listeners: ' + listeners_count );
    ...
    return { // Virtual_HTTP_Servers.prototype
      _add: function( servers, options ) {
        ...
        servers.forEach( function( server ) {
          out.push( extend( {}, server, {
            handler    : new HTTP_Server_Handler(),
            http_server: virtual_http.createServer( filter ).listen( server.http_server )
          } ) );
        } );
        
        return Super._add.call( this, out, options );
      }, // _add()
      
      _remove: function( servers, options ) {
        var that = this, out = [];
        
        servers.forEach( function( server ) {
          var p = that._index_of( server );
        ...
        return Super._remove.call( this, out, options );
      } // _remove()
