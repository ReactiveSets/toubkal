          ...
          server: socketio
            ...
            .on( 'connection', function( socket ) {
              ...
              socket.on( 'disconnect', function() {
                ...
                var updated = extend( {}, connection, { state: 'disconnected' } );
              ...
              socket.on( 'xs_connection', function( _connection ) {
                ...
                if ( ! connection ) {
                  // ToDo: use uuid for connection ids to allow reconnections to another server
                  // as well as server restarts, without the risk of collisions
                  connection = { id: id = ++that.last_connection_id, state: 'connected', socket: socket };
                   
                  Set.prototype.add.call( that, [ connection ] );
                } else {
                  // update connection
                  
                  var updated = extend( {}, connection, { state: 'connected' } );
                ...
                socket.emit( 'xs_connection', { id: id } );
              } ) // on xs_connection
            } )
        };
