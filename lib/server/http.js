  ...
  Pipelet.build( 'serve', Serve, {
    ...
    add: function( files, options ) {
      for ( var i = -1, l = files.length; ++i < l; ) {
        var file = files[ i ], uri_path = file.uri_path || ( '/' + file.name );
        
        var mime_type = file.mime_type = file.mime_type || mime.lookup( uri_path );
        
        this.files[ uri_path ] = file;
        
        de&&ug( 'Serve.add(), uri_path: ' + uri_path + ', mime type: ' + mime_type );
      }
    ...
    remove: function( files, options ) {
      for ( var u, i = -1, l = files.length; ++i < l; ) {
        ...
        this.files[ file.uri_path || ( '/' + file.name ) ] = u;
      }
