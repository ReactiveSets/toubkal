  ...
  */
  ...
  Set.build( 'socket_io_clients', Socket_IO_Clients, {
    add: function( servers ) {
      var that = this, socket_io_servers = this.socket_io_servers;
      ...
      function start_server( address ) {
        ...
        io.configure( function() {
          ...
          de&&log( "socket_io_clients(), io.configure(), transports: " + log.s( options.transports ) );
        ...
        function on_connection( socket ) {
          ...
          function on_disconnect() {
            if ( connection ) {
              ...
              if ( options.remove_timeout ) {
                disconnection_timeout = setTimeout( remove_connection, options.remove_timeout * 1000 );
              } else {
                remove_connection();
              }
    ...
    remove: function( servers ) {
      var socket_io_servers = this.socket_io_servers;
      ...
      for ( var i = -1, l = servers.length; ++i < l; ) {
        ...
        if ( socket_io_servers[ address ] ) {
          log( 'socket_io_clients(), remove server at address ' + address );
        ...
        log( 'socket_io_clients(), error: remove no server at address ' + address );
      }
      ...
      .alter( function( file ) {
        ...
        if ( file.uri === u && name !== u ) {
          switch ( name.charAt( 0 ) ) {
            ...
            return file; // cannot convert absolute path to uri
          }
