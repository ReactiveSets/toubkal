...
/* -------------------------------------------------------------------------------------------
   de&&ug()
*/
var de = false, ug = log.bind( null, 'http' );
...
/* -------------------------------------------------------------------------------------------
   ...
   Example:
     rs.set( [ { id: 1, ip_address: '0.0.0.0', port: 80 } ] ).http_server()
*/
...
function HTTP_Listen( listen_trigger, options ) {
  ...
  listen_trigger.greedy( options )._output.on_complete( function() {
    this.triggered = true;
...
var virtual_http = ( function() {
  ...
  function Server( filter ) {
    switch( typeof filter ) {
      ...
      case 'object':
        if ( toString.call( filter ) === '[object Array]' ) {
          var hostnames = {};
  ...
  set: function( http_server, http_handlers ) {
    ...
    function set_http_handler( handler ) {
      ...
      switch( typeof routes ) {
        ...
        case 'object':
          if ( toString.call( routes ) === '[object Array]' ) {
...
function Serve_HTTP_Servers( http_handlers, options ) {
  ...
  switch( typeof http_handlers ) {
    ...
    case 'object':
      if ( toString.call( http_handlers ) === '[object Array]' ) {
...
/* --------------------------------------------------------------------------
   ...
   Parameters:
     - options:
       ...
       - cache_control_max_age: Cache control, default max age is
         Serve.max_age (1 hour) to avoid fetching the same asset twice in the
         same session, yet allow frequent updates.
         ...
         stored as this._max_age
       
       - session_options (Object): options for express-session. If present,
         invoke express-session middleware.
       
   File attributes (all optional except path or uri):
     - path          (String): file path, must be defined unless uri provided
     - uri           (String): served uri, defaults to ( '/' + path )
     - content       (String): plain text content, must be defined unless get_content() or redirect is provided
     - mime_type     (String): for Content-type, defaults to mime.lookup( uri )
     - etag          (String): defaults to crypto.createHash( 'sha1' ).update( content ).digest( 'base64' )
     - cache_control (String): defaults to 'public'
     - max_age      (Integer): for Cache-Control header, defaults to this._max_age (see options above)
     - gzip          (String): gzipped content, defaults to zlib.gzip()
     - deflate       (String): deflated content, defaults to zlib.deflate()
     - get_content (Function): retrieves unavailable content assynchronously: function( error, content )
     - redirect      (Object):
       - status_code  (Integer): defaults to 301
       - url           (String): redirect-to location url, must be defined
*/
...
Greedy.Build( 'serve', Serve, {
  handler: function request_handler( request, response, next ) {
    ...
    function send_error( error, error_code ) {
      ...
      de&&ug( 'Serve.handler()', { pathname: pathname, error_code: error_code, error: error } );
  ...
  _add: function( files, options ) {
    for ( var i = -1, l = files.length; ++i < l; ) {
      ...
      de&&ug( this._get_name( '_add' ), { uri: uri, mime_type: mime_type } );
    }
