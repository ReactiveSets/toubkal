...
/* ----------------------------------------------------------------------------
    @pipelet http_listen( listen_trigger, options )
    
    @short Listens to http servers when ```listen_trigger``` completes a @@transaction
    
    @parameters
    - **listen_trigger** (@class:Pipelet\): on complete, will trigger source
      http servers listening.
      
    - **options** (Object): optional @class:Pipelet options
    
    @examples
    - Listen to HTTP servers when ```files``` are ready.
...
function HTTP_Listen( listen_trigger, options ) {
  ...
  listen_trigger.greedy()._output.on( 'complete', function() {
    this.triggered = true;
