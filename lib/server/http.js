       ...
       - http_handlers:
         ...
         - (Function): a handler for the 'request' listener only. Equivalent to
           defining http_handlers as [
             { name: 'request', 'handler': http_handlers }
           ]
         
         - (Array of Objects): which Objects have the following attributes:
         
           - name (String): the name of the HTTP listener, which can be:
             - 'request'
             - 'checkContinue'
             - 'connect'
             - 'upgrade'
         
           - handler (Function): a function which signature depends on 'name':
     ...
     Examples:
       ...
       Using multiple instances of serve_http_servers() and a Connect application:
         ...
         http_servers
           ...
           .serve_http_servers( [
             { 'name': 'checkContinue', 'handler': check_continue_handler, 'routes': [ '/application', '/uploads' ] },
             { 'name': 'upgrade'      , 'handler': websockets_handler    , 'routes': '/sockets' }
           ] )
         ;
