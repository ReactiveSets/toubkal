...
var XS = require( './configuration.js' ).XS
  , xs = XS.xs
  , extend = XS.extend
  , log = XS.log
  , Pipelet = XS.Pipelet
...
function Send_Mail( configuration, options ) {
  ...
  configuration
    .filter( function( v ) { return 'nodemailer' == v.module } )
    
    //ToDo: refactor using filter query
    // .filter( [ { nodemailer: v.module } ] )
  ...
  add: function( emails, options ) {
    ...
    function send( email ) {
      ...
      transport.sendMail( email, function( error, response ) {
        if ( error ) {
          out.push( { model: 'email_send_error', id: id, error: '' + error } );
        } else {
          out.push( { model: 'sent_email', id: id } );
        }
        
        if ( --l ) return;
        
        that.emit_add( out, options )
      } );
    } // send()
  } // add()
