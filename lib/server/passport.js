...
/* -------------------------------------------------------------------------------------------
   ...
   - options (Object): Set options:
     - name (String): debugging name, can be set only during the firs invocation of
       passport_strategies()
...
Passport_Profiles.Output = Pipelet.Output.subclass(
  ...
  function ( Super ) { return {
    ...
    update_upstream_query: function( changes, input ) {
      this.source = null; // Disable upstream query updates
      
      Super.update_upstream_query.call( this, changes, input );
    } // Passport_Profiles.Output..update_upstream_query()
  }; }
); // Passport_Profiles.Output()
