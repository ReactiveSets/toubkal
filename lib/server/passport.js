...
/* -------------------------------------------------------------------------------------------
   ...
   Where attributes should be set as follows:
    - pipelet     : "passport_strategies"
    - module      : passport strategy required npm module name
...
function Passport_Strategies( users, options ) {
  ...
  var passport_strategies = rs
    ...
    .trace( 'strategies from config' )
    
    // join with the common strategies
    .join( common_strategies, [ [ 'id', 'id' ] ], join_strategies )
  ;
...
Set.Build( 'passport', Passport, function( Super ) { return {
  ...
  _make_passport_routes: function() {
    ...
    de&&ug( 'passport routes', that._passport_routes );
  ...
  _add: function( strategies, options ) {
    ...
    for( var i = -1; ++i < l; ) {
      ...
      if( ! routes[ strategy_route ] )
        passport_routes.push( { path: strategy_route, strategie_name: strategy_name, scope: strategy.scope, callbackURL: callbackURL } );
    } // for()
