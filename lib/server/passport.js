      ...
      if( ! user_provider_emails.length ) {
        ...
        var uuid   = uuid_v4()
          ...
          , emails = profile_emails
              .map( function( email ) {
                return extend_2( { flow: 'user_provider_email', user_id: uuid, provider_id: profile.id }, email )
              } )
        ;
        
        users._add( [ extend_2( { user_id: uuid, flow: 'user_provider_profile' }, profile ), user ].contact( emails ) );
        ...
        _user_provider_email._fetch( function( user_provider_emails ) {
          ...
          if( ! user_provider_emails.length ) {
            ...
            added = [ extend_2( { user_id: user_id, flow: 'user_provider_profile' }, profile ); ];
