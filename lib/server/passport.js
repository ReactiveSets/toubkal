...
Compose( 'passport_strategies_configuration', function( source, callback_base, options ) {
  ...
  function merge_strategy( configuration, common ) {
    ...
    if ( ! strategy.name ) {
      ...
      if ( pound == -1 ) {
        fatal( 'no pound (#) found in id: ' + id )
      }
      
      strategy.name = id.substr( id.indexOf( '#' ) + 1 );
      
      if ( ! strategy.name.length ) {
        fatal( 'no strategy name found in id: ' + id );
      }
    }
    ...
    function fatal( message ) {
      throw new Error( 'Passport configuration error, ' + message );
    } // fatal()
  } // merge_strategy()
...
/* -------------------------------------------------------------------------------------------
   ...
   Initialize passport strategies from source strategies description.
    ...
    function make_passport_routes() {
      ...
      function get_routes() {
        var routes          = this._routes
          , base_route      = this._options.base_route
          , redirects       = { successRedirect: base_route + '/profile', failureRedirect: base_route + '/login', scope: [ 'email' ] }
          , passport_routes = this._passport_routes
        ...
        for( var i = -1; ++i < len; ) {
          var route   = passport_routes[ i ]
            ...
            , scope   = route.scope
            , options = { scope: [ 'email' ] } // passport authentication options
          ;
