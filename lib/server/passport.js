    ...
    ```javascript
      ...
      var clients = http_servers.dispatch( rs.express_application(), function( source, options ) {
        rs
          ...
          .serve( source,
            { routes: [ '/lib', '/node_modules' ], session_options: this.session_options }
          )
        ;
        
        // Use session_options to initialize socket_io_clients
        return source
          .socket_io_clients( { session_options: this.session_options } )
        ;
      } );
...
rs
  ...
  */
  .Singleton( 'passport_instance', function( source, options ) {
    
    return source
      
      .namespace()
      
      .set( [ { id: 1, instance: new Passport() } ] )
    ;
  } ) // passport_instance()
  ...
  /* --------------------------------------------------------------------------
      ...
      @short Initialise passport instance express middleware and express router
      
      @emits
      - **id** (integer): 1
      - **application** (Object): express application instance
      ...
      - Pipelet passport_strategies_routes()
  */
  .Singleton( 'express_use_passport', function( source, passport_route, options ) {
...
rs.Singleton( 'passport_strategies_configuration', function( source, callback_base, options ) {
  ...
  function prepare_strategy( configuration ) {
    var strategy = extend( { lookup_user }, configuration )
...
/* -------------------------------------------------------------------------------------------
    @pipelet passport_strategies( passport_instance, options )
    ...
    - Pipelet trace()
*/
function Passport_Strategies( passport_instance, options ) {
  ...
  passport_instance.api.fetch( ( values ) => {
    ...
    if ( passport )
      this._passport_lib_instance = passport.instance
    ;
  } );
} // Passport_Strategies()
...
Set.Build( 'passport_strategies', Passport_Strategies, function ( Super ) {
  ...
  return {
    _add_value: function( t, strategy ) {
      ...
      that._passport_lib_instance.use( strategy_name, strategy_processor );
    ...
    _remove_value: function( t, strategy ) {
      ...
      if ( position != -1 ) {
        ...
        this._passport_lib_instance.unuse( strategy_name );
...
/* ----------------------------------------------------------------------------
    @pipelet passport_strategies_routes( passport_instance, options )
...
function Passport_Strategies_Routes( passport_instance, options ) {
  var that = this;
  ...
  passport_instance.api.fetch( function( values ) {
    let passport = values[ 0 ]
      , instance = passport && passport.instance
    ;
    
    if ( instance )
      that._passport_lib_instance = instance;
  } );
  ...
  __transform: function( values ) {
    ...
    values.forEach( function( strategy ) {
      var strategy_name          = strategy.name
        ...
        , passport_login_handler = that._passport_lib_instance.authenticate( strategy_name, {
      ...
      if ( callback_url ) {
        ...
        out.push( {
          ...
          handler: that._passport_lib_instance.authenticate(
            strategy_name,
