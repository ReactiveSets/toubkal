...
/* -------------------------------------------------------------------------------------------
   passport_strategies_menu( route, render, options )
   
   Strategies are first ordered by "order" attribute if present, or by "name".
   
   Then render() is called with all ordered strategies to return the menu.
   
   This is a stateful synchronous greedy pipelet.
   
   Parameters:
   - route    (String): e.g. /passport/login
   
   - render (Function): called to render menu on strategies change, signature:
     String render( strategies )
     - strategies (Array of strategies): from strategy configuration
     
     Must return a string with the html content of the menu page.
     
   - options (Object):
     - name (String): optional debuging name
   
   Usage:
     rs
       .passport_strategies_menu( 'passport/login', function( strategies ) {
         return
           '<html>' +
             '<head>' +
...
*/
Compose( 'passport_strategies_menu', function( source, route, render, options ) {
  var object_clone = extend.object_clone
    , name = options.name
  ;
  ...
  return rs
    ...
    .alter( function( strategies ) {
      strategies = object_clone( strategies.content );
    ...
    }, { no_clone: true, name: name + '_render' } )
  ;
} ); // passport_strategies_menu()
