...
Set.Build( 'passport_strategies', Passport_Strategies, function ( Super ) { return {
  ...
  _remove_value: function( t, strategy ) {
    var position      = this.index_of( strategy )
      , strategy_name = strategy.name
    ;
...
/* -------------------------------------------------------------------------------------------
   passport_routes( application_configuration, options )
   ...
   Parameters:
   - application_configuration (Function): called once to initialize express (or another
     framework). Signature: application_configuration( router ):
     - router (Function): express middleware use it: app.use( router() )
...
function Passport_Routes( application_configuration, options ) {
