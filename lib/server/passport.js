...
/* -------------------------------------------------------------------------------------------
   rs.passport_strategies( callback_url, options )
   
   Process strategies to configure Passport. Reads configuration then joins
   passport_strategies with common strategies.
   
   Usage:
     databace.passport_profiles()._add_destination( database );
     
     rs.passport_strategies()
   ...
   - options      (Object):
     ...
     - user_flow      (String): dataflow name in user's database, default is "user_profile"
...
*/
function Passport_Strategies( callback_url, options ) {
  Pipelet.call( this, options );
  
  this._user_flow = this._options.user_flow || 'user_profile';
...
Pipelet.Build( 'passport_strategies', Passport_Strategies, {
  __transform: function( strategies, options, caller ) {
    var out   = []
      , that  = this
    ;
    
    if( caller === 'remove' ) return strategies; // ToDo: handle errors on added strategies, cannot be removed
    ...
    // ToDo: _user_lookup_and_create(), manage multiple users identities and providers
    function user_lookup_and_create( strategy_name ) {
      var users = passport_profiles
        , flow  = this._user_flow
      ;
      ...
      return function( profile, done ) {
        ...
        var _profile = profile //._json
          , id       = _profile.id
          , _query   = [ { flow: flow, provider_name: strategy_name, provider_id : id } ]
          , user_profiles = []
        ;
        
        // fetch user profiles
        users._input.fetch_source( lookup_and_create, _query );
        
        function lookup_and_create( profiles, no_more ) {
          ...
          if ( l == 0 ) {
            ...
            de&&ug( 'lookup_and_create(), new user:', user );
          } else if ( l == 1 ) {
            // user found
            user = user_profiles[ 0 ];
            
            de&&ug( 'lookup_and_create(), found user:', user );
          } else {
...
/* -------------------------------------------------------------------------------------------
   users_profiles.passport_profiles( options )
   
   Performs passport serialization, emits new user profiles.
   
   This is a singelton, the first instance is always returned. Parameters are only interpreted
   on the first instanciation, ignored for all other instanciations.
   
   Parameters:
   - options (optional Object):
     - user_flow (String): default is "user_profile"
*/
...
function Passport_Profiles( options ) {
  ...
  options = extend( { user_flow: 'user_profile' }, options );
  
  Pipelet.call( this, options );
  
  this._user_flow = options.user_flow;
  ...
  function serialize( user, done ) {
    de&&ug( 'serializeUser(), user:', user );
    
    done( null, user.id );
  } // serialize()
  
  function deserialize( user_id, done ) {
    var flow = that._user_flow
      , user_profiles = []
    ;
    
    that._input.fetch_source( rx, [ { flow: flow, id: user_id } ] );
    ...
    function rx( profiles, no_more ) {
      ...
      if ( no_more ) {
        ...
        if ( ! user )
          return error( 'not found in dataflow ' + flow );
...
/* -------------------------------------------------------------------------------------------
   stragegies.passport( options )
   
   From passport strategies, start strategies and configure passport routes.
...
Set.Build( 'passport', Passport, function( Super ) { return {
  _add: function( strategies, options ) {
    ...
    // ToDo: only add strategies which were successfuly started
