  ...
  Set.Build( 'passport', Passport, function( Super ) { return {
    _get_routes: function() {
      ...
      for( var i = -1; ++i < len; ) {
        ...
        app_routes.push( { path: base_route + '/' + name, label: name } );
        
        if( ! handler ) handler = passport.authenticate( name );
      ...
      routes[ base_route + '/login' ] = {
        ...
        handler: function( request, response, next ) {
          ...
          for( var i = app_routes.length; i; ) {
            var app_route = app_routes[ --i ];
            
            s += '<a href="' + app_route.path + '">Login with ' + app_route.label + '</a> ';
          }
