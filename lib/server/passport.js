  ...
  .Singleton( 'passport_strategies_configuration', function( source, callback_base, options ) {
    ...
    function prepare_strategy( configuration ) {
      ...
      if ( ! strategy_name ) {
        ...
        strategy.name = strategy_name = id.slice( id.indexOf( '#' ) + 1 );
      ...
      if ( ! strategy.module ) {
        strategy.module = 'passport-' + strategy_name;
      }
      
      if ( strategy_name != 'local' && ! credentials.callbackURL ) {
      ...
      .map( function( fetched ) {
        ...
        function add_strategy( strategy, i ) {
          ...
          const
            ...
            credentials            = strategy.credentials,
            local_startegy_options = extend( { usernameField: 'username', passwordField: 'password' }, strategy.options ),
            parameters             = [ strategy_name == 'local' ? local_strategy_user_lookup : user_lookup_and_create ],
            users                  = rs.passport_profiles()
          ;
          
          // de&&ug( 'add_strategy, credentials', credentials, local_startegy_options );
          
          strategy_name == 'local' 
            ? local_startegy_options && parameters.unshift( local_startegy_options )
          ...
          function user_lookup_and_create( token, token_secret, profile, done ) {
          ...
          function local_strategy_user_lookup( username, password, done ) {
            var username_field = local_startegy_options.usernameField
              , password_field = local_startegy_options.passwordField
              , _query         = [ { [ username_field ]: username } ]
              , user_profiles  = []
              , name           = 'local_strategy_user_lookup(), '
            ;
            
            // ug( name + 'strategy_name: ' + strategy_name + ', fields' + username_field + ', ' + password_field, 'arguments.length' );
            ...
            function fetched( profiles, no_more ) {
              ...
              if ( l == 0 ) {
                // no user profile matched, create a new user profile
                de&&ug( name + ', no user found, username:', username );
...
Pipelet.Build( 'passport_profiles', Passport_Profiles ).singleton();
  ...
  .Compose( 'passport_strategies_routes', function( source, options ) {
    ...
    return source
      ...
      .flat_map( function( strategy ) {
        ...
        strategy_name == 'local' || callback_route( strategy.callback_url );
        ...
        function authenticate_handler( options ) {
          ug( 'authenticate_handler', options )
        ...
        function login_route() {
          ...
          function handler( request, response, next ) {
            ...
            if( strategy_name == 'local' ) {
              strategy.passport.authenticate( strategy_name, function( err, user, message ) {
                de&&ug( 'local_strategy_handler()', err, user, message );
                
                if( err ) return next( err );
                
                if( ! user ) return response.send( message );
