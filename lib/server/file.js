...
Pipelet.Compose( 'configuration', function( source, options ) {
  ...
  var filepath = options.filepath // ToDo: normalize filepath
    , configuration = configurations[ filepath ]
  ;
  
  if ( configuration ) return configuration;
  
  var flow  = options.flow
    , input = source.flow( flow )
  ;
  ...
  */
  ...
  return configurations[ filepath ] = rs.encapsulate( input, output );
} ); // configuration()
