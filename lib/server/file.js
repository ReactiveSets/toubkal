...
function path_join( name, base_directory ) {
  if( ! path.isAbsolute( name ) ) {
    if( name.charAt( 0 ) == '~' ) {
      ...
      name = base_directory + separator + name;
    }
  }
...
/* ----------------------------------------------------------------------------
    ...
    - base_directories: to prepend to ```"path"``` attribute:
      ...
      - (Array of Strings): join all paths to prepend 
...
rs.Compose( 'path_join', function( source, base_directories, options ) {
  if( is_array( base_directories ) )
    base_directories = path.join.apply( path, base_directories )
  ;
...
/* ----------------------------------------------------------------------------
    ...
    - directory (String): entry directory without trailing separator,
      relative to option ```"base_directory"``` or ```process.cwd()```
    - inode: stats.ino
    - mode: stats.mode
...
rs
  /* --------------------------------------------------------------------------
      ...
      - **options** (Object): pipelet map() asynchronous options. Except:
        - **limit**: for @@function:cancelable_limiter() to limit the number
        of concurrent fs.stat() running in parralel. Default is 0 (unlimited).
      ...
      - **error** (Error): optional, if there is an error and it is not
        "ENOENT".
      
      - **error_pipelet** (String):, if there is an error, pipelet name
      ...
      @examples
      This example is a manual test available in ```toubkal/test/manual/fs_stat.js```:
      ...
      2018/11/22 19:29:55.149 - Trace( fs_stat )..fetched(),  {
        ...
        "values": [
          ...
          },
          {
            ...
            "error": {},
            "error_pipelet": "fs_stat (test/manual/fs_stat.js:7:4)"
          }
        ]
      }
      ```
  ...
  */
  .Compose( 'fs_stat', function( source, options ) {
    options = extend_2( { limit: 0 }, options );
    
    return source.map( stat, options );
    
    function stat( file, next ) {
      if ( file.error ) return next( file );
      
      try {
        fs.stat( file.path, function( error, stat ) {
          if ( error && error.code != 'ENOENT' ) {
            emit_error( error );
          
          } else {
            next( null, { source: file, stat: stat } );
      ...
      function emit_error( error ) {
        next( null, { source: file, error: error, error_pipelet: options.name } );
      }
    } // stat()
  } ) // pipelet fs_stat()
;
...
/* ----------------------------------------------------------------------------
    ...
    - options (Object): optional attributes:
      ...
      - fork_tag (String): tagging output transactions
...
rs.Multiton( 'configuration', configuration_multiton_name, function configuration( source, options ) {
  ...
  var output = rs
    ...
    .attribute_to_values( options ) // ToDo: use flat_map()
    .alter( function( _ ) { _.flow = _.flow || flow } )
    .optimize()
    .unique()
  ;
  ...
  /* Possible implementation to write to configuration file
     Requires:
     - implement option operations of pipelet optimize()
     - new pipelet fs_write()
  */
  /*
  var fetched = source
    .flow( flow )
  ...
  fetched
    ...
    .map( function( fetched ) {
      ...
      source.removes.forEach( function( removed ) {
        var i = index_of( added );
        
        if ( i >= 0 ) out.splice( i, 1 );
      } );
      
      source.adds.forEach( function( added ) {
        if ( index_of( added ) == -1 ) out,push( added );
      } );
      
      source.updates.forEach( function( updated ) {
