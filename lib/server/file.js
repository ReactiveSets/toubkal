...
( function( exports ) {
  ...
  */
  function Watch( options ) {
    return File_Set.call( this, extend( {}, options, { key: [ 'name' ] } ) );
  } // Watch()
  ...
  File_Set.Build( 'watch', Watch, function( Super ) { return {
    _add: function( files ) {
      ...
      function add_file( file ) {
        var _, filename = that._get_path( file.name ), v, previous, encoding = file.encoding;
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Options:
       - base_directory: (string) the name of the base directory for all directories
  */
  function Watch_Directories( options ) {
    File_Set.call( this, extend( {}, options, { key: [ 'path' ] } ) );
        ...
        function add_directory( directory ) {
          ...
          function readdir() {
            fs.readdir( path, function( error, entries ) {
              if ( error ) {
                that._error( '_add', 'fs.readdir() path: ' + path + ', error: ' + error );
              }
              ...
              Object.keys( previous_entries ).forEach( function( entry ) {
                if ( entries.indexOf( entry ) == -1 ) {
                  ...
                  Super._remove.call( that, [ previous_entries[ entry ] ] );
          ...
          function stat( entry ) {
            ...
            fs.stat( _path, function( error, stats ) {
              if ( error ) {
                that._error( '_add', 'fs.stat() path: ' + _path + ', error: ' + error );
              }
              ...
              if ( previous_stats ) {
                for( var p in stats ) {
                  if ( ( stats[ p ] ).valueOf() != ( previous_stats[ p ] ).valueOf() ) {
                    ...
                    that._transaction( 2, {}, function( t, l ) {
                      Super._remove.call( that, [ previous_stats ], t.next().get_emit_options() );
                      Super._add   .call( that, [ stats          ], t.next().get_emit_options() );
                    } );
        ...
        function remove_directory( directory ) {
          ...
          if ( d ) {
            if ( --d.count ) {
              ...
              Super._remove.call( that, Object.keys( entries ).map( function( e ) { return entries[ e ] } ) );
