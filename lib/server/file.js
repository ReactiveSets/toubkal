...
/* -------------------------------------------------------------------------------------------
    ...
    The source must describe files, using a "path" attribute. If the path does not start with
    '.' or '~', the path is prepended with './' to read from the current directory.
    ...
    - options (Object): pipelet options
      - base_directory (String): the path of the base directory for all source files, default
        is directory where the process was launched.
    
    Warning: if a file does not exist when added, watch() cannot set a file watcher and will
    not attempt to add one if the file later appears in the directory. If files not always
    expected to be present when added, one should use watch_directory() upstream of watch()
    in order to add and remove watchers reactively.
...
/* -------------------------------------------------------------------------------------------
   ...
   ToDo: try https://github.com/paulmillr/chokidar as a replacement to fs.watch() to fix a number of problems
*/
  ...
  return {
    _add_value: function( t, directory ) {
      var that           = this
        ...
        , directory_key  = directory.path // ToDo: normalize path
...
/* -------------------------------------------------------------------------------------------
   ...
   This is an assynchronous, stateful, greedy pipelet.
...
/* -------------------------------------------------------------------------------------------
    ...
    @short JSON file configuration
    
    @description:
      Read (-only for now) a JSON configuration file and emits a dataflow of changes on that
      file.
      
      To determine independent values, the JSON file must contain one Array of Objects where
      each object has a unique key [ "id" ] by default. For an example of valid JSON
      configuration file, check the example bellow.
      
      A number of Toubkal pipelets expect a format where the following attributes are
      required for configuration objects, refer to each pipelet for further details:
        - "id" (String): a unique key for all configuration value. If should start with
          the name of the pipelet, e.g. "passport_strategies", and if there are multiple
          configuration values required by the pipelet, it should be followed by "#" and
          a unique string for each value, e.g. "passport_strategies#facebook".
        
        - "pipelet" (String): the name of the pipelet handling the configuration value. It
          may be used by pipelets handling multiple configuration values to subscribe to
          all their configuration values.
      
      This is a multiton, asynchronous, stateful, greedy pipelet.
    
    @parameters:
      - options (Object): optional attributes:
        - key  (Array of Strings): List of attributes defining unique configuration values,
                                   inherited from upstream pipelet, default is [ "id" ].
                                   
        - flow           (String): Configuration dataflow name, default is 'configuration',
                                   writing to the configuration requires the flow attribute to
                                   match inputs.
        
        - filepath       (String): Configuration file path, defaults to ~/config.rs.json
        
        - base_directory (String): If filepath is not an absolute directory, base_directory
                                   is prepended to filepath to determine the absolute location
                                   of the configuration file.
    
    @examples:
    
      - A configuration file for toubkal pipelets @@dropbox_public_urls() and @@passport_strategies():
        [
          {
...
/*
   ...
   ToDo: configuration(), use watch_directory() to monitor configuration file existance.
*/
...
rs.Multiton( 'configuration', configuration_multiton_name, function configuration( source, options ) {
  ...
  var flow  = options.flow
    , rs    = source.namespace()
    //, input = rs.flow( flow )//.no_fetch() ToDo: suppress fetch from source, only let query updates through
  ;
  ...
  var output = rs
    ...
    //.union( [ input ] )
    .optimize()
    // ToDo: the set should filter-out duplicate values, i.e. having same id and _v
    // .trace( name )
    // .set( [] ) // ToDo: requires a key for configuration objects, or use of a cache
  ;
