      ...
      function add_file( file ) {
        ...
        fs.readFile( filename, 'utf8', function( error, content ) {
          if ( error ) {
            ...
            file.sha1 = crypto.createHash( 'sha1' ).update( content );
            
            file.etag = file.sha1.digest( 'base64' );
            
            de&&ug( 'watch(), read file "' + filename + '", length: ' + content.length + ', etag: ' + file.etag );
            ...
            watcher.on( 'change', function( event ) {
              ...
              read_timeout = setTimeout( function() { fs.readFile( filename, 'utf8', function( error, content ) {
                ...
                if ( content != file.content ) {
                  ...
                  v.sha1 = crypto.createHash( 'sha1' ).update( content );
                  
                  v.etag = v.sha1.digest( 'base64' );
                  
                  de&&ug( 'watch(), read file "' + filename + '", length: ' + content.length + ', etag: ' + v.etag );
