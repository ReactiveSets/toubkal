        ...
        fs.readFile( filename, 'utf8', function( error, data ) {
          if ( error ) {
            ...
            watcher.on( 'change', function( event ) {
              ...
              read_timeout = setTimeout( function() { fs.readFile( filename, 'utf8', function( error, data ) {
                if ( error ) {
                  de&&ug( 'watch(): no longer able to read fle "' + filename + '", data: ' + data + ', error: ' + log.s( error ) );
                } else {
                  de&&ug( 'watch(), read file "' + filename + '", length: ' + data.length );
                }
