...
( function( exports ) {
  ...
  */
  ...
  XS.Compose( 'configuration', function( source, options ) {
    ...
    configurations
      .filter( function( v ) { return v.name === filename } )
      .fetch_all( function( values ) { configuration = values[ 0 ]; } )
    ;
    // fetch_all() must be synchronous because composition cannot be assynchronous
    // and Compose does not provide an initialization function
    
    if ( configuration ) return configuration.pipelet;
    ...
    configurations
      .add( [ { flow: 'configuration_pipelets', name: filename, pipelet: that } ] )
      
      .trace( 'configuration_pipelets', { exclude: [ 'pipelet' ] } )
    ;
