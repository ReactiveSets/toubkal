...
RS.path_join = path_join;

/* ----------------------------------------------------------------------------
    @pipelet path_join( base_directory )
    ...
    - Finally path is normalized using ```path.normalize()```
*/
rs.Compose( 'path_join', function( source, base_directory, options ) {
  base_directory = base_directory ? base_directory + separator : '';
  
  return source
    .alter( function( file ) {
      file.path = path_join( file.path, base_directory );
    }, options )
  ;
} ); // path_join()
