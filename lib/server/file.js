...
/* -------------------------------------------------------------------------------------------
   de&&ug()
*/
var de = true, ug = log.bind( null, 'file' );
        ...
        fs.stat( entry_path, function( error, stats ) {
          ...
          if ( error ) {
            ...
            if ( previous_stats ) {
              ...
              de&&ug( _name + 'added: ' + log.s( new_stats ) );
      ...
      function make_directory( dirname, callback, last ) {
        fs.stat( dirname, function( error, stat ) {
          ...
          if ( error.code != 'ENOENT' ) {
            de&&ug( 'Path ' + dirname + ' cannot be stat-ed, error: ' + error.code );
...
require( '../core/operations_optimizer' );
...
/* -------------------------------------------------------------------------------------------
   ...
   Parameters:
     - options: (Object) optional attributes:
       ...
       - flow: (String) configuration dataflow name, default is 'configuration', writing
           to the configuration requires the flow attribute to match inputs.
       
       - filepath: (String) the path of the configuration file to read,
           defaults to ~/config.rs.json
           
       - base_directory (String): if filepath is not an absolute directory, base_directory
           is prepended to filepath to determine the absolute location of the configuration
           file.
           
   ToDo: more tests for configuration()
*/
var configurations = {}; // configurations multiton instances

Pipelet.Compose( 'configuration', function( source, options ) {
  options = extend_2( {
    flow: 'configuration', filepath: '~/config.rs.json', key: [ 'id' ]
  }, options );
  
  var filepath = options.filepath
    , name = options.name = ( 'configuration( ' + filepath + ' )' )
    , normalized_path
    , watch_options = extend( {}, options, { key: [ 'path' ] } )
  ;
  
  if ( options.base_directory ) {
    normalized_path = path.resolve( options.base_directory, filepath );
  } else {
    normalized_path = path.normalize( filepath );
  }
  
  de&&ug( name + ', normalized_path:', normalized_path );
  
  var configuration = configurations[ normalized_path ];
  ...
  var flow  = options.flow
    , input = source.flow( flow )//.no_fetch() ToDo: suppress fetch from source, only let query updates through
  ;
  ...
  */
  ...
  return configurations[ normalized_path ] = rs.encapsulate( input, output );
} ); // configuration()
  ...
  return {
    __transform: function( modules ) {
      ...
      while ( module = modules[ ++i ] ) {
        ...
        // ToDo: handle exception from require.resolve()
        files.push( extend_2( {
