  ...
  */
  ...
  Set.Build( 'watch', Watch, {
    _add: function( files ) {
      var that = this
        , added = []
        , base = this._options.base_directory
      ;
      ...
      function add_file( file ) {
        ...
        fs.readFile( filename, encoding, function( error, content ) {
            ...
            watcher.on( 'change', function( event ) {
              ...
              read_timeout = setTimeout( function() { fs.readFile( filename, encoding, function( error, content ) {
                if ( error ) {
                  ...
                  return Set.prototype._remove.call( that, [ previous ] );
                }
                ...
                if ( content != previous.content ) {
                  ...
                  Set.prototype._update.call( that, [ [ previous, v ] ] );
          ...
          added.length && Set.prototype._add.call( that, added );
        } );
      }
    }, // _add()
    ...
    _remove: function( files ) {
      ...
      removed.length && Set.prototype._remove.call( this, removed );
  ...
  Pipelet.Build( 'content_to_set', function( options ) {
    ...
    
    {
      ...
      _update: function( updates, options ) {
        ...
        return this.__emit_operations( update[ 1 ].content, update[ 0 ].content, null, options ); // ToDo: check if [] are required around adds and removes
      },
    }
  );
