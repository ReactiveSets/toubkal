...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Resolve node_module file names and make uri_path for uglify() source map and
     http_server serve().
  */
  XS.Compose( 'require_resolve', function( source, options ) {
    return source
      .alter( function( file ) {
        ...
        file.uri_path = '/node_modules/' + name;
        
        de&&ug( 'require_resolve(), absolute file name: ' + log.s( file ) + ', uri path:' + file.uri_path );
