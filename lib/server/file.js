...
( function( exports ) {
  ...
  */
  function Watch_Directories( options ) {
    return File_Set.call( this, extend( {}, options, { key: [ 'path' ] } ) )
  } // Watch_Directories()
        ...
        function add_directory( directory ) {
          ...
          function readdir() {
            fs.readdir( path, function( error, entries ) {
              ...
              Object.keys( previous_entries ).forEach( function( entry ) {
                if ( entries.map( function( e ) { return path + '/' + e } ).indexOf( entry ) == -1 ) {
          ...
          function stat( entry ) {
            ...
            fs.stat( entry, function( error, stats ) {
              if ( error ) {
                that._error( '_add', 'fs.stat() path: ' + entry + ', error: ' + error );
              }
