...
( function( exports ) {
  ...
  */
    ...
    return {
      _add: function( directories, options ) {
        ...
        function add_directory( directory ) {
          var directory_path = that._get_path( directory.path )
            , d              = _directories[ directory_path ]
          ;
          ...
          function readdir() {
            fs.readdir( directory_path, function( error, entries ) {
              if ( error ) {
                de&&ug( name + 'fs.readdir() path: ' + directory_path + ', error: ' + error );
              } else {
                ...
                Object.keys( previous_entries ).forEach( function( entry ) {
                  if ( entries.indexOf( entry ) == -1 ) {
                    de&&ug( name + 'readdir(), entry "' + entry + '" was deleted from path: ' + directory_path );
          ...
          function stat( entry ) {
            ...
            fs.stat( entry_path, function( error, stats ) {
              if ( error ) {
                de&&ug( name + 'fs.stat() path: ' + entry_path + ', error: ' + error );
                
                var previous_entry = previous_entries[ entry ];
                
                if ( previous_entry ) {
                  de&&ug( name + 'after fs.stat() failed, remove entry ' + entry_path );
      ...
      _remove: function( directories, options ) {
        ...
        function remove_directory( directory ) {
          ...
          if ( d ) {
            if ( --d.count ) {
              de&&ug( name + 'directory "' + directory_path + '" is still watched ' + d.count + ' time(s)' );
            } else {
              de&&ug( name + 'directory "' + directory_path + '" un-watched' );
              d.watcher.close();
              d.watcher = null;
              ...
              delete _directories[ directory_path ];
            }
          } else {
            de&&ug( name + 'directory "' + directory_path + '" not watched ' );
          }
        } // remove_directory()
      } // _remove()
    };
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Source:
       - options.attribute_name (Array of Objects)
  ...
  */
  ...
  function Require_Resolve( options ) {
    return Pipelet.call( this, extend_2( { key: [ 'name' ] }, options ) );
  } // Require_Resolve()
