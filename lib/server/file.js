    ...
    return {
      _add: function( directories, options ) {
        ...
        function add_directory( directory ) {
          var directory_key  = directory.path
          ...
          var directory_path = that._get_path( directory_key );
          
          try {
            var watcher = fs.watch( directory_path );
          } catch( e ) {
            // ToDo: send error to global error dataflow
            de&&ug( name + 'add_directory(), path: ' + directory_path + ', error: ' + e );
          ...
          function watch_handler( event, entry ) {
            ...
            // Wait 300 ms before processing to give a chance for all changes to become visible
            setTimeout( function() {
              if ( entry ) {
                stat( entry );
              } else {
      ...
      _x_add_value: function( t, directory ) {
        var that           = this
          , name           = de && this._get_name( '_add_value' )
          , _directories   = this._directories
          , directory_key  = directory.path
          , d              = _directories[ directory_key ]
          , adds = [], removes = [], count = 1
        ;
        
        de&&ug( name + 'directory: ' + log.s( directory ) );
        ...
        function watch_handler( event, entry ) {
          ...
          if ( t ) {
            ...
            that._transaction( 1, {}, function( _t, l ) {
              t = _t;
