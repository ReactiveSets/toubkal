...
rs
  .Compose( 'debug', function( source, debug, options ) {
    return debug
...
// ToDo: make all exports singleton pipelets
...
rs
  /* --------------------------------------------------------------------------
      ...
      @short Provides minified asset for browser source map support
  ...
  .Singleton( 'source_map_support_min', function( source, options ) {
    return source
      .set( [ {
        ...
        uri: '/lib/browser-source-map-support.js.min'
      } ] )
