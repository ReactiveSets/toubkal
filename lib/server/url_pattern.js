...
( 'url_pattern', [ '../core/pipelet', [ 'UrlPattern', 'url-pattern' ] ], function( rs, Url_Pattern ) {
  ...
  /* --------------------------------------------------------------------------
      @pipelet url_pattern( pattern, options )
      ...
      - **options** (Object): optional @@class:Pipelet options
        - **segment_name_charset** (String): change the set of chars allowed in named segment names, default is: 'a-zA-Z0-9_-'
        - **attribute**            (String): name of the attribute to parse, default id: 'hash'
      ...
      ```javascript
        rs
          ...
          .url_pattern( '/api/users/:id', { attribute: 'content' } )
        ;
  ...
  */
  ...
  rs.Compose( 'url_pattern', function( source, pattern, options ) {
    var attribute = options.attribute || 'hash'
      , type      = toString.call( pattern )
    ;
    ...
    return source
      .map( function( value ) {
        var v = pattern.match( decodeURIComponent( value[ attribute ] ) );
