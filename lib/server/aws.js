...
function EC2_Describe_Spot_Price_History( start, instance_types, options ) {
  ...
  if ( instance_types ) {
    if ( instance_types instanceof Set ) {
      instance_types.on( 'change', function( operation, values, options ) {
  ...
  if ( products ) {
    if ( products instanceof Set ) {
      products.on( 'change', function( operation, values, options ) {
...
Set.build( 'ec2_describe_spot_price_history', EC2_Describe_Spot_Price_History, {
  add: function( regions ) {
    ...
    function get_prices( region, parameters ) {
      de&&ug( 'ec2_describe_spot_price_history, get_prices(): parameters: ' + log.s( parameters ) );
      ...
      function receiver( e, prices ) {
        ...
        de&&ug( 'EC2 Spot Price History for region "' + region + '": ' + log.s( prices ) );
        
        // Add region into returned prices 
        for ( var i = -1, l = prices.length; ++i < l; ) {
          prices[ i ].RegionName = region;
        }
        
        Set.prototype.add.call( that, prices );
        
        if ( ! parameters.NextToken && --count == 0 ) {
          that.emit( 'complete', that.a );
        }
      } // receiver()
