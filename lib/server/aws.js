  ...
  add: function( regions, options ) {
    ...
    for ( var i = -1; ++i < regions.length; ) {
      ...
      ( function( region ) { 
        ...
        function error( e ) {
          --count || that.emit_add( [], options ); // no more
...
function EC2_Describe_Spot_Price_History( start, instance_types, options ) {
  ...
  if ( instance_types ) {
    if ( typeof instance_types.on == 'function' ) {
      instance_types.on( 'complete', function() {
        that.fetch_all( set_instance_types );
      } );
    } else {
  ...
  if ( products ) {
    if ( typeof products.on == 'function' ) {
      products.on( 'complete', function() {
        that.fetch_all( set_products );
      } );
    } else {
...
Set.build( 'ec2_describe_spot_price_history', EC2_Describe_Spot_Price_History, {
  add: function( zones, options ) {
    ...
    de&&ug( 'ec2_describe_spot_price_history..add(): zones: ' + log.s( zones ) );
    ...
    function get_prices( region, zone, parameters ) {
      ...
      function receiver( e, prices ) {
        ...
        try {
          ...
          if ( end ) {
            ...
            Set.prototype.add.call( that, prices, --that.count ? options_more : options );
          } else {
            // There's more to fetch
            Set.prototype.add.call( that, prices, options_more );
            
            // Query next prices if more needed and available
            get_prices( region, zone, parameters );
          }
        } catch( e ) {
          de&&ug( 'exception, EC2_Describe_Spot_Price_History, region/zone: "' + zone + '", error: ' + e + ', stack trace: ' + e.stack );
      ...
      function error( e ) {
        --that.count || that.emit_add( [], options ); // emit dummy add when no more
        
        de&&ug( 'EC2_Describe_Spot_Price_History, region/zone: "' + zone + '", error ' + e )
      } // error()
    } // get_prices()
  } // add()
