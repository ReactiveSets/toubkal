    ...
    for ( var i = -1; ++i < regions.length; ) {
      ...
      ( function( region ) { 
        region_clients[ region ].describeAvailabilityZones( function( e, availability_zones ) {
          ...
          Set.prototype.add.call( that, availability_zones, --count ? options_more : options );
        } );
...
Set.build( 'ec2_describe_spot_price_history', EC2_Describe_Spot_Price_History, {
  add: function( zones, options ) {
    var options_more = XS.more( options ), more = options && options.more;
    
    de&&ug( 'ec2_describe_spot_price_history..add(): zones: ' + log.s( zones ) + ', options: ' + log.s( options ) );
    
    if ( zones.length === 0 ) return this;
    
    var that = this, count = zones.length;
    
    for ( var i = -1; ++i < zones.length; ) {
      var zone = zones[ i ];
      
      get_prices( zone.RegionName, zone.ZoneName, extend( {}, this.parameters ) ); // copy parameters to allow for next token
    }
    ...
    function get_prices( region, zone, parameters ) {
      ...
      de&&ug( 'ec2_describe_spot_price_history, zone: "' + zone + '", get_prices(): parameters: ' + log.s( parameters ) );
      ...
      function receiver( e, prices ) {
        ...
        parameters.NextToken = prices.NextToken;
        
        prices = prices.SpotPriceHistory;
        
        de&&ug( 'EC2 Spot Price History for zone "' + zone + '", prices: ' + prices.length );
        
        try {
          // Add region into returned prices, parse spot prices as floats, truncate results to
          // return results from requested start date 
          for ( var i = -1, l = prices.length, start = that.start.valueOf(), end = false; ++i < l; ) {
            var p = prices[ i ];
          ...
          Set.prototype.add.call( that, prices, --count ? options_more : options );
          
          // Query next prices if more needed and available
          end || parameters.NextToken && get_prices( region, parameters );
        } catch( e ) {
          de&&ug( 'exception, EC2_Describe_Spot_Price_History, zone ' + zone + ', error ' + e + ', stack trace: ' + e.stack );
      ...
      function error( e ) {
        --count || more || that.emit_add( that, [] ); // emit dummy add when no more
