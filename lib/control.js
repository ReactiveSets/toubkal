  ...
  */
  ...
  extend( Control.Drop_Down.prototype, {
    ...
    add: function( objects, options ) {
      ...
      this.selected = to_value( select.options[ select.selectedIndex ].value );
    ...
    remove: function( objects, options ) {
      ...
      for( var i = -1; ++i < len; ) {
        ...
        if( selected === objects[ i ].id ) this.selected = to_value( select.options[ 0 ].value );
      }
    ...
    update: function( updates, options ) {
      ...
      for( var i = len, j = 0; i; ) {
        ...
        if( from !== to ) select.add( o, select.options[ to + 1 ] );
      }
