  ...
  */
  ...
  extend( Control.Radio.prototype, {
    ...
    remove: function( objects ) {
      ...
      for( var i = rl; i; ) {
        for( var j = len, radio = radios[ --i ]; j; ) {
          var o = objects[ --j ];
          
          if( radio.getAttribute( "name" ) == o.id ) node.removeChild( radio.parentNode );
        }
      }
    ...
    update: function( updates ) {
      ...
      for( var i = rl; i; ) {
        for( var j = len, radio = radios[ --i ]; j; ) {
          ...
          if( radio.getAttribute( "name" ) == u0.id ) {
            if( u0.checked !== u1.checked ) radio.checked         = u1.checked;
            if( u0.label   !== u1.label   ) labels[ i ].innerText = u1.label;
  ...
  /* -------------------------------------------------------------------------------------------
     Control.Drop_Down()
  */
  Pipelet.prototype.drop_down = function( node, organizer, options ) {
    return new Control.Drop_Down( node, organizer, extend( { key: this.key }, options ) ).set_source( this );
  };
  
  Control.Drop_Down = function( node, organizer, options ) {
  ...
  extend( Control.Drop_Down.prototype, {
    ...
    add: function( objects ) {
      ...
      if( added.length === 0 ) added.push( objects[ 0 ] );
    ...
    remove: function( objects ) {
      ...
      for( var i = ol; i; ) {
        for( var j = len, option = options[ --i ]; j; ) {
          var o = objects[ --j ];
          
          if( option.value == o.id ) select.remove( i );
        }
      }
      
      Ordered_Set.prototype.remove.call( this, objects );
      
      if( this.get().length === 0 ) Ordered_Set.prototype.add.call( this, [ this.source.a[ 0 ] ] );
    ...
    update: function( updates ) {
      ...
      var node    = this.node
        , drop_downs  = node.getElementsByTagName( "input" )
        , labels  = node.getElementsByTagName( "label" )
        , removed = []
        , added   = []
        , len     = updates.length
        , rl      = drop_downs.length
      ;
      
      if( ! len ) return this;
      
      for( var i = rl; i; ) {
        for( var j = len, drop_down = drop_downs[ --i ]; j; ) {
          var u  = updates[ --j ]
            , u0 = u[ 0 ]
            , u1 = u[ 1 ]
          ;
          
          if( drop_down.getAttribute( "name" ) == u0.id ) {
            if( u0.checked !== u1.checked ) drop_down.checked         = u1.checked;
            if( u0.label   !== u1.label   ) labels[ i ].innerText = u1.label;
