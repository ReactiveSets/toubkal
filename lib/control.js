  ...
  extend( Control.Drop_Down.prototype, {
    ...
    add: function( objects, options ) {
      ...
      for( var i = len; i; ) {
        ...
        if( o.selected ) selected = location;
        
        select.add( option, select.options[ location ] );
      }
      
      this.selected = to_value( select.options[ select.selectedIndex = selected ].value );
    ...
    remove: function( objects, options ) {
      ...
      for( var i = -1; ++i < len; ) {
        select.remove( locations[ i ] );
        
        if( selected === objects[ i ].id ) selected = to_value( select.options[ 0 ].value );
      }
    ...
    update: function( updates, options ) {
      ...
      for( var i = len, j = 0, options = select.options; i; ) {
        var move = moves[ --i ]
          , from = move.from
          , to   = move.to
          , u    = updates[ j++ ]
        ...
        if( u0.id    !== u1.id    ) o.value = u1.id;
        if( u0.label !== u1.label ) o.innerText = u1.label;
        
        if( u1.selected ) selected = to;
        
        if( from !== to ) select.add( o, options[ to + 1 ] );
      }
