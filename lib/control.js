...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     Control.Radiobox()
  */
  Pipelet.prototype.radiobox = function( node, organizer, options ) {
    return new Control.Radiobox( node, organizer, extend( { key: this.key }, options ) ).set_source( this );
  };
  
  Control.Radiobox = function( node, organizer, options ) {
    de&&ug( "new Control.Radiobox()" );
  ...
  subclass( Single_Choice, Control.Radiobox );
  
  extend( Control.Radiobox.prototype, {
    add: function( objects ) {
      de&&ug( "Radiobox::add(), objects: " + log.s( objects ) );
      ...
      for( var i = -1; ++i < l; ) {
        ...
        radiobox.setAttribute( "name", o.id    );
        
        if( default_value && default_value.id === o.id ) {
          radiobox.checked = true;
          added.push( o );
        }
        
        radiobox.onclick = toggle;
      ...
      function toggle() {
        ...
        for( var i = l; i; ) {
          ...
          if( p.length ) Ordered_Set.prototype.remove.call( that, p )
    ...
    remove: function( objects ) {
      de&&ug( "Radiobox::remove, objects: " + log.s( objects ) );
      
      var node       = this.node
        , radioboxes = node.getElementsByTagName( "input" )
        , len        = objects.length
        , rl         = radioboxes.length
      ;
      
      for( var i = rl; i; ) {
        for( var j = len, radiobox = radioboxes[ --i ]; j; ) {
          if( radiobox.getAttribute( "name" ) == objects[ --j ].id ) node.removeChild( radiobox.parentNode );
        }
      }
    ...
    update: function( updates ) {
      de&&ug( "Radiobox::update, updates: " + log.s( updates ) );
      ...
      for( var i = rl; i; ) {
        for( var j = len, radiobox = radioboxes[ --i ]; j; ) {
          ...
          if( radiobox.getAttribute( "name" ) == u0.id ) {
            if( u0.checked !== u1.checked ) radiobox.checked      = u1.checked;
            if( u0.label   !== u1.label   ) labels[ i ].innerText = u1.label;
