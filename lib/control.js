...
( function( exports ) {
  ...
  */
  ...
  extend( Control.Checkbox.prototype, {
    ...
    update: function( updates ) {
      ...
      var l = updates.length, id = this.value.id, v = {};
  ...
  */
  Fork.prototype.checkbox_group = function( node, organizer, options ) {
  ...
  subclass( Control, Control.Checkbox_Group );
  
  extend( Control.Checkbox_Group.prototype, {
    set_value: function( v ) {
      Control.prototype.set_value.call( this, v );
      
      de&&ug( "Checkbox_Group::set_value(), value: " + log.s( this.value ) );
      
      var checkboxes = this.node.getElementsByTagName( "input" )
        , vl         = v.length
      ...
      for( var i = cl; i; ) {
        ...
        for( var j = vl; j; ) if( checkbox.getAttribute( "name" ) == v[ --j ].id ) checkbox.checked = true;
      }
    ...
    add: function( objects ) {
      ...
      for( var i = -1; ++i < l; ) {
        ...
        s += '<div><input name="' + o.id + '" type="checkbox" /><label>' + o.label + '</label></div>';
        
        o.checked && added.push( o )
      }
