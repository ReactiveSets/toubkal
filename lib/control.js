...
( function( exports ) {
  ...
  */
  ...
  function Control( node, organizer, options ) {
    ...
    Ordered.call( this, organizer, options );
    
    return this;
  } // Control()
  
  subclass( Ordered, Control );
  
  extend( Control.prototype, {
    fetch: Pipelet.prototype.fetch,
    
    transform: function( values, options ) {
      var out   = []
        , l     = values.length
      ...
      // out = index !== void 0 ? [ values[ index ] ] : [];
      
      de&&ug( 'Control..transform(), out: ' + log.s( out ) );
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     Control.Radio()
  */
  Pipelet.prototype.radio = function( node, organizer, options ) {
    return new Control.Radio( node, organizer, extend( { key: this.key }, options ) ).set_source( this );
  };
  
  Control.Radio = function( node, organizer, options ) {
  ...
  extend( Control.Radio.prototype, {
    ...
    add: function( added, options ) {
      de&&ug( "Radio::add(), added: " + log.s( added ) );
    ...
    remove: function( removed, options ) {
      de&&ug( "Radio::remove, removed: " + log.s( removed ) );
    ...
    insert_: function( at, value ) {
      ...
      label.innerText = value.label || value.id;
      
      radio.setAttribute( "name", value.id );
      radio.setAttribute( "type", "radio"  );
      
      if ( value.checked ) {
        radio.checked = true;
        this.value    = value;
        this.selected_index = at;
      } else if ( this.selected_index >= at ) {
        this.selected_index += 1;
      }
      
      div .appendChild( radio );
      div .appendChild( label );
      node.insertBefore( div, node.childNodes[ at ] );
      
      console.log( "here", value, at );
    ...
    remove_: function( from , value ) {
      ...
      if ( this.selected_index > from ) {
        this.selected_index -= 1;
      } else if ( this.selected_index === from ) {
        this.selected_index = u;
        this.value = u;
      }
    ...
    update_: null // force remove_ + insert_, ToDo: needs to be implemented to not loose selected index when not changed
  } ); // Control.Radio()
  ...
  */
  ...
  extend( Control.Drop_Down.prototype, {
    ...
    get_value: function() {
      ...
      this.selected_index = this.select.selectedIndex;
    ...
    add: function( objects, options ) {
      ...
      Order.insert_at( objects, options, function( at, value ) { that.insert_( at, value ) } );
      
      this.selected_index = this.select.selectedIndex;
      
      if( this.value === undefined ) this.value = objects[ this.selected_index ];
    ...
    remove: function( objects, options ) {
      ...
      Order.remove_from( objects, options, function( from, value ) { that.remove_( from, value ) } );
    ...
    update: function( updates, options ) {
      ...
      Order.update_from_to( updates, options, 
        function( at  , value ) { that.insert_( at  , value ) },
        function( from, value ) { that.remove_( from, value ) }
      );
    ...
    insert_: function( at, value ) {
      ...
      option.innerText = value.label || value.id;
      option.value     = value.id;
      
      this.select.add( option, this.select.options[ at ] );
