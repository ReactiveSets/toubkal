  ...
  Load_Images.Build( 'bootstrap_carousel', Bootstrap_Carousel, {
    _process_options: function( options ) {
      var default_options = {
        ...
        , auto_start: true    // prevent carousel from auto-cycling
      };
    ...
    _set_bootstrap_carousel_nodes: function( dom_node ) {
      ...
      add_class( dom_node , 'carousel ' + options.transition + ( options.auto_start ? ' play' : ' pause' ) );
      ...
      function make_carousel_controls() {
        ...
        if( options.play ) {
          ...
          $play.innerHTML = '<span class="glyphicon glyphicon-' + ( options.auto_start ? 'pause' : 'play' ) + '"></span>';
    ...
    _add_value: function( transaction, value ) {
      ...
      function inistialize_carousel( values ) {
        ...
        ! that._options.auto_start && $( $carousel ).carousel( 'pause' );
