  ...
  */
  Load_Images.Build( 'bootstrap_carousel', Bootstrap_Carousel, {
    _process_options: function( options ) {
      ...
      this._carousel_options = extend( default_options, options.carousel_options );
      /*
    ...
    _set_events_handlers: function() {
      ...
      function keyup( e ) {
        ...
        if( that.options.input_events ) {
          ...
          that.emit_add( [ { image_id: image.id, direction: direction, current_index: index } ] );
        } else {
    ...
    _add_value: function( transaction, value ) {
      ...
      this._start_carousel && this.fetch_all( inistialize_carousel );
      ...
      function inistialize_carousel( values ) {
        ...
        de&&ug( '_inistialize_carousel()' );
        
        $( that.bootstrap_carousel ).carousel( that._carousel_options );
    ...
    _remove_value: function( transaction, value ) {
      ...
      var $inner = this.carousel_inner
        , $item  = $inner.childNodes[ 0 ]
      ;
      
      for( $item = $inner.childNodes[ 0 ]
        ; $item.getElementsByTagName( 'img' )[ 0 ] && $item.getElementsByTagName( 'img' )[ 0 ].getAttribute( 'xs_uri' ) !== value.uri
