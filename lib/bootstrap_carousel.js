  ...
  Load_Images.build( 'bootstrap_carousel', Bootstrap_Carousel, {
    process_options: function( options ) {
      ...
      if( options.input_events ) {
        options.input_events.on( 'add', function( operation, values, options ) {
    ...
    set_bootstrap_carousel_nodes: function( dom_node ) {
      if( XS.is_DOM( dom_node ) ) {
        ...
        if( options.controls ) {
          ...
          controls_node.insertBefore( left_node , controls_node.childNodes[ 0 ] );
          controls_node.insertBefore( right_node, controls_node.childNodes[ 1 ] );
          ...
          if( controls.play ) {
            ...
            controls_node.insertBefore( play_node, controls_node.childNodes[ 1 ] );
          }
          ...
          if( controls.play ) {
            ...
            controls_node.insertBefore( matrix_node, controls_node.childNodes[ 0 ] );
          }
    ...
    set_events_handlers: function() {
      var that      = this
        ...
        , stat      = false                  // control visibility
        , timer     = 0
      ;
      
      // stop / play carousel
      _controls.onclick = function( e ) {
        var target = e.target;
        
        console.log( has_class( target, 'icon-play'  ), target );
        
        if( has_class( target, 'icon-play'  ) ) {
          $( carousel ).carousel( 'cycle' );
          
          remove_class( target, 'icon-play'  );
          add_class   ( target, 'icon-pause' );
          
          return;
        }
        
        if( has_class( target, 'icon-pause' ) ) {
          $( carousel ).carousel( 'pause' );
          
          remove_class( target, 'icon-pause'  );
          add_class   ( target, 'icon-play' );
      ...
      // display controls on mouse move
      $( carousel )
        .mousemove( show_controls )
        // .mouseover( hide_controls )
        // .mousestop( hide_controls )
      ;
      ...
      function show_controls() {
        ...
        $( _controls ).fadeIn( 1000 );
      } // show_controls()
      
      function hide_controls() {
        if( ! stat ) return;
        
        timer = setTimeout( _hide, 1500 );
      }
