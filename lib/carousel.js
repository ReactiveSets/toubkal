  ...
  Load_Images.build( 'carousel', Carousel, {
    ...
    set_carousel_nodes: function( dom_node ) {
      ...
      if( XS.is_DOM( dom_node ) ) {
        ...
        dom_node.setAttribute( 'data-interval', '2000' );
    ...
    add: function( added, options ) {
      ...
      for( var i = -1; ++i < l; ) {
        ...
        this.fetch_all( start_carousel );
      }
      
      return this;
      
      function start_carousel( values ) {
        if( values.length === 2 ) $( that.carousel ).carousel( { interval: that.options.interval } );
      }
    }, // add()
    ...
    remove: function( removed, options ) {
      ...
      for( var i = l; i; ) {
        var name = removed[ --i ].name
          , item = carousel_inner.childNodes[ 0 ]
        ;
        
        while( item ) {
          if( item.getElementsByTagName( 'img' )[ 0 ].src === name ) {
