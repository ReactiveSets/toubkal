    ...
    set_node: function( dom_node, options ) {
      if( XS.is_DOM( dom_node ) ) {
        ...
        if( options.controls ) {
          ...
          left.href = right.href = '#my_carousel';
  ...
  Pipelet.build( 'carousel_inner', Carousel_Inner, {
    set_node: function( dom_node, options ) {
      if( XS.is_DOM( dom_node ) ) {
        ...
        console.log( dom_node.childNodes[ 0 ] );
        
        dom_node.insertBefore( this.carousel_inner, dom_node.childNodes[ 0 ] );
       } else {
        throw( "node is not a DOM element" );
      }
    ...
    add: function( added, options ) {
      ...
      for( var i = len; i; ) {
        ...
        if( XS.has_class( item, 'item' ) || XS.has_class( item, 'carousel-inner' ) || XS.has_class( item, 'carousel-control' ) ) continue;
