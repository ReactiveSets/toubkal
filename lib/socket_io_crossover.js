  ...
  */
  function Socket_IO_Crossover( socket, options ) {
    ...
    this.socket = socket
      ...
      .on( 'xs_fetch', function( data ) {
        ...
        var query = data.query; // ToDo validate query (Array of Objects) or undefined
      ...
      .on( 'xs_query', function( data ) {
        de&&ug( 'received xs_query: ' + log.s( data ) );
        
        var source = that._source;
        
        // ToDo: validate data.adds, data.removes
        
        Pipelet.prototype._update_upstream_query.call( that, data.adds || [], data.removes || [] );
      } )
  ...
  Pipelet.subclass( Socket_IO_Crossover, {
    ...
    send: function( method, data ) {
      if ( this.disconnected ) {
        de&&ug( 'queue: "' + method + '", ' + log.s( data ) );
        
        this.emit_queue.push( [ method, data ] );
      } else {
        de&&ug( 'send: "' + method + '", ' + log.s( data ) );
    ...
    _update_upstream_query: function( adds, removes ) {
      var al = adds.length, rl = removes.length, o = {};
      
      if ( al || rl ) {
        if ( al ) o.adds    = adds;
        if ( rl ) o.removes = removes;
        
        return this.send( 'xs_query', o )
      }
