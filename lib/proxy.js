...
( function( exports ) {
  ...
  */
  ...
  Pipelet.subclass( "proxy", Proxy, {
    ...
    add: function( objects, by_subscriber ) {
      ...
      de&&mand( !objects || this.is_source() || by_subscriber );
      return this.send( { 
        name:         'add',
        objects:       objects,
    ...
    // Process queued messages, both those sent and those received, asap.
      ...
      while ( action = this.receiveQueue.shift() ) {
        ...
        case 'propose_add'    : this.source.propose_add(    action.objects ); break;
        case 'propose_remove' : this.source.propose_remove( action.objects ); break;
        case 'propose_update' : this.source.propose_update( action.objects ); break;
        case 'propose_clear'  : this.source.propose_clear(  action.objects ); break;
        default:
    ...
    // Private. called when contact is lost with remote proxy.
      ...
      // ToDo: Tell the source xro destination about that? how?
      if( !this.is_source() ){
  ...
  */
  ...
  xs.broadcast.subclass( "publish", Publish, {
    
    factory: function( name, options ) {
      return new Publish( this, name, options );
    },
    ...
    add: function( objects ){
      return Broadcast.prototype.add.call( this, objects );
    },
  ...
  */
  ...
  xs.broadcast.subclass( "subscribe", Subscribe, {
    
    factory: function( p1, p2 ) {
      // xs.subscribe( "some_publisher") usage
      if( this === the_void ) {
        return new Subscribe( p1, p2 );
      // some_publisher.subscribe() usage
      } else {
        return new Subscribe( this, p1 );
      }
    },
    ...
    // dependents.
    add:    function( x ) { return this.connections_add(    x ); },
    update: function( x ) { return this.connections_update( x ); },
    remove: function( x ) { return this.connections_remove( x ); },
    clear:  function(   ) { return this.connections_clear(    ); },
    ...
    disconnect: function( target ) {
      // ToDo: feature interactions with set_source()...
      if ( target === this.proxy_sub ) {
        this.disconnection();
        return this;
      }
      // Invoke super
      return Broadcast.disconnect.call( this, target);
    },
  ...
  */
  ...
  xs.datalet.subclass( "tracer", Tracer, {
    factory: function( options ){ new Tracer( this, options ); return this; },
  ...
  */
  ...
  eval( XS.export_code( 'XS', [ 'Proxy', 'Publish', 'Subscribe' ] ) );
