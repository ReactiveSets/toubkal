...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Which both mean "year in ( 2009, 2010, 2011, 2012 ) OR year == 2005".
     
     When expressions involve more than one attribute, these can be specified
     using JavaScript Objects, e.g. "sales / count > 1000" can be expressed as:
     ...
     an expression returns false, or fails, the entire execution of the
     expression will fail and return false. This allow to express things like:
     ...
     return false, and can be expressed as:
     
       sales: [ '/' [ { $: 0 } '!=', { _: 'count' } ], '>', 1000 ]
     
     Nested attributes are supported using JavaScript Objects using the '.'
     attribute, e.g.:
     ...
     Which all mean user.age > 18. Using this method rather than the more
     intuitive 'user.age' notation has two significant advantages:
     a) attributes can contain dots, there is no ambiguity
     ...
     Any depth can be reached using the following notation:
     
       user: [ { '.': [ 'profile', 'age' ] }, '>', 18 ]
       
     Meaning 'user.profile.age > 18'.
     
     The current attribute can be changed using this notation:
     
     user: [ { __: [ 'profile', 'age' ] }, '>', 18 ]
     
     One may implement SQL, or any other language, expression using a compiler
     but this would have to be optional, at least on the client side to avoid
     the extra cost of such compiler.
