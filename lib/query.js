...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     In "this" context, developpers have access to the following attributes:
       ...
       - _ (String or Array): current attribute specification e.g.:
         'count', or [ 'user', 'profile' ]
  ...
  */
  ...
  var Operator = Query.Operator = function( name, f, options ) {
    if ( operators[ name ] ) throw new Error( 'Query Operator "' + name + '" already defined' );
  ...
  function equals( a, b ) {
    ...
    // They have the same number of own properties
    
    for ( i = -1; ++i < l; ) {
      var p = keys[ i ];
      
      if ( equals( a[ p ], b[ p ] ) ) continue;
