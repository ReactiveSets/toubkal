  ...
  extend_2( Query_Tree.prototype, {
    ...
    route: function( operation, values ) {
      ...
      switch( operation ) {
        case 'update':
          ...
          for ( i = -1, rl = subscribers.length; ++i < rl; ) {
            var r = subscribers[ i ], p = r.p, input = p._input, added = [], removed = [], updated = [];
      ...
      for ( i = -1, l = subscribers.length; ++i < l; ) {
        subscribers[ i ].p._input.source_subscriber_index = 0;
      }
      ...
      function route( node, value, add_value ) {
        ...
        for( i = -1; ++i < rl; ) {
          var p = node_subscribers[ i ]
            , input = p._input
            , subscriber_index = input.source_subscriber_index // ToDo: stop using p to store source_subscriber_index
          ;
      ...
      function route_clear( node, subscribers ) {
        ...
        for( i = -1; ++i < rl; ) {
          var p = node_subscribers[ i ], input = p._input;
