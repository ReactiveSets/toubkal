...
( function( exports ) {
  ...
  */
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Terms can express more complex expression using JavaScript Arrays as values, e.g.
     "year: [ '>=', 2013 ]" means "year >= 2013". At this time Queries only support this
     the semantic of [ operator, value ] meaning "( attribute operator value ) == true",
     but much more complex expressions can be expressed such as the two equivalent
     expressions:
       year: [ 'and', '>=', 2009, '<=', 2012 ]
     ...
     The following:
       year: [ 'or', '=', 2005, 'and', '>=', 2009, '<=', 2012 ]
     
     Would mean "year == 2005 or ( year >= 2009 AND year <= 2012 )".
     
     When grouping is required, on can use additional nested Arrays, e.g.:
       year: [ 'or', [ 'in', 2009, 2010, 2011, 2012 ], '=', 2005 ]
     ...
     count ? sales / count > 1000 : false
     
     count: [ '?', { 'sales': [ '/', {} ] } '>', 1000, [ false ] ]
     
     count: [ { 'sales': [ '/', [ { __: 0 }, '!=' {} ] ] } '>', 1000 ]
     
     sales: [ { '/', [ { __: 0 }, '!=' { _: 'count' } }, '>', 1000 ]
      ...
      function route_value( that, transaction, transaction_id, node, subscribers, value, pushed ) {
        ...
        for( i = -1; key = keys[ ++i ]; ) {
          ...
          var branch = branches[ key ]
            , child  = branch[ v ] || evaluate( branch, v )
          ;
