  ...
  */
  Query.evaluate = function( value, attribute, expression ) {
    ...
    if ( expression instanceof Array ) {
      return evaluate( expression, 0 ) === fail ? false: true;
    } else {
      return evaluate_object( left, expression ) === fail ? false: true;
    }
    
    function evaluate_object( value, object ) {
      for ( var a in object ) {
    ...
    function evaluate( expression, i ) {
        ...
        switch( type ) {
          ...
          case 'object': // this is a subexpression
            if ( first instanceof Array ) {
              left = evaluate( first, 0 );
            } else {
              left = evaluate_object( left, first )
            }
          break;
              ...
              default:
                ...
                if ( operator ) {
                  var f = operator[ 0 ]
                    ...
                    , no_fail = ! ( options && options.accepts_fail )
                    , parameters
                    , _left = left
                  ;
