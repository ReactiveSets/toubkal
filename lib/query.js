...
( function( exports ) {
  ...
  */
  ...
  extend_2( Query_Tree.prototype, {
    ...
    __query_tree_route: function( operation, values ) {
        ...
        case 'update':
          ...
          for ( i = -1, rl = subscribers.length; ++i < rl; ) {
            ...
            if ( operations ) {
              ...
              if ( operations > 1 ) t = { count: operations };
              
              if ( removed.length ) {
                subscribers_operations[ 0 ][ 1 ].push( { p: p, v: removed, t: t } );
              }
              
              if ( added.length ) {
                subscribers_operations[ 1 ][ 1 ].push( { p: p, v: added  , t: t } );
              }
              
              if ( updated.length ) {
                subscribers_operations[ 2 ][ 1 ].push( { p: p, v: updated, t: t } );
              }
            }
          } // for all subscribers
        break;
      ...
      function route( node, value, add_value ) {
        ...
        for( i = -1; ++i < rl; ) {
          var p = node_subscribers[ i ]
            , subscriber_index = p.source_subscriber_index // ToDo: stop using p to store source_subscriber_index
          ;
      ...
      function route_clear( node, subscribers ) {
        ...
        for( i = -1; ++i < rl; ) {
          ...
          if ( p.source_subscriber_index ) continue; 
          
          // This pipelet is not marked for clearing yet
          subscribers.push( { p: p } );
    ...
    // ToDo: unsused function __end_source_transactions()
