  ...
  */
  ...
  extend_2( Query_Tree.prototype, {
    ...
    __query_tree_emit: function( operation, values, options ) {
      ...
      // ToDo: DRY route_value() and route_update()
        ...
        for( i = -1; key = keys[ ++i ]; ) {
          ...
          if ( child ) {
            ...
            if ( typeof expressions == 'object' && expressions instanceof Array ) {
              evaluate( value, key, expressions, function( child ) {
                route_value( that, transaction, transaction_id, child, subscribers, value, pushed );
              } );
            }
          }
      ...
      function evaluate( value, attribute, expressions, route_child ) {
        var i = -1, expression, Query_evaluate = Query.evaluate;
        
        while ( expression = expressions[ ++i ] ) {
          if ( Query_evaluate( value, attribute, expression.e ) ) {
            route_child( expression.child );
          }
        }
      } // evaluate()
