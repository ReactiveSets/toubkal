...
( 'cancelable', [ './console_logger', './javascript' ], function( Console_Logger, javascript ) {
  ...
  /* --------------------------------------------------------------------------
      ...
      - **linit**:
        ...
        - (Function) **get( count, got ) -> cancel**: a cancelable_limiter()
        instance, e.g. from a shared limiter.
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      ...
      - **limit**: for @function:cancelable_limiter() to limit concurrent
      operations.
      ...
      ```( value, next, i ) -> cancel```:
        ...
        Signature: ```( errors, value, more ) -> undefined```:
          ...
          - **errors** (Array of Error): or null if there are no errors
          
          - **value**: transformed value that will be emited by next() once
          all transformed values are collected.
          
          - **more** (Boolean): optional, to indicate that there are more
          values to come, calling done() at least one more time with an
          additional value. This can be used to implement an asynchronous
          flat_map().
        
        - **i** (Number): the position of value in values.
      
      - **done** (Function): called when all values are collected of
      when canceled. Signature: ```( errors, values )```:
        
        - **errors** (Array of Errors or null ): optional errors. If the
        last error is 'canceled', cancelable_map() was canceled before
        all values were processed.
        
        - **values** (Array): all values transformed by f(), even if
        there are errors.
      
      - **cancel_on_error** (Boolean): if true, upon the first error
      emitted by done(), cancelable_map() will cancel all ongoing and
      remaining tasks and call next() with errors.
  ...
  function cancelable_map( values, limit, f, next, cancel_on_error ) {
    ...
    */
    ...
    function cancel( errors, slot ) {
      errors && add_errors( errors, slot );
      
      add_errors( 'canceled', slot );
    ...
    function add_errors( errors, slot ) {
      push.apply( all_errors = all_errors || [], ( is_array( errors ) ? errors : [ errors ] ).map( to_Error ) )
      
      function to_Error( error ) {
        error = javascript.is_class( 'Error' )( error ) ? error : new Error( error );
        
        if ( is_number( slot ) ) error.slot = slot;
    ...
    function create_slot_and_done( slot, done ) {
      ...
      return function( errors, value, more ) {
        if ( ! canceled ) {
          ...
          if ( errors )
            ...
            ( is_function( cancel_on_error )
              ...
              : cancel_on_error
            ) ? cancel( errors, slot ) : add_errors( errors, slot )
        ...
        function add_error( message ) {
          ...
          errors.push( new Error( message + ', slot: '  + slot ) );
        }
