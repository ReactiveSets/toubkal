...
( 'event_emitter', [ './console_logger', './loggable' ], function( Console_Logger, Loggable ) {
  ...
  /* --------------------------------------------------------------------------
      ...
      @short Simple event emitter, compatible with node EventEmitter.
  ...
  Loggable.subclass( 'Event_Emitter', Event_Emitter, function() {
    return {
      ...
      */
      ...
      /* ----------------------------------------------------------------------
          @method Event_Emitter.._emit_event( event_name, arguments )
      ...
      _emit_event: function( event_name, a ) {
        ...
        if ( l ) {
          ...
          for( var i = -1, listener; listener = listeners[ ++i ]; ) {
            if ( listener.once ) {
              // call this._remove_listener( listeners, i-- )
              debug && log( name + ', removing "once" event listener, position:', i );
              
              listeners.splice( i--, 1 );
              
              l -= 1;
              
              // ToDo: emit remove_listener event
            }
