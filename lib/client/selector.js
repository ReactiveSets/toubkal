  ...
  rs
    ...
    */
    .Compose( '$add_class', function( $source, css_class, options ) {
      return $source
        .alter( function( _, operation ) {
          _.$node
            && ( operation == 'add' || operation == 'fetch' ? add_class : remove_class )( v.$node, css_class )
          ;
        }, options )
      ;
    } ) // $add_class()
    ...
    /* -----------------------------------------------------------------------------------------
        ...
        @short Emits only source values which have a $node which has css_class set
    ...
    .Compose( '$has_class', function( $source, css_class, options ) {
      return $source
        .map( function( _ ) {
          ...
          if ( $node && has_class( $node, css_class ) )
            return _
          ;
        } )
      ;
