  ...
  */
  var d = document, dom_selectors = {
    ...
    , '[': function( attribute ) {    // [attribute]
        ...
        attribute = attribute.slice( 0, -1 ); // remove leading ]
  ...
  */
  rs
    /* ------------------------------------------------------------------------
        ...
        @short Add css_class to $node attribute
        ...
        @description:
        Source values provide ```"$node"``` attribute, which should be a DOM
        node to add a class to. If none is present, nothing is done.
    ...
    */
    .Compose( '$add_class', function( $source, css_class, options ) {
      return $source
        .alter( function( _ ) {
          _.$node && add_class( _.$node, css_class );
        }, options )
      ;
    } ) // $add_class()
    
    /* ------------------------------------------------------------------------
        @pipelet $remove_class( css_class, options )
        
        @short Remove css_class to $node attribute
        ...
        This is a @@stateless, @@greedy, @@synchronous pipelet.
    */
    .Compose( '$remove_class', function( $source, css_class, options ) {
      return $source
        .alter( function( _ ) {
          _.$node && remove_class( _.$node, css_class );
        }, options )
      ;
    } ) // $remove_class()
