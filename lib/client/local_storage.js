...
undefine()( 'local_storage', [ '../core/pipelet' ], function( rs ) {
  ...
  /* --------------------------------------------------------------------------
      ...
      - flow (String): unique flow attribute value to differentiate
        singletons and serve as a prefix to all key values in Web
        Storage API.
  ...
  */
  ...
  Set.Build( 'local_storage', Local_Storage, function( Super ) { return {
    _make_flow_key: function( value ) {
      return this._flow + '#' + this._make_key( value );
    }, // _make_flow_key()
    ...
    _add: function( values, options ) {
      ...
      values.forEach( function( value ) {
        var key      = that._make_flow_key( value )
