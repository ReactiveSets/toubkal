  ...
  */
  Pipelet.build( 'url_events', URL_Events, {
    fetch: function( receiver ) {
      ...
      return receiver( [ { url: this.url } ], true );
    }, // fetch()
  ...
  /* -------------------------------------------------------------------------------------------
     ...
     Parameters: optional object
       ...
       - slashes_denote_host : treat '//foo/bar' as { host: 'foo', pathname: '/bar' } rather than { pathname: '//foo/bar' }. default: false
  */
  ...
  Pipelet.build( 'url_parse', URL_Parse, {
    parse: function( url, options ) {
      var value    = { href: url }
        ...
        , slashes_denote_host = options.slashes_denote_host
      ;
      ...
      // value.slashes is necessary for url stringify function if needed
      if( slashes_denote_host || protocol || rest.match( /^\/\/[^@\/]+@[^@\/]+/ ) ) {
      ...
      var hash_index = rest.indexOf( "#" )
        , hash       = rest.substr( hash_index )
        , qs_index   = hash.indexOf( '?' )
        , query_string = qs_index !== -1 ? hash.substr( qs_index + 1 ) : ''
      ;
      
      // parse query string into an object
      if( parse_query_string ) value.query = qs_index !== -1 ? parse_query_string( query_string ) : {};
      
      rest = rest.substr( 0, hash_index );
    ...
    add: function( added, options ) {
      ...
      for( var i = -1; ++i < l; ) {
        ...
        if( url === undefined ) {
          de&&ug( 'add(), url not defined, value: ' + log.s( added[ i ] ) );
        } else {
          var value = this.parse( url, o );
