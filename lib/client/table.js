...
*/
undefine()( 'table', [ '../core/pipelet' ], function( rs ) {
  'use strict';
  ...
  /* -------------------------------------------------------------------------------------------
     de&&ug()
  */
  var de = false, ug = RS.log.bind( null, 'table' );
  ...
  */
  function Table_Columns( columns, table, options ) {
    ...
    this._input.add_source( columns._output || columns );
  } // Table_Columns()
  
  Ordered.subclass( 'Table_Columns', Table_Columns, {
    _add: function( added, options ) {
      de&&ug( "Table_Columns.._add(), added:", added );
    ...
    _remove: function( removed, options ) {
      de&&ug( "Table_Columns.._remove(), removed:", removed );
    ...
    _update: function( updates, options ) {
      de&&ug( "Table_Columns.._update(), updated:", updates );
    ...
    insert_: function( at, v ) {
      ...
      for( var i = l; i; ) {
        _add_cell( this.table.body.getElementsByTagName( "tr" )[ --i ].insertCell( at ), lines[ i ][ v.id ], align );
      }
  ...
  /* -------------------------------------------------------------------------------------------
      @pipelet table( $node, columns, options )
      ...
      @parameters:
        ...
        - options (Object):
          ...
          - class_names (String): table element css classes, separated by spaces
  */
  ...
  Ordered.Build( 'table', Table, {
    ...
    init: function() {
      var that    = this
        ...
        , header  = that.header = table.createTHead()
        , body    = that.body   = document.createElement( "tbody" )
        , options = that._options
      ;
      
      table.appendChild( body );
      table.setAttribute( "class", options.class_names || "rs_table" );
      table.createCaption();
      
      header.insertRow( 0 );
      
      that.node.appendChild( table );
      
      options.caption && that.set_caption( options.caption );
    }, // init()
    ...
    _remove: function( removed, options ) {
      de&&ug( "Table.._remove(), removing:", removed );
    ...
    _update: function( updates, options ) {
      de&&ug( "Table.._update(), updating:", updates );
