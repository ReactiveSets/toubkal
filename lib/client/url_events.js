...
( 'url_events', [ '../core/pipelet' ], function( rs ) {
  ...
  /* --------------------------------------------------------------------------
      ...
      Where @@pipelet:last() transforms a stream of url adds into url updates.
      
      Also using pipelet url_parse().
      
      @description
      This is a @@singleton, @@stateful, source, add-only pipelet.
      
      The state has a single value containing the attribute ```"url"``` with
      the most recent value of ```document.location.href```.
      
      This state is updated each time a change is detected either using
