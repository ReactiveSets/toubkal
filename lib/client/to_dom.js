...
( 'to_dom', [ '../core/pipelet' ], function( rs ) {
  ...
  */
  ...
  /*
     ToDo: to_dom(): implement pipelet selector as a filter to source data, rendering conditionally
     ToDo: rename to_dom() pipelet into $to_dom() or $()
     ToDo: implement $select() pipelet to use as a pipelet selector input to to_dom()
     ToDo: implement pipelet processing the output of to_dom() as a selector, having an input dataflow as a parameter, allows chaining
     
     Example: playing with possible API:
     
     - Render list of all "tablet" products names and prices
     
     rs
       .Singleton( 'products', function( source, options ) {
         return rs
           .socket_io_server()
           .flow( 'products' )
         ;
       ...
       .$select( '#products-list', { line: 'tablets' } )
       
       .$products()
       
       .$( null, function render_name( document, $name, product ) {
         $name.innerHTML = product.name;
       } )
     ;
  */
