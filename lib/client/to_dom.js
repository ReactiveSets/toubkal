...
( 'to_dom', [ '../core/pipelet' ], function( rs ) {
  ...
  /* -------------------------------------------------------------------------------------------
      ...
      @description:
        ...
        they may also be used as html bookmarks or references in CSS rules and
        are set by the following process:
  ...
  */
  ...
  function $TO_DOM( selector, options ) {
    ...
    Set.call( that, [], extend( {}, options, { name: name } ) );
    ...
    return {
      ...
      _update_value: function( t, removed, added, move ) {
        ...
        if ( ! $node
          ...
          || node.key != this._make_key( removed )
        ) {
