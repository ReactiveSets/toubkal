...
( 'to_dom', [ '../core/pipelet' ], function( rs ) {
  ...
  /* --------------------------------------------------------------------------
      ...
      - **attributes** (Object): optional DOM Element attributes, or properties.
        When setting, the *style* attribute is set after parsing style
        *property_name / value* pairs, setting ```Element.style.property_name```
        to the changed, or new, value.
        For attributes other than *style*, if the added property is a string
        or if its name starts with *"data-"*, it is set using
        ```Element.setAttribute()```, otherwise it is set by setting the
        ...
        otherwise it is removed using ```Element.removeAttribute()```. 
  ...
  */
      ...
      _update_value: function( t, removed, added, move ) {
        ...
        function remove_attribute( a ) {
          ...
          if ( typeof $node[ a ] == "boolean" ) {
            ...
            // ToDo: for data-, consider delete $node.dataset[ toCamelCase( a.substring( 5 ) ) ]
  ...
  function set_node_attribute( name, $node, a, added, removed ) {
    ...
    } else if ( typeof added == 'string' || a.substring( 0, 5 ) == 'data-' ) {
      // ToDo: for 'data-', consider setting $node.dataset[ toCamelCase( a.substring( 5 ) ) ] = added
      // this would allow to set complex types in addition to strings
