  ...
  Set.Build( '$to_dom', $TO_DOM, function( Super ) {
    ...
    return {
      _get_dom_node: function( selector ) {
        ...
        $node
          || this._error( '_get_dom_node()', 'element ( ' + selector + ' ) not found' )
        ;
      }, // _get_dom_node()
      ...
      _add_value: function( t, value ) {
        ...
        if ( $parent ) {
          ...
          if ( that._b_index_of( value ) == -1 && !$e ) {
            de&&ug( this._get_name( '_add_value' ), 'creating and appending child:', value );
            
            $e = document.createElement( value.tag || that._tag );
            
            $e.id = id;
            
            if ( value.content ) $e.innerHTML = value.content;
            
            extend( $e, value.attributes );
          ...
          de&&ug( this._get_name( '_add_value' ), 'no parent' );
      ...
      _remove_value: function( t, value ) {
        ...
        if ( $parent
          ...
          de&&ug( this._get_name( '_remove_value' ), 'removing child:', value );
          
          $parent.removeChild( $e );
          
          value.$node = $e;
        } else {
          de&&ug( this._get_name( '_remove_value' ), 'cannot remove child:', value, 'parent:', $parent, '' );
