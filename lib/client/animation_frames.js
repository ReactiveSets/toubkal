...
undefine()( 'animation_frames', [ '../core/pipelet' ], function( rs ) {
  ...
  */
  ...
  return rs.Compose( 'animation_frames', singleton, function( source, options ) {
    var state = { flow: flow, id: 1 }
  ...
  */
  .Compose( 'animation_frames_polyfill', singleton, function( source, options ) {
    var animation_frames = rs.animation_frames( options )
      , state            = animation_frames._fetch_all()[ 0 ]
    ;
    
    return state.native ? animation_frames :
      rs
        .beat( 17 )
