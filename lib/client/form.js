  ...
  */
  ...
  Pipelet.Build( 'form_display_errors', Form_Display_Errors, {
    __transform: function( values, options, caller ) {
      ...
      var flow_name  = form.flow
        , flow_value = $form_node.flow.value
      ;
      
      if( flow_name == flow_value || flow_name == flow_value + '_origin' ) return values;
      ...
      for( var i = len; i; ) {
        var error            = errors[ --i ]
          , $form_field      = $form_node[ error.field.id ]
