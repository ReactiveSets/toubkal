...
( 'form', [ [ '../core/order', '../core' ] ], function( rs ) {
  ...
  */
  function Form_Submit( dom_node, options ) {
    ...
    this._form_value     = null;
    this._source_state   = null;
    
    var that           = this
      , $submit_button = this._$submit_button = dom_node.querySelector( 'input[type="submit"]' )
      , prevent_submit = ! this._options.action
    ;
    
    dom_node.querySelector( 'form' ).onsubmit = function() {
      var previous = that._form_value
        , value    = that._source_state
      ;
      
      if( previous ) {
        ! value_equals( previous, value ) && that.__emit_update( [ [ previous, value ] ] )
      } else {
  ...
  Pipelet.Build( 'form_submit', Form_Submit, {
    ...
    _set_source_state: function( value ) {
      this._source_state = value;
      
      return this;
    }, // _set_form_value()
    
    _set_submit_button_state: function( state ) {
      
      this._$submit_button.disabled = state;
    ...
    _add: function( added ) {
      ...
      this._set_source_state( added[ 0 ] );
    ...
    _update: function( updates ) {
      ...
      this._set_source_state( updates[ 0 ][ 1 ] );
