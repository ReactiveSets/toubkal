...
( 'form', [ [ '../core/order', '../core' ] ], function( rs ) {
  ...
  */
  ...
  /* --------------------------------------------------------------------------
      @pipelet form_process_fields( form_process, options )
      
      @short Creates form in DOM
      ...
      Emits values through form_process that instanciates this pipelet
      and must provide:
      - _$form
      - _form_value
      - __emit_update()
      - __emit_add()
  */
  function Form_Process_Fields( form_process, options ) {
  ...
  Ordered.Build( 'form_process_fields', Form_Process_Fields, {
  ...
  */
  rs.Compose( 'form_process', function( source, dom_node, flow, form_fields, options ) {
    options = extend_2( { display_button: true }, options );
    
    var rs   = source.namespace()
      , that = rs.pass_through( options )
    ;
    
    set_node.call( that, dom_node );
    
    that._form_value = null;
    
    if( RS.is_array( form_fields ) ) form_fields = rs.set( form_fields );
    
    form_fields.form_process_fields( that, options );
    
    return that
      .set_flow( flow + '_origin' )
    ;
    
    function set_node( dom_node ) {
